# IDX Super Scanner v9.1.1 — Database-First Macro Scanner
...
## v9.1.1 hotfix
- Menghapus FutureWarning pandas pada boolean database readiness/refresh queue.
- Memperbaiki parsing boolean agar string `False` tidak salah dianggap `True`.
- Tidak mengubah schema database maupun scoring contract.

## v9.1.2 database acquisition recovery

The database-first backfill now uses independent evidence queues, current IDX host failover, hardened Yahoo statement parsing, Twelve Data history fallback, real zero-gain stall detection, and persistent market/news evidence through the existing Supabase source-events table. See `PATCH_NOTES_V9_1_2.md`.

...
AI_VERSION = "7.10.0-expanded-causal-oos-data-completion-ai"
...
class _RidgeModel:
    ...
    def predict(self, frame: pd.DataFrame) -> np.ndarray:
        x = _matrix(frame, self.feature_names)
        x = np.where(np.isfinite(x), x, self.scaler.median)
        return self.scaler.transform(x) @ self.coef + self.intercept
...
def _fit_scaler(x: np.ndarray) -> _Scaler:
    ...
    # Treat such columns as genuinely unavailable and impute a neutral numeric
    # zero only inside the scaler; upstream coverage gates still keep them from
    # becoming model evidence.
    array = np.asarray(x, dtype=float)
    if array.ndim != 2:
        array = np.atleast_2d(array)
    median = np.zeros(array.shape[1], dtype=float)
    for idx in range(array.shape[1]):
        finite = array[:, idx][np.isfinite(array[:, idx])]
        if finite.size:
            median[idx] = float(np.median(finite))
...
def _fit_ridge_regression(
    ...
    """Fit a small, regularised outcome model without adding heavy runtimes."""
    names = _feature_names()
    x_raw = _matrix(frame, names)
    y = pd.to_numeric(target, errors="coerce").to_numpy(dtype=float)
    valid = np.isfinite(y)
    ...
    minimum = int(config.min_training_events if minimum_events is None else minimum_events)
    if len(y) < max(12, minimum):
        return None
    valid_frame = frame.loc[valid].copy()
    scaler = _fit_scaler(x_raw)
    x = scaler.transform(np.where(np.isfinite(x_raw), x_raw, scaler.median))
    weights = (
        _recency_weights(valid_frame, config.recency_half_life_days)
        * _outcome_quality_weights(valid_frame)
    )
    weights = np.clip(weights, 0.05, None)
    intercept = float(np.average(y, weights=weights))
    centered = y - intercept
    root_weight = np.sqrt(weights / max(1e-9, float(np.mean(weights))))
    weighted_x = x * root_weight[:, None]
    weighted_y = centered * root_weight
    penalty = max(1e-6, float(config.l2))
    gram = weighted_x.T @ weighted_x + penalty * len(y) * np.eye(weighted_x.shape[1])
    try:
        coef = np.linalg.solve(gram, weighted_x.T @ weighted_y)
    except np.linalg.LinAlgError:
        coef = np.linalg.pinv(gram) @ (weighted_x.T @ weighted_y)
    return _RidgeModel(
...
def _rule_to_ai_training_frame(events: pd.DataFrame, source: str = "UNKNOWN") -> pd.DataFrame:
    ...
    quality = pd.to_numeric(series("quality_score", np.nan), errors="coerce")
    out["structure_score"] = pd.to_numeric(series("structural_quality_score"), errors="coerce").fillna(quality)
    out["timing_score"] = pd.to_numeric(series("timing_score"), errors="coerce")
    silent = pd.to_numeric(series("silent_accumulation_score"), errors="coerce")
    ...
    flow_observed = silent.notna() | pd.to_numeric(series("cmf20"), errors="coerce").notna() | pd.to_numeric(series("volume_ratio"), errors="coerce").notna()
    flow_fallback = flow_fallback.where(flow_observed, np.nan)
    out["flow_score"] = pd.to_numeric(series("flow_score"), errors="coerce").combine_first(flow_fallback)
    ...
    out["data_quality_score"] = pd.to_numeric(series("data_quality_score"), errors="coerce").combine_first(pd.to_numeric(series("data_completeness_score"), errors="coerce"))
    out["validation_score"] = pd.to_numeric(series("validation_score"), errors="coerce").combine_first(pd.to_numeric(series("validation_gate_score"), errors="coerce"))
    ...
    tp1_before_sl_source = (
        series("tp1_before_sl", np.nan)
        if "tp1_before_sl" in out
        else series("tp1_hit", np.nan)
    )
    out["success_target"] = _binary_outcome(tp1_before_sl_source)
    out.loc[~out["fill_target"].isin([0, 1]), "fill_target"] = np.nan
    out.loc[~out["success_target"].isin([0, 1]), "success_target"] = np.nan
    out["r_multiple"] = pd.to_numeric(series("r_multiple", np.nan), errors="coerce")
    out["mfe_r"] = pd.to_numeric(
        series("mfe_r", np.nan) if "mfe_r" in out else series("maximum_favorable_excursion_r", np.nan),
        errors="coerce",
    )
    out["mae_r"] = pd.to_numeric(
        series("mae_r", np.nan) if "mae_r" in out else series("maximum_adverse_excursion_r", np.nan),
        errors="coerce",
    )
    out["gross_return_pct"] = pd.to_numeric(series("gross_return_pct", np.nan), errors="coerce")
    out["roundtrip_cost_pct"] = pd.to_numeric(series("roundtrip_cost_pct", np.nan), errors="coerce")
    out["fill_delay_bars"] = pd.to_numeric(
        series("fill_delay_bars", np.nan), errors="coerce",
    )
    out["bars_to_exit"] = pd.to_numeric(
        series("bars_to_exit", np.nan), errors="coerce",
    )
    net_return = pd.to_numeric(series("net_return_pct", np.nan), errors="coerce")
    stop_fraction = pd.to_numeric(out.get("stop_pct"), errors="coerce")
    derived_net_return = 100.0 * out["r_multiple"] * stop_fraction
    out["net_return_pct"] = net_return.where(net_return.notna(), derived_net_return)
...
def _prepare_training_history(
    ...
    if "signal_date" in history.columns:
        history["signal_date"] = pd.to_datetime(
            history["signal_date"], errors="coerce", utc=True,
        ).dt.tz_convert(None).dt.normalize()
    history["_event_identity"] = _event_identity(history)
    history = history.sort_values(
        ["_source_priority", "signal_date"], na_position="first", kind="mergesort",
    )
...
def _execution_drawdown_pct(net_returns_pct: pd.Series) -> float:
    values = pd.to_numeric(net_returns_pct, errors="coerce").dropna().to_numpy(dtype=float)
    if len(values) == 0:
        return np.nan
    equity = np.cumprod(1.0 + np.clip(values / 100.0, -0.99, 10.0))
    peak = np.maximum.accumulate(np.r_[1.0, equity])[1:]
    drawdown = 100.0 * (equity / np.maximum(peak, 1e-9) - 1.0)
    return float(abs(np.min(np.r_[0.0, drawdown])))


def _execution_profit_factor(r_multiple: pd.Series) -> float:
    values = pd.to_numeric(r_multiple, errors="coerce").dropna().to_numpy(dtype=float)
...
def _discrete_fill_hazard(
    history: pd.DataFrame,
    row: Mapping[str, Any],
    total_fill_probability: float,
    config: LocalAIConfig,
) -> tuple[float, float, float, str]:
    """Distribute P(fill≤window) across bars with a censored discrete hazard."""
    if history is None or history.empty or not np.isfinite(total_fill_probability):
        return np.nan, np.nan, np.nan, "NO_HAZARD_HISTORY"
    usable = history.copy()
    usable["_fill"] = pd.to_numeric(
        usable.get("fill_target"), errors="coerce",
    )
    usable["_delay"] = pd.to_numeric(
        usable.get("fill_delay_bars"), errors="coerce",
    )
    usable = usable[usable["_fill"].isin([0.0, 1.0])].copy()
    if usable.empty:
        return np.nan, np.nan, np.nan, "NO_HAZARD_HISTORY"
    strategy = _normalize_strategy(row.get("strategy"))
    strategy_rows = usable[
        usable.get(
            "strategy", pd.Series("UNKNOWN", index=usable.index),
        ).map(_normalize_strategy).eq(strategy)
    ]
    minimum = max(12, int(config.min_strategy_events))
    if len(strategy_rows) >= minimum:
        usable = strategy_rows
        state = "STRATEGY_DISCRETE_HAZARD"
    else:
        state = "GLOBAL_DISCRETE_HAZARD"
    window = max(1, int(config.memory_entry_window_bars))
    hazards: list[float] = []
    survival = 1.0
    raw_events: list[float] = []
    for bar in range(window):
        filled = usable["_fill"].eq(1.0)
        # No-fill rows are right-censored at the end of the configured window.
        at_risk = (~filled) | usable["_delay"].fillna(window).ge(bar)
        events = filled & usable["_delay"].round().eq(bar)
        risk_count = int(at_risk.sum())
        event_count = int(events.sum())
        hazard = (event_count + 0.5) / (risk_count + 1.0) if risk_count else 0.0
        hazard = float(np.clip(hazard, 0.0, 1.0))
        hazards.append(hazard)
        raw_events.append(survival * hazard)
        survival *= 1.0 - hazard
    raw_total = float(sum(raw_events))
    if raw_total <= 1e-12:
        return np.nan, np.nan, np.nan, "INSUFFICIENT_HAZARD_EVENTS"
    total = float(np.clip(total_fill_probability, 0.0, 1.0))
    scaled_events = [total * value / raw_total for value in raw_events]
    p1 = scaled_events[0]
    p3 = sum(scaled_events[: min(3, window)])
    expected_delay = (
        sum((bar + 1) * probability for bar, probability in enumerate(scaled_events))
        / total
        if total > 1e-12 else np.nan
    )
    return p1, p3, expected_delay, state


def _execution_ai_bundle(
    filled_history: pd.DataFrame,
    current: pd.DataFrame,
    config: LocalAIConfig,
) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    """Train/evaluate expected-R, MFE and MAE models on chronological outcomes."""
    empty_predictions = {
        "expected_r": np.full(len(current), np.nan),
        "mfe_r": np.full(len(current), np.nan),
        "mae_r": np.full(len(current), np.nan),
    }
    if filled_history is None or filled_history.empty:
        return empty_predictions, {
            "state": "NO_EXECUTION_HISTORY", "sample_size": 0,
            "evaluation_size": 0, "expectancy_r": np.nan,
            "profit_factor": np.nan, "max_drawdown_pct": np.nan,
        }
    usable = filled_history.copy()
    usable["r_multiple"] = pd.to_numeric(usable.get("r_multiple"), errors="coerce")
    usable = usable[usable["r_multiple"].notna()].copy()
    if len(usable) < max(12, int(config.min_training_events)):
        return empty_predictions, {
            "state": "INSUFFICIENT_EXECUTION_EVENTS", "sample_size": int(len(usable)),
            "evaluation_size": 0, "expectancy_r": np.nan,
            "profit_factor": np.nan, "max_drawdown_pct": np.nan,
        }
    train, evaluation = _chronological_split(
        usable, max(0.20, min(0.35, float(config.calibration_fraction))),
    )
    evaluation_r = pd.to_numeric(evaluation.get("r_multiple"), errors="coerce").dropna()
    evaluation_net = pd.to_numeric(
        evaluation.get("net_return_pct", pd.Series(np.nan, index=evaluation.index)),
        errors="coerce",
    )
    if evaluation_net.notna().sum() < len(evaluation_r):
        stop_fraction = pd.to_numeric(
            evaluation.get("stop_pct", pd.Series(np.nan, index=evaluation.index)),
            errors="coerce",
        )
        fallback_net = 100.0 * pd.to_numeric(evaluation.get("r_multiple"), errors="coerce") * stop_fraction
        evaluation_net = evaluation_net.where(evaluation_net.notna(), fallback_net)
    expectancy = float(evaluation_r.mean()) if len(evaluation_r) else np.nan
    profit_factor = _execution_profit_factor(evaluation_r)
    max_drawdown = _execution_drawdown_pct(evaluation_net)

    state_reasons: list[str] = []
    if len(evaluation_r) < int(config.min_evaluation_events):
        state_reasons.append("EVALUATION_SAMPLE_LOW")
    if not np.isfinite(expectancy) or expectancy < float(config.min_expectancy_r):
        state_reasons.append("EXPECTANCY_NOT_POSITIVE")
    if not np.isfinite(profit_factor) or profit_factor < float(config.min_profit_factor):
        state_reasons.append("PROFIT_FACTOR_LOW")
    if not np.isfinite(max_drawdown) or max_drawdown > float(config.max_oos_drawdown_pct):
        state_reasons.append("DRAWDOWN_TOO_HIGH")
    state = "VALIDATED_EXECUTION_AI" if not state_reasons else "EXECUTION_SHADOW_ONLY"

    predictions: dict[str, np.ndarray] = {}
    regression_mae: dict[str, float] = {}
    for target in ("r_multiple", "mfe_r", "mae_r"):
        values = pd.to_numeric(usable.get(target, pd.Series(np.nan, index=usable.index)), errors="coerce")
        labelled = usable[values.notna()].copy()
        labelled[target] = values[values.notna()]
        minimum_fit = max(12, int(config.min_training_events * 0.60))
        live_model = _fit_ridge_regression(labelled, labelled[target], config, minimum_fit)
        predictions[target if target != "r_multiple" else "expected_r"] = (
            live_model.predict(current) if live_model is not None else np.full(len(current), np.nan)
        )
        train_values = pd.to_numeric(train.get(target, pd.Series(np.nan, index=train.index)), errors="coerce")
        train_labelled = train[train_values.notna()].copy()
        train_labelled[target] = train_values[train_values.notna()]
        eval_values = pd.to_numeric(
            evaluation.get(target, pd.Series(np.nan, index=evaluation.index)), errors="coerce",
        )
        audit_model = _fit_ridge_regression(
            train_labelled, train_labelled.get(target, pd.Series(dtype=float)),
            config, minimum_fit,
        )
        if audit_model is not None and eval_values.notna().any():
            predicted_eval = audit_model.predict(evaluation.loc[eval_values.notna()])
            regression_mae[target] = float(
                np.mean(np.abs(predicted_eval - eval_values[eval_values.notna()].to_numpy(dtype=float)))
            )
        else:
            regression_mae[target] = np.nan
    for key, values in empty_predictions.items():
        predictions.setdefault(key, values)
    return predictions, {
        "state": state,
        "gate_reasons": " | ".join(state_reasons) or "ALL_EXECUTION_GATES_PASSED",
        "sample_size": int(len(usable)),
        "train_size": int(len(train)),
        "evaluation_size": int(len(evaluation_r)),
        "expectancy_r": expectancy,
        "profit_factor": profit_factor,
        "max_drawdown_pct": max_drawdown,
        "expected_r_mae": regression_mae.get("r_multiple"),
        "mfe_r_mae": regression_mae.get("mfe_r"),
        "mae_r_mae": regression_mae.get("mae_r"),
    }
...
def enrich_profit_ranking_with_ai(
    ...
    score_source = result.get("profit_conviction_score", pd.Series(np.nan, index=result.index))
    rule_score = pd.to_numeric(score_source, errors="coerce")
    rule_score_available = rule_score.notna()
    ...
    if (not cfg.enabled) or cfg.mode.upper() == "RULE_ONLY":
        for column in (
            ...
            "ai_fill_probability_1d_pct", "ai_fill_probability_3d_pct",
            "ai_expected_fill_delay_bars",
            ...
            "ai_tp1_probability_upper_pct", "ai_expected_r", "ai_net_expectancy_r",
            "ai_mfe_r", "ai_mae_r", "ai_oos_expectancy_r",
            "ai_oos_profit_factor", "ai_oos_max_drawdown_pct", "ai_drift_score",
        ...
        result["ai_execution_validation_state"] = "DISABLED_RULE_ONLY"
        result["ai_fill_hazard_state"] = "DISABLED_RULE_ONLY"
        result["ai_execution_gate_reasons"] = "RULE_ONLY"
        result["hybrid_conviction_score"] = rule_score.round(1)
        result["final_score"] = rule_score.round(1)
    ...
    execution_predictions, execution_audit = _execution_ai_bundle(
        filled_history, result, cfg,
    )
    execution_state = _safe_text(execution_audit.get("state")) or "NO_EXECUTION_HISTORY"
    execution_validated = execution_state == "VALIDATED_EXECUTION_AI"
    ...
    fill_probability_1d: list[float] = []
    fill_probability_3d: list[float] = []
    expected_fill_delay: list[float] = []
    fill_hazard_states: list[str] = []
    ...
    for idx, row in result.iterrows():
        ...
        hazard_1d, hazard_3d, hazard_delay, hazard_state = _discrete_fill_hazard(
            history, row, p_fill, cfg,
        )
        ...
        model_r = (
            float(execution_predictions["expected_r"][idx])
            if idx < len(execution_predictions["expected_r"])
            and np.isfinite(execution_predictions["expected_r"][idx])
            else np.nan
        )
        e_r = model_r if np.isfinite(model_r) else similar_r if np.isfinite(similar_r) else base_r
        trade_probability = p_fill * p_success
        expectancy_score = 50.0 + 32.0 * math.tanh(e_r) if np.isfinite(e_r) else 50.0
        ai_edge = 0.72 * (100.0 * trade_probability) + 0.28 * expectancy_score
        ...
        if not execution_validated:
            gate_reasons.append(
                _safe_text(execution_audit.get("gate_reasons"))
                or "EXECUTION_VALIDATION_PENDING"
            )
        ...
        if (
            ...
            and np.isfinite(joint_brier_skill)
            and joint_brier_skill > cfg.min_brier_skill
        ):
        ...
        rule_value = _safe_number(rule_score.iloc[idx], np.nan)
        if not np.isfinite(rule_value):
            effective_weight = 0.0
            influence = False
            gate_reasons.append("RULE_SCORE_MISSING")
            hybrid = np.nan
        else:
            hybrid = (
                (1.0 - effective_weight) * rule_value + effective_weight * ai_edge
                if influence else rule_value
            )
        ...
        elif joint_validated and execution_validated and influence:
        ...
        fill_probability_1d.append(
            hazard_1d if probability_available else np.nan
        )
        fill_probability_3d.append(
            hazard_3d if probability_available else np.nan
        )
        expected_fill_delay.append(
            hazard_delay if probability_available else np.nan
        )
        fill_hazard_states.append(hazard_state)
        ...
        if probability_available:
            ...
            if np.isfinite(hazard_3d):
                explanation += f"; P(fill≤3 bar) {hazard_3d*100:.0f}%"
            if np.isfinite(hazard_delay):
                explanation += f"; E[fill] {hazard_delay:.1f} bar"
            explanation += f"; expected R {e_r:.2f}" if np.isfinite(e_r) else "; expected R belum stabil"
            mfe_prediction = (
                float(execution_predictions["mfe_r"][idx])
                if idx < len(execution_predictions["mfe_r"])
                and np.isfinite(execution_predictions["mfe_r"][idx])
                else np.nan
            )
            mae_prediction = (
                float(execution_predictions["mae_r"][idx])
                if idx < len(execution_predictions["mae_r"])
                and np.isfinite(execution_predictions["mae_r"][idx])
                else np.nan
            )
            if np.isfinite(mfe_prediction):
                explanation += f"; MFE {mfe_prediction:.2f}R"
            if np.isfinite(mae_prediction):
                explanation += f"; MAE {mae_prediction:.2f}R"
        else:
    ...
    result["ai_fill_probability_1d_pct"] = np.round(
        100.0 * np.asarray(fill_probability_1d, dtype=float), 1,
    )
    result["ai_fill_probability_3d_pct"] = np.round(
        100.0 * np.asarray(fill_probability_3d, dtype=float), 1,
    )
    result["ai_expected_fill_delay_bars"] = np.round(
        np.asarray(expected_fill_delay, dtype=float), 2,
    )
    result["ai_fill_hazard_state"] = fill_hazard_states
    ...
    result["ai_net_expectancy_r"] = result["ai_expected_r"]
    result["ai_mfe_r"] = np.round(
        np.asarray(execution_predictions.get("mfe_r"), dtype=float), 3,
    )
    result["ai_mae_r"] = np.round(
        np.asarray(execution_predictions.get("mae_r"), dtype=float), 3,
    )
    result["ai_oos_expectancy_r"] = _safe_number(execution_audit.get("expectancy_r"), np.nan)
    result["ai_oos_profit_factor"] = _safe_number(execution_audit.get("profit_factor"), np.nan)
    result["ai_oos_max_drawdown_pct"] = _safe_number(execution_audit.get("max_drawdown_pct"), np.nan)
    result["ai_execution_validation_state"] = execution_state
    result["ai_execution_gate_reasons"] = _safe_text(execution_audit.get("gate_reasons"))
    ...
    result["final_score"] = pd.to_numeric(result.get(sort_column), errors="coerce").fillna(rule_score).round(1)
    result["_silent_rank"] = pd.to_numeric(
        result.get("silent_accumulation_score", pd.Series(0.0, index=result.index)), errors="coerce"
    ).fillna(0.0)
    ...
    result = result.sort_values(
        ["final_score", "_silent_rank", "_ai_readiness", "target_quality_score", "liquidity_score"],
        ascending=[False, False, True, False, False],
        kind="stable",
    ).drop(columns=["_ai_readiness", "_silent_rank"]).reset_index(drop=True)
    result["profit_rank"] = np.arange(1, len(result) + 1)
    ...
    audit = pd.DataFrame([
        ...
        {"metric": "Execution AI state", "value": execution_state},
        {"metric": "Execution train/evaluation", "value": f"{execution_audit.get('train_size', 0)}/{execution_audit.get('evaluation_size', 0)}"},
        {"metric": "Execution OOS expectancy R", "value": execution_audit.get("expectancy_r")},
        {"metric": "Execution OOS profit factor", "value": execution_audit.get("profit_factor")},
        {"metric": "Execution OOS max drawdown %", "value": execution_audit.get("max_drawdown_pct")},
        {"metric": "Expected-R prediction MAE", "value": execution_audit.get("expected_r_mae")},
        {"metric": "MFE prediction MAE", "value": execution_audit.get("mfe_r_mae")},
        {"metric": "MAE prediction MAE", "value": execution_audit.get("mae_r_mae")},
        ...
        {"metric": "Storage", "value": "Persistent database + local cache/CSV fallback"},
    ])
    return result, audit
...
def _register_current(memory: pd.DataFrame, ranking: pd.DataFrame, prepared: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for _, row in ranking.iterrows():
        ...
        record.update({
            ...
            "filled": np.nan,
            "tp1_hit": np.nan,
            "tp1_before_sl": np.nan,
            "r_multiple": np.nan,
            "mfe_r": np.nan,
            "mae_r": np.nan,
            "gross_return_pct": np.nan,
            "net_return_pct": np.nan,
            "roundtrip_cost_pct": np.nan,
            "fill_delay_bars": np.nan,
            "bars_to_exit": np.nan,
            "competing_risk_event": "OPEN",
            "result": "OPEN",
...
def _resolve_one(row: pd.Series, frame: pd.DataFrame, cfg: LocalAIConfig) -> dict[str, Any]:
        ...
        if buy_stop:
            ...
            if np.isfinite(open_v) and atr_value > 0 and open_v > level + cfg.max_entry_gap_atr * atr_value:
                result.update({
                    ...
                    "fill_delay_bars": 0,
                    "competing_risk_event": "NO_FILL_GAP_CANCEL",
            ...
            if np.isfinite(open_v) and open_v <= stop:
                result.update({
                    ...
                    "roundtrip_cost_pct": 0.0,
                    "fill_delay_bars": 0,
                    "competing_risk_event": "NO_FILL_INVALIDATED",
    ...
    if fill_position is None:
        if len(local) >= int(cfg.memory_entry_window_bars):
            result.update({
                "memory_state": "RESOLVED", "filled": False,
                "tp1_hit": False, "tp1_before_sl": False,
                "result": "NO_FILL", "r_multiple": 0.0,
                "gross_return_pct": 0.0, "net_return_pct": 0.0,
                "roundtrip_cost_pct": 0.0,
                "fill_delay_bars": int(cfg.memory_entry_window_bars),
                "competing_risk_event": "NO_FILL_CENSORED",
            })
        return result
    ...
    exit_bar_offset = max(0, len(future) - 1)
    maximum_high = fill_price
    minimum_low = fill_price
    for bar_offset, (candle_date, candle) in enumerate(future.iterrows()):
        ...
        if bar_offset == 0 and not fill_at_open and (stop_touched or tp1_touched):
            ...
            break
        # When fill occurred at the open, the whole bar is observable after
        # entry. For an intrabar fill, start excursions on the next bar because
        # daily OHLC cannot order pre-fill versus post-fill extremes.
        if fill_at_open or bar_offset > 0:
            if np.isfinite(high_v):
                maximum_high = max(maximum_high, high_v)
            if np.isfinite(low_v):
                minimum_low = min(minimum_low, low_v)
        if np.isfinite(open_v) and open_v <= stop:
    ...
    gross_return = exit_price / fill_price - 1.0 if np.isfinite(exit_price) else np.nan
    applied_cost = max(0.0, cfg.roundtrip_cost_pct)
    net_return = gross_return - applied_cost if np.isfinite(gross_return) else np.nan
    r_multiple = net_return / risk_pct if np.isfinite(risk_pct) and risk_pct > 0 else np.nan
    risk_amount = fill_price - stop
    mfe_r = (maximum_high - fill_price) / risk_amount if risk_amount > 0 else np.nan
    mae_r = (minimum_low - fill_price) / risk_amount if risk_amount > 0 else np.nan
    mfe_pct = 100.0 * (maximum_high / fill_price - 1.0) if fill_price > 0 else np.nan
    mae_pct = 100.0 * (minimum_low / fill_price - 1.0) if fill_price > 0 else np.nan
    ...
    result.update({
        ...
        "tp1_hit": tp1_hit,
        "tp1_before_sl": tp1_hit,
        "tp2_hit": tp2_hit,
        "outcome_ambiguous": outcome_ambiguous,
        "result": outcome,
        "r_multiple": round(float(r_multiple), 4) if np.isfinite(r_multiple) else np.nan,
        "expectancy_after_cost_r": round(float(r_multiple), 4) if np.isfinite(r_multiple) else np.nan,
        "mfe_r": round(float(mfe_r), 4) if np.isfinite(mfe_r) else np.nan,
        "mae_r": round(float(mae_r), 4) if np.isfinite(mae_r) else np.nan,
        "mfe_pct": round(float(mfe_pct), 4) if np.isfinite(mfe_pct) else np.nan,
        "mae_pct": round(float(mae_pct), 4) if np.isfinite(mae_pct) else np.nan,
        "gross_return_pct": round(100.0 * float(gross_return), 4) if np.isfinite(gross_return) else np.nan,
        "net_return_pct": round(100.0 * float(net_return), 4) if np.isfinite(net_return) else np.nan,
        "roundtrip_cost_pct": round(100.0 * applied_cost, 4),
        "cost_r": round(float(applied_cost / risk_pct), 4) if np.isfinite(risk_pct) and risk_pct > 0 else np.nan,
        "fill_delay_bars": int(fill_position),
        "bars_to_exit": int(exit_bar_offset + 1),
        "competing_risk_event": (
            "TP1_FIRST" if outcome == "WIN_TP1"
            else "SL_FIRST" if outcome in {"LOSS", "LOSS_GAP"}
            else "AMBIGUOUS" if "AMBIGUOUS" in outcome
            else "TIME_EXIT"
        ),
        "fill_slippage_pct": round(100.0 * (fill_price / entry - 1.0), 4) if entry > 0 else np.nan,
...
def update_outcome_memory(
    ...
    cfg = config or LocalAIConfig()
    existing_memory = (
        existing.copy() if isinstance(existing, pd.DataFrame) else pd.DataFrame()
    )
    for date_column, normalise_day in (
        ("signal_date", True), ("fill_date", False),
        ("exit_date", False), ("resolved_at", False),
    ):
        if date_column not in existing_memory.columns:
            continue
        parsed_dates = pd.to_datetime(
            existing_memory[date_column], errors="coerce", utc=True,
        ).dt.tz_convert(None)
        existing_memory[date_column] = (
            parsed_dates.dt.normalize() if normalise_day else parsed_dates
        )
    memory = _register_current(existing_memory, ranking, prepared)
    if memory.empty:
        return memory
    resolved = []
    ...
    if "signal_date" in output.columns:
        output["signal_date"] = pd.to_datetime(
            output["signal_date"], errors="coerce", utc=True,
        ).dt.tz_convert(None).dt.normalize()
    output = output.sort_values(
        "signal_date", na_position="last", kind="mergesort",
    ).tail(int(cfg.memory_max_rows)).reset_index(drop=True)
    save_memory(output, cfg.memory_max_rows)
    return output



def validation_events_to_memory(events: pd.DataFrame | None) -> pd.DataFrame:
    """Convert chronological walk-forward events into durable AI memory.

    Only genuine OOS rows are persisted.  Trigger candidates remain labelled as
    research outcomes and never become live-production approvals.
    """
    if events is None or events.empty:
        return pd.DataFrame()
    frame = events.copy()
    if "is_oos" in frame:
        frame = frame[frame["is_oos"].fillna(False).astype(bool)]
    elif "oos_eligible" in frame:
        frame = frame[frame["oos_eligible"].fillna(False).astype(bool)]
    if frame.empty:
        return pd.DataFrame()
    rows: list[dict[str, Any]] = []
    for raw in frame.to_dict("records"):
        ticker = _safe_text(raw.get("ticker")).upper()
        strategy = _normalize_strategy(raw.get("setup", raw.get("strategy")))
        signal_date = pd.to_datetime(raw.get("signal_date"), errors="coerce")
        entry = _safe_number(raw.get("entry", raw.get("planned_entry")), np.nan)
        stop = _safe_number(raw.get("stop_loss", raw.get("stop")), np.nan)
        tp1 = _safe_number(raw.get("tp1"), np.nan)
        if not ticker or pd.isna(signal_date) or not all(np.isfinite(x) for x in (entry, stop, tp1)):
            continue
        signal_id = _signal_identifier(ticker, strategy, signal_date, entry, stop, tp1)
        filled_raw = raw.get("filled")
        filled = bool(filled_raw) if not pd.isna(filled_raw) else False
        result = _safe_text(raw.get("result")) or ("RESOLVED" if filled else "NO_FILL")
        record = dict(raw)
        record.update({
            "signal_id": signal_id,
            "ticker": ticker,
            ...
            "memory_state": "RESOLVED",
            "entry": entry,
            "trigger_price": _safe_number(raw.get("trigger_price", raw.get("planned_entry")), entry),
            "stop_loss": stop,
            "tp1": tp1,
            "tp2": _safe_number(raw.get("tp2"), np.nan),
            "filled": filled,
            "tp1_before_sl": raw.get("tp1_before_sl", raw.get("tp1_hit", False)),
            "result": result,
            "r_multiple": _safe_number(raw.get("r_multiple"), 0.0 if not filled else np.nan),
            "expectancy_after_cost_r": _safe_number(raw.get("r_multiple"), 0.0 if not filled else np.nan),
            "outcome_quality": "CHRONOLOGICAL_OOS_CAUSAL",
            "validation_event_tier": _safe_text(raw.get("validation_event_tier")) or "TRIGGER_CANDIDATE",
            "production_gate_pass": bool(raw.get("production_gate_pass", False)),
    ...
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame(rows).drop_duplicates("signal_id", keep="last").reset_index(drop=True)
...
def memory_summary(memory: pd.DataFrame | None) -> pd.DataFrame:
    ...
    r_values = pd.to_numeric(
        filled.get("r_multiple", pd.Series(np.nan, index=filled.index)),
        errors="coerce",
    )
    net_values = pd.to_numeric(
        filled.get("net_return_pct", pd.Series(np.nan, index=filled.index)),
        errors="coerce",
    )
    mfe_values = pd.to_numeric(
        filled.get("mfe_r", pd.Series(np.nan, index=filled.index)),
        errors="coerce",
    )
    mae_values = pd.to_numeric(
        filled.get("mae_r", pd.Series(np.nan, index=filled.index)),
        errors="coerce",
    )
    return pd.DataFrame([
        ...
        {"metric": "Expectancy after cost (R)", "value": round(float(r_values.mean()), 3) if r_values.notna().any() else np.nan},
        {"metric": "Profit factor (R)", "value": round(_execution_profit_factor(r_values), 3) if r_values.notna().any() else np.nan},
        {"metric": "Maximum drawdown", "value": round(_execution_drawdown_pct(net_values), 2) if net_values.notna().any() else np.nan},
        {"metric": "Median MFE (R)", "value": round(float(mfe_values.median()), 3) if mfe_values.notna().any() else np.nan},
        {"metric": "Median MAE (R)", "value": round(float(mae_values.median()), 3) if mae_values.notna().any() else np.nan},
        {"metric": "Persistence", "value": "Persistent database; local CSV remains a fail-soft fallback"},
    ])
...
def enrich_multibagger_with_peer_ai(
    ...
    out = candidates.copy()
    if not enabled:
        out["ai_multibagger_peer_score"] = np.nan
        out["ai_multibagger_forward_score"] = np.nan
        out["ai_multibagger_absolute_score"] = np.nan
    ...
    if np.isfinite(matrix).any():
        center = np.asarray([
            np.nanmedian(matrix[:, column]) if np.isfinite(matrix[:, column]).any() else 50.0
            for column in range(matrix.shape[1])
        ], dtype=float)
        filled_matrix = np.where(np.isfinite(matrix), matrix, center)
        mad = np.asarray([
            np.nanmedian(np.abs(filled_matrix[:, column] - center[column])) * 1.4826
            for column in range(filled_matrix.shape[1])
        ], dtype=float)
        mad = np.where(mad > 1e-6, mad, 20.0)
        z = (filled_matrix - center) / mad
        outlier = np.sqrt(np.mean(np.clip(z, -8, 8) ** 2, axis=1))
    else:
        outlier = np.zeros(len(out), dtype=float)
    ...
    impact = pd.to_numeric(out.get("future_fundamental_impact_score", pd.Series(np.nan, index=out.index)), errors="coerce")
    forward_components = pd.DataFrame({
        "impact": impact,
        "revenue": np.clip(50 + revenue_uplift * 1.4, 0, 100),
        "ebitda": np.clip(50 + ebitda_uplift * 1.2, 0, 100),
        "success": success_probability.clip(0, 100),
        "debt": np.clip(65 - debt_change * 0.8, 0, 100),
    }, index=out.index)
    forward_weights = pd.Series({"impact": 0.30, "revenue": 0.22, "ebitda": 0.20, "success": 0.18, "debt": 0.10})
    forward_den = forward_components.notna().mul(forward_weights, axis=1).sum(axis=1)
    forward_score = forward_components.mul(forward_weights, axis=1).sum(axis=1, min_count=1).div(forward_den.where(forward_den.gt(0)))
    forward_coverage = forward_den / forward_weights.sum()
    forward_score = forward_score.where(forward_coverage.ge(0.50))
    ...
    business_quality = (10.0 * score10).where(score10.notna(), score100).clip(0.0, 100.0)
    consensus = pd.to_numeric(out.get("fundamental_consensus_score", pd.Series(np.nan, index=out.index)), errors="coerce").clip(0.0, 100.0)
    fundamental_coverage = pd.to_numeric(out.get("fundamental_coverage", pd.Series(np.nan, index=out.index)), errors="coerce").clip(0.0, 100.0)
    ...
    grade_score = grade.map({"A": 100.0, "B": 75.0, "C": 45.0, "D": 15.0})
    absolute_components = pd.DataFrame({
        "business": business_quality,
        "consensus": consensus,
        "coverage": fundamental_coverage,
        "grade": grade_score,
        "official": official.astype(float) * 100.0,
    }, index=out.index)
    absolute_weights = pd.Series({"business": 0.50, "consensus": 0.20, "coverage": 0.15, "grade": 0.10, "official": 0.05})
    absolute_den = absolute_components.notna().mul(absolute_weights, axis=1).sum(axis=1)
    absolute_score = absolute_components.mul(absolute_weights, axis=1).sum(axis=1, min_count=1).div(absolute_den.where(absolute_den.gt(0)))
    absolute_score = absolute_score.where(absolute_den.ge(0.70))

    combined_components = pd.DataFrame({"peer": peer_score, "absolute": absolute_score, "forward": forward_score}, index=out.index)
    combined_weights = pd.Series({"peer": 0.50, "absolute": 0.30, "forward": 0.20})
    combined_den = combined_components.notna().mul(combined_weights, axis=1).sum(axis=1)
    combined = combined_components.mul(combined_weights, axis=1).sum(axis=1, min_count=1).div(combined_den.where(combined_den.gt(0)))
    combined = (combined - outlier_risk).clip(0, 100).where(combined_den.ge(0.70))
    ...
    confidence_score = np.clip(
        ...
        * official_support * integrity_support * np.where(pd.Series(combined, index=out.index).notna(), 1.0, 0.0),
        0,
        1,
    )
...
__all__.extend(["validation_events_to_memory", "save_memory"]) if "__all__" in globals() else None

...
AI_VERSION = "6.2.0-validated-hybrid-ai"
...
class LocalAIConfig:
    ...
    max_unvalidated_weight: float = 0.05
...
def enrich_profit_ranking_with_ai(
    ...
    are visible in shadow form and receive at most a very small Bayesian weight;
    a model that fails to beat the chronological naive baseline receives zero.
    """
    ...
    for idx, row in result.iterrows():
        ...
        if joint_validated and np.isfinite(joint_brier_skill) and joint_brier_skill > cfg.min_brier_skill:
            ...
            maximum_weight = min(float(cfg.max_unvalidated_weight), float(cfg.max_weight), 0.05)
            performance_support = 0.35
        effective_weight = (
            maximum_weight
            * confidence_score
            * global_support

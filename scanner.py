...
def analyze_portfolio_positions(
    ...
    for _, position in portfolio.iterrows():
        ...
        scanner_setup_ready = scanner_status in {"EXECUTION_READY", "PENDING_DATA"}
...
def download_intraday_ohlcv(
    ...
    The function is deliberately separate from the daily scanner. Provider
    failure returns an audit report and empty histories; it never fabricates
    an intraday signal from daily data.
    """
    import yfinance as yf
    ...
    failed: dict[str, str] = {}
    warnings: dict[str, str] = {}
    for start in range(0, len(requested), max(1, int(batch_size))):
        ...
        try:
            ...
            for ticker in batch:
                ...
                if not frame.empty:
                    ...
                    failed[ticker] = "Intraday batch kosong"
        except Exception as exc:
            for ticker in batch:
                failed[ticker] = f"Intraday batch gagal: {type(exc).__name__}"

    # A bounded individual retry avoids losing a whole shortlist because one
    # batch response had an unexpected MultiIndex layout.
    ...
    def retry_one(ticker: str) -> tuple[str, pd.DataFrame, str | None]:
        try:
            ...
            return ticker, clean, None if not clean.empty else "Intraday individual kosong"
        except Exception as exc:
            return ticker, pd.DataFrame(), f"{type(exc).__name__}: {str(exc)[:100]}"
    ...
    report = DownloadReport(
        ...
        failed=failed,
        provider="Yahoo Finance via yfinance (intraday)",
...
def _intraday_session(frame: pd.DataFrame) -> pd.DataFrame:
    if frame is None or frame.empty:
        return pd.DataFrame()
    ...
    return out[pd.Index(out.index).map(lambda x: pd.Timestamp(x).date() == last_date)].copy()


def _intraday_metrics(frame: pd.DataFrame) -> dict[str, float]:
    session = _intraday_session(frame)
    ...
    cum_volume = session["Volume"].cumsum().replace(0, np.nan)
    session_vwap = float((typical * session["Volume"]).cumsum().iloc[-1] / cum_volume.iloc[-1]) if pd.notna(cum_volume.iloc[-1]) else np.nan
    ...
    late_n = min(4, max(1, len(session) // 3))
    ...
    opening_n = min(4, len(session))
    opening_mean = float(session["Volume"].head(opening_n).mean())
    remainder = session["Volume"].iloc[opening_n:]
    remainder_mean = float(remainder.mean()) if len(remainder) else np.nan
    opening_ratio = opening_mean / remainder_mean if np.isfinite(remainder_mean) and remainder_mean > 0 else np.nan
    orb_n = min(2, len(session))
    orb_high = float(session["High"].head(orb_n).max())
    orb_low = float(session["Low"].head(orb_n).min())
    intraday_return = last_close / float(session["Open"].iloc[0]) - 1 if float(session["Open"].iloc[0]) > 0 else np.nan
    return {
        "intraday_bars": float(len(session)),
...
def scan_bsjp_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        im = _intraday_metrics(intraday.get(ticker, pd.DataFrame()))
        has_intraday = im["intraday_bars"] >= 8
        ...
        session = _intraday_session(intraday.get(ticker, pd.DataFrame()))
        ...
        if ready:
            ...
            status = "BSJP_DATA_REQUIRED"
            action = "FETCH_15M_DATA"
        elif not in_window:
    ...
    result = pd.DataFrame(rows)
    if not result.empty:
        rank = {"BSJP_READY": 0, "BSJP_WATCHLIST": 1, "BSJP_DATA_REQUIRED": 2}
...
def scan_bpjs_candidates(
    ...
    now_jkt = _jakarta_timestamp(now)
    minute = now_jkt.hour * 60 + now_jkt.minute
    in_window = now_jkt.weekday() < 5 and (9 * 60 + 15) <= minute <= (10 * 60 + 45)
    rows: list[dict[str, Any]] = []
    for ticker, frame in prepared.items():
        if frame is None or frame.empty or len(frame) < 60:
            continue
        ...
        im = _intraday_metrics(intraday.get(ticker, pd.DataFrame()))
        has_intraday = im["intraday_bars"] >= 3
        session = _intraday_session(intraday.get(ticker, pd.DataFrame()))
        ...
        rows.append({
            ...
            "room_to_ara_pct": room_to_ara,
            "intraday_bars": int(im["intraday_bars"]),
            "execution_window": "09:15–10:45 WIB",
    ...
    result = pd.DataFrame(rows)
    if not result.empty:
        rank = {"BPJS_READY": 0, "BPJS_WATCHLIST": 1, "BPJS_DATA_REQUIRED": 2}
...
def scan_multibagger_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        balance_score = 0.0
        balance_score += 5.0 if np.isfinite(ocf) and ocf > 0 else 0.0
        balance_score += 4.0 if np.isfinite(fcf) and fcf > 0 else 0.0
        balance_score += 3.0 if not np.isfinite(debt_equity) or debt_equity <= 0.8 else 1.0 if debt_equity <= 1.5 else 0.0
        balance_score += 3.0 if not np.isfinite(cash_to_debt) or cash_to_debt >= 0.5 else 1.0 if cash_to_debt >= 0.2 else 0.0
        valuation_score = 0.0
        ...
        if total < 60:
            continue
        if total >= 82 and coverage >= 70 and adtv >= 1_500_000_000 and not severe_flags:
        ...
        rows.append({
            ...
            "red_flags": red_flags,
            "note": "Candidate ranking, not a forecast or guaranteed multiple",
        })
    result = pd.DataFrame(rows)
    if not result.empty:
...
def scan_ara_hunter_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        accumulation, up_down = _silent_accumulation_metrics(frame)
        im = _intraday_metrics(intraday.get(ticker, pd.DataFrame()))
        has_intraday = im["intraday_bars"] >= 6
...
def build_specialty_screens(
    ...
    del market_context  # reserved for future per-strategy regime calibration
    return {
        "sniper": scan_sniper_entries(core_signals, prepared, config=config),
        "bsjp": scan_bsjp_candidates(prepared, intraday=intraday, core_signals=core_signals, config=config, now=now),
        "bpjs": scan_bpjs_candidates(prepared, intraday=intraday, core_signals=core_signals, config=config, now=now),
        "multibagger": scan_multibagger_candidates(prepared, fundamentals, core_signals=core_signals, config=config),
        "ara_hunter": scan_ara_hunter_candidates(prepared, intraday=intraday, core_signals=core_signals, config=config),
    }
...
def _market_status_data_coverage(row: Mapping[str, Any], cfg: ScanConfig) -> float:
    ...
    if quote_verified:
        return max(85.0, component_score)
    if final_ohlcv:
        # This confirms recent trading activity but not every official status
        # page, therefore it cannot be treated as 100% official coverage.
        return max(70.0, component_score)
...
def _quote_data_coverage(row: Mapping[str, Any], cfg: ScanConfig) -> float:
    ...
    data_age = _finite(row.get("absolute_data_age_days"), 999)
    final_ohlcv = data_age <= cfg.max_absolute_data_age_days and not _truthy(row.get("current_bar_incomplete", False))
    return 85.0 if final_ohlcv else 25.0
...
def _layer_data_coverage(row: Mapping[str, Any], cfg: ScanConfig) -> dict[str, float]:
    ...
    core_trade_fields = ("ticker", "quality_score", "entry", "stop_loss", "tp1", "tp2", "rr1", "rr2", "stop_pct")
...
__version__ = "4.2.2-data-completeness-80"
__all__ = list(dict.fromkeys(__all__ + [

...
def _download_ohlcv_v431(tickers: Iterable[str], period: str='3y', batch_size: int=30, itick_api_token: str='') -> tuple[dict[str, pd.DataFrame], DownloadReport]:
    ...
    report = DownloadReport(
        ...
        provider='Database/cache-first: verified cache → Yahoo incremental → IDX official → iTick optional',
    ...
    *,
    ...
    for pos in candidates:
        ...
        if cached_gate is None:
            ...
            # Historical validation only records EXECUTION_READY events. These
            # gates are hard blockers in ``ScanEngine._finalize``; running four
            # full detectors and the accumulation model after one has failed
            # cannot create an event and previously dominated scan CPU.
            if base_blockers or context.regime not in {'RISK_ON', 'NEUTRAL'}:
                ...
                continue
            profile = (
                accumulation_cache.get(int(pos))
            ...
            blockers, metrics = _historical_gate_inputs(
                snapshot,
                config,
        ...
        if blockers or context.regime not in {'RISK_ON', 'NEUTRAL'}:
            continue
        if not profile:
            profile = _silent_accumulation_profile(snapshot)
            if accumulation_cache is not None:
                accumulation_cache[int(pos)] = profile
        if setup == 'REVERSAL_ACCUMULATION':
            plan = detector(snapshot, ticker, accumulation_profile=profile)
        else:
        ...
        event = _simulate_order(
            ...
            context.regime,
            config,
            silent_accumulation_score=_finite(
                metrics.get('silent_accumulation_score'),
                np.nan,
            ),
        )
        events.append(event)
...
class ScanConfig:
    ...
    fundamental_backfill_max_per_scan: int = 40
    ...
    narrative_growth_max_adjustment_points: float = 8.0
    narrative_turnaround_max_adjustment_points: float = 10.0
    narrative_swing_max_adjustment_points: float = 8.0
...
__version__ = '7.6.4-evidence-acquisition'

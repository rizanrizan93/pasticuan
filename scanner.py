...
__version__ = "4.3.2-eod-integrity"

IDX_DAILY_FINAL_HOUR = 16
IDX_DAILY_FINAL_MINUTE = 20
IDX_CACHE_SCHEMA_VERSION = 2
...
def apply_execution_snapshot_gate(
    ...
    for idx, row in out.iterrows():
        ...
        session_not_final = now.weekday() < 5 and not idx_daily_bar_is_final(now)
        if session_not_final or market_state == "REGULAR":
            out.at[idx, "pending_close"] = True
            _append_pipe(out, idx, "evidence_warnings", "Sesi reguler: setup dipertahankan, keputusan direct-order menunggu candle EOD final")
            if np.isfinite(divergence) and divergence > tolerance:

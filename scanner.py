...
def _silent_accumulation_profile(frame: pd.DataFrame) -> dict[str, object]:
    ...
    """
    unavailable = {
        'silent_accumulation_version': 'V4_ADAPTIVE_PERSISTENCE',
    ...
    score = max(0.0, min(100.0, base_score + v3_adjustment + v4_adjustment))
    ...
    elif score >= 75 and distribution_days <= 2 and confidence >= 65 and accumulation_persistence_score >= 52 and (absorption_days >= 1 or churning_days >= 2 or accumulation_days >= 6):
...
__version__ = '7.0.0-database-first-incremental'

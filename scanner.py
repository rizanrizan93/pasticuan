...
def _silent_accumulation_profile(frame: pd.DataFrame) -> dict[str, object]:
    ...
    cmf20_series = money_flow.rolling(20, min_periods=10).sum() / volume.rolling(20, min_periods=10).sum().replace(0, np.nan)
    cmf60_series = money_flow.rolling(60, min_periods=25).sum() / volume.rolling(60, min_periods=25).sum().replace(0, np.nan)
    ...
    obv_slope20_series = obv_local.diff(20) / total_volume20
    adl_slope20_series = adl_local.diff(20) / total_volume20
    typical = (high + low + close) / 3.0
    vwap20 = (typical * volume).rolling(20, min_periods=10).sum() / total_volume20
    total_volume60 = volume.rolling(60, min_periods=25).sum().replace(0, np.nan)
    vwap60 = (typical * volume).rolling(60, min_periods=25).sum() / total_volume60
    ...
    row = recent.iloc[-1]
    cmf20_value = _finite(row.get('CMF20'), _finite(cmf20_series.iloc[-1], np.nan))
    cmf60_value = _finite(row.get('CMF60'), _finite(cmf60_series.iloc[-1], np.nan))
    obv_slope20 = _finite(row.get('OBV_SLOPE20'), _finite(obv_slope20_series.iloc[-1], np.nan))
    adl_slope20 = _finite(row.get('ADL_SLOPE20'), _finite(adl_slope20_series.iloc[-1], np.nan))
    close_value = _finite(close.iloc[-1], 0.0)
    vwap20_value = _finite(row.get('VWAP20'), _finite(vwap20.iloc[-1], np.nan))
    vwap60_value = _finite(row.get('VWAP60'), _finite(vwap60.iloc[-1], np.nan))
    ...
    ema100_value = _finite(close.ewm(span=100, adjust=False).mean().iloc[-1], np.nan)
...
def _historical_gate_inputs(df: pd.DataFrame, config: ScanConfig):
    ...
    profile = _silent_accumulation_profile(df)
...
__version__ = '7.1.2-mixed-datetime-normalization'

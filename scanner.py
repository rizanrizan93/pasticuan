...
def _silent_accumulation_profile(frame: pd.DataFrame) -> dict[str, object]:
    """Return an auditable price-volume accumulation profile.

    This is deliberately a *proxy*, not proof of beneficial-owner activity.
    It combines money-flow persistence, value asymmetry, pullback volume dry-up,
    price acceptance, relative strength and explicit distribution penalties.
    All features are causal and use only bars available at the evaluation date.
    """
    unavailable = {
    ...
    if _finite(vol_ratio.iloc[-1], 0.0) >= 1.6 and _finite(close_location.iloc[-1], 0.5) <= 0.35:
        penalty += 7.0
    score = max(0.0, min(100.0, flow_score + persistence_score + asymmetry_score + dryup_score + acceptance_score + relative_score - penalty))
    zero_volume = float(volume.tail(60).eq(0).mean())
    available = sum(np.isfinite(v) for v in (cmf20_value, cmf60_value, obv_slope20, adl_slope20, up_down20, up_down60, weighted_clv20, pullback_ratio, vwap20_value, vwap60_value, relative, contraction))
    coverage = 100.0 * available / 12.0
    confidence = max(0.0, min(100.0, 0.55 * coverage + 0.45 * min(100.0, len(recent) / 60.0 * 100.0) - 60.0 * zero_volume))
    if distribution_days >= 4 or (cmf20_value < -0.08 and adl_slope20 < 0):
        state = 'DISTRIBUTION_RISK'
    elif score >= 75 and distribution_days <= 2 and confidence >= 65:
        state = 'SILENT_ACCUMULATION_CONFIRMED'
    elif score >= 62 and distribution_days <= 3:
    ...
    if np.isfinite(pullback_ratio): positives.append(f'pullback volume {pullback_ratio:.2f}x')
    negatives = []
    ...
    reason = '; '.join(positives[:5] + negatives[:3]) or 'Price-volume evidence belum cukup.'
    return {
...
__version__ = '6.8.0-eoff-multibagger-accumulation-research'

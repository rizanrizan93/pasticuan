    ...
    def run_batch_download(targets: list[str], incremental: bool) -> None:
        ...
        for start_idx in range(0, len(targets), max(1, int(batch_size))):
            ...
            kwargs: dict[str, Any] = {
                ...
                'repair': False, 'actions': False, 'threads': 4,
    ...
    if unresolved:
        ...
        if yahoo_chart_direct is not None:
            def direct_one(ticker: str):
                ...
                incremental = not cached.empty
                try:
                    direct_timeout = max(4, int(os.environ.get('IDX_SCANNER_YAHOO_TIMEOUT_SECONDS', '8')))
                    direct_retries = max(0, min(2, int(os.environ.get('IDX_SCANNER_YAHOO_RETRY_COUNT', '1'))))
            ...
            direct_workers = max(2, min(12, int(os.environ.get('IDX_SCANNER_YAHOO_MAX_WORKERS', '8'))))
...
def enrich_fundamentals_with_history(fundamentals: pd.DataFrame, history: pd.DataFrame, now: Any | None=None) -> pd.DataFrame:
    ...
    snapshot_score = cached_snapshot_score.combine_first(current_score)
    current_coverage = (
    ...
    snapshot_coverage = cached_snapshot_coverage.combine_first(current_coverage)
...
def download_ohlcv(tickers: Iterable[str], period: str='5y', batch_size: int=30, itick_api_token: str='') -> tuple[dict[str, pd.DataFrame], DownloadReport]:

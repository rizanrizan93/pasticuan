...
def fetch_yahoo_fundamental_history(tickers: Iterable[str], max_workers: int=2, max_tickers: int=40) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Fetch bounded statement history while respecting provider cooldown."""
    ...
    if health:
        ...
        return _fundamental_empty_history(), reports
    ...
    if _is_yahoo_blocking_error(first_code):
        ...
        return normalize_fundamental_history(pd.DataFrame(rows)), pd.DataFrame(reports, columns=report_columns)
    if first_rows:
        _clear_yahoo_provider_unavailable()
...
__version__ = '6.6.6-provider-guard-batch2'

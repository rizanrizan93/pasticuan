...
def _download_ohlcv_v431(tickers: Iterable[str], period: str='3y', batch_size: int=30, itick_api_token: str='') -> tuple[dict[str, pd.DataFrame], DownloadReport]:
    ...
    def run_batch_download(targets: list[str], incremental: bool) -> None:
        ...
        for start_idx in range(0, len(targets), max(1, int(batch_size))):
            ...
            if incremental:
                ...
                overlap_start = min(latest_dates) - pd.Timedelta(days=10)
                kwargs['start'] = overlap_start.strftime('%Y-%m-%d')
                kwargs['end'] = (_expected_last_completed_daily_date() + pd.Timedelta(days=2)).strftime('%Y-%m-%d')
            else:
    ...
    def retry_one(ticker: str) -> tuple[str, pd.DataFrame, str | None, bool]:
        ...
        try:
            if incremental:
                start_date = (pd.Timestamp(cached.index[-1]).normalize() - pd.Timedelta(days=10)).strftime('%Y-%m-%d')
                end_date = (_expected_last_completed_daily_date() + pd.Timedelta(days=2)).strftime('%Y-%m-%d')
...
__version__ = '7.4.0-scipy-statistical-quality-momentum'

...
CORE_PIPELINE_CACHE_VERSION = '7.16.0-persistent-core-indicator-signal-cache'
...
def _silent_accumulation_profile(frame: pd.DataFrame) -> dict[str, object]:
    ...
    ema100_value = _finite(
        row.get('EMA100'),
        _finite(close.ewm(span=100, adjust=False).mean().iloc[-1], np.nan),
    )
    ...
    elif np.isfinite(ema100_value) and close_value >= ema100_value:
...
class ScanConfig:
    ...
    selector_max_ai_weight: float = 0.30
    ...
    narrative_growth_max_adjustment_points: float = 5.0
    narrative_turnaround_max_adjustment_points: float = 7.0
    narrative_swing_max_adjustment_points: float = 5.0
    ...
    time_cycle_core_max_weight: float = 0.04
...
def fetch_resilient_market_status(tickers: Iterable[str], config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    if not current.empty and 'ticker' in current:
        ...
        current = current.sort_values(['ticker', '_route_ok', 'market_status_provider_attempt'], ascending=[True, False, False], kind='stable').drop_duplicates('ticker', keep='first').drop(columns='_route_ok')
    ...
    resolved['market_status_route_state'] = tier.map({'LIVE': 'LIVE_AUTHORITY_VERIFIED', 'CACHE_FALLBACK': 'CACHE_AUTHORITY_FALLBACK'}).fillna('UNAVAILABLE_FAIL_CLOSED')
    resolved['market_status_score_eligible'] = resolved.apply(lambda row: usable(row.to_dict()), axis=1)
    return resolved
...
def fetch_resilient_news_review(tickers: Iterable[str], lookback_days: int=7, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for attempt in range(max(1, cfg.provider_retry_count)):
        if not unresolved:
            break
        frame = fetch_automatic_news_review(sorted(unresolved), lookback_days=lookback_days)
        if frame is None or frame.empty:
            continue
        frame = frame.copy()
    ...
    if not current.empty and 'ticker' in current:
        ...
        current = current.sort_values(['ticker', '_route_ok', 'news_provider_attempt'], ascending=[True, False, False], kind='stable').drop_duplicates('ticker', keep='first').drop(columns='_route_ok')
    elif not current.empty:
        current = pd.DataFrame()
    ...
    tier = resolved.get('evidence_source_tier', pd.Series('UNRESOLVED', index=resolved.index)).fillna('UNRESOLVED').astype(str).str.upper()
    resolved['news_route_state'] = tier.map({'LIVE': 'LIVE_COMPLETE', 'CACHE_FALLBACK': 'CACHE_VERIFIED_FALLBACK'}).fillna('UNAVAILABLE_NOT_SCORED')
    resolved['news_score_eligible'] = resolved.apply(lambda row: usable(row.to_dict()), axis=1)
    return resolved
...
def finalize_execution_integrity(signals: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        for key, value in plan_contract_values.items():
            out.at[idx, key] = value
        if candidate and not hard and not confirmed and not plan_contract_valid:
...
__version__ = '7.16.3-database-write-readback-observability'

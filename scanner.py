...
def _silent_accumulation_profile(frame: pd.DataFrame) -> dict[str, object]:
    """Return Silent Accumulation v3 price-volume evidence.

    The model remains a proxy. It does not identify beneficial owners. Version
    3 preserves the v2 money-flow framework and adds causal absorption,
    effort-versus-result, failed-absorption, lower-wick support and persistent
    bid diagnostics. The final adjustment is bounded so one micro-pattern
    cannot dominate the broader 20/60-day evidence.
    """
    unavailable = {
        'silent_accumulation_version': 'V3_ABSORPTION',
    ...
    score = max(0.0, min(100.0, base_score + v3_adjustment))
    ...
    confidence = max(0.0, min(100.0, 0.55 * coverage + 0.45 * min(100.0, len(recent) / 60.0 * 100.0) - 60.0 * zero_volume))
    if distribution_days >= 4 or failed_absorption_days >= 2 or (cmf20_value < -0.08 and adl_slope20 < 0):
        state = 'DISTRIBUTION_RISK'
    elif score >= 75 and distribution_days <= 2 and confidence >= 65 and (absorption_days >= 1 or churning_days >= 2 or accumulation_days >= 6):
...
__version__ = '6.8.1-research-maintenance-database-ready'

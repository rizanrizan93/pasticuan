...
def fetch_one_fundamental(ticker: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {'ticker': ticker, 'fundamental_score': np.nan, 'fundamental_coverage': 0.0, 'fundamental_reliability': 'NONE', 'fundamental_red_flags': '', 'latest_statement_date': pd.NaT, 'statement_age_days': np.nan, 'fundamental_error_code': classify_provider_error(exc), 'fundamental_error': f'{type(exc).__name__}: {str(exc)[:100]}', 'fundamental_provider': 'Yahoo Finance via yfinance', 'fundamental_fetched_at': pd.Timestamp.now(tz='Asia/Jakarta').isoformat()}
...
def _yahoo_statement_rows(ticker: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    try:
        ...
        return rows, {'ticker': ticker, 'provider': 'YAHOO', 'status': status, 'rows': len(rows), 'error': ' | '.join(errors)}
    except Exception as exc:
        return [], {'ticker': ticker, 'provider': 'YAHOO', 'status': 'FAILED', 'rows': 0, 'error': f'{type(exc).__name__}: {str(exc)[:140]}'}
...
def fetch_yahoo_fundamental_history(tickers: Iterable[str], max_workers: int=2, max_tickers: int=40) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Fetch bounded quarterly/annual statement history from the free baseline."""
    names = list(dict.fromkeys(tickers))[:max(0, int(max_tickers))]
    if not names:
        return _fundamental_empty_history(), pd.DataFrame(columns=['ticker', 'provider', 'status', 'rows', 'error'])
    rows: list[dict[str, Any]] = []
    reports: list[dict[str, Any]] = []
    with ThreadPoolExecutor(max_workers=min(max(1, int(max_workers)), len(names))) as pool:
        futures = {pool.submit(_yahoo_statement_rows, ticker): ticker for ticker in names}
        for future in as_completed(futures):
            statement_rows, report = future.result()
            rows.extend(statement_rows)
            reports.append(report)
    return normalize_fundamental_history(pd.DataFrame(rows)), pd.DataFrame(reports)
...
def fetch_twelve_data_fundamental_history(tickers: Iterable[str], api_key: str, max_tickers: int=20, timeout: int=15) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return normalize_fundamental_history(pd.DataFrame(all_rows)), pd.DataFrame(reports)
...
def enrich_fundamentals_with_history(fundamentals: pd.DataFrame, history: pd.DataFrame, now: Any | None=None) -> pd.DataFrame:
    ...
    out['fundamental_score_snapshot'] = pd.to_numeric(out.get('fundamental_score'), errors='coerce')
    out['fundamental_coverage_snapshot'] = pd.to_numeric(out.get('fundamental_coverage'), errors='coerce').fillna(0.0)
    out['fundamental_score'] = pd.to_numeric(out.get('fundamental_score'), errors='coerce').astype(float)
    out['fundamental_coverage'] = pd.to_numeric(out.get('fundamental_coverage'), errors='coerce').astype(float)
...
def fetch_automatic_independent_prices(tickers: Iterable[str], reference_date: Any | None=None, twelve_data_api_key: str='', itick_api_token: str='', primary_reference: Mapping[str, tuple[Any, float]] | None=None, primary_source_tiers: Mapping[str, str] | None=None, config: ScanConfig | None=None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    covered = _aligned_automatic_tickers(cached, primary_reference, cfg)
    if covered:
    ...
    covered.update(_aligned_automatic_tickers(official, primary_reference, cfg))
    remaining = [ticker for ticker in names if ticker not in covered]
    ...
    if itick_names and _safe_text(itick_api_token):
        ...
        covered.update(_aligned_automatic_tickers(itick, primary_reference, cfg))
        remaining = [ticker for ticker in names if ticker not in covered]
...
_SPECIALTY_EXPORTS = {
    ...
    'scan_ara_hunter_candidates', 'build_specialty_screens', 'build_daily_opportunity_board',
    'parse_orderbook_snapshot_csv', 'apply_ara_external_confirmation',
    '_intraday_metrics',
}
...
__version__ = '5.6.1-safe-signal-semantics'

...
def _write_daily_ohlcv_cache(
    ticker: str, frame: pd.DataFrame, source_family: str = "UNKNOWN"
) -> None:
...
def _expected_last_completed_daily_date(now: Any | None = None) -> pd.Timestamp:
    """Best-effort expected date for the latest *completed* IDX daily bar.

    Before the post-close settlement window, today's still-forming daily candle
    is intentionally excluded. IDX holidays are handled conservatively by the
    provider/cache fallback rather than guessed from a hard-coded calendar.
    """
    stamp = pd.Timestamp(now) if now is not None else pd.Timestamp.now(tz="Asia/Jakarta")
    if stamp.tzinfo is None:
        stamp = stamp.tz_localize("Asia/Jakarta")
    else:
        stamp = stamp.tz_convert("Asia/Jakarta")
    day = stamp.normalize()
    if day.weekday() < 5 and (stamp.hour, stamp.minute) >= (17, 30):
...
def _daily_cache_is_current(frame: pd.DataFrame, now: Any | None = None) -> bool:
    ...
    if clean.empty:
        return False
    latest = pd.Timestamp(clean.index[-1]).normalize()
    return latest >= _expected_last_completed_daily_date(now)
...
def _align_secondary_ohlcv_to_cached(
    ...
    overlap = base[["Close"]].rename(columns={"Close": "base"}).join(
        incoming[["Close"]].rename(columns={"Close": "secondary"}), how="inner"
    ).dropna()
    if len(overlap) < 3:
...
def _itick_period_limit(period: str, interval: str) -> int:
    if interval == "1d":
        return {"2y": 540, "3y": 800, "5y": 1_350}.get(str(period).lower(), 800)
    return 1_000 if interval == "5m" else 500
...
def fetch_itick_ohlcv(
    ...
    for ticker in names:
        if not _reserve_itick_free_call():
            reports.append({
                "ticker": ticker, "status": "RATE_BUDGET_EXHAUSTED", "bars": 0,
                "source": "ITICK_FREE", "error": "Batas internal 4 panggilan/menit tercapai",
            })
            continue
        ...
        try:
            response = requests.get(
                ...
                params={
                    ...
                    "code": code,
                    "kType": ktype,
                    "limit": str(_itick_period_limit(period, interval)),
                },
            ...
            raw = pd.DataFrame(values).rename(columns={
            ...
            reports.append({
                "ticker": ticker, "status": "OK", "bars": len(frame),
                "source": "ITICK_FREE", "error": "",
            })
        except Exception as exc:
            safe_error = str(exc).replace(token, "***")
            reports.append({
                "ticker": ticker, "status": "FAILED", "bars": 0,
                "source": "ITICK_FREE", "error": f"{type(exc).__name__}: {safe_error[:140]}",
            })
...
def _idx_eod_patch_frame(row: pd.Series) -> pd.DataFrame:
    ...
    }], index=[pd.Timestamp(row.get("Date")).normalize()])
    return _clean_ohlcv(frame, strict=True)
...
def download_ohlcv(
    ...
    # A current cache is a previously verified completed EOD series. Reusing it
    # prevents needless full-history downloads and materially lowers rate-limit
    # pressure on a free Streamlit deployment.
    ...
    # Official IDX can cheaply restore the latest completed bar when historical
    # cache exists. It is not used to fabricate a full history from one row.
    if unresolved:
        try:
            official, _ = fetch_idx_official_eod_quotes(
                unresolved,
                reference_date=_expected_last_completed_daily_date(),
                lookback_days=7,
                timeout=8,
            )
        except Exception:
        ...
        if official is not None and not official.empty:
            for ticker, group in official.groupby("ticker", sort=False):
                ...
                warnings[str(ticker)] = (
                    f"Yahoo gagal; bar EOD terakhir ditambal dari IDX resmi (gap kalender {gap_days} hari)"
                )

    unresolved = [ticker for ticker in requested if ticker not in histories]
    if unresolved and str(itick_api_token or "").strip():
        secondary, secondary_report = fetch_itick_ohlcv(
            unresolved,
            api_token=itick_api_token,
            period=period,
            interval="1d",
            max_tickers=len(unresolved),
        )
    ...
    # Last resort: retain stale history for research. Existing execution gates
    # explicitly prevent CACHE_FALLBACK from becoming a direct BUY_LIMIT.
    unresolved = [ticker for ticker in requested if ticker not in histories]
    ...
    report = DownloadReport(
        ...
        sorted(histories),
        failed,
        provider="Free multi-source: cache → Yahoo → IDX official → iTick optional",
...
class ScanConfig:
    ...
    # A second price family is required by the real-money dashboard. v4.2.6
    # retrieves it automatically from official IDX EOD, Google Finance, or an
    # optional server-side Twelve Data key. Missing verification creates
    # READY_FOR_PRICE_VERIFY instead of treating Yahoo plus its cache as two
    # independent sources.
...
def download_intraday_ohlcv(
    ...
    report = DownloadReport(
        ...
        failed=failed,
        provider=f"Free intraday: Yahoo {interval} → iTick optional",
        adjusted_prices=True,
        downloaded_at=pd.Timestamp.now(tz="Asia/Jakarta").isoformat(),
        warnings=warnings,
        source_tiers=source_tiers,
...
def scan_bpjs_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        if ready:
            ...
            action = "RUN_AGAIN_09_20_10_45_WIB"
        else:
            status = "BPJS_WATCHLIST"
...
def build_independent_price_validation(
    ...
    config: ScanConfig | None = None,
    now: Any | None = None,
    primary_source_tiers: Mapping[str, str] | None = None,
) -> pd.DataFrame:
    ...
    for ticker, ticker_data in independent_data.groupby("ticker", sort=False):
        ...
        primary_family = _primary_source_family_from_tier(
            (primary_source_tiers or {}).get(str(ticker), "")
        )
        ...
        for family, group in ticker_data.groupby("independent_source_family", sort=False):
            ...
            same_provider = bool(
                str(family) == "YAHOO"
                or (primary_family != "UNKNOWN" and str(family) == primary_family)
            )
            history_ok = bool(
                not needs_history
                or (
...
# =============================================================================
# v4.2.6 zero-upload automatic evidence orchestration
# =============================================================================

_AUTOMATIC_INDEPENDENT_COLUMNS = [
    "ticker", "Date", "Open", "High", "Low", "Close", "Volume",
    "independent_source", "independent_source_family",
]

_AUTOMATIC_PROVIDER_REPORT_COLUMNS = [
    "provider", "scope", "status", "rows", "asof", "error",
]
...
def _parse_idx_api_date(value: object, fallback: pd.Timestamp) -> pd.Timestamp:
    ...
    epoch_match = re.search(r"/Date\((-?\d{10,13})", text)
    if epoch_match:
...
def _normalize_idx_stock_summary_payload(
    payload: object,
    requested: set[str],
    ...
    for item in raw_rows:
        ...
        rows.append({
            "ticker": ticker,
            "Date": _parse_idx_api_date(item.get("Date"), fallback_date),
            "Open": _external_number(item.get("OpenPrice", item.get("OpeningPrice"))),
            "High": _external_number(item.get("High", item.get("HighestPrice"))),
            "Low": _external_number(item.get("Low", item.get("LowestPrice"))),
            "Close": close,
            "Volume": _external_number(item.get("Volume", item.get("TradedVolume"))),
...
def fetch_idx_official_eod_quotes(
    tickers: Iterable[str],
    ...
    for trade_date in _candidate_idx_summary_dates(reference_date, lookback_days):
        ...
        try:
            ...
            reports.append({
                "provider": "IDX_OFFICIAL_STOCK_SUMMARY",
                "scope": date_text,
                "status": "OK" if found else "EMPTY",
                "rows": len(found),
                "asof": trade_date,
                "error": "" if found else (seed_error or "ticker kandidat tidak ditemukan"),
            })
        except Exception as exc:
            reports.append({
                "provider": "IDX_OFFICIAL_STOCK_SUMMARY",
...
def _google_finance_quote_from_html(html_text: str, ticker: str) -> dict[str, Any]:
    ...
    return {
        ...
        "price": float(price),
        "timestamp": pd.Timestamp(quote_time).tz_convert("Asia/Jakarta").tz_localize(None),
        "currency": currency or "IDR",
        "exchange": exchange or "IDX",
    }
...
def fetch_google_finance_quotes(
    ...
    def one(ticker: str) -> tuple[dict[str, Any] | None, dict[str, Any]]:
        ...
        for _ in range(max(1, int(retry_count))):
            try:
                ...
                row = {
                    ...
                    "Close": quote["price"],
                    "Volume": np.nan,
                    "independent_source": "GOOGLE_FINANCE_PUBLIC_QUOTE",
        ...
        return None, {
            ...
            "rows": 0,
            "asof": pd.NaT,
            "error": last_error or "respons tidak tersedia",
        }
...
def _aligned_automatic_tickers(
    ...
    for ticker, group in data.groupby("ticker", sort=False):
        ...
        tolerance = max(
            config.max_secondary_price_divergence_pct,
            2.0 * idx_tick_size(reference_close) / reference_close,
        )
...
def fetch_automatic_independent_prices(
    tickers: Iterable[str],
    reference_date: Any | None = None,
    twelve_data_api_key: str = "",
    primary_reference: Mapping[str, tuple[Any, float]] | None = None,
    primary_source_tiers: Mapping[str, str] | None = None,
    config: ScanConfig | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    primary_families = {
        ticker: _primary_source_family_from_tier((primary_source_tiers or {}).get(ticker, ""))
        for ticker in names
    }
    official_names = [ticker for ticker in names if primary_families.get(ticker) != "IDX_OFFICIAL"]
    official, official_report = fetch_idx_official_eod_quotes(
        official_names,
        reference_date=reference_date,
        lookback_days=cfg.idx_summary_lookback_days,
    ...
    if skipped_idx:
        report_frames.append(_automatic_provider_report([{
            "provider": "IDX_OFFICIAL_STOCK_SUMMARY",
            "scope": ticker,
            "status": "SKIPPED_SAME_PRIMARY",
            "rows": 0,
            "asof": pd.NaT,
            "error": "Primary OHLCV terakhir berasal dari IDX; gunakan keluarga independen lain",
        } for ticker in skipped_idx]))
    ...
    google, google_report = fetch_google_finance_quotes(
        remaining,
        max_tickers=len(remaining),
        timeout=cfg.automatic_provider_timeout_seconds,
        max_workers=cfg.google_finance_max_workers,
    )
    if not google.empty:
    ...
    if remaining and _safe_text(twelve_data_api_key):
        twelve, twelve_report = fetch_twelve_data_eod(
            remaining,
        ...
        if not twelve_report.empty:
            normalized_report = pd.DataFrame({
                "provider": "TWELVE_DATA",
                "scope": twelve_report.get("ticker", ""),
                "status": twelve_report.get("status", "FAILED"),
                "rows": pd.to_numeric(twelve_report.get("bars", 0), errors="coerce").fillna(0).astype(int),
                "asof": pd.NaT,
                "error": twelve_report.get("error", ""),
            })
            report_frames.append(_automatic_provider_report(normalized_report.to_dict("records")))
    elif remaining:
        report_frames.append(_automatic_provider_report([{
            "provider": "TWELVE_DATA",
            "scope": f"{len(remaining)} unresolved ticker",
            "status": "OPTIONAL_NOT_CONFIGURED",
            "rows": 0,
            "asof": pd.NaT,
            "error": "Fallback berbayar tidak dikonfigurasi; tidak diperlukan bila IDX/Google berhasil",
        }]))

    data = pd.concat(data_frames, ignore_index=True) if data_frames else _empty_automatic_independent_data()
    if not data.empty:
        data = data.drop_duplicates(
            ["ticker", "independent_source_family", "Date"], keep="last"
        ).reset_index(drop=True)
    report = pd.concat(report_frames, ignore_index=True) if report_frames else _automatic_provider_report([])
    return data, report


__version__ = "4.3.1-free-multisource"
__all__ = list(dict.fromkeys(__all__ + [
...
# =============================================================================
# v4.3.2 EOD integrity, calibrated execution funnel, and setup-specific flow
# =============================================================================

__version__ = "4.3.3-premarket-session-fix"
...
def idx_daily_bar_is_final(now: Any | None = None) -> bool:
    ...
    return bool(
        stamp.weekday() < 5
        and (stamp.hour, stamp.minute) >= (IDX_DAILY_FINAL_HOUR, IDX_DAILY_FINAL_MINUTE)
    )
...
def idx_core_waits_for_eod(
    now: Any | None = None,
    market_state: str = "UNKNOWN",
    quote_time: Any | None = None,
    current_bar_incomplete: bool = False,
) -> bool:
    ...
    if pd.notna(parsed_quote):
        ...
        if parsed_quote.date() == stamp.date():
            return True
        # A CLOSED quote from a previous date inside the clock window is most
        # likely a holiday/non-trading day, so yesterday's final EOD remains usable.
        if state in {"CLOSED", "POST"} and parsed_quote.date() < stamp.date():
            return False

    # Within normal market hours an unavailable/unknown quote is treated
    # conservatively as an active session. This avoids issuing direct orders
    # against yesterday's candle while today's bar may still be forming.
...
_load_daily_ohlcv_cache_v431 = _load_daily_ohlcv_cache
...
def _cache_meta_proves_final(frame: pd.DataFrame, now: Any | None = None) -> bool:
    ...
    if pd.notna(written):
        if latest < written.normalize():
            return True
        if latest == written.normalize() and (written.hour, written.minute) >= (
            IDX_DAILY_FINAL_HOUR, IDX_DAILY_FINAL_MINUTE
        ):
            return True
    # Legacy cache without metadata is safe only when its last date is strictly
    # before today. A same-day legacy cache is refreshed because it may contain
    # an intraday partial candle from v4.3.1.
...
_download_ohlcv_v431 = download_ohlcv
...
_download_benchmark_v431 = download_benchmark
...
def _ready_distance_atr_for_setup(setup: str, cfg: ScanConfig) -> float:
    limits = {
        "PULLBACK_CONTINUATION": 0.45,
        "BREAKOUT_RETEST": 0.45,
        "REVERSAL_ACCUMULATION": 0.35,
        "UNICORN_SNIPER_ICT": 0.30,
    }
...
def _tradeability_v432(self: ScanEngine, frame: pd.DataFrame, asof: pd.Timestamp):
    ...
    # Silent accumulation is not universally required. It remains a hard gate
    # only for reversal/sniper in the setup finalizer below.
    blockers = [
        item for item in blockers
        if not str(item).startswith("Silent-accumulation proxy")
    ]
...
_apply_execution_snapshot_gate_v431 = apply_execution_snapshot_gate
...
def apply_execution_snapshot_gate(
    ...
    for idx, row in out.iterrows():
        ...
        wait_for_eod = idx_core_waits_for_eod(
            now=now_jkt,
            market_state=market_state,
            quote_time=row.get("quote_time"),
            current_bar_incomplete=_truthy(row.get("current_bar_incomplete", False)),
        )
        out.at[idx, "execution_session_phase"] = (
            "REGULAR_WAIT_EOD" if wait_for_eod
            else "PREMARKET_PREVIOUS_EOD" if now_jkt.weekday() < 5 and not idx_regular_decision_window(now_jkt) and not idx_daily_bar_is_final(now_jkt)
            else "POST_CLOSE_FINAL_EOD"
        )
        if wait_for_eod:
...
_finalize_execution_integrity_v431 = finalize_execution_integrity
...
def finalize_execution_integrity(signals: pd.DataFrame, config: ScanConfig | None = None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        # Strong flow may promote a candidate whose only remaining failure was
        # the confidence threshold. Weak flow can demote an otherwise marginal
        # pullback/breakout, but it is not a universal hard blocker.
        failures = [x for x in _safe_text(row.get("execution_gate_failures")).split(" | ") if x]
        ...
        if modifier < 0 and adjusted_confidence < cfg.min_execution_confidence and out.at[idx, "status"] == "EXECUTION_READY":
            ...
            out.at[idx, "status"] == "PENDING_DATA"
            and not failures
            and adjusted_confidence >= cfg.min_execution_confidence
            and _truthy(row.get("portfolio_selected", False))
            and not _safe_text(row.get("critical_blockers"))
        ):
...
__all__ = list(dict.fromkeys(__all__ + [
    ...
    "idx_core_waits_for_eod",
    "IDX_DAILY_FINAL_HOUR",
    "IDX_DAILY_FINAL_MINUTE",
    "IDX_REGULAR_DECISION_START_HOUR",
    "IDX_REGULAR_DECISION_START_MINUTE",
...
# =============================================================================
# v4.4.0 Analyst Fusion decision engine
# =============================================================================

__version__ = "4.4.0-analyst-fusion"

# READY_NOT_SELECTED is a valid trade plan that lost the portfolio ranking, not
# a data failure. Keeping it separate prevents a budget constraint from being
# misdiagnosed as PENDING_DATA.
STATUS_ORDER = {
    "EXECUTION_READY": 0,
...
_ANALYST_SOFT_BLOCKER_PREFIXES = (
    "Retest/reclaim/entry trigger belum lengkap",
    "Quality score ",
    "Confirmation retest harus terjadi pada bar terakhir",
    "volume pullback belum kontraksi",
    "relative strength vs IHSG negatif",
    "CMF/OBV belum mendukung",
    "ADX < 18",
)
...
_ANALYST_HARD_BLOCKER_TOKENS = (
    ...
    "Data tertinggal",
    "Data absolut",
    "Daily bar hari ini belum dianggap final",
    "Nilai transaksi bar terakhir sangat rendah",
    "dekat/terkunci ARA",
    "Zona berumur",
    "Masa berlaku setup sudah habis",
    "terlalu jauh",
    "menutup jauh di bawah zona entry",
    "Jarak SL",
    "Level entry/SL tidak valid",
    "fraksi harga IDX",
    "Urutan SL < entry < TP1 < TP2",
    "di luar rentang auto-rejection",
    "Risk/reward di bawah minimum",
    "Regime IHSG RISK_OFF",
    "Regime IHSG tidak dapat diverifikasi",
    "Flow proxy ",
    "Strict Unicorn memerlukan",
    "Base terlalu lebar",
    "Breakout ditolak",
)
...
def _analyst_hard_blockers(row: Mapping[str, Any]) -> list[str]:
    hard: list[str] = []
    for item in _pipe_parts(row.get("blockers")):
        if item.startswith(_ANALYST_SOFT_BLOCKER_PREFIXES):
            continue
        if any(token in item for token in _ANALYST_HARD_BLOCKER_TOKENS):
            hard.append(item)
    critical = _safe_text(row.get("critical_blockers"))
    if critical:
        hard.extend(_pipe_parts(critical) or [critical])
    if _truthy(row.get("validation_critical_blocker", False)):
        hard.append("OOS edge negatif")
    if _truthy(row.get("fundamental_critical_blocker", False)):
        hard.append("Fundamental distress")
    if _truthy(row.get("market_status_critical_blocker", False)):
        hard.append("Status IDX negatif")
    if _truthy(row.get("news_critical_blocker", False)):
        hard.append("Berita material negatif")
    if _truthy(row.get("quote_critical_blocker", False)):
...
def _analyst_setup_mode(row: Mapping[str, Any], cfg: ScanConfig) -> tuple[str, list[str]]:
    """Return the professional order-plan mode and its explanation.

    The strict engine requires a same-bar trigger. The analyst engine also
    accepts a controlled resting limit order when the setup already has a valid
    structure, risk plan, and observable retest/pullback zone. Reversal and ICT
    sniper remain strict because premature limits have asymmetric failure risk.
    """
    setup = _safe_text(row.get("setup"))
    ...
    reasons: list[str] = []
    if technical_strict:
        return "TRIGGER_CONFIRMED", ["Trigger strict core scanner sudah lengkap"]

    # Pullback limit: trend/pullback detector has already validated the long-term
    # trend and value area. Require B+ quality, proximity, and at least neutral
    # flow. This mirrors a professional resting limit order, not a market chase.
    if setup == "PULLBACK_CONTINUATION" and action in {
        "WAIT_PULLBACK_CONFIRMATION", "WAIT_STRICT_FLOW_CONFIRMATION"
    }:
        if quality >= 78.0 and distance <= 0.75 and smart >= 45.0:
            reasons.extend([
                "Uptrend dan pullback value-area sudah terdeteksi",
                "Entry menggunakan resting limit; bukan market chase",
            ])
            return "LIMIT_PULLBACK_ZONE", reasons

    # Breakout retest may be planned as a limit only after the retest itself and
    # its reclaim have already been observed. A breakout that has not retested
    # remains watchlist.
    if setup == "BREAKOUT_RETEST" and action in {
        "WAIT_RETEST", "WAIT_CURRENT_RETEST_CONFIRMATION"
    }:
        retest_seen = "Retest level breakout terdeteksi" in evidence
        reclaim_seen = "Retest ditutup dengan reclaim" in evidence
        if quality >= 78.0 and distance <= 0.65 and retest_seen and reclaim_seen:
            reasons.extend([
                "Breakout, retest, dan reclaim sudah pernah terobservasi",
                "Limit ditempatkan di zona retest yang masih valid",
            ])
            return "LIMIT_BREAKOUT_RETEST", reasons

    # Reversal can use a resting limit only after CHOCH/BOS is explicit and flow
    # is strong. This is intentionally rarer than pullback/breakout.
    if setup == "REVERSAL_ACCUMULATION" and action == "WAIT_RETEST":
        structure_seen = "CHOCH/BOS bullish terkonfirmasi" in evidence
        if quality >= 82.0 and distance <= 0.45 and smart >= 70.0 and structure_seen:
            reasons.extend([
                "CHOCH/BOS dan akumulasi kuat sudah terkonfirmasi",
                "Entry menunggu retest struktur, bukan menebak bottom",
            ])
            return "LIMIT_CHOCH_RETEST", reasons

    return "WATCH_ONLY", reasons


def apply_analyst_fusion_gate(
    signals: pd.DataFrame,
    config: ScanConfig | None = None,
    minimum_score: float = 80.0,
) -> pd.DataFrame:
    """Build a deterministic professional-analyst decision layer.

    This is not an embedded language model. It encodes the same hierarchy used
    in a professional review: market structure first, then execution mechanics,
    smart-money proxy, context, fundamentals, and evidence reliability. Missing
    optional providers reduce conviction; explicit adverse evidence still blocks.
    """
    cfg = config or ScanConfig()
    if signals.empty:
        return signals.copy()
    out = signals.copy()
    for column, default in (
        ("analyst_pre_budget_ready", False),
        ("analyst_fusion_score", 0.0),
        ("analyst_order_mode", "WATCH_ONLY"),
    ...
    for idx, row in out.iterrows():
        mode, mode_reasons = _analyst_setup_mode(row, cfg)
        quality = max(0.0, min(100.0, _finite(row.get("quality_score"), 0.0)))
        risk_conf = _risk_layer_confidence(row, cfg)
        smart = max(0.0, min(100.0, _finite(row.get("silent_accumulation_score"), 50.0)))
        market_conf = max(0.0, min(100.0, _finite(row.get("market_status_confidence"), 55.0)))
        news_conf = max(0.0, min(100.0, _finite(row.get("news_confidence"), 52.0)))
        fundamental_conf = max(0.0, min(100.0, _finite(row.get("fundamental_confidence"), 50.0)))
        validation_conf = max(0.0, min(100.0, _finite(row.get("validation_confidence"), 45.0)))
        quote_conf = max(0.0, min(100.0, _finite(row.get("quote_confidence"), 68.0)))
        universe_conf = max(0.0, min(100.0, _finite(row.get("universe_confidence"), 62.0)))
        distance = _finite(row.get("distance_atr"), 999.0)
        setup = _safe_text(row.get("setup"))
        regime = _safe_text(row.get("market_regime")).upper()
        source_tier = _safe_text(row.get("ohlcv_source_tier")).upper()
        final_bar = not _truthy(row.get("current_bar_incomplete", False))
        fresh_daily = bool(
            source_tier not in {"CACHE_FALLBACK", "UNAVAILABLE", ""}
            and _finite(row.get("absolute_data_age_days"), 999.0) <= cfg.max_absolute_data_age_days
            and final_bar
        )
        hard = _analyst_hard_blockers(row)
        if regime in {"RISK_OFF", "UNKNOWN"}:
            hard.append(f"Market regime {regime or 'UNKNOWN'}")
        if not fresh_daily:
            hard.append("OHLCV final/live tidak memadai")
        hard = list(dict.fromkeys(hard))

        # Trigger/order-mode quality. Resting limits are discounted relative to
        # an already-confirmed strict trigger.
        mode_score = {
            "TRIGGER_CONFIRMED": 100.0,
            "LIMIT_PULLBACK_ZONE": 84.0,
            "LIMIT_BREAKOUT_RETEST": 87.0,
            "LIMIT_CHOCH_RETEST": 86.0,
            "WATCH_ONLY": 35.0,
        }.get(mode, 35.0)
        distance_score = max(0.0, 100.0 - 45.0 * max(0.0, distance)) if np.isfinite(distance) else 0.0
        strict_flow_setup = setup in {"REVERSAL_ACCUMULATION", "UNICORN_SNIPER_ICT"}
        flow_weight = 0.12 if strict_flow_setup else 0.08
        technical_weight = 0.24 if strict_flow_setup else 0.28
        # The weights sum to 1.0. For reversal/sniper, 4% moves from generic
        # technical quality to flow confirmation. Fundamental/news/OOS remain
        # soft; distress or demonstrated negative edge is a hard blocker.
        weights = {
            "technical": (quality, technical_weight),
            "order_mode": (mode_score, 0.20),
            "risk": (risk_conf, 0.18),
            "distance": (distance_score, 0.08),
            "flow": (smart, flow_weight),
            "market": (market_conf, 0.07),
            "quote": (quote_conf, 0.04),
            "fundamental": (fundamental_conf, 0.03),
            "validation": (validation_conf, 0.02),
            "news": (news_conf, 0.01),
            "universe": (universe_conf, 0.01),
        }
        score = round(sum(value * weight for value, weight in weights.values()), 1)
        strict = bool(
            _truthy(row.get("technical_setup_ready", False))
            and _truthy(row.get("independent_price_verified", False))
            and _finite(row.get("fundamental_coverage"), 0.0) >= cfg.min_direct_fundamental_coverage
        )
        candidate = bool(
            mode != "WATCH_ONLY"
            and not hard
        ...
        basis = list(mode_reasons)
        basis.append(f"Analyst Fusion {score:.1f}/100")
        if not _truthy(row.get("independent_price_verified", False)):
            basis.append("Harga akhir wajib dicocokkan di Stockbit sebelum submit")
        if _finite(row.get("fundamental_coverage"), 0.0) < cfg.min_direct_fundamental_coverage:
            basis.append("Fundamental parsial diperlakukan sebagai soft evidence")
        if hard:
            basis.append("Hard block: " + "; ".join(hard))
        ...
        out.at[idx, "strict_execution_ready"] = strict
        out.at[idx, "analyst_candidate_reason"] = (
            "PASS" if candidate else "WATCH_ONLY" if mode == "WATCH_ONLY"
            else "HARD_BLOCK" if hard else "RISK_OR_SCORE_BELOW_THRESHOLD"
        )
    return out


def enforce_analyst_portfolio_budget(
    signals: pd.DataFrame,
    config: ScanConfig | None = None,
    current_positions: int = 0,
    current_open_risk_idr: float = 0.0,
    ...
    """Allocate portfolio slots from Analyst Fusion candidates.

    Unlike the old gate, an unselected valid plan becomes READY_NOT_SELECTED,
    never PENDING_DATA. This removes the circular dependency between evidence
    status and portfolio allocation.
    """
    cfg = config or ScanConfig()
    if signals.empty:
        return signals.copy()
    ...
    out["execution_rank"] = np.nan
    max_risk = cfg.account_size_idr * cfg.max_portfolio_risk_pct
    remaining_risk = max(0.0, max_risk - max(0.0, float(current_open_risk_idr)))
    if cash_on_hand_idr is None:
        remaining_cash = max(0.0, cfg.account_size_idr - max(0.0, float(current_invested_idr)))
    else:
        remaining_cash = max(0.0, float(cash_on_hand_idr))
    slots = max(0, int(cfg.max_positions) - max(0, int(current_positions)))
    candidates = out.index[out["analyst_pre_budget_ready"].map(_truthy)].tolist()
    if not candidates:
        out["portfolio_remaining_risk_idr"] = remaining_risk
        out["portfolio_remaining_cash_idr"] = remaining_cash
        return out
    sort_cols = [
        column for column in (
            "analyst_fusion_score", "quality_score", "silent_accumulation_score",
            "rr2", "adtv20_idr"
        ) if column in out
    ]
    ranked = out.loc[candidates].sort_values(sort_cols, ascending=False, na_position="last")
    ...
    for idx, row in ranked.iterrows():
        ...
        if ticker in selected_tickers:
            reason = "Setup alternatif ticker yang sama"
        elif rank >= slots:
            reason = "Slot posisi portofolio sudah penuh"
        elif not np.isfinite(risk) or risk <= 0 or risk > remaining_risk:
            reason = "Risiko agregat portofolio melampaui batas"
        elif not np.isfinite(capital) or capital <= 0 or capital > remaining_cash:
            reason = "Cash on hand tidak cukup"
        if reason:
            _append_pipe(out, idx, "portfolio_blockers", reason)
            continue
...
_finalize_execution_integrity_v433 = finalize_execution_integrity
...
def finalize_execution_integrity(signals: pd.DataFrame, config: ScanConfig | None = None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        # v4.3.3 used a blanket PENDING_CLOSE override for every detected setup.
        # Only actionable candidates should wait for EOD; ordinary watchlist rows
        # retain their actual technical status.
        if pending_close:
        ...
        if candidate and selected and not hard:
            ...
            if _truthy(row.get("requires_stockbit_price_check", True)):
                _append_pipe(
                    out, idx, "evidence_warnings",
                    "Sebelum submit, cocokkan harga terakhir dan bid/offer di Stockbit; provider independen otomatis belum terverifikasi",
                )
            ...
            # Preserve stricter adverse status from the base engine.
...
# =============================================================================
# v4.5.0 Signal-First + ARA Hunter calibration
# =============================================================================

__version__ = "4.5.0-signal-first"

# Preserve the v4.4 implementations for targeted extension.
_intraday_metrics_v440 = _intraday_metrics
_apply_analyst_fusion_gate_v440 = apply_analyst_fusion_gate
_finalize_execution_integrity_v440 = finalize_execution_integrity
...
def _intraday_metrics(
    ...
    if session_date_text:
        try:
            ...
            after_final = (reference.hour, reference.minute) >= (
                IDX_DAILY_FINAL_HOUR, IDX_DAILY_FINAL_MINUTE
            )
            premarket = (reference.hour, reference.minute) < (
                IDX_REGULAR_DECISION_START_HOUR, IDX_REGULAR_DECISION_START_MINUTE
            )
            complete = bool(
                session_date < today
                or (session_date == today and after_final)
                or (reference.weekday() >= 5 and session_date <= today)
                or (premarket and session_date < today)
            )
        except Exception:
            complete = False
...
def _specialty_hard_context_map(signals: pd.DataFrame | None) -> dict[str, str]:
    ...
    if signals is None or signals.empty or "ticker" not in signals:
        return result
    hard_tokens = (
        "SUSPENS", "FCA", "FULL CALL AUCTION", "PENGHENTIAN SEMENTARA",
        "DATA ABSOLUT", "OHLCV FINAL/LIVE TIDAK MEMADAI", "KONFLIK QUOTE",
        "KONFLIK HARGA", "UNAVAILABLE",
    )
    for ticker, group in signals.groupby("ticker"):
        messages: list[str] = []
        for _, row in group.iterrows():
            candidates = [
                ...
                _safe_text(row.get("analyst_hard_blockers")),
            ]
            if _truthy(row.get("market_status_critical_blocker", False)):
...
def specialty_intraday_shortlist(
    ...
    """Build a balanced shortlist that explicitly reserves capacity for ARA momentum.

    v4.4 ranked mostly by liquidity, so fast movers could be omitted before
    intraday data was even requested. The new shortlist merges liquid/core names
    with a dedicated momentum bucket.
    """
    rows: list[dict[str, Any]] = []
    core_tickers: set[str] = set()
    if core_signals is not None and not core_signals.empty and "ticker" in core_signals:
    ...
    for ticker, frame in prepared.items():
        ...
        breakout = close > _finite(row.get("HIGH20_PREV"), float("inf"))
        liquid_score = 0.0
        liquid_score += min(35.0, 35.0 * adtv / 5_000_000_000.0) if adtv > 0 else 0.0
        liquid_score += min(20.0, 10.0 * max(0.0, vol_ratio - 0.5))
        liquid_score += 10.0 if close_location >= 0.70 else 5.0 if close_location >= 0.55 else 0.0
        liquid_score += 10.0 if cmf_v > 0.05 else 5.0 if cmf_v > 0 else 0.0
        liquid_score += 8.0 if rs_v > 0 else 0.0
        liquid_score += 7.0 if breakout else 0.0
        liquid_score += 5.0 if ticker in core_tickers else 0.0
        ara_score = 0.0
        ara_score += min(30.0, max(0.0, (vol_ratio - 1.0) * 15.0))
        ara_score += 20.0 if close_location >= 0.85 else 12.0 if close_location >= 0.70 else 0.0
    ...
    total = max(1, int(max_candidates))
    momentum_slots = max(10, min(total // 2, 45))
    liquid_slots = max(1, total - momentum_slots)
    liquid = ranked.sort_values(["liquid_score", "adtv"], ascending=[False, False]).head(liquid_slots)
    momentum = ranked.sort_values(["ara_score", "adtv"], ascending=[False, False]).head(momentum_slots)
    merged = list(dict.fromkeys(liquid["ticker"].tolist() + momentum["ticker"].tolist()))
    if len(merged) < total:
        remainder = ranked.sort_values(["liquid_score", "ara_score", "adtv"], ascending=False)
        merged.extend([ticker for ticker in remainder["ticker"].tolist() if ticker not in merged])
    return merged[:total]
...
def scan_ara_hunter_candidates(
    ...
    """Detect pre-ARA momentum and today's confirmed ARA without account-risk gates.

    ARA Hunter is a signal screen, not a portfolio allocator. High volatility,
    wide stops, regime, and account size are disclosed but do not suppress a
    technically valid signal. Suspensions/FCA and unusable market data remain
    hard blockers.
    """
    cfg = config or ScanConfig()
    intraday = intraday or {}
    ...
    for ticker, frame in prepared.items():
        ...
        intraday_usable = bool(
            im["intraday_bars"] >= 6
            and (_truthy(im.get("intraday_fresh")) or _truthy(im.get("intraday_session_complete")))
        )
        intraday_confirm = bool(
            intraday_usable
            and _finite(im["intraday_last"], 0.0) >= _finite(im["session_vwap"], float("inf"))
            and _finite(im["late_volume_acceleration"], 0.0) >= 1.10
            and _finite(im["session_close_location"], 0.0) >= 0.70
        )
        near_or_locked = near_upper_auto_rejection(prev_close, close, _finite(row.get("High"), close))
        ara_confirmed_today = bool(
            near_or_locked and daily_return >= 0.88 * idx_ara_pct(prev_close)
        )
        context_blocker = critical_map.get(ticker, "")

        score = 0.0
        score += 20.0 if vol_ratio >= 3.0 else 16.0 if vol_ratio >= 2.0 else 10.0 if vol_ratio >= 1.4 else 0.0
        score += 12.0 if value_today >= 20_000_000_000 else 9.0 if value_today >= 8_000_000_000 else 5.0 if value_today >= 1_000_000_000 else 0.0
        score += 14.0 if close_location >= 0.90 else 10.0 if close_location >= 0.78 else 5.0 if close_location >= 0.65 else 0.0
        score += 11.0 if body_atr >= 1.0 else 8.0 if body_atr >= 0.65 else 4.0 if body_atr >= 0.45 else 0.0
        score += 12.0 if cmf_v >= 0.10 and obv_up else 8.0 if cmf_v > 0 or obv_up else 0.0
        score += 10.0 if breakout20 else 5.0 if daily_return >= 0.04 else 0.0
        score += 9.0 if 0.03 <= daily_return <= 0.20 else 5.0 if 0.015 <= daily_return <= 0.25 else 0.0
        score += 5.0 if 52 <= rsi_v <= 82 and 50 <= mfi_v <= 88 else 0.0
        score += 4.0 if accumulation >= 70 else 2.0 if accumulation >= 55 else 0.0
        score += 8.0 if intraday_confirm else 3.0 if _truthy(im.get("intraday_session_complete")) else 0.0
        if opening_gap > 0.12:
            score -= 8.0
        if room < 0.02 and not ara_confirmed_today:
            score -= 8.0
        if context_blocker:
        ...
        daily_valid = bool(
            value_today >= 1_000_000_000.0
            and vol_ratio >= 1.35
            and close_location >= 0.68
            and body_atr >= 0.45
            and (cmf_v > 0 or obv_up)
            and (breakout20 or daily_return >= 0.035)
            and 0.012 <= daily_return <= 0.25
            and opening_gap <= 0.12
            and not context_blocker
        )
        ready_gate = bool(
            daily_valid
            and adtv >= 500_000_000.0
            and value_today >= 2_000_000_000.0
            and vol_ratio >= 1.50
        ...
        if ara_confirmed_today and not context_blocker:
            status = "ARA_CONFIRMED_TODAY"
            signal_valid = True
            instruction = "OBSERVE_NEXT_BASE_NO_CHASE"
        elif ready_gate:
            status = "ARA_HUNTER_READY"
            signal_valid = True
            instruction = "BUY_LIMIT_USER_SIZE"
        elif daily_valid and score >= 62.0:
            status = "ARA_HUNTER_CANDIDATE"
            signal_valid = True
            instruction = "WAIT_INTRADAY_TRIGGER"
        elif score >= 52.0:
            status = "ARA_HUNTER_WATCHLIST"
            signal_valid = False
        ...
        if stop >= entry:
            stop = round_idx_price(entry - 3 * idx_tick_size(entry), "down")
        risk_pct = (entry - stop) / entry if entry > 0 else np.nan
        risk_grade = "VERY_HIGH" if ara_confirmed_today or risk_pct >= 0.08 else "HIGH"
        warnings: list[str] = []
        if market_regime in {"RISK_OFF", "UNKNOWN"}:
            warnings.append(f"Regime {market_regime}; tidak memblokir signal-first")
        if not intraday_usable:
            warnings.append("Intraday tidak tersedia/final; keputusan memakai daily momentum")
        if near_or_locked:
            warnings.append("Sudah dekat/terkunci ARA; jangan market chase")
        rows.append({
            "ticker": ticker,
            "ara_hunter_status": status,
            ...
            "up_down_value_ratio20": up_down,
            "intraday_confirmed": intraday_confirm,
            "intraday_session_complete": _truthy(im.get("intraday_session_complete")),
            "intraday_data_state": im["intraday_data_state"],
            "intraday_session_date": im["intraday_session_date"],
            "intraday_age_minutes": im["intraday_age_minutes"],
            "market_regime": market_regime,
            ...
            "account_risk_gate_applied": False,
            "risk_class": risk_grade,
    ...
    if not result.empty:
        rank = {
            "ARA_HUNTER_READY": 0,
            "ARA_CONFIRMED_TODAY": 1,
            "ARA_HUNTER_CANDIDATE": 2,
            "ARA_HUNTER_WATCHLIST": 3,
        }
        result["_rank"] = result["ara_hunter_status"].map(rank).fillna(9)
        result = result.sort_values(
            ["_rank", "ara_hunter_score", "value_today_idr"],
...
def _analyst_setup_mode(row: Mapping[str, Any], cfg: ScanConfig) -> tuple[str, list[str]]:
    """Signal-first order mode: direct detector triggers remain valid regardless of account risk."""
    setup = _safe_text(row.get("setup"))
    action = _safe_text(row.get("action"))
    evidence = _safe_text(row.get("evidence"))
    quality = _finite(row.get("quality_score"), 0.0)
    ...
    if setup == "PULLBACK_CONTINUATION" and action in {
        "WAIT_PULLBACK_CONFIRMATION", "WAIT_STRICT_FLOW_CONFIRMATION"
    } and quality >= 75.0 and distance <= 0.90:
        return "LIMIT_PULLBACK_ZONE", ["Struktur uptrend dan value-area pullback masih valid"]
    if setup == "BREAKOUT_RETEST" and action in {
        "WAIT_RETEST", "WAIT_CURRENT_RETEST_CONFIRMATION"
    }:
        retest_seen = "Retest level breakout terdeteksi" in evidence
        reclaim_seen = "Retest ditutup dengan reclaim" in evidence
        if quality >= 75.0 and distance <= 0.80 and retest_seen and reclaim_seen:
            return "LIMIT_BREAKOUT_RETEST", ["Breakout-retest-reclaim sudah terobservasi"]
    if setup == "REVERSAL_ACCUMULATION" and action == "WAIT_RETEST":
        structure_seen = "CHOCH/BOS bullish terkonfirmasi" in evidence
        if quality >= 78.0 and distance <= 0.60 and smart >= 60.0 and structure_seen:
            return "LIMIT_CHOCH_RETEST", ["CHOCH/BOS dan akumulasi telah terkonfirmasi"]
    return "WATCH_ONLY", []


def _signal_first_hard_blockers(row: Mapping[str, Any], cfg: ScanConfig) -> list[str]:
    """Keep only structural/data/trading impossibilities as hard blockers."""
    hard: list[str] = []
    ...
    action = _safe_text(row.get("action"))
    if action in {"TOO_EXTENDED_WAIT_NEW_BASE", "NO_SETUP"}:
        hard.append("Setup tidak berada pada entry state yang valid")
    blockers = _pipe_parts(row.get("blockers"))
    true_invalid_tokens = (
        "Riwayat hanya", "Data absolut", "Masa berlaku setup sudah habis",
        "Harga sudah menutup jauh di bawah zona entry", "Level entry/SL tidak valid",
        "fraksi harga IDX", "Urutan SL < entry < TP1 < TP2", "Breakout ditolak",
        "Zona berumur", "Jarak ke zona tidak dapat dihitung",
    )
    ...
    if _truthy(row.get("current_bar_incomplete", False)) or _truthy(row.get("pending_close", False)):
...
def apply_analyst_fusion_gate(
    ...
    """Promote valid setups without cash, lot, account-risk, or portfolio gates."""
    cfg = config or ScanConfig()
    if signals.empty:
        return signals.copy()
    out = _apply_analyst_fusion_gate_v440(signals, cfg, minimum_score=minimum_score)
    for column, default in (
        ("account_risk_gate_applied", False),
        ("signal_risk_grade", "NORMAL"),
        ("signal_risk_warnings", ""),
        ("setup_valid_signal", False),
    ):
        if column not in out:
            out[column] = default
    for idx, row in out.iterrows():
        ...
        smart = max(0.0, min(100.0, _finite(row.get("silent_accumulation_score"), 50.0)))
        distance = _finite(row.get("distance_atr"), 999.0)
        distance_score = max(0.0, 100.0 - 35.0 * max(0.0, distance)) if np.isfinite(distance) else 0.0
        mode_score = {
            "TRIGGER_CONFIRMED": 100.0,
            "LIMIT_PULLBACK_ZONE": 86.0,
            "LIMIT_BREAKOUT_RETEST": 88.0,
            "LIMIT_CHOCH_RETEST": 86.0,
            "WATCH_ONLY": 30.0,
        }.get(mode, 30.0)
        market_conf = max(0.0, min(100.0, _finite(row.get("market_status_confidence"), 55.0)))
        ...
        validation_conf = max(0.0, min(100.0, _finite(row.get("validation_confidence"), 45.0)))
        score = round(
            quality * 0.45 + mode_score * 0.25 + distance_score * 0.10
            + smart * 0.10 + market_conf * 0.04 + quote_conf * 0.03
            + fundamental_conf * 0.02 + validation_conf * 0.01,
            1,
        )
        candidate = bool(mode != "WATCH_ONLY" and not hard and score >= float(minimum_score))
        risk_grade, risk_flags = _risk_disclosure(row, cfg)
        basis = reasons + [f"Signal-First score {score:.1f}/100"]
        if risk_flags:
...
def enforce_analyst_portfolio_budget(
    ...
    sort_cols = [
        c for c in ("analyst_fusion_score", "quality_score", "silent_accumulation_score", "rr2", "adtv20_idr")
        if c in out
    ]
...
# v4.5 final override: sizing remains an optional calculator and can never
# downgrade a valid setup or add an account blocker.
def attach_position_sizing(signals: pd.DataFrame, config: ScanConfig | None = None) -> pd.DataFrame:
    cfg = config or ScanConfig()
    if signals.empty:
        return signals.copy()
    ...
    for _, row in out.iterrows():
        sized_rows.append(
            size_stockbit_order(
                _finite(row.get("entry"), np.nan),
                _finite(row.get("stop_loss"), np.nan),
                cfg,
            )
        )
    sizing = pd.DataFrame(sized_rows, index=out.index)
...
__all__ = list(dict.fromkeys(__all__ + ["attach_position_sizing"]))

# =============================================================================
# v4.6.0 ARA Intelligence
# Separate PRE-ARA prediction from next-session ARA continuation.  Real broker
# summary / market-depth remain optional evidence; automatic decisions use
# explicit OHLCV microstructure proxies and never claim to observe the queue.
# =============================================================================

__version__ = "4.6.0-ara-intelligence"
...
def _safe_ratio(numerator: Any, denominator: Any, default: float = np.nan) -> float:
    num = _finite(numerator, np.nan)
    den = _finite(denominator, np.nan)
    if not np.isfinite(num) or not np.isfinite(den) or den == 0:
        return default
    return float(num / den)


def _ara_intraday_microstructure(
    frame: pd.DataFrame,
    ara_price: float,
    now: Any | None = None,
) -> dict[str, Any]:
    ...
    empty = {
        "proxy_intraday_bars": 0,
        "proxy_signed_volume_imbalance": np.nan,
        "proxy_clv_pressure": np.nan,
        "proxy_vwap_hold_ratio": np.nan,
        "proxy_late_buy_imbalance": np.nan,
        "proxy_late_volume_share": np.nan,
        "proxy_directional_efficiency": np.nan,
        "proxy_ara_lock_ratio": np.nan,
        "proxy_final_lock_bars": 0,
        "proxy_relock_count": 0,
        "proxy_max_unlock_pct": np.nan,
        "proxy_range_compression": np.nan,
        "orderflow_proxy_score": np.nan,
        "queue_proxy_score": np.nan,
    }
    ...
    def positive_component(value: float, weak: float, strong: float) -> float:
        if not np.isfinite(value):
            return 45.0
        if value <= weak:
            return max(0.0, 50.0 * (value - (-1.0)) / max(weak + 1.0, 1e-9))
        if value >= strong:
            return 100.0
        return 50.0 + 50.0 * (value - weak) / max(strong - weak, 1e-9)

    flow_score = _clip_score(
        0.25 * positive_component(signed_imbalance, 0.00, 0.45)
        + 0.20 * positive_component(clv_pressure, 0.00, 0.65)
        + 0.20 * max(0.0, min(100.0, vwap_hold * 100.0))
        + 0.20 * positive_component(_finite(late_buy_imbalance, 0.0), 0.00, 0.50)
        + 0.15 * positive_component(directional_efficiency, 0.00, 0.60)
    )
    ...
    queue_score = _clip_score(
        35.0 * lock_ratio
        + 20.0 * min(1.0, final_lock_bars / 5.0)
        + 15.0 * min(1.0, relock_count / 3.0)
        + 15.0 * unlock_component / 100.0
        + 15.0 * compression_component / 100.0
    )

    return {
        "proxy_intraday_bars": int(len(session)),
        "proxy_signed_volume_imbalance": signed_imbalance,
        "proxy_clv_pressure": clv_pressure,
        "proxy_vwap_hold_ratio": vwap_hold,
        "proxy_late_buy_imbalance": late_buy_imbalance,
        "proxy_late_volume_share": late_share,
        "proxy_directional_efficiency": directional_efficiency,
        "proxy_ara_lock_ratio": lock_ratio,
        "proxy_final_lock_bars": int(final_lock_bars),
        "proxy_relock_count": relock_count,
        "proxy_max_unlock_pct": max_unlock_pct,
        "proxy_range_compression": range_compression,
        "orderflow_proxy_score": round(flow_score, 1),
        "queue_proxy_score": round(queue_score, 1),
    }
...
def _ara_history_profile(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    result = {
        "historical_ara_events": 0,
        "historical_next_day_positive_rate": np.nan,
        "historical_next_day_strong_rate": np.nan,
        "historical_next_day_ara_rate": np.nan,
        "historical_median_next_day_return": np.nan,
        "historical_pre_ara_samples": 0,
        "historical_pre_ara_hit_rate": np.nan,
    }
    ...
    for i in range(1, len(data) - 1):
        ...
        candidate = bool(
            np.isfinite(vol_ratio) and vol_ratio >= 1.5
            and np.isfinite(close_location) and close_location >= 0.72
            and np.isfinite(body_atr) and body_atr >= 0.45
            and 0.015 <= ret <= 0.25
            and (not np.isfinite(high20) or current_close > high20 or ret >= 0.04)
            and ((np.isfinite(cmf) and cmf > 0) or (np.isfinite(obv) and obv > 0))
        )
        if candidate:
    ...
    if next_returns:
        result.update({
            "historical_ara_events": len(next_returns),
            "historical_next_day_positive_rate": float(np.mean(np.array(next_returns) > 0)),
            "historical_next_day_strong_rate": float(np.mean(next_strong_flags)),
            "historical_next_day_ara_rate": float(np.mean(next_ara_flags)),
            "historical_median_next_day_return": float(np.median(next_returns)),
        })
    if pre_hits:
        result.update({
            "historical_pre_ara_samples": len(pre_hits),
            "historical_pre_ara_hit_rate": float(np.mean(pre_hits)),
        })
    return result
...
def parse_orderbook_snapshot_csv(source: bytes | BinaryIO | pd.DataFrame) -> pd.DataFrame:
    ...
    aliases = {
        "time": "timestamp", "datetime": "timestamp", "asof": "timestamp",
        "bid": "bid_price", "bidprice": "bid_price", "buy_price": "bid_price",
        "bid_volume": "bid_lots", "bid_vol": "bid_lots", "buy_lots": "bid_lots",
        "offer": "offer_price", "ask": "offer_price", "ask_price": "offer_price",
        "offerprice": "offer_price", "sell_price": "offer_price",
        "offer_volume": "offer_lots", "ask_lots": "offer_lots", "sell_lots": "offer_lots",
    }
    ...
    if not required.issubset(frame.columns):
        raise ValueError(
            "Order-book CSV wajib memiliki ticker, timestamp, bid_price, bid_lots, offer_price, dan offer_lots"
        )
    frame["ticker"] = frame["ticker"].map(normalize_idx_ticker)
    ...
    for ticker, group in frame.groupby("ticker", sort=False):
        ...
        signal = (
            "STRONG_BUY_QUEUE" if np.isfinite(imbalance) and imbalance >= 0.35
            else "BUY_QUEUE" if np.isfinite(imbalance) and imbalance >= 0.12
            else "SELL_HEAVY" if np.isfinite(imbalance) and imbalance <= -0.25
            else "BALANCED"
        )
        rows.append({
            "ticker": ticker,
            "orderbook_asof": latest,
            "orderbook_levels": int(len(snapshot)),
            "orderbook_bid_lots": bid_lots,
            "orderbook_offer_lots": offer_lots,
            "orderbook_imbalance": imbalance,
            "orderbook_best_bid": best_bid,
            "orderbook_best_offer": best_offer,
            "orderbook_spread_pct": spread_pct,
            "orderbook_signal": signal,
            "orderbook_note": "Snapshot pengguna; bukan feed real-time otomatis",
        })
    return pd.DataFrame(rows)


def apply_ara_external_confirmation(
    ara: pd.DataFrame,
    ...
    if not broker_summary.empty:
        keep = [c for c in (
            "ticker", "broksum_asof", "broksum_days", "broksum_net_ratio", "broksum_signal",
            "top_net_buy_brokers", "top_net_sell_brokers", "broksum_note"
        ) if c in broker_summary]
...
def scan_ara_hunter_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        daily_score += 12.0 if value_today >= 20_000_000_000 else 9.0 if value_today >= 8_000_000_000 else 5.0 if value_today >= 1_000_000_000 else 0.0
        daily_score += 14.0 if close_location >= 0.90 else 10.0 if close_location >= 0.78 else 5.0 if close_location >= 0.65 else 0.0
        daily_score += 11.0 if body_atr >= 1.0 else 8.0 if body_atr >= 0.65 else 4.0 if body_atr >= 0.45 else 0.0
        daily_score += 12.0 if cmf_v >= 0.10 and obv_up else 8.0 if cmf_v > 0 or obv_up else 0.0
        daily_score += 10.0 if breakout20 else 5.0 if daily_return >= 0.04 else 0.0
        daily_score += 9.0 if 0.03 <= daily_return <= 0.20 else 5.0 if 0.015 <= daily_return <= 0.25 else 0.0
        ...
        # If intraday is unavailable, derive a deliberately lower-confidence
        # flow proxy from daily accumulation metrics rather than fabricating a
        # fresh order-flow reading.
        flow_proxy = _finite(micro.get("orderflow_proxy_score"), np.nan)
        if not np.isfinite(flow_proxy):
            flow_proxy = _clip_score(
                35.0 + 20.0 * max(-1.0, min(1.0, cmf_v))
                + 15.0 * (1.0 if obv_up else 0.0)
                + 15.0 * max(0.0, min(1.0, close_location))
                + 15.0 * max(0.0, min(1.0, _finite(up_down, 1.0) / 2.0))
            )
        ...
        pre_score = _clip_score(0.55 * daily_score + 0.30 * flow_proxy + 0.10 * queue_proxy + 0.05 * pre_hist_score)
        ...
        continuation_daily = _clip_score(
            20.0 * min(1.0, close_location)
            + 18.0 * min(1.0, max(0.0, vol_ratio) / 4.0)
            + 16.0 * (1.0 if breakout20 else 0.4)
            + 16.0 * min(1.0, accumulation / 80.0)
            + 15.0 * (1.0 if body_atr >= 0.65 else 0.55)
            + 15.0 * (1.0 if opening_gap <= 0.10 else 0.25)
        )
        ...
        if np.isfinite(hist_strong) or np.isfinite(hist_positive):
            hist_score = _clip_score(
                20.0 + 50.0 * _finite(hist_strong, 0.0) + 30.0 * _finite(hist_positive, 0.0)
            )
        continuation_score = _clip_score(
            0.30 * continuation_daily + 0.30 * flow_proxy + 0.30 * queue_proxy + 0.10 * hist_score
        )
        if np.isfinite(prior_5d_return) and prior_5d_return >= 0.70:
            continuation_score = max(0.0, continuation_score - 10.0)
        if opening_gap > 0.15:
            continuation_score = max(0.0, continuation_score - 8.0)

        daily_valid = bool(
            value_today >= 750_000_000.0
            and vol_ratio >= 1.25
            and close_location >= 0.65
            and body_atr >= 0.40
            and (cmf_v > -0.03 or obv_up)
            and (breakout20 or daily_return >= 0.025)
            and 0.008 <= daily_return <= 0.28
            and opening_gap <= 0.15
            and not context_blocker
        )
        ...
        payload = {
            ...
            "signal_valid": signal_valid,
            "ara_model_score": round(model_score, 1),
            "ara_hunter_score": round(model_score, 1),
            "daily_momentum_score": round(daily_score, 1),
            "orderflow_proxy_score": round(flow_proxy, 1),
            "queue_proxy_score": round(queue_proxy, 1),
            "proxy_confidence": "HIGH" if intraday_usable else "MEDIUM_LOW",
            "last_price": close,
            "previous_close": prev_close,
            "ara_price": ara_price,
            "room_to_ara_pct": room,
            ...
            "up_down_value_ratio20": up_down,
            "intraday_session_complete": _truthy(im.get("intraday_session_complete")),
            "intraday_data_state": im.get("intraday_data_state"),
            "intraday_session_date": im.get("intraday_session_date"),
            "intraday_age_minutes": im.get("intraday_age_minutes"),
            "market_regime": market_regime,
            ...
            "warnings": " • ".join(warnings),
            "blockers": context_blocker,
            "observed_broker_summary": False,
            "observed_orderbook": False,
        }
        payload.update(micro)
        # Preserve the effective fallback-aware scores.  The raw intraday
        # helper returns NaN when no session is available; those NaNs must not
        # overwrite the daily proxy used by the model.
    ...
    if not result.empty:
        rank = {
            "ARA_CONTINUATION_VERIFIED_ORDERFLOW": 0,
            "PRE_ARA_READY": 1,
            "ARA_CONTINUATION_READY": 2,
            "PRE_ARA_CANDIDATE": 3,
            "ARA_CONTINUATION_CANDIDATE": 4,
            "ARA_CONFIRMED_ONLY": 5,
            "PRE_ARA_WATCHLIST": 6,
        }
        result["_rank"] = result["ara_hunter_status"].map(rank).fillna(9)
        result = result.sort_values(
            ["_rank", "ara_model_score", "value_today_idr"],
            ascending=[True, False, False],
        ).drop(columns="_rank").reset_index(drop=True)
    return result
...
__all__ = list(dict.fromkeys(__all__ + [
    ...
    "_ara_intraday_microstructure",
    "_ara_history_profile",
    "__version__",
]))

# =============================================================================
# v4.7.0 signal calibration: specialty READY repair + complete core action map
# =============================================================================

__version__ = "4.7.0-signal-calibration"
...
def _critical_context_map(signals: pd.DataFrame | None) -> dict[str, str]:
    ...
    if signals is None or signals.empty or "ticker" not in signals:
        return result
    critical_tokens = (
        "SUSPENS", "FCA", "SPECIAL MONITOR", "PEMANTAUAN KHUSUS",
        "KONFLIK QUOTE", "KONFLIK HARGA", "OHLCV TIDAK TERSEDIA",
        "CORPORATE ACTION BELUM DISESUAIKAN",
    )
    for ticker, group in signals.groupby("ticker"):
        messages: list[str] = []
        for _, row in group.iterrows():
...
def _specialty_risk_warnings(
    *,
    ...
    room_to_ara: float = np.nan,
) -> list[str]:
    warnings: list[str] = []
    if market_regime in {"RISK_OFF", "UNKNOWN"}:
        warnings.append(f"Regime IHSG {market_regime}")
    if adtv < cfg.min_adtv_idr:
        warnings.append(f"ADTV rendah Rp{adtv/1e9:.2f} miliar")
...
def scan_bsjp_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        daily_components = {
            ...
            "Volume harian belum menguat": vol_ratio >= 0.90,
            "CMF masih lemah": cmf_v >= -0.03,
            "OBV belum naik": obv_up,
        ...
        session_components = {
            "Harga belum di atas VWAP sesi": above_vwap,
            "Late volume belum akseleratif": late_volume,
            "Close sesi belum dekat high": location_ok,
            "Momentum sesi masih negatif": positive_session,
        }
        ...
        score += 12.0 if vol_ratio >= 1.50 else 8.0 if vol_ratio >= 1.10 else 4.0 if vol_ratio >= 0.90 else 0.0
        ...
        risk = max(entry - stop, idx_tick_size(entry))
        tp1 = round_idx_price(entry + max(1.3 * risk, 0.015 * entry), "up")
        tp2 = round_idx_price(entry + max(2.0 * risk, 0.028 * entry), "up")
        stop_pct = (entry - stop) / entry if entry > 0 else np.nan

        setup_valid = bool(
            in_window and has_intraday and above_vwap
            and session_hits >= 2 and daily_hits >= 4 and score >= 68.0
            and not context_blocker
        )
        if setup_valid:
        ...
        warnings = _specialty_risk_warnings(
            market_regime=market_regime, adtv=adtv, cfg=cfg, stop_pct=stop_pct,
            room_to_ara=room_to_ara,
            conditions={**daily_components, **session_components},
        )
        rows.append({
            "ticker": ticker, "bsjp_status": status, "bsjp_score": round(score, 1),
            "setup_valid": setup_valid, "action": action, "last_price": close,
            "intraday_last": intraday_last, "entry": entry, "stop_loss": stop,
            "morning_tp1": tp1, "morning_tp2": tp2, "risk_pct": stop_pct,
            "daily_return_pct": daily_return, "volume_ratio": vol_ratio,
            "adtv20_idr": adtv, "cmf20": cmf_v, "relative_strength60": rs_v,
            "session_vwap": im.get("session_vwap"),
            "session_close_location": im.get("session_close_location"),
            "late_volume_acceleration": im.get("late_volume_acceleration"),
            "room_to_ara_pct": room_to_ara, "daily_confirmation_count": daily_hits,
            "session_confirmation_count": session_hits,
            "intraday_bars": int(im["intraday_bars"]),
            "intraday_interval_minutes": im["intraday_interval_minutes"],
            "intraday_data_state": im["intraday_data_state"],
            "intraday_session_date": im["intraday_session_date"],
            "intraday_age_minutes": im["intraday_age_minutes"],
            "intraday_fresh": im["intraday_fresh"], "market_regime": market_regime,
            "execution_window": "14:30–15:49 WIB", "exit_window": "09:00–10:00 WIB next session",
            "order_instruction": "BUY_LIMIT_USER_SIZE" if setup_valid else "WATCH_ONLY",
            "account_risk_gate_applied": False, "risk_class": "HIGH_OVERNIGHT_GAP_RISK",
            "warnings": " • ".join(warnings), "blockers": context_blocker,
            **_specialty_sizing(entry, stop, cfg, position_cap=1.0, risk_cap=1.0),
        })

    result = pd.DataFrame(rows)
    if not result.empty:
        rank = {"BSJP_READY": 0, "BSJP_WATCHLIST": 1, "BSJP_WAIT_SESSION_BARS": 2,
                "BSJP_STALE_INTRADAY": 3, "BSJP_DATA_UNAVAILABLE": 4}
        result["_rank"] = result["bsjp_status"].map(rank).fillna(9)
        result = result.sort_values(["_rank", "bsjp_score", "adtv20_idr"], ascending=[True, False, False]).drop(columns="_rank").reset_index(drop=True)
    return result
...
def scan_bpjs_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        location_ok = has_intraday and _finite(im.get("session_close_location"), 0.0) >= 0.60
        ...
        conditions = {
            ...
            "Harga belum di atas VWAP": above_vwap,
            "ORB belum ditembus": orb_break,
            "Opening volume belum menguat": opening_volume,
            "Close sesi belum dekat high": location_ok,
        ...
        setup_valid = bool(
            in_window and has_intraday and above_vwap and orb_break
            and score >= 68.0 and not context_blocker
        )
        if setup_valid:
        ...
        rows.append({
            ...
            "intraday_age_minutes": im["intraday_age_minutes"],
            "intraday_fresh": im["intraday_fresh"], "market_regime": market_regime,
            "execution_window": "09:20–10:45 WIB", "mandatory_exit": "Before regular-market close",
            "order_instruction": "BUY_LIMIT_USER_SIZE" if setup_valid else "WATCH_ONLY",
            "account_risk_gate_applied": False, "risk_class": "HIGH_INTRADAY_EXECUTION_RISK",
            "warnings": " • ".join(warnings), "blockers": context_blocker,
            **_specialty_sizing(entry, stop, cfg, position_cap=1.0, risk_cap=1.0),
        })
    ...
    result = pd.DataFrame(rows)
    if not result.empty:
        rank = {"BPJS_READY": 0, "BPJS_WATCHLIST": 1, "BPJS_WAIT_OPENING_BARS": 2,
                "BPJS_STALE_INTRADAY": 3, "BPJS_DATA_UNAVAILABLE": 4}
        result["_rank"] = result["bpjs_status"].map(rank).fillna(9)
        result = result.sort_values(["_rank", "bpjs_score", "adtv20_idr"], ascending=[True, False, False]).drop(columns="_rank").reset_index(drop=True)
    return result
...
def _analyst_setup_mode(row: Mapping[str, Any], cfg: ScanConfig) -> tuple[str, list[str]]:
    ...
    if setup == "BREAKOUT_RETEST" and action in {"WAIT_RETEST", "WAIT_CURRENT_RETEST_CONFIRMATION"}:
        retest_seen = "Retest level breakout terdeteksi" in evidence
        if quality >= 70.0 and distance <= 1.20 and retest_seen:
    ...
    if setup == "REVERSAL_ACCUMULATION" and action in {"WAIT_RETEST", "WAIT_HIGHER_LOW_AND_FLOW"}:
        ...
        if quality >= 70.0 and distance <= 1.00 and structure_seen and sweep_seen and smart >= 45.0:
    ...
    if setup == "UNICORN_SNIPER_ICT" and action in {"WAIT_FVG_RETRACE", "WAIT_STRICT_UNICORN_CONFLUENCE"}:
        core_smc = all(token in evidence for token in (
            "Sell-side liquidity sweep", "Bullish BOS dengan displacement", "Bullish FVG valid"
        ))
        if quality >= 68.0 and distance <= 1.00 and core_smc:
...
def apply_analyst_fusion_gate(
    ...
    out = signals.copy()
    defaults = {
        "account_risk_gate_applied": False, "signal_risk_grade": "NORMAL",
        "signal_risk_warnings": "", "setup_valid_signal": False,
        "analyst_order_mode": "WATCH_ONLY", "analyst_fusion_score": 0.0,
        "analyst_pre_budget_ready": False, "analyst_hard_blockers": "",
        "analyst_decision_basis": "", "analyst_candidate_reason": "WATCH_ONLY",
        "requires_stockbit_price_check": True,
    }
    for column, default in defaults.items():
        if column not in out:
    ...
    mode_scores = {
        "TRIGGER_CONFIRMED": 100.0, "LIMIT_PULLBACK_ZONE": 88.0,
        "LIMIT_BREAKOUT_RETEST": 88.0, "LIMIT_CHOCH_RETEST": 84.0,
        "LIMIT_FVG_RETRACE": 82.0, "WATCH_ONLY": 25.0,
    }
    for idx, row in out.iterrows():
        mode, reasons = _analyst_setup_mode(row, cfg)
        hard = _signal_first_hard_blockers(row, cfg)
        ...
        score = round(
            quality * 0.52 + mode_scores.get(mode, 25.0) * 0.28
            + distance_score * 0.12 + smart * 0.08,
            1,
        )
...
__all__ = list(dict.fromkeys(__all__ + [
    "scan_bsjp_candidates", "scan_bpjs_candidates", "apply_analyst_fusion_gate",
    "_analyst_setup_mode", "_critical_context_map", "__version__",
]))
...
def specialty_intraday_shortlist(
    ...
    for ticker, frame in prepared.items():
        ...
        liquid = min(35.0, 35.0 * adtv / 5_000_000_000.0) if adtv > 0 else 0.0
        ...
        bsjp_score += 15.0 if close_location >= 0.70 else 8.0 if close_location >= 0.55 else 0.0
        bsjp_score += 10.0 if rs_v > 0 else 4.0 if rs_v >= -0.03 else 0.0
        bsjp_score += 8.0 if -0.03 <= daily_return <= 0.10 else 0.0
        ...
        ara_score += 5.0 if cmf_v > 0 or rs_v > 0 else 0.0
        rows.append({
            "ticker": ticker, "core_score": core_score, "bpjs_score": bpjs_score,
            "bsjp_score": bsjp_score, "ara_score": ara_score, "adtv": adtv,
        })
    ...
    # Round-robin fill prevents overlap among the top buckets from shrinking
    # actual coverage below the requested intraday capacity.
    pointers = {name: 0 for name in buckets}
    ordered_map = {
        name: ranked.sort_values([name, "adtv"], ascending=[False, False])["ticker"].tolist()
        for name in buckets
    }
...
__all__ = list(dict.fromkeys(__all__ + ["specialty_intraday_shortlist"]))

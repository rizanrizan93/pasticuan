...
def _finalize_execution_integrity_v433(signals: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    out['status_rank'] = out['status'].map(STATUS_ORDER).fillna(99)
    return out
STATUS_ORDER = {'EXECUTION_READY': 0, 'ENTRY_PLAN_READY': 1, 'READY_NOT_SELECTED': 2, 'READY_FOR_PRICE_VERIFY': 3, 'PENDING_CLOSE': 4, 'PENDING_DATA': 5, 'WATCHLIST_ENTRY': 6, 'BLOCKED_CONTEXT': 7, 'REJECT': 8}
...
_SPECIALTY_EXPORTS = {
    ...
    'scan_ara_hunter_candidates', 'build_specialty_screens',
...
__version__ = '5.2.0-price-structure-targets'

...
validation, execution-state logic, portfolio analytics, charting, and the stable primitives used by the focused Multibagger/Core Swing module.
"""
...
class SetupPlan:
    ...
    reclaim_trigger_price: float | None = None
    retest_reference_price: float | None = None
    trigger_basis: str = ''
    trigger_valid_until: Any = None
...
def prepare_indicators(df: pd.DataFrame, benchmark: pd.DataFrame | None=None) -> pd.DataFrame:
    ...
    out = out.sort_index()
    # A missing volume observation is not a zero-volume trading day. Preserve
    # the distinction in an audit field, then exclude that bar from every
    # price-volume indicator instead of fabricating neutral/negative flow.
    out['_VOLUME_OBSERVED'] = out['Volume'].notna().astype(float)
    out['VOLUME_DATA_COVERAGE20'] = (
        100.0
        * out['_VOLUME_OBSERVED'].rolling(20, min_periods=1).mean()
    )
    out = out.dropna(subset=['Open', 'High', 'Low', 'Close', 'Volume'])
    out['Volume'] = out['Volume'].clip(lower=0)
    out = out.drop(columns=['_VOLUME_OBSERVED'])
    ...
    out['CMF60'] = cmf(out, 60)
    out['OBV'] = obv(out)
    out['OBV_SLOPE10'] = out['OBV'].diff(10) / out['Volume'].rolling(20).mean().replace(0, np.nan)
    out['OBV_SLOPE20'] = out['OBV'].diff(20) / out['Volume'].rolling(20).sum().replace(0, np.nan)
    money_flow_multiplier = (2.0 * out['Close'] - out['High'] - out['Low']) / (out['High'] - out['Low']).replace(0, np.nan)
    out['ADL'] = (money_flow_multiplier.fillna(0.0) * out['Volume']).cumsum()
    out['ADL_SLOPE20'] = out['ADL'].diff(20) / out['Volume'].rolling(20).sum().replace(0, np.nan)
    ...
    out['VWAP60'] = (typical * out['Volume']).rolling(60).sum() / out['Volume'].rolling(60).sum().replace(0, np.nan)
...
def _plan_prices(
    ...
    *,
    ...
    trigger = _finite(plan.trigger, np.nan)
    entry = _finite(plan.entry, np.nan)
    execution_price = max(entry, trigger) if np.isfinite(trigger) else entry
    if (
        not np.isfinite(execution_price)
        or execution_price <= _finite(plan.stop_loss, np.nan)
        or _finite(plan.tp1, np.nan) <= execution_price
        or _finite(plan.tp2, np.nan) < _finite(plan.tp1, np.nan)
    ):
        plan.invalidated = True
        plan.blockers.append(
            'Urutan execution price/trigger harus memenuhi SL < entry/trigger < TP1 <= TP2'
        )
        plan.reason = (
            'Entry plan dibatalkan karena trigger atau harga eksekusi sudah mencapai/melewati target struktur'
        )
        plan.rr1 = None
        plan.rr2 = None
        return plan
    # For conditional buy-stop plans, entry is the executable trigger price.
    # Entry_low/high retain the observation/retest zone for chart execution.
    if np.isfinite(trigger) and trigger > entry:
        plan.entry = round_idx_price(trigger, 'up')
        execution_price = float(plan.entry)
    risk = execution_price - plan.stop_loss
    if risk <= 0:
        plan.invalidated = True
        plan.blockers.append('Risk distance dari execution price ke structural stop tidak valid')
        plan.reason = 'Entry plan dibatalkan karena risk distance tidak positif'
        plan.rr1 = None
        plan.rr2 = None
        return plan
    plan.rr1 = round((plan.tp1 - execution_price) / risk, 2)
    plan.rr2 = round((plan.tp2 - execution_price) / risk, 2)
    return plan
...
def _detect_breakout_v21(df: pd.DataFrame, ticker: str) -> SetupPlan:
    ...
    # Daily scanner trigger: reclaim the broken resistance by one IDX tick.
    # Do not chase the latest candle high; snapshot/gap gates handle cases where
    # the next session opens materially above this conditional trigger.
    reclaim_trigger = round_idx_price(resistance + idx_tick_size(resistance), 'up')
    if retest_date is not None:
        rpos = int(np.flatnonzero(df.index == retest_date)[-1])
        retest_high = _finite(df.iloc[rpos].get('High'), resistance)
    else:
        retest_high = resistance
    trigger_extension_atr = max(0.0, (reclaim_trigger - resistance) / atr_v)
    if retest_date is not None and confirmation and in_retest_area and trigger_extension_atr <= 0.25:
        raw_entry = reclaim_trigger
        plan.entry_type = 'CONDITIONAL_BUY_STOP_RECLAIM'
        plan.action = 'READY_TRIGGER'
    else:
        raw_entry = reclaim_trigger
        plan.entry_type = 'WAIT_RETEST_THEN_BUY_STOP_RECLAIM'
    ...
    plan.trigger = reclaim_trigger
    plan.reclaim_trigger_price = reclaim_trigger
    plan.retest_reference_price = round_idx_price(resistance, 'nearest')
    plan.trigger_basis = 'BROKEN_55D_RESISTANCE_PLUS_ONE_IDX_TICK'
    plan.trigger_valid_until = plan.valid_until
    plan.trigger_instruction = (
        f'Pasang alert/buy-stop bersyarat di {reclaim_trigger:.0f}; aktif hanya setelah harga masuk zona retest '
        f'{round_idx_price(zone_low, "down"):.0f}-{round_idx_price(zone_high, "up"):.0f}. '
        'Batalkan bila close menembus invalidasi atau harga gap melewati batas eksekusi.'
    )
...
def _detect_reversal_v21(df: pd.DataFrame, ticker: str) -> SetupPlan:
    ...
    choch_trigger = structure_level + idx_tick_size(structure_level)
    if choch and in_zone and confirmation:
        raw_entry = max(close, _finite(row['High']) + idx_tick_size(close))
        choch_trigger = raw_entry
        plan.entry_type = 'BUY_STOP_AFTER_CHOCH'
        plan.action = 'READY_TRIGGER'
    elif choch:
        raw_entry = choch_trigger
        plan.entry_type = 'WAIT_CHOCH_RETEST_THEN_BUY_STOP_RECLAIM'
        plan.action = 'WAIT_RETEST'
    else:
        raw_entry = choch_trigger
        plan.entry_type = 'BUY_STOP_AFTER_CHOCH'
    ...
    plan.entry_low, plan.entry_high = (zone_low, zone_high)
    plan.trigger = choch_trigger
    plan.reclaim_trigger_price = choch_trigger
    plan.trigger_basis = 'CHOCH_STRUCTURE_RECLAIM_PLUS_ONE_IDX_TICK'
    plan.distance_atr = round(_distance_to_zone(close, zone_low, zone_high, atr_v), 2)
...
def _detect_unicorn_v21(df: pd.DataFrame, ticker: str) -> SetupPlan:
    ...
    fvg_reclaim_trigger = zone_high + idx_tick_size(max(zone_high, close))
    if in_zone and confirmation:
        raw_entry = max(close, _finite(row['High']) + idx_tick_size(close))
        fvg_reclaim_trigger = raw_entry
        plan.entry_type = 'BUY_STOP_FVG_RECLAIM'
        plan.action = 'READY_TRIGGER'
    else:
        raw_entry = fvg_reclaim_trigger
        plan.entry_type = 'WAIT_FVG_RETRACE_THEN_BUY_STOP_RECLAIM'
    ...
    plan.entry_low, plan.entry_high = (zone_low, zone_high)
    plan.trigger = fvg_reclaim_trigger
    plan.reclaim_trigger_price = fvg_reclaim_trigger
    plan.trigger_basis = 'FVG_ZONE_UPPER_BOUNDARY_PLUS_ONE_IDX_TICK'
...
def detect_all_setups(
    df: pd.DataFrame,
    ticker: str,
    accumulation_profile: Mapping[str, object] | None = None,
) -> list[SetupPlan]:
    plans: list[SetupPlan] = []
    for detector in SETUP_DETECTORS:
        if detector is detect_reversal_accumulation:
            plans.append(detector(df, ticker, accumulation_profile=accumulation_profile))
        else:
            plans.append(detector(df, ticker))
    return plans
import re
...
def _daily_ohlcv_cache_binary_path(ticker: str) -> Path:
    """Safe NumPy companion for fast 400-symbol warm starts.

    Pickle is intentionally not used here. Cache directories are operational
    state and must not become an arbitrary-code execution boundary when a
    deployment mounts a shared or user-writable volume.
    """
    return _daily_ohlcv_cache_path(ticker).with_suffix('.npz')


def _write_daily_ohlcv_binary_cache(
    ticker: str, frame: pd.DataFrame,
) -> None:
    """Atomically write the fixed OHLCV contract without object arrays."""
    if frame is None or frame.empty:
        return
    required = ('Open', 'High', 'Low', 'Close', 'Volume')
    if any(column not in frame.columns for column in required):
        return
    path = _daily_ohlcv_cache_binary_path(ticker)
...
def _read_daily_ohlcv_binary_cache(ticker: str) -> pd.DataFrame:
    """Load a safe fixed-schema NumPy cache; never enable pickle decoding."""
    path = _daily_ohlcv_cache_binary_path(ticker)
    if not path.is_file():
        return pd.DataFrame()
    try:
        required = {'index_ns', 'Open', 'High', 'Low', 'Close', 'Volume'}
        ...
        if len(lengths) != 1 or not lengths or next(iter(lengths)) == 0:
            return pd.DataFrame()
        index_ns = arrays.pop('index_ns')
        if index_ns.dtype.kind not in {'i', 'u'}:
            return pd.DataFrame()
        frame = pd.DataFrame(
            {column: arrays[column].astype(float, copy=False) for column in (
                'Open', 'High', 'Low', 'Close', 'Volume',
            )},
            index=pd.to_datetime(index_ns.astype('int64', copy=False)),
        )
...
def _load_daily_ohlcv_cache_v431(ticker: str) -> pd.DataFrame:
    try:
        ...
        binary_path = _daily_ohlcv_cache_binary_path(ticker)
        binary_current = binary_path.is_file() and (
            not path.is_file()
            or binary_path.stat().st_mtime_ns >= path.stat().st_mtime_ns
        )
        if binary_current:
            binary = _read_daily_ohlcv_binary_cache(ticker)
            if not binary.empty:
                return binary
        if not path.is_file():
            return pd.DataFrame()
        ...
        if not clean.empty:
            # One-time, safe migration from the portable CSV. Legacy .pkl
            # companions are deliberately ignored and never deserialised.
            _write_daily_ohlcv_binary_cache(ticker, clean)
        return clean
    except Exception:
        return pd.DataFrame()
...
def ohlcv_quality_issues(frame: pd.DataFrame) -> list[str]:
    ...
    jump_dates = {pd.Timestamp(value).date().isoformat() for value in frame.index[jumps.gt(0.8).fillna(False)]}
    attrs = dict(getattr(frame, 'attrs', {}) or {})
    verified_action_dates = set(str(value) for value in attrs.get('corporate_action_split_dates', []) or [])
    unexplained = jump_dates - verified_action_dates
    if unexplained:
        issues.append('Lompatan adjusted price >80% tanpa corporate-action event terverifikasi')
    elif jump_dates:
        attrs['corporate_action_quality_state'] = 'ADJUSTED_SPLIT_VERIFIED'
...
def _merge_ohlcv_history(base: pd.DataFrame, update: pd.DataFrame) -> pd.DataFrame:
    ...
    if right.empty:
        return left
    merged_attrs = dict(getattr(left, 'attrs', {}) or {})
    for key, value in dict(getattr(right, 'attrs', {}) or {}).items():
        if key in {'corporate_action_split_dates', 'corporate_action_dividend_dates'}:
            merged_attrs[key] = sorted(set((merged_attrs.get(key, []) or []) + (value or [])))
        else:
            merged_attrs[key] = value
    merged = _clean_ohlcv(pd.concat([left, right], axis=0), strict=True)
...
def _yahoo_quote_summary_enabled() -> bool:
    """The endpoint commonly returns 401; enable only for verified deployments."""
    return str(os.environ.get('IDX_SCANNER_ENABLE_YAHOO_QUOTE_SUMMARY', '')).strip().lower() in {
        '1', 'true', 'yes', 'on', 'enabled',
    }
...
def _download_ohlcv_v431(tickers: Iterable[str], period: str='3y', batch_size: int=30, itick_api_token: str='') -> tuple[dict[str, pd.DataFrame], DownloadReport]:
    """Database/cache-first incremental daily OHLCV orchestration.

    Current verified caches generate zero provider calls. Stale caches request
    only a short overlapping date window; only symbols without any history use
    a full-period download. Provider failures fall back to IDX EOD, optional
    iTick and finally the last verified cache.
    """
    try:
        if not _yfinance_fallback_enabled():
            raise RuntimeError('yfinance fallback disabled; using bounded direct JSON first')
        import yfinance as yf
        yahoo_import_error = ''
    except Exception as exc:
        yf = None
        yahoo_import_error = f'{type(exc).__name__}: {str(exc)[:120]}'
    ...
    if len(requested) >= 16:
        with ThreadPoolExecutor(max_workers=min(8, len(requested))) as pool:
            cached_histories = dict(zip(
                requested, pool.map(_load_daily_ohlcv_cache, requested),
            ))
    else:
        cached_histories = {
            ticker: _load_daily_ohlcv_cache(ticker) for ticker in requested
        }
    cache_hits = 0
    ...
    incremental_targets = [ticker for ticker in provider_targets if not cached_histories.get(ticker, pd.DataFrame()).empty]
    full_targets = [ticker for ticker in provider_targets if cached_histories.get(ticker, pd.DataFrame()).empty]
    skip_individual_retry: set[str] = set()
    if yf is None:
        skip_individual_retry.update(provider_targets)
        for ticker in provider_targets:
            failed[ticker] = f'Yahoo provider unavailable: {yahoo_import_error}'
            warnings[ticker] = 'Dependency yfinance tidak tersedia; mencoba IDX/iTick/cache fallback'

    def run_batch_download(targets: list[str], incremental: bool) -> None:
        nonlocal incremental_refreshes, full_refreshes, provider_calls, downloaded_bars
        for start_idx in range(0, len(targets), max(1, int(batch_size))):
            batch = targets[start_idx:start_idx + max(1, int(batch_size))]
            if not batch:
                continue
            kwargs: dict[str, Any] = {
                'interval': '1d', 'group_by': 'ticker', 'auto_adjust': True,
                'repair': False, 'actions': False, 'threads': 8,
                'progress': False, 'timeout': max(8, int(os.environ.get('IDX_SCANNER_YF_BATCH_TIMEOUT_SECONDS', '15'))),
            }
            if incremental:
                latest_dates = [pd.Timestamp(cached_histories[ticker].index[-1]).normalize() for ticker in batch if not cached_histories[ticker].empty]
                overlap_start = min(latest_dates) - timedelta(days=10)
                kwargs['start'] = overlap_start.strftime('%Y-%m-%d')
                kwargs['end'] = (_expected_last_completed_daily_date() + timedelta(days=2)).strftime('%Y-%m-%d')
            else:
                ...
                for ticker in batch:
                    ...
                    if not live.empty:
                        ...
                        if incremental:
                            ...
                            prior_date = pd.Timestamp(cached_histories[ticker].index[-1]).date().isoformat()
                            new_bars = max(0, len(merged) - len(cached_histories[ticker]))
                            warnings[ticker] = f'Incremental OHLCV dari {prior_date}; {new_bars} bar baru'
                        else:
                            full_refreshes += 1
                            source_tiers[ticker] = 'LIVE_YAHOO_FULL'
                        _write_daily_ohlcv_cache(ticker, merged, 'YAHOO')
                        if quality:
                            warnings[ticker] = ' • '.join(x for x in (warnings.get(ticker, ''), ' • '.join(quality)) if x)
                    else:
    ...
    if yf is not None:
        run_batch_download(incremental_targets, incremental=True)
        run_batch_download(full_targets, incremental=False)

    missing = [ticker for ticker in provider_targets if ticker not in histories and ticker not in skip_individual_retry]

    def retry_one(ticker: str) -> tuple[str, pd.DataFrame, str | None, bool]:
        cached = cached_histories.get(ticker, pd.DataFrame())
        incremental = not cached.empty
        try:
            if incremental:
                start_date = (pd.Timestamp(cached.index[-1]).normalize() - timedelta(days=10)).strftime('%Y-%m-%d')
                end_date = (_expected_last_completed_daily_date() + timedelta(days=2)).strftime('%Y-%m-%d')
                frame = yf.Ticker(ticker).history(start=start_date, end=end_date, interval='1d', auto_adjust=True, repair=False, actions=False, timeout=20)
            else:
            ...
            return (ticker, clean, audit, incremental)
        except Exception as exc:
            return (ticker, pd.DataFrame(), f'{type(exc).__name__}: {str(exc)[:100]}', incremental)

    # Individual yfinance retries are deliberately OFF by default. If one batch
    # provider is degraded, retrying hundreds of symbols one-by-one (20s each,
    # two workers) can turn a 400-name scan into a 1h+ job. Go directly to the
    # bounded Yahoo chart fallback instead. Operators can explicitly re-enable
    # the legacy route for diagnostics.
    enable_individual_retry = str(os.environ.get('IDX_SCANNER_ENABLE_INDIVIDUAL_YF_RETRY', 'false')).strip().lower() in {'1','true','yes','on'}
    if missing and enable_individual_retry:
        provider_calls += len(missing)
        retry_workers = max(2, min(6, int(os.environ.get('IDX_SCANNER_YF_RETRY_WORKERS', '4'))))
        with ThreadPoolExecutor(max_workers=min(retry_workers, len(missing))) as pool:
            futures = [pool.submit(retry_one, ticker) for ticker in missing]
            for future in as_completed(futures):
                ticker, live, error, incremental = future.result()
                if not live.empty:
                    downloaded_bars += len(live)
                    merged = _merge_ohlcv_history(cached_histories.get(ticker, pd.DataFrame()), live)
                    histories[ticker] = merged
                    if incremental:
                        incremental_refreshes += 1
                        source_tiers[ticker] = 'LIVE_YAHOO_INCREMENTAL_RETRY'
                    else:
                        full_refreshes += 1
                        source_tiers[ticker] = 'LIVE_YAHOO_FULL_RETRY'
                    _write_daily_ohlcv_cache(ticker, merged, 'YAHOO')
        ...
        for ticker in missing:
            warnings[ticker] = 'Batch Yahoo belum menghasilkan data; individual yfinance retry dilewati untuk menjaga bounded runtime'

    # Dependency-light direct chart fallback.  This route uses the same Yahoo
    # upstream but does not depend on the yfinance package or its crumb cache.
    unresolved = [ticker for ticker in requested if ticker not in histories]
    if unresolved:
        try:
            from free_data_providers import yahoo_chart_direct
        except Exception:
            yahoo_chart_direct = None
        if yahoo_chart_direct is not None:
            def direct_one(ticker: str):
                cached = cached_histories.get(ticker, pd.DataFrame())
                incremental = not cached.empty
                try:
                    direct_timeout = max(4, int(os.environ.get('IDX_SCANNER_YAHOO_TIMEOUT_SECONDS', '6')))
                    direct_retries = max(0, min(1, int(os.environ.get('IDX_SCANNER_YAHOO_RETRY_COUNT', '0'))))
                    direct, meta = yahoo_chart_direct(
                        ticker, period=period,
                        start=(pd.Timestamp(cached.index[-1]).normalize() - timedelta(days=10)) if incremental else None,
                        end=(_expected_last_completed_daily_date() + timedelta(days=2)) if incremental else None,
                        timeout=direct_timeout,
                        retry_count=direct_retries,
                        retry_backoff=0.25,
                    )
                    return ticker, _completed_daily_frame(direct), meta, incremental, ''
                except Exception as exc:
                    return ticker, pd.DataFrame(), {}, incremental, f'{type(exc).__name__}: {str(exc)[:100]}'

            provider_calls += len(unresolved)
            direct_workers = max(4, min(16, int(os.environ.get('IDX_SCANNER_YAHOO_MAX_WORKERS', '12'))))
            workers = min(direct_workers, max(1, len(unresolved)))
            with ThreadPoolExecutor(max_workers=workers) as pool:
                futures = [pool.submit(direct_one, ticker) for ticker in unresolved]
                for future in as_completed(futures):
                    ticker, direct, meta, incremental, error = future.result()
                    cached = cached_histories.get(ticker, pd.DataFrame())
                    if direct.empty:
                        failed[ticker] = f'Yahoo direct gagal/kosong: {error or "NO_DATA"}'
                        continue
                    quality = ohlcv_quality_issues(direct)
                    merged = _merge_ohlcv_history(cached, direct)
                    histories[ticker] = merged
                    downloaded_bars += len(direct)
                    if incremental:
                        incremental_refreshes += 1
                        source_tiers[ticker] = 'LIVE_YAHOO_DIRECT_INCREMENTAL'
                    else:
                        full_refreshes += 1
                        source_tiers[ticker] = 'LIVE_YAHOO_DIRECT_FULL'
                    _write_daily_ohlcv_cache(ticker, merged, 'YAHOO_DIRECT')
                    failed.pop(ticker, None)
                    state = 'current' if _daily_cache_is_current(merged) else 'stale_after_direct_refresh'
    ...
    report = DownloadReport(
        requested, sorted(histories), failed,
        provider='Database/cache-first: verified cache → bounded Yahoo direct chart → optional yfinance → IDX official → iTick optional',
        adjusted_prices=True, downloaded_at=pd.Timestamp.now(tz='Asia/Jakarta').isoformat(),
        warnings=warnings, source_tiers=source_tiers, cache_hits=cache_hits,
        incremental_refreshes=incremental_refreshes, full_refreshes=full_refreshes,
        provider_calls=provider_calls, downloaded_bars=downloaded_bars,
    )
...
def _download_benchmark_v431(period: str='3y') -> pd.DataFrame:
    ...
    try:
        from free_data_providers import yahoo_chart_direct
        frame, _ = yahoo_chart_direct(
            '^JKSE', period=period,
            timeout=max(4, int(os.environ.get('IDX_SCANNER_YAHOO_TIMEOUT_SECONDS', '8'))),
            retry_count=max(0, min(2, int(os.environ.get('IDX_SCANNER_YAHOO_RETRY_COUNT', '1')))),
            retry_backoff=0.25,
        )
        clean = _completed_daily_frame(frame)
        if not clean.empty:
            merged = _merge_ohlcv_history(cached, clean)
            _write_daily_ohlcv_cache('^JKSE', merged, 'YAHOO_DIRECT')
            return merged
    except Exception:
        pass
    return cached
...
def score_fundamentals(info: dict[str, Any]) -> dict[str, Any]:
    ...
    metrics = {
        ...
        'insider_ownership_pct': _num(info.get('heldPercentInsiders')),
        'institution_ownership_pct': _num(
...
def parse_broker_summary_csv(
    ...
    *,
    ...
    """Aggregate broker-code transactions with an explicit trust boundary.

    A user-uploaded CSV remains useful observed flow, but it cannot certify its
    own provenance and therefore cannot unlock direct-evidence allocation.
    A future authenticated provider may set ``source_verified=True`` at the
    integration boundary after it has verified the payload independently.
    Broker codes are never treated as beneficial-owner identities.
    """
    frame = _read_csv(source)
    ...
    frame['broker_code'] = (
        frame['broker_code'].fillna('').astype(str).str.strip().str.upper()
    )
    frame['buy'] = pd.to_numeric(frame[buy_col], errors='coerce')
    ...
    valid = (
        frame['ticker'].notna()
        & frame['date'].notna()
        & frame['broker_code'].str.fullmatch(r'[A-Z0-9]{2,8}', na=False)
        & frame['buy'].notna() & frame['sell'].notna()
        & frame['buy'].ge(0.0) & frame['sell'].ge(0.0)
        & frame['gross'].gt(0.0)
        & frame['date'].le(reference)
    )
    invalid_rows = int((~valid).sum())
    frame = frame.loc[valid].copy()
    empty_columns = [
        'ticker', 'broksum_asof', 'broksum_days', 'broksum_net',
        'broksum_net_ratio', 'broksum_signal',
    ...
    duplicate_rows = int(frame.duplicated(
        ['ticker', 'date', 'broker_code'], keep=False,
    ).sum())
    frame = frame.groupby(
        ['ticker', 'date', 'broker_code'], as_index=False, sort=False,
    )[['buy', 'sell', 'net', 'gross']].sum()
    source_kind = str(source_type or 'USER_UPLOAD').strip().upper()
    verified_source = bool(source_verified) and source_kind in {
        'PROVIDER_API', 'BROKER_EXPORT_VERIFIED', 'DATABASE_VERIFIED',
    }
    global_quality = max(
        0.0,
        min(100.0, 100.0 * (input_rows - invalid_rows) / max(input_rows, 1)),
    )
    if duplicate_rows:
        global_quality = max(
            0.0, global_quality - min(25.0, float(duplicate_rows)),
        )
    ...
    for ticker, group in frame.groupby('ticker', sort=False):
        ...
        rows.append({
            ...
            'broksum_source_type': source_kind,
            'broksum_source_name': str(source_name or '').strip(),
            'broksum_source_verified': verified_source,
            'broksum_provenance_state': (
            ...
            'broksum_data_quality_pct': round(global_quality, 1),
            'broksum_measure': 'VALUE' if buy_col == 'buy_value' else 'VOLUME',
            'broksum_note': (
                'Proxy kode broker; bukan identitas beneficial owner. '
                + ('Provenance direct terverifikasi.' if direct_eligible
                   else 'Tidak memenuhi direct-evidence provenance gate.')
            ),
        })
    return pd.DataFrame(rows)
...
def attach_broker_summary(signals: pd.DataFrame, broksum: pd.DataFrame) -> pd.DataFrame:
    ...
    if broksum is None or broksum.empty:
        ...
        out['broksum_source_verified'] = False
        out['broksum_direct_evidence_eligible'] = False
        out['broksum_provenance_state'] = 'NOT_AVAILABLE'
        return out
...
class ScanEngine:
    ...
    def _market_context(self, prepared: dict[str, pd.DataFrame], benchmark: pd.DataFrame | None) -> tuple[MarketContext, pd.DataFrame | None]:
        ...
        expected_session = _expected_last_completed_daily_date(now_jakarta)
        benchmark_last = pd.Timestamp(benchmark.index[-1]).normalize()
        benchmark_age = max(0, (today - benchmark_last).days)
        benchmark_session_lag = idx_session_lag(benchmark_last, expected_session)
        if benchmark_session_lag != 0:
            context.reason = (
                f'Data IHSG belum mencapai sesi selesai {expected_session.date()} '
                f'(lag {benchmark_session_lag} sesi); regime tidak dapat dipercaya'
            )
            return (context, bench_ind)
    ...
    def _tradeability(
        ...
        now_jakarta = pd.Timestamp.now(tz='Asia/Jakarta')
        today = now_jakarta.tz_localize(None).normalize()
        last_session = pd.Timestamp(frame.index[-1]).normalize()
        expected_session = _expected_last_completed_daily_date(now_jakarta)
        data_lag_sessions = idx_session_lag(last_session, expected_session)
        daily_session_current = data_lag_sessions == 0
        absolute_age = max(0, (today - last_session).days)
        ...
        if not daily_session_current:
            blockers.append(
                f'Data belum mencapai sesi IDX terakhir selesai {expected_session.date()} '
                f'(lag {data_lag_sessions} sesi)'
            )
        ...
        profile = (
            ...
            if accumulation_profile is not None
            else _silent_accumulation_profile(frame)
        )
        smart_score = _finite(profile.get('silent_accumulation_score'), 0.0)
        up_down = _finite(profile.get('up_down_value_ratio20'), np.nan)
        metrics: dict[str, float | int | str] = {
            'last_price': close,
            'last_date': pd.Timestamp(frame.index[-1]).date().isoformat(),
            'data_lag_days': lag,
            'absolute_data_age_days': absolute_age,
            'data_lag_sessions': data_lag_sessions,
            'expected_last_completed_session': expected_session.date().isoformat(),
            'daily_session_current': daily_session_current,
            'current_bar_incomplete': int(current_bar_incomplete),
            'adtv20_idr': adtv,
            'atr_pct': atr_pct,
            # Structural references are exported for research-only accumulation planning.
            # They are never sufficient for an executable order without the setup/real-money gates.
            'atr14': _number(last.get('ATR14')),
            'ema20': _number(last.get('EMA20')),
            'ema50': _number(last.get('EMA50')),
            'ema200': _number(last.get('EMA200')),
            'vwap20': _number(last.get('VWAP20')),
            'last_pivot_low': _number(last.get('LAST_PIVOT_LOW')),
            'last_pivot_high': _number(last.get('LAST_PIVOT_HIGH')),
    ...
    def _core_cache_ttl_seconds(self) -> float:
        hours = max(0.25, float(getattr(self.config, 'incremental_cache_max_age_hours', 8.0) or 8.0))
        return 3600.0 * hours

    @staticmethod
    def _core_cache_report_row(
        report: list[dict[str, Any]], *, stage: str, state: str,
        cache_key: str, load_ms: float = 0.0, build_ms: float = 0.0,
        store_ms: float = 0.0, payload_bytes: int = 0, detail: str = '',
    ) -> None:
        report.append({
            'stage': str(stage), 'cache_state': str(state),
            'cache_key_prefix': str(cache_key)[:12],
            'load_ms': round(float(load_ms), 2),
            'build_ms': round(float(build_ms), 2),
            'store_ms': round(float(store_ms), 2),
            'payload_bytes': int(payload_bytes or 0),
            'detail': str(detail or ''),
        })

    def _core_load_or_build_stage(
        self, *, store: IncrementalEvidenceStore | None,
        report: list[dict[str, Any]], stage: str, cache_key: str,
        builder: Any, validator: Any | None = None,
    ) -> Any:
        enabled = bool(getattr(self.config, 'core_incremental_cache_stage_enabled', True))
        if store is None or not enabled:
            started = time.perf_counter()
            payload = builder()
            self._core_cache_report_row(
                report, stage=stage, state='DISABLED_REBUILT', cache_key=cache_key,
                build_ms=1000.0 * (time.perf_counter() - started),
            )
            return payload
        lookup = store.get(stage, cache_key)
        valid = lookup.state == 'HIT'
        if valid and validator is not None:
            try:
                valid = bool(validator(lookup.payload))
            except Exception:
                valid = False
        if valid:
            self._core_cache_report_row(
                report, stage=stage, state='HIT', cache_key=cache_key,
                load_ms=lookup.elapsed_ms, payload_bytes=lookup.payload_bytes,
            )
        ...
        build_ms = 1000.0 * (time.perf_counter() - started)
        stored = store.put(
            stage, cache_key, payload,
            ttl_seconds=self._core_cache_ttl_seconds(),
            metadata={'pipeline_version': CORE_PIPELINE_CACHE_VERSION},
        )
        state = lookup.state if lookup.state != 'HIT' else 'SEMANTIC_INVALID'
        self._core_cache_report_row(
            report, stage=stage, state=f'{state}_REBUILT', cache_key=cache_key,
            load_ms=lookup.elapsed_ms, build_ms=build_ms,
            store_ms=stored.elapsed_ms, payload_bytes=stored.payload_bytes,
            detail='; '.join(part for part in (lookup.detail, stored.detail) if part),
        )
        return payload

    @staticmethod
    def _core_partition_index(ticker: str, partitions: int) -> int:
        token = hashlib.sha256(str(ticker).upper().encode('utf-8')).digest()
        return int.from_bytes(token[:4], 'big') % max(1, int(partitions))

    def _core_runtime_partition(self) -> str:
        now = pd.Timestamp.now(tz='Asia/Jakarta')
        return f"{now.date().isoformat()}|decision_window={int(idx_regular_decision_window(now))}"

    def _scan_from_prepared(
        self, prepared: dict[str, pd.DataFrame], benchmark: pd.DataFrame | None,
        *, store: IncrementalEvidenceStore | None,
        report: list[dict[str, Any]], partition_keys: dict[int, str],
        config_fingerprint: str, runtime_partition: str,
    ) -> dict[str, Any]:
        context, _ = self._market_context(prepared, benchmark)
        asof_candidates = [
            pd.Timestamp(frame.index[-1]) for frame in prepared.values()
        ...
        if not asof_candidates:
            return {
                'signals': pd.DataFrame(), 'universe': pd.DataFrame(),
                'prepared': {}, 'market_context': MarketContext(),
                'core_incremental_cache_report': pd.DataFrame(report),
            }
        asof = max(asof_candidates)
        context_fp = fingerprint_value(asdict(context))
        all_partition_fp = fingerprint_value(partition_keys)
        summary_key = combine_fingerprints(
            CORE_PIPELINE_CACHE_VERSION, 'CORE_SUMMARY', all_partition_fp,
            context_fp, config_fingerprint, asof.isoformat(), runtime_partition,
        )
        full_enabled = bool(getattr(self.config, 'core_incremental_cache_full_result_enabled', True))
        if store is not None and full_enabled:
            lookup = store.get('core_summary', summary_key)
            payload = lookup.payload
            valid = (
                lookup.state == 'HIT' and isinstance(payload, dict)
                and isinstance(payload.get('signals'), pd.DataFrame)
                and isinstance(payload.get('universe'), pd.DataFrame)
                and isinstance(payload.get('silent_profiles'), dict)
            )
            if valid:
                for ticker, profile in payload['silent_profiles'].items():
                    frame = prepared.get(ticker)
                    if isinstance(frame, pd.DataFrame):
                        frame.attrs['_silent_accumulation_profile'] = dict(profile or {})
                self._core_cache_report_row(
                    report, stage='core_summary', state='HIT', cache_key=summary_key,
                    load_ms=lookup.elapsed_ms, payload_bytes=lookup.payload_bytes,
                )
                return {
                    'signals': payload['signals'], 'universe': payload['universe'],
                    'prepared': prepared, 'market_context': payload.get('market_context', context),
                    'asof': payload.get('asof', asof), 'config': asdict(self.config),
                    'core_incremental_cache_report': pd.DataFrame(report),
                }
            self._core_cache_report_row(
                report, stage='core_summary',
                state=f"{lookup.state if lookup.state != 'HIT' else 'SEMANTIC_INVALID'}_REBUILD_REQUIRED",
                cache_key=summary_key, load_ms=lookup.elapsed_ms,
                payload_bytes=lookup.payload_bytes, detail=lookup.detail,
            )

        partitions = max(1, min(64, int(getattr(self.config, 'core_incremental_cache_partitions', 16) or 16)))
        grouped: dict[int, list[str]] = {index: [] for index in range(partitions)}
        for ticker in sorted(prepared, key=lambda value: str(value).upper()):
            grouped[self._core_partition_index(ticker, partitions)].append(ticker)

        local_bundles: dict[str, dict[str, Any]] = {}
        for bucket, tickers in grouped.items():
            if not tickers:
                continue
            local_key = combine_fingerprints(
                CORE_PIPELINE_CACHE_VERSION, 'LOCAL_ANALYSIS', partition_keys[bucket],
                config_fingerprint, asof.isoformat(), runtime_partition,
            )
            stage = f'core_local_partition_{bucket:02d}'

            def build_local(names: tuple[str, ...] = tuple(tickers)) -> dict[str, dict[str, Any]]:
                result: dict[str, dict[str, Any]] = {}
                for ticker in names:
                    frame = prepared[ticker]
                    profile = _silent_accumulation_profile(frame)
                    frame.attrs['_silent_accumulation_profile'] = profile
                    blockers, metrics = self._tradeability(
                        frame, asof, accumulation_profile=profile,
                    )
                    plans = detect_all_setups(
                        frame, ticker, accumulation_profile=profile,
                    )
                    result[ticker] = {
                        'accumulation_profile': profile,
            ...
            def valid_local(payload: Any, names: tuple[str, ...] = tuple(tickers)) -> bool:
                return (
                    isinstance(payload, dict) and set(payload) == set(names)
                    and all(
                        isinstance(payload.get(name), dict)
                        and isinstance(payload[name].get('plans'), list)
                        and isinstance(payload[name].get('metrics'), dict)
                        for name in names
                    )
                )

            bundle = self._core_load_or_build_stage(
                store=store, report=report, stage=stage, cache_key=local_key,
                builder=build_local, validator=valid_local,
            )
            local_bundles.update(bundle)

        signal_rows: list[dict[str, Any]] = []
        universe_rows: list[dict[str, Any]] = []
        silent_profiles: dict[str, dict[str, Any]] = {}
        for ticker, frame in prepared.items():
            bundle = local_bundles[ticker]
            profile = dict(bundle.get('accumulation_profile') or {})
            frame.attrs['_silent_accumulation_profile'] = profile
            silent_profiles[ticker] = profile
            trade_blockers = list(bundle.get('trade_blockers') or [])
            metrics = dict(bundle.get('metrics') or {})
            plans = list(bundle.get('plans') or [])
            finalized = [
                self._finalize(plan, frame, context, trade_blockers, metrics)
                for plan in plans
            ]
            ...
            best = sorted(
                candidates, key=lambda value: (
                    value['status_rank'], -value['quality_score'], value.get('setup', ''),
                ),
            )[0]
            setup_detected = bool(best['detected'])
            universe_rows.append({
                'ticker': ticker,
                'best_setup': best['setup'] if setup_detected else 'NO_SETUP',
                'setup_detected': setup_detected,
                'status': best['status'],
                'quality_score': best['quality_score'] if setup_detected else np.nan,
                'grade': best['grade'] if setup_detected else 'N/A',
                'setup_diagnostic_score': best['quality_score'],
                'reason': best['reason'], 'blockers': best['blockers'],
                **metrics,
            })
        ...
        if not universe.empty:
            ...
            universe = universe.sort_values(
                ['status_rank', 'setup_diagnostic_score', 'adtv20_idr'],
                ascending=[True, False, False],
            ).drop(columns='status_rank').reset_index(drop=True)

        if store is not None and full_enabled:
            stored = store.put(
                'core_summary', summary_key,
                {
                    'signals': signals, 'universe': universe,
                    'market_context': context, 'asof': asof,
            ...
            self._core_cache_report_row(
                report, stage='core_summary', state=stored.state,
                cache_key=summary_key, store_ms=stored.elapsed_ms,
                payload_bytes=stored.payload_bytes, detail=stored.detail,
            )
        return {
            'signals': signals, 'universe': universe, 'prepared': prepared,
            'market_context': context, 'asof': asof, 'config': asdict(self.config),
            'core_incremental_cache_report': pd.DataFrame(report),
        }
    ...
    def scan(self, histories: dict[str, pd.DataFrame], benchmark: pd.DataFrame | None=None) -> dict[str, Any]:
        ...
        valid_histories = {
            ticker: frame for ticker, frame in histories.items()
            if isinstance(frame, pd.DataFrame) and not frame.empty
        }
        ...
        cache_on = (
            incremental_cache_enabled(self.config)
            and bool(getattr(self.config, 'core_incremental_cache_enabled', True))
        )
        store: IncrementalEvidenceStore | None = None
        if cache_on:
            try:
                store = IncrementalEvidenceStore(
                    max_entries=max(48, int(getattr(self.config, 'incremental_cache_max_entries', 48))),
                    max_bytes=max(16, int(getattr(self.config, 'incremental_cache_max_bytes_mb', 1024))) * 1024 * 1024,
                    read_only=bool(getattr(self.config, 'incremental_cache_read_only', False)),
                )
            except Exception as exc:
                self._core_cache_report_row(
                    report, stage='core_cache_initialisation', state='ERROR_FAIL_SOFT',
                    cache_key='', detail=f'{type(exc).__name__}: {exc}',
                )
                store = None

        benchmark_fp = fingerprint_dataframe(benchmark)
        history_fps = {
            ticker: fingerprint_dataframe(frame)
            for ticker, frame in sorted(valid_histories.items(), key=lambda item: str(item[0]).upper())
        }
        config_payload = {
            key: value for key, value in asdict(self.config).items()
            if not key.startswith('incremental_cache_')
            and not key.startswith('core_incremental_cache_')
        }
        config_fp = fingerprint_value(config_payload)
        partitions = max(1, min(64, int(getattr(self.config, 'core_incremental_cache_partitions', 16) or 16)))
        grouped: dict[int, list[str]] = {index: [] for index in range(partitions)}
        for ticker in history_fps:
            grouped[self._core_partition_index(ticker, partitions)].append(ticker)
        ...
        for bucket, tickers in grouped.items():
            if not tickers:
                continue
            partition_key = combine_fingerprints(
                CORE_PIPELINE_CACHE_VERSION, 'PREPARED', benchmark_fp,
                fingerprint_value([(ticker, history_fps[ticker]) for ticker in tickers]),
            )
            partition_keys[bucket] = partition_key
            stage = f'core_prepared_partition_{bucket:02d}'

            def build_prepared(names: tuple[str, ...] = tuple(tickers)) -> dict[str, pd.DataFrame]:
                result: dict[str, pd.DataFrame] = {}
                for ticker in names:
                    frame = prepare_indicators(valid_histories[ticker], benchmark)
                    # A raw history can become empty after mandatory OHLCV/volume
                    # sanitation. Excluding it here prevents the legacy index[-1]
                    # crash and keeps universe coverage auditable.
                    if isinstance(frame, pd.DataFrame) and not frame.empty:
                        result[ticker] = frame
                return result

            def valid_prepared(payload: Any, names: tuple[str, ...] = tuple(tickers)) -> bool:
                return (
                    isinstance(payload, dict)
                    and set(payload).issubset(set(names))
                    and all(isinstance(frame, pd.DataFrame) and not frame.empty for frame in payload.values())
                )

            bundle = self._core_load_or_build_stage(
                store=store, report=report, stage=stage, cache_key=partition_key,
                builder=build_prepared, validator=valid_prepared,
            )
        ...
        # Remove empty buckets from the aggregate fingerprint. A ticker whose
        # raw bars are all unusable is intentionally absent from prepared.
        active_buckets = {
            bucket: key for bucket, key in partition_keys.items()
            if any(self._core_partition_index(ticker, partitions) == bucket for ticker in prepared)
        }
        return self._scan_from_prepared(
            prepared, benchmark, store=store, report=report,
            partition_keys=active_buckets, config_fingerprint=config_fp,
            runtime_partition=self._core_runtime_partition(),
        )
import math
...
class BacktestEvent:
    ...
    structural_quality_score: float | None = None
    validation_event_tier: str = 'PRODUCTION_READY'
    production_gate_pass: bool = True
    production_gate_blockers: str = ''
...
def _simulate_order(
    ...
    smart_score = _finite(silent_accumulation_score, np.nan)
    if not np.isfinite(smart_score):
...
def simulate_setup(
    ...
    config: ScanConfig,
    *,
    accumulation_cache: dict[int, dict[str, object]] | None = None,
    gate_cache: dict[int, tuple[list[str], dict[str, object], MarketContext]] | None = None,
) -> list[BacktestEvent]:
    """Generate causal validation events without weakening the live order gate.

    A detector-confirmed plan with valid entry/stop/target geometry is retained as
    ``TRIGGER_CANDIDATE`` even when liquidity, regime, or accumulation blocks a
    live order.  This expands genuine OOS evidence.  Production execution still
    requires ``EXECUTION_READY`` and is recorded separately on each event.
    """
    ...
    for pos in candidates:
        ...
        cached_gate = gate_cache.get(int(pos)) if gate_cache is not None else None
        if cached_gate is None:
            base_blockers, _ = _historical_gate_inputs_v300(snapshot, config)
            context = _historical_context(snapshot)
            profile = accumulation_cache.get(int(pos)) if accumulation_cache is not None else None
            if profile is None:
                profile = _silent_accumulation_profile(snapshot)
                if accumulation_cache is not None:
                    accumulation_cache[int(pos)] = profile
            blockers, metrics = _historical_gate_inputs(snapshot, config, accumulation_profile=profile)
            blockers = list(dict.fromkeys(list(base_blockers) + list(blockers)))
            if context.regime not in {'RISK_ON', 'NEUTRAL'}:
                blockers.append(f'Market regime {context.regime}')
            if gate_cache is not None:
                gate_cache[int(pos)] = (list(blockers), dict(metrics), context)
        else:
            blockers, metrics, context = cached_gate
            blockers = list(blockers)
            metrics = dict(metrics)
            profile = accumulation_cache.get(int(pos), {}) if accumulation_cache is not None else {}
        if not profile:
            profile = _silent_accumulation_profile(snapshot)
            if accumulation_cache is not None:
                accumulation_cache[int(pos)] = profile
        try:
            plan = detector(snapshot, ticker, accumulation_profile=profile) if setup == 'REVERSAL_ACCUMULATION' else detector(snapshot, ticker)
        except Exception:
            continue
        if not bool(getattr(plan, 'detected', False)):
            continue
        finalized = engine._finalize(plan, snapshot, context, blockers, metrics)
        production_pass = str(finalized.get('status', '')).upper() == 'EXECUTION_READY'
        values = [getattr(plan, name, None) for name in ('entry', 'stop_loss', 'tp1', 'tp2')]
        try:
            entry, stop, tp1, tp2 = (float(value) for value in values)
        except (TypeError, ValueError):
            continue
        geometry_valid = all(np.isfinite(value) and value > 0 for value in (entry, stop, tp1, tp2)) and stop < entry < tp1 <= tp2
        if not geometry_valid:
            continue
        quality = _finite(finalized.get('quality_score'), _finite(getattr(plan, 'setup_score', np.nan), np.nan))
        if not np.isfinite(quality):
            continue
        event = _simulate_order(
            df, pos, ticker, setup, plan, float(quality),
            context.regime, config,
            silent_accumulation_score=_finite(metrics.get('silent_accumulation_score'), np.nan),
        )
        event.production_gate_pass = bool(production_pass)
        event.production_gate_blockers = ' • '.join(dict.fromkeys(str(value) for value in blockers if str(value)))
        event.validation_event_tier = 'PRODUCTION_READY' if production_pass else 'TRIGGER_CANDIDATE'
        events.append(event)
        if event.filled and event.exit_date is not None:
            exit_positions = np.flatnonzero(df.index == event.exit_date)
            exit_pos = int(exit_positions[-1]) if len(exit_positions) else pos
...
def run_walkforward_validation(prepared: dict[str, pd.DataFrame], config: ScanConfig | None=None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for ticker, frame in prepared.items():
        if len(frame) < 225:
            continue
        accumulation_cache: dict[int, dict[str, object]] = {}
        gate_cache: dict[int, tuple[list[str], dict[str, object], MarketContext]] = {}
        for setup in SETUPS:
            all_events.extend(
                asdict(event)
                for event in simulate_setup(
                    frame,
                    ticker,
                    setup,
                    cfg,
...
def select_walkforward_universe(
    prepared: Mapping[str, pd.DataFrame],
    max_tickers: int = 60,
) -> tuple[dict[str, pd.DataFrame], pd.DataFrame]:
    """Select a deterministic, liquidity-stratified validation cohort.

    Full-universe chronological validation can dominate a Streamlit Community
    Cloud scan because each historical candidate rebuilds the live detector.
    This selector bounds that cost without cherry-picking current score,
    setup, or return. Symbols are sampled deterministically inside four
    liquidity strata. ``max_tickers <= 0`` keeps the full eligible universe.
    """
    eligible: list[dict[str, object]] = []
    for ticker, frame in prepared.items():
        if frame is None or frame.empty or len(frame) < 225:
            continue
        row = frame.iloc[-1]
        adtv = _finite(row.get('ADTV20'), np.nan)
        if not np.isfinite(adtv):
            close = pd.to_numeric(frame.get('Close'), errors='coerce')
            volume = pd.to_numeric(frame.get('Volume'), errors='coerce')
            adtv = _finite((close * volume).tail(20).mean(), 0.0)
        eligible.append({
            'ticker': str(ticker),
            'bars': int(len(frame)),
            'last_date': pd.Timestamp(frame.index[-1]).date().isoformat(),
            'adtv20_idr': float(max(0.0, adtv)),
            '_stable_key': hashlib.sha256(str(ticker).encode('utf-8')).hexdigest(),
        })
    if not eligible:
        return {}, pd.DataFrame(columns=[
            'ticker', 'bars', 'last_date', 'adtv20_idr', 'liquidity_stratum',
            'selected', 'selection_policy',
        ])

    ordered = sorted(eligible, key=lambda row: (float(row['adtv20_idr']), str(row['ticker'])))
    total = len(ordered)
    for rank, row in enumerate(ordered):
        bucket = min(3, int(4 * rank / max(1, total)))
        row['liquidity_stratum'] = ('LOW', 'MID_LOW', 'MID_HIGH', 'HIGH')[bucket]

    limit = int(max_tickers)
    if limit <= 0 or total <= limit:
        selected_names = {str(row['ticker']) for row in ordered}
        policy = 'FULL_ELIGIBLE_UNIVERSE'
    else:
        selected_names: set[str] = set()
        base, remainder = divmod(limit, 4)
        for bucket_index, bucket_name in enumerate(('LOW', 'MID_LOW', 'MID_HIGH', 'HIGH')):
            bucket_rows = sorted(
                (row for row in ordered if row['liquidity_stratum'] == bucket_name),
                key=lambda row: str(row['_stable_key']),
            )
            quota = base + (1 if bucket_index < remainder else 0)
            selected_names.update(str(row['ticker']) for row in bucket_rows[:quota])
        if len(selected_names) < limit:
            remaining = sorted(
                (row for row in ordered if str(row['ticker']) not in selected_names),
                key=lambda row: str(row['_stable_key']),
            )
            selected_names.update(
                str(row['ticker'])
                for row in remaining[: limit - len(selected_names)]
            )
        policy = 'DETERMINISTIC_LIQUIDITY_STRATIFIED'

    selected = {
        ticker: frame
        for ticker, frame in prepared.items()
        if str(ticker) in selected_names
    }
    audit = pd.DataFrame(ordered).drop(columns=['_stable_key'])
    audit['selected'] = audit['ticker'].isin(selected_names)
    audit['selection_policy'] = policy
    audit['eligible_universe_tickers'] = total
    audit['selected_tickers'] = len(selected)
    audit['selection_coverage_pct'] = round(100.0 * len(selected) / total, 1)
    audit = audit.sort_values(
        ['selected', 'liquidity_stratum', 'adtv20_idr', 'ticker'],
        ascending=[False, True, False, True],
        kind='stable',
    ).reset_index(drop=True)
    return selected, audit

def run_adaptive_walkforward_validation(
    prepared: Mapping[str, pd.DataFrame],
    config: ScanConfig | None=None,
    *,
    initial_tickers: int=60,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Expand a deterministic OOS cohort until evidence targets or the cap.

    Evidence is never fabricated and quality gates are not relaxed.  The cohort
    expands 60 -> 120 -> configured cap, accumulating only newly selected
    tickers.  AI remains shadow-only when a setup still lacks genuine events.
    """
    cfg = config or ScanConfig()
    eligible_count = sum(1 for frame in (prepared or {}).values() if frame is not None and len(frame) >= 225)
    cap = eligible_count if int(cfg.validation_max_tickers) <= 0 else min(eligible_count, int(cfg.validation_max_tickers))
    if cap <= 0:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    targets = []
    size = min(max(20, int(initial_tickers)), cap)
    while True:
        targets.append(size)
        if size >= cap:
            break
        size = min(cap, max(size + 1, size * 2))
    accumulated: list[pd.DataFrame] = []
    processed: set[str] = set()
    audit_rows: list[dict[str, Any]] = []
    last_universe_audit = pd.DataFrame()
    for iteration, target in enumerate(targets, start=1):
        cohort, universe_audit = select_walkforward_universe(prepared, max_tickers=target)
        last_universe_audit = universe_audit
        delta = {ticker: frame for ticker, frame in cohort.items() if ticker not in processed}
        if delta:
            _, new_events = run_walkforward_validation(delta, cfg)
            if new_events is not None and not new_events.empty:
                accumulated.append(new_events)
            processed.update(delta)
        events = pd.concat(accumulated, ignore_index=True, sort=False) if accumulated else pd.DataFrame()
        if not events.empty:
            events = _assign_oos_folds(events, cfg)
        eligible = events[events.get('oos_eligible', pd.Series(False, index=events.index)).fillna(False).astype(bool)].copy() if not events.empty else pd.DataFrame()
        setup_counts: dict[str, dict[str, int]] = {}
        sufficient = True
        for setup in SETUPS:
            subset = eligible[eligible.get('setup', pd.Series('', index=eligible.index)).astype(str).eq(setup)] if not eligible.empty else pd.DataFrame()
            filled = int(pd.to_numeric(subset.get('filled', pd.Series(dtype=float)), errors='coerce').fillna(0).astype(bool).sum()) if not subset.empty else 0
            signals = int(len(subset))
            setup_counts[setup] = {'signals': signals, 'filled': filled}
            if signals < int(cfg.min_oos_signal_events) or filled < int(cfg.min_oos_filled_events):
                sufficient = False
        audit_rows.append({
            'iteration': iteration, 'target_tickers': target, 'processed_tickers': len(processed),
            'oos_events': int(len(eligible)), 'evidence_target_met': bool(sufficient),
            'setup_counts': json.dumps(setup_counts, sort_keys=True),
            'policy': 'ADAPTIVE_DETERMINISTIC_LIQUIDITY_STRATIFIED',
        })
        if sufficient:
            break
    final_events = pd.concat(accumulated, ignore_index=True, sort=False) if accumulated else pd.DataFrame()
    if not final_events.empty:
        final_events = _assign_oos_folds(final_events, cfg)
    stats = aggregate_backtest(final_events, cfg)
    expansion_audit = pd.DataFrame(audit_rows)
    if not last_universe_audit.empty:
        last_universe_audit = last_universe_audit.copy()
        last_universe_audit['adaptive_iterations'] = len(audit_rows)
        last_universe_audit['adaptive_evidence_target_met'] = bool(audit_rows[-1]['evidence_target_met']) if audit_rows else False
        last_universe_audit.attrs['expansion_audit'] = expansion_audit
    return stats, final_events, last_universe_audit
...
def detect_pullback_continuation(df: pd.DataFrame, ticker: str) -> SetupPlan:
    ...
    if confirmation and distance <= 0.45:
        reclaim_trigger = max(
            close, _finite(row.get('High'), close) + idx_tick_size(close),
        )
        raw_entry = reclaim_trigger
        plan.entry_type = 'BUY_STOP_REACCELERATION'
        plan.action = 'READY_TRIGGER'
    else:
        # A waiting pullback is not a limit order that can be filled before the
        # reclaim condition.  The zone remains the observation area, while the
        # executable order price is one IDX tick above its upper boundary.
        # Using the latest candle high here produced impossible plans where the
        # trigger sat above TP1/TP2 after price had already extended away.
        reclaim_trigger = zone_high + idx_tick_size(max(zone_high, close))
        raw_entry = reclaim_trigger
        plan.entry_type = 'WAIT_VALUE_ZONE_THEN_BUY_STOP_RECLAIM'
    ...
    plan.trigger = reclaim_trigger
    plan.reclaim_trigger_price = reclaim_trigger
    plan.trigger_basis = 'VALUE_ZONE_UPPER_BOUNDARY_PLUS_ONE_IDX_TICK'
    plan.trigger_instruction = (
        'Tunggu harga masuk value area, lalu gunakan buy-stop reclaim satu fraksi '
        'di atas batas atas zona; jangan mengejar high candle yang sudah extended.'
    )
    plan.distance_atr = round(distance, 2)
    plan.evidence = [
...
def detect_reversal_accumulation(
    df: pd.DataFrame,
    ticker: str,
    accumulation_profile: Mapping[str, object] | None = None,
) -> SetupPlan:
    ...
    profile = (
        accumulation_profile
        if accumulation_profile is not None
        else _silent_accumulation_profile(df)
    )
    accumulation_score = _finite(profile.get('silent_accumulation_score'), 0.0)
    up_down = _finite(profile.get('up_down_value_ratio20'), np.nan)
...
def _is_official_idx_host(host: object) -> bool:
    value = str(host or '').strip().lower().rstrip('.')
    return value in {'idx.id', 'idx.co.id'} or value.endswith('.idx.id') or value.endswith('.idx.co.id')


def _idx_url_candidates(url: str) -> tuple[str, ...]:
    """Try the official anti-bot mirror, current host, then legacy host."""
    raw = str(url or '').strip()
    candidates: list[str] = []
    if '://www.idx.id/' in raw:
        candidates.extend([
            raw.replace('://www.idx.id/', '://block.idx.id/'),
            raw,
            raw.replace('://www.idx.id/', '://www.idx.co.id/'),
        ])
    elif '://idx.id/' in raw:
        candidates.extend([
            raw.replace('://idx.id/', '://block.idx.id/'),
            raw,
            raw.replace('://idx.id/', '://idx.co.id/'),
        ])
    elif '://block.idx.id/' in raw:
        candidates.extend([
            raw,
            raw.replace('://block.idx.id/', '://www.idx.id/'),
            raw.replace('://block.idx.id/', '://www.idx.co.id/'),
        ])
    elif '://www.idx.co.id/' in raw:
        candidates.extend([
            raw.replace('://www.idx.co.id/', '://block.idx.id/'),
            raw.replace('://www.idx.co.id/', '://www.idx.id/'),
            raw,
        ])
    elif '://idx.co.id/' in raw:
        candidates.extend([
            raw.replace('://idx.co.id/', '://block.idx.id/'),
            raw.replace('://idx.co.id/', '://idx.id/'),
            raw,
        ])
    else:
...
def _is_exact_official_idx_url(value: object) -> bool:
    try:
        ...
        return False
    return parsed.scheme == 'https' and _is_official_idx_host(parsed.hostname) and bool(parsed.path and parsed.path != '/')
...
def parse_market_status_csv(source: bytes | BinaryIO | pd.DataFrame) -> pd.DataFrame:
    ...
    coverage flag is true, and the source host is an official idx.id/idx.co.id domain.
    """
    frame = _read_csv(source)
    frame.columns = [str(column).strip().lower() for column in frame.columns]
...
IDX_AUTOMATION_URLS = {'stock_list': 'https://block.idx.id/id/data-pasar/data-saham/daftar-saham/', 'watchlist': 'https://www.idx.id/id/perusahaan-tercatat/daftar-efek-pemantauan-khusus/', 'suspension': 'https://www.idx.id/id/berita/suspensi', 'long_suspension': 'https://www.idx.id/id/perusahaan-tercatat/suspensi-6-bulan/', 'corporate_actions': 'https://www.idx.id/id/perusahaan-tercatat/aksi-korporasi'}
...
def parse_news_review_csv(source: bytes | BinaryIO | pd.DataFrame) -> pd.DataFrame:
    ...
    frame['news_summary'] = _column(frame, 'summary', '').fillna('').astype(str)
    ...
    frame['news_event_type'] = _column(frame, 'event_type', '').fillna('').astype(str).str.upper()
    frame['news_published_at'] = pd.to_datetime(
        _column(frame, 'published_at', pd.NaT), errors='coerce',
    )
    frame['news_source_family'] = _column(
        frame, 'source_family', '',
    ).fillna('').astype(str)
    ...
    for ticker, group in frame.dropna(subset=['ticker']).groupby('ticker', sort=False):
        ...
        if status == 'COMPLETE' and (not structural_complete):
            status = 'INCOMPLETE'
        event_payload = []
        for item in material.to_dict('records'):
            event_payload.append({
                'headline': item.get('news_title', ''),
                'summary': item.get('news_summary', ''),
                'source_url': item.get('news_source_url', ''),
                'source_family': (
                    item.get('news_source_family')
                    or item.get('news_provider')
                    or latest.get('news_provider', '')
                ),
                'event_date': (
                    pd.Timestamp(item['news_published_at']).isoformat()
                    if pd.notna(item.get('news_published_at'))
                    else pd.Timestamp(item['news_reviewed_at']).isoformat()
                    if pd.notna(item.get('news_reviewed_at'))
                    else ''
                ),
                'detected_at': (
                    pd.Timestamp(item['news_reviewed_at']).isoformat()
                    if pd.notna(item.get('news_reviewed_at'))
                    else ''
                ),
                'event_type': item.get('news_event_type', ''),
                'sentiment': item.get('news_sentiment', 'NEUTRAL'),
                'materiality': item.get('news_materiality', 'LOW'),
                'official_verified': bool(item.get('news_verified', False)),
            })
        rows.append({
            'ticker': ticker,
            ...
            'items_reviewed': (
                int(latest['items_reviewed'])
                if pd.notna(latest['items_reviewed']) else np.nan
            ),
            'coverage_start': latest['coverage_start'],
            'coverage_end': latest['coverage_end'],
            'news_provider': latest['news_provider'],
            'verified_catalyst_count': int(
                (
                    (material['news_sentiment'] == 'POSITIVE')
            ...
            'catalyst_summary': ' | '.join(
                material.tail(3)['news_title'].tolist()
            ),
            'news_sources': ' | '.join(
                material.tail(3)['news_source_url'].tolist()
            ),
            'narrative_events_json': json.dumps(
...
def aggregate_backtest(events: pd.DataFrame, config: ScanConfig) -> pd.DataFrame:
    columns = ['setup', 'signal_events_oos', 'all_signal_events', 'production_ready_events', 'trigger_candidate_events', 'historical_events', 'filled_events', 'entry_fill_rate_5d', 'entry_fill_ci_low', 'entry_fill_ci_high', 'historical_hit_rate', 'bayes_probability', 'tp1_ci_low', 'tp1_ci_high', 'expectancy_r', 'profit_factor', 'max_losing_streak', 'median_fill_bars', 'median_time_to_tp1_bars', 'tp1_time_p25', 'tp1_time_p75', 'sample_reliability', 'validation_scope']
    if events.empty:
        return pd.DataFrame(columns=columns)
...
def _longest_true_run(values: pd.Series) -> int:
    """Return the longest consecutive True run for a causal boolean series."""
    longest = current = 0
...
def _silent_accumulation_profile(frame: pd.DataFrame) -> dict[str, object]:
    """Return Silent Accumulation v4 adaptive price-volume evidence.

    The model remains a proxy. It does not identify beneficial owners. Version
    4 preserves the v3 absorption framework and adds accumulation persistence,
    regime-aware weighting, supply-pressure memory and confidence diagnostics.
    Every new adjustment is bounded so one micro-pattern cannot dominate the
    broader 20/60-day evidence. The model remains a proxy, not beneficial-owner
    identification.
    """
    unavailable = {
        'silent_accumulation_version': 'V5_LIQUIDITY_CALIBRATED_OUTCOME_READY',
        'silent_accumulation_score': 0.0,
        'silent_accumulation_raw_score': 0.0,
        'silent_accumulation_liquidity_adjustment': 0.0,
        'silent_accumulation_liquidity_min_confirmation': 100.0,
        'silent_accumulation_calibration_policy': 'NO_DATA',
        'liquidity_bucket': 'UNKNOWN',
        'silent_accumulation_base_score_v2': 0.0,
        'silent_accumulation_v3_adjustment': 0.0,
        'silent_accumulation_v4_adjustment': 0.0,
        'silent_accumulation_adaptive_adjustment': 0.0,
        'accumulation_persistence_score': 0.0,
        'accumulation_positive_windows_pct': 0.0,
        'accumulation_longest_run': 0,
        'accumulation_regime': 'UNKNOWN',
        'accumulation_weight_profile': 'NO_DATA',
        'silent_accumulation_state': 'INSUFFICIENT_HISTORY',
        'silent_accumulation_confidence': 0.0,
        'silent_accumulation_reason': 'Minimum 25 bar OHLCV diperlukan.',
        'up_down_value_ratio20': np.nan,
        'up_down_value_ratio60': np.nan,
        'weighted_close_location20': np.nan,
        'cmf20_accumulation': np.nan,
        'cmf60_accumulation': np.nan,
        'adl_slope20': np.nan,
        'obv_slope20': np.nan,
        'pullback_volume_ratio20': np.nan,
        'accumulation_days20': 0,
        'distribution_days20': 0,
        'churning_support_days20': 0,
        'absorption_confirmed_days20': 0,
        'failed_absorption_days20': 0,
        'effort_result_absorption20': 0,
        'effort_result_distribution20': 0,
        'lower_wick_support_days20': 0,
        'persistent_bid_score': 0.0,
        'supply_pressure_ratio20': np.nan,
        'silent_accumulation_data_coverage': 0.0,
    }
    ...
    if not required.issubset(recent.columns):
        unavailable['silent_accumulation_state'] = 'MISSING_OHLCV'
        unavailable['silent_accumulation_reason'] = 'Kolom High/Low/Close/Volume tidak lengkap.'
        return unavailable
    ...
    volume_raw = pd.to_numeric(recent['Volume'], errors='coerce')
    observed_volume_coverage = float(
        100.0 * volume_raw.tail(min(60, len(volume_raw))).notna().mean()
    )
    prepared_volume_coverage = pd.to_numeric(
        recent.get(
            'VOLUME_DATA_COVERAGE20',
            pd.Series(np.nan, index=recent.index),
        ),
        errors='coerce',
    )
    last_prepared_coverage = (
        float(prepared_volume_coverage.dropna().iloc[-1])
        if prepared_volume_coverage.notna().any()
        else observed_volume_coverage
    )
    effective_volume_coverage = min(
        observed_volume_coverage,
        last_prepared_coverage,
    )
    if effective_volume_coverage < 90.0:
        unavailable['silent_accumulation_state'] = 'MISSING_VOLUME_DATA'
        unavailable['silent_accumulation_reason'] = (
            'Coverage volume terbaru '
            f'{effective_volume_coverage:.1f}% (<90%); flow tidak dinilai.'
        )
        unavailable['silent_accumulation_data_coverage'] = round(
            effective_volume_coverage, 1,
        )
        return unavailable
    volume = volume_raw.clip(lower=0.0)
    open_ = pd.to_numeric(recent.get('Open', close.shift(1)), errors='coerce')
    spread = (high - low).replace(0, np.nan)
    close_location = ((close - low) / spread).clip(0.0, 1.0)
    lower_wick = ((np.minimum(open_, close) - low) / spread).clip(0.0, 1.0)
    returns = close.pct_change()
    value = pd.to_numeric(recent.get('VALUE', close * volume), errors='coerce').fillna(0.0).clip(lower=0.0)
    # ``prepare_indicators`` already calculated these rolling fields over the
    # full causal prefix. Reusing them avoids rebuilding several pandas rolling
    # graphs for every historical candidate in chronological validation.
    # Fallbacks preserve support for raw OHLCV frames used by public helpers.
    prepared_vol_ratio = pd.to_numeric(
        recent.get('VOL_RATIO', pd.Series(np.nan, index=recent.index)),
        errors='coerce',
    )
    if prepared_vol_ratio.notna().tail(60).any():
        vol_ratio = prepared_vol_ratio
    else:
        vol_ma20 = volume.rolling(20, min_periods=10).mean().replace(0, np.nan)
        vol_ratio = volume / vol_ma20
    value_median20 = value.rolling(20, min_periods=10).median().replace(0, np.nan)
    value_effort = value / value_median20

    def value_ratio(window: int) -> float:
        v = value.tail(window)
        r = returns.tail(window)
        up = float(v[r > 0].sum())
        down = float(v[r < 0].sum())
        return up / down if down > 0 else 3.0 if up > 0 else np.nan

    up_down20 = value_ratio(20)
    up_down60 = value_ratio(60)
    tail20 = recent.tail(20).index
    value20 = value.loc[tail20]
    clv20 = close_location.loc[tail20]
    weighted_clv20 = float((clv20 * value20).sum() / value20.sum()) if float(value20.sum()) > 0 else np.nan
    up_vol = volume.tail(20)[returns.tail(20) > 0]
    down_vol = volume.tail(20)[returns.tail(20) < 0]
    up_median = float(up_vol.median()) if len(up_vol) else np.nan
    down_median = float(down_vol.median()) if len(down_vol) else np.nan
    pullback_ratio = down_median / up_median if np.isfinite(up_median) and up_median > 0 and np.isfinite(down_median) else np.nan
    accumulation_days = int(((close_location.tail(20) >= 0.62) & (returns.tail(20) >= -0.005) & (vol_ratio.tail(20).between(0.60, 1.80))).sum())
    distribution_mask = (returns.tail(20) < -0.005) & (vol_ratio.tail(20) >= 1.30) & (close_location.tail(20) <= 0.38)
    distribution_days = int(distribution_mask.sum())
    churning_days = int(((returns.tail(20).abs() <= 0.015) & (vol_ratio.tail(20) >= 1.30) & (close_location.tail(20) >= 0.58)).sum())

    # V3 absorption evidence. A candidate event must be high-effort, recover
    # away from the low, and then avoid an immediate lower low. Confirmation
    # uses only bars that already exist at evaluation time.
    ...
    confirmed_absorption = pd.Series(False, index=recent.index)
    failed_absorption = pd.Series(False, index=recent.index)
    positions = list(range(max(0, len(recent) - 23), len(recent) - 1))
    for pos in positions:
        if not bool(candidate_absorption.iloc[pos]):
            continue
        future = recent.iloc[pos + 1 : min(len(recent), pos + 4)]
        if future.empty:
            continue
        event_low = _finite(low.iloc[pos], np.nan)
        event_close = _finite(close.iloc[pos], np.nan)
        future_low = pd.to_numeric(future['Low'], errors='coerce').min()
        future_close = pd.to_numeric(future['Close'], errors='coerce').max()
        if np.isfinite(event_low) and np.isfinite(event_close) and np.isfinite(future_low) and np.isfinite(future_close):
            holds_low = future_low >= event_low * 0.997
            recovers = future_close >= event_close * 1.003
            confirmed_absorption.iloc[pos] = bool(holds_low and recovers)
            failed_absorption.iloc[pos] = bool(future_low < event_low * 0.988)

    effort_absorption_mask = (
        (value_effort.tail(20) >= 1.25)
        & (returns.tail(20).abs() <= 0.015)
        & (close_location.tail(20) >= 0.60)
    )
    effort_distribution_mask = (
        (value_effort.tail(20) >= 1.25)
        & (returns.tail(20) <= 0.005)
        & (close_location.tail(20) <= 0.40)
    )
    lower_wick_support_mask = (
        (vol_ratio.tail(20) >= 1.00)
        & (lower_wick.tail(20) >= 0.35)
        & (close_location.tail(20) >= 0.55)
    )
    absorption_days = int(confirmed_absorption.tail(20).sum())
    failed_absorption_days = int(failed_absorption.tail(20).sum())
    effort_absorption_days = int(effort_absorption_mask.sum())
    effort_distribution_days = int(effort_distribution_mask.sum())
    lower_wick_support_days = int(lower_wick_support_mask.sum())
    accumulation_value = float(value.tail(20)[(close_location.tail(20) >= 0.58) & (returns.tail(20) >= -0.005)].sum())
    supply_value = float(value.tail(20)[(close_location.tail(20) <= 0.42) & (returns.tail(20) <= 0.005)].sum())
    supply_pressure_ratio = supply_value / accumulation_value if accumulation_value > 0 else np.nan

    row = recent.iloc[-1]
    cmf20_value = _finite(row.get('CMF20'), np.nan)
    cmf60_value = _finite(row.get('CMF60'), np.nan)
    obv_slope20 = _finite(row.get('OBV_SLOPE20'), np.nan)
    adl_slope20 = _finite(row.get('ADL_SLOPE20'), np.nan)
    vwap20_value = _finite(row.get('VWAP20'), np.nan)
    vwap60_value = _finite(row.get('VWAP60'), np.nan)
    if not all(
        np.isfinite(value)
        for value in (
            cmf20_value,
        ...
        mfm = (2.0 * close - high - low) / spread
        money_flow = mfm.fillna(0.0) * volume
        total_volume20 = volume.rolling(20, min_periods=10).sum().replace(0, np.nan)
        total_volume60 = volume.rolling(60, min_periods=25).sum().replace(0, np.nan)
        if not np.isfinite(cmf20_value):
            cmf20_value = _finite(
                (
                    money_flow.rolling(20, min_periods=10).sum()
                    / total_volume20
                ).iloc[-1],
                np.nan,
            )
        if not np.isfinite(cmf60_value):
            cmf60_value = _finite(
                (
                    money_flow.rolling(60, min_periods=25).sum()
                    / total_volume60
                ).iloc[-1],
                np.nan,
            )
        if not np.isfinite(obv_slope20):
            obv_local = (np.sign(close.diff()).fillna(0.0) * volume).cumsum()
            obv_slope20 = _finite(
                (obv_local.diff(20) / total_volume20).iloc[-1],
                np.nan,
            )
        if not np.isfinite(adl_slope20):
            adl_local = money_flow.cumsum()
            adl_slope20 = _finite(
                (adl_local.diff(20) / total_volume20).iloc[-1],
                np.nan,
            )
        typical = (high + low + close) / 3.0
        if not np.isfinite(vwap20_value):
            vwap20_value = _finite(
                ((typical * volume).rolling(20, min_periods=10).sum() / total_volume20).iloc[-1],
                np.nan,
            )
        if not np.isfinite(vwap60_value):
            vwap60_value = _finite(
                ((typical * volume).rolling(60, min_periods=25).sum() / total_volume60).iloc[-1],
                np.nan,
            )
    close_value = _finite(close.iloc[-1], 0.0)
    relative = _finite(row.get('REL_STRENGTH60'), np.nan)
    ema20_value = _finite(row.get('EMA20'), _finite(close.ewm(span=20, adjust=False).mean().iloc[-1], np.nan))
    ema50_value = _finite(row.get('EMA50'), _finite(close.ewm(span=50, adjust=False).mean().iloc[-1], np.nan))
    true_range = pd.concat([(high-low), (high-close.shift(1)).abs(), (low-close.shift(1)).abs()], axis=1).max(axis=1)
    atr14 = _finite(row.get('ATR14'), _finite(true_range.rolling(14, min_periods=7).mean().iloc[-1], np.nan))
    atr_median60 = _finite(true_range.rolling(14, min_periods=7).mean().rolling(60, min_periods=25).median().iloc[-1], np.nan)
    contraction = atr14 / atr_median60 if np.isfinite(atr14) and np.isfinite(atr_median60) and atr_median60 > 0 else np.nan

    # V4 regime and persistence layer. This uses only information available at
    # the evaluated bar. It changes emphasis, not the underlying evidence.
    ema200_value = _finite(
        row.get('EMA200'),
        _finite(close.ewm(span=200, adjust=False).mean().iloc[-1], np.nan),
    )
    return20 = _finite(close.pct_change(20).iloc[-1], np.nan)
    if np.isfinite(contraction) and contraction >= 1.30:
        accumulation_regime = 'VOLATILE'
        accumulation_weight_profile = 'DISTRIBUTION_AND_ABSORPTION_FIRST'
    elif all(np.isfinite(v) for v in (close_value, ema20_value, ema50_value)) and close_value >= ema20_value >= ema50_value and return20 >= 0.02:
        accumulation_regime = 'TRENDING_UP'
        accumulation_weight_profile = 'RELATIVE_STRENGTH_AND_ACCEPTANCE'
    elif all(np.isfinite(v) for v in (close_value, ema20_value, ema50_value)) and close_value <= ema20_value <= ema50_value and return20 <= -0.02:
        accumulation_regime = 'TRENDING_DOWN'
        accumulation_weight_profile = 'ABSORPTION_AND_RISK_CONTROL'
    elif np.isfinite(ema200_value) and close_value >= ema200_value:
        accumulation_regime = 'SIDEWAYS_ABOVE_LONG_TREND'
        accumulation_weight_profile = 'DRYUP_ABSORPTION_PERSISTENCE'
    else:
        accumulation_regime = 'SIDEWAYS_OR_BASE'
        accumulation_weight_profile = 'DRYUP_ABSORPTION_PERSISTENCE'

    full_distribution = (returns < -0.005) & (vol_ratio >= 1.30) & (close_location <= 0.38)
    full_positive = (
        ((close_location >= 0.58) & (returns >= -0.005) & vol_ratio.between(0.60, 1.80))
        | confirmed_absorption
        | ((lower_wick >= 0.35) & (close_location >= 0.55) & (vol_ratio >= 1.00))
    )
    persistence_window = full_positive.tail(60).fillna(False)
    negative_window = (full_distribution | failed_absorption).tail(60).fillna(False)
    rolling_net = (
        persistence_window.astype(float).rolling(10, min_periods=7).sum()
        - 1.5 * negative_window.astype(float).rolling(10, min_periods=7).sum()
    )
    valid_net = rolling_net.dropna()
    positive_windows_pct = 100.0 * float((valid_net >= 2.0).mean()) if len(valid_net) else 0.0
    longest_run = _longest_true_run(persistence_window)
    negative_share = float(negative_window.mean()) if len(negative_window) else 1.0
    accumulation_persistence_score = max(0.0, min(100.0,
        0.55 * positive_windows_pct
        + 25.0 * min(1.0, longest_run / 8.0)
        + 20.0 * (1.0 - min(1.0, negative_share / 0.20))
    ))

    flow_score = 0.0
    flow_score += 12.0 if cmf20_value >= 0.10 else 9.0 if cmf20_value >= 0.05 else 5.0 if cmf20_value > 0 else 0.0
    flow_score += 8.0 if cmf60_value >= 0.06 else 5.0 if cmf60_value > 0 else 0.0
    flow_score += 10.0 if np.isfinite(weighted_clv20) and weighted_clv20 >= 0.62 else 6.0 if np.isfinite(weighted_clv20) and weighted_clv20 >= 0.55 else 2.0 if np.isfinite(weighted_clv20) and weighted_clv20 >= 0.50 else 0.0
    persistence_score = 0.0
    persistence_score += 9.0 if adl_slope20 >= 0.08 else 6.0 if adl_slope20 > 0 else 0.0
    persistence_score += 8.0 if obv_slope20 >= 0.08 else 5.0 if obv_slope20 > 0 else 0.0
    persistence_score += min(5.0, 0.7 * accumulation_days + 1.2 * churning_days)
    asymmetry_score = 0.0
    asymmetry_score += 9.0 if np.isfinite(up_down20) and up_down20 >= 1.35 else 6.0 if np.isfinite(up_down20) and up_down20 >= 1.15 else 3.0 if np.isfinite(up_down20) and up_down20 >= 1.0 else 0.0
    asymmetry_score += 7.0 if np.isfinite(up_down60) and up_down60 >= 1.20 else 4.0 if np.isfinite(up_down60) and up_down60 >= 1.05 else 0.0
    dryup_score = 10.0 if np.isfinite(pullback_ratio) and pullback_ratio <= 0.75 else 7.0 if np.isfinite(pullback_ratio) and pullback_ratio <= 0.95 else 3.0 if np.isfinite(pullback_ratio) and pullback_ratio <= 1.10 else 0.0
    acceptance_score = 0.0
    acceptance_score += 4.0 if np.isfinite(vwap20_value) and close_value >= vwap20_value else 0.0
    acceptance_score += 3.0 if np.isfinite(vwap60_value) and close_value >= vwap60_value else 0.0
    acceptance_score += 3.0 if np.isfinite(ema20_value) and np.isfinite(ema50_value) and close_value >= ema20_value >= ema50_value else 1.5 if np.isfinite(ema50_value) and close_value >= ema50_value else 0.0
    acceptance_score += 2.0 if np.isfinite(contraction) and contraction <= 0.95 else 1.0 if np.isfinite(contraction) and contraction <= 1.10 else 0.0
    relative_score = 10.0 if relative >= 0.08 else 7.0 if relative > 0 else 3.0 if np.isfinite(relative) and relative >= -0.03 else 0.0
    penalty = min(24.0, 5.0 * distribution_days)
    if cmf20_value < -0.05:
        penalty += 7.0
    if _finite(vol_ratio.iloc[-1], 0.0) >= 1.6 and _finite(close_location.iloc[-1], 0.5) <= 0.35:
        penalty += 7.0
    base_score = max(0.0, min(100.0, flow_score + persistence_score + asymmetry_score + dryup_score + acceptance_score + relative_score - penalty))

    positive_adjustment = min(12.0, 2.5 * absorption_days + 0.55 * lower_wick_support_days + 1.0 * effort_absorption_days)
    negative_adjustment = min(18.0, 4.0 * failed_absorption_days + 2.0 * effort_distribution_days)
    if np.isfinite(supply_pressure_ratio):
        if supply_pressure_ratio <= 0.65:
            positive_adjustment += 2.0
        elif supply_pressure_ratio >= 1.35:
            negative_adjustment += 4.0
    v3_adjustment = max(-18.0, min(14.0, positive_adjustment - negative_adjustment))
    persistence_adjustment = max(-6.0, min(6.0, (accumulation_persistence_score - 50.0) * 0.12))
    if accumulation_regime == 'TRENDING_UP':
        adaptive_adjustment = 0.03 * (relative_score - 5.0) + 0.05 * (acceptance_score - 6.0)
    elif accumulation_regime in {'SIDEWAYS_ABOVE_LONG_TREND', 'SIDEWAYS_OR_BASE'}:
        adaptive_adjustment = 0.18 * (dryup_score - 5.0) + 0.20 * min(8.0, absorption_days + 0.5 * lower_wick_support_days)
    elif accumulation_regime == 'TRENDING_DOWN':
        adaptive_adjustment = 0.70 * absorption_days + 0.18 * lower_wick_support_days - 1.5 * distribution_days - 2.0 * failed_absorption_days
    else:  # VOLATILE
        adaptive_adjustment = 0.15 * absorption_days - 1.8 * distribution_days - 2.5 * failed_absorption_days
    adaptive_adjustment = max(-6.0, min(6.0, adaptive_adjustment))
    v4_adjustment = max(-10.0, min(10.0, persistence_adjustment + adaptive_adjustment))
    raw_score = max(0.0, min(100.0, base_score + v3_adjustment + v4_adjustment))
    persistent_bid_score = max(0.0, min(100.0,
        12.0 * absorption_days + 4.0 * lower_wick_support_days + 5.0 * churning_days
        + 4.0 * effort_absorption_days - 15.0 * failed_absorption_days - 9.0 * effort_distribution_days
    ))
    zero_volume = float(volume.tail(60).eq(0).mean())
    feature_values = (
        cmf20_value, cmf60_value, obv_slope20, adl_slope20, up_down20, up_down60,
        weighted_clv20, pullback_ratio, vwap20_value, vwap60_value, relative, contraction,
        persistent_bid_score, supply_pressure_ratio,
    )
    available = sum(np.isfinite(v) for v in feature_values)
    coverage = 100.0 * available / len(feature_values)
    confidence = max(0.0, min(100.0,
        0.48 * coverage
        + 0.32 * min(100.0, len(recent) / 60.0 * 100.0)
        + 0.20 * accumulation_persistence_score
        - 60.0 * zero_volume
        - (8.0 if accumulation_regime == 'VOLATILE' else 0.0)
    ))
    adtv20_idr = _finite(value.tail(20).mean(), 0.0)
    calibration = calibrate_silent_accumulation(
        raw_score,
        adtv20_idr,
        data_coverage=coverage,
        distribution_days=distribution_days,
        failed_absorption_days=failed_absorption_days,
    )
    score = float(calibration.calibrated_score)
    confidence = max(0.0, min(100.0, confidence * calibration.confidence_multiplier))
    if distribution_days >= 4 or failed_absorption_days >= 2 or (cmf20_value < -0.08 and adl_slope20 < 0):
        state = 'DISTRIBUTION_RISK'
    elif score >= max(75.0, calibration.minimum_confirmation_score) and distribution_days <= 2 and confidence >= 65 and accumulation_persistence_score >= 52 and (absorption_days >= 1 or churning_days >= 2 or accumulation_days >= 6):
        state = 'SILENT_ACCUMULATION_CONFIRMED'
    elif score >= 62 and distribution_days <= 3 and failed_absorption_days <= 1:
        state = 'EARLY_ACCUMULATION'
    elif score >= 45:
        state = 'NEUTRAL_FLOW'
    else:
        state = 'WEAK_OR_DISTRIBUTION'
    positives = []
    if cmf20_value > 0: positives.append(f'CMF20 {cmf20_value:.2f}')
    if adl_slope20 > 0: positives.append('ADL naik')
    if obv_slope20 > 0: positives.append('OBV naik')
    if np.isfinite(up_down20): positives.append(f'up/down value {up_down20:.2f}x')
    if absorption_days: positives.append(f'{absorption_days} absorption terkonfirmasi')
    if lower_wick_support_days: positives.append(f'{lower_wick_support_days} lower-wick support')
    if accumulation_persistence_score >= 60: positives.append(f'persistence {accumulation_persistence_score:.0f}')
    positives.append(f'regime {accumulation_regime}')
    positives.append(f'liquidity {calibration.bucket}')
    negatives = []
    if distribution_days: negatives.append(f'{distribution_days} distribution day')
    if failed_absorption_days: negatives.append(f'{failed_absorption_days} failed absorption')
    if effort_distribution_days: negatives.append(f'{effort_distribution_days} effort/result distribution')
    if cmf20_value < 0: negatives.append(f'CMF20 {cmf20_value:.2f}')
    reason = '; '.join(positives[:6] + negatives[:4]) or 'Price-volume evidence belum cukup.'
    return {
        'silent_accumulation_version': 'V5_LIQUIDITY_CALIBRATED_OUTCOME_READY',
        'silent_accumulation_score': round(score, 1),
        'silent_accumulation_raw_score': round(raw_score, 1),
        'silent_accumulation_liquidity_adjustment': round(calibration.adjustment, 1),
        'silent_accumulation_liquidity_min_confirmation': round(calibration.minimum_confirmation_score, 1),
        'silent_accumulation_calibration_policy': calibration.policy,
        'liquidity_bucket': calibration.bucket,
        'silent_accumulation_base_score_v2': round(base_score, 1),
        'silent_accumulation_v3_adjustment': round(v3_adjustment, 1),
        'silent_accumulation_v4_adjustment': round(v4_adjustment, 1),
        'silent_accumulation_adaptive_adjustment': round(adaptive_adjustment, 1),
        'accumulation_persistence_score': round(accumulation_persistence_score, 1),
        'accumulation_positive_windows_pct': round(positive_windows_pct, 1),
        'accumulation_longest_run': int(longest_run),
        'accumulation_regime': accumulation_regime,
        'accumulation_weight_profile': accumulation_weight_profile,
        'silent_accumulation_state': state,
        'silent_accumulation_confidence': round(confidence, 1),
        'silent_accumulation_reason': reason,
        'up_down_value_ratio20': round(up_down20, 3) if np.isfinite(up_down20) else np.nan,
        'up_down_value_ratio60': round(up_down60, 3) if np.isfinite(up_down60) else np.nan,
        'weighted_close_location20': round(weighted_clv20, 3) if np.isfinite(weighted_clv20) else np.nan,
        'cmf20_accumulation': round(cmf20_value, 4) if np.isfinite(cmf20_value) else np.nan,
        'cmf60_accumulation': round(cmf60_value, 4) if np.isfinite(cmf60_value) else np.nan,
        'adl_slope20': round(adl_slope20, 4) if np.isfinite(adl_slope20) else np.nan,
        'obv_slope20': round(obv_slope20, 4) if np.isfinite(obv_slope20) else np.nan,
        'pullback_volume_ratio20': round(pullback_ratio, 3) if np.isfinite(pullback_ratio) else np.nan,
        'accumulation_days20': accumulation_days,
        'distribution_days20': distribution_days,
        'churning_support_days20': churning_days,
        'absorption_confirmed_days20': absorption_days,
        'failed_absorption_days20': failed_absorption_days,
        'effort_result_absorption20': effort_absorption_days,
        'effort_result_distribution20': effort_distribution_days,
        'lower_wick_support_days20': lower_wick_support_days,
        'persistent_bid_score': round(persistent_bid_score, 1),
        'supply_pressure_ratio20': round(supply_pressure_ratio, 3) if np.isfinite(supply_pressure_ratio) else np.nan,
        'silent_accumulation_data_coverage': round(coverage, 1),
    }

def _silent_accumulation_metrics(frame: pd.DataFrame) -> tuple[float, float]:
    profile = _silent_accumulation_profile(frame)
    return (
        _finite(profile.get('silent_accumulation_score'), 0.0),
        _finite(profile.get('up_down_value_ratio20'), np.nan),
    )

def _historical_gate_inputs(
    df: pd.DataFrame,
    config: ScanConfig,
    accumulation_profile: Mapping[str, object] | None = None,
):
    ...
    profile = (
        dict(accumulation_profile)
        if accumulation_profile is not None
        else _silent_accumulation_profile(df)
    )
    metrics.update(profile)
    minimum = float(getattr(config, 'silent_accumulation_min_score', 62.0))
    distribution_limit = int(getattr(config, 'silent_accumulation_max_distribution_days', 3))
    if config.real_money_mode and (
        safe_number(profile.get('silent_accumulation_score'), 0.0) < minimum
        or int(safe_number(profile.get('distribution_days20'), 0.0)) > distribution_limit
    ):
        blockers.append('Price-volume accumulation belum lolos gate')
    return (blockers, metrics)
...
def classify_provider_error(exc: BaseException) -> str:
    ...
    if (
        '404' in text or 'no data' in text or 'no price data' in text
        or 'not found' in text or 'possibly delisted' in text
        or 'PRICESMISSING' in name
    ):
...
def _fundamental_failure_row(ticker: str, code: str, message: object) -> dict[str, Any]:
    return {
        ...
        'fundamental_complete_for_multibagger': False,
        'fundamental_lineage_notes': '',
        'fundamental_red_flags': '',
        ...
        'fundamental_provider': 'Yahoo Fundamentals public routes',
...
def _mark_yahoo_provider_unavailable(code: object, reason: object) -> None:
    ...
    payload = {
        ...
        'blocked_until': (now + timedelta(minutes=cooldown_minutes)).isoformat(),
    }
    path = _yahoo_provider_health_path()
    tmp = path.with_suffix('.tmp')
    try:
...
def _direct_fundamental_history_cache_path(ticker: str) -> Path:
    safe = re.sub('[^A-Z0-9._-]', '_', str(ticker).upper())
    root = _cache_root() / 'fundamental_timeseries_direct'
    root.mkdir(parents=True, exist_ok=True)
    return root / f'{safe}.json'


def _write_recent_direct_fundamental_history(
    ticker: str, history: pd.DataFrame,
) -> None:
    normalized = normalize_fundamental_history(history)
    if normalized.empty:
        return
    path = _direct_fundamental_history_cache_path(ticker)
    tmp = path.with_name(f'{path.name}.tmp')
    try:
        checked_at = pd.Timestamp.now(tz='Asia/Jakarta').isoformat()
        records = json.loads(normalized.to_json(
...
def _load_recent_direct_fundamental_history(
    ticker: str, max_age_hours: float=6.0,
) -> pd.DataFrame:
    path = _direct_fundamental_history_cache_path(ticker)
    try:
        if not path.is_file():
        ...
        frame.attrs['provider_checked_at'] = payload.get('provider_checked_at', '')
        return normalize_fundamental_history(frame)
    except Exception:
        return _fundamental_empty_history()
...
def fetch_one_fundamental(ticker: str) -> dict[str, Any]:
    ...
    info: dict[str, Any] = {}
    provider = ''
    recent = _load_recent_direct_fundamental_history(ticker)
    if recent is not None and not recent.empty:
        enriched = enrich_fundamentals_with_history(
            pd.DataFrame({'ticker': [ticker]}), recent, now=now,
        )
        if enriched is not None and not enriched.empty:
            history_row = enriched.iloc[0].to_dict()
            coverage = _finite(history_row.get('fundamental_coverage'), 0.0)
            score = _finite(history_row.get('fundamental_score'), np.nan)
            if coverage > 0.0 and np.isfinite(score):
                history_row.update({
                    'ticker': ticker, 'fundamental_error_code': '',
                    'fundamental_error': '',
                    'fundamental_provider': 'Yahoo Fundamentals Timeseries Direct (same-run cache)',
                    'fundamental_fetched_at': pd.Timestamp.now(tz='Asia/Jakarta').isoformat(),
                })
                return history_row
        ...
        try:
            ...
            enriched = (
                enrich_fundamentals_with_history(
                    pd.DataFrame({'ticker': [ticker]}), normalized, now=now,
                )
                if not normalized.empty else pd.DataFrame()
            )
            if enriched is not None and not enriched.empty:
                history_row = enriched.iloc[0].to_dict()
                coverage = _finite(history_row.get('fundamental_coverage'), 0.0)
                ...
                if coverage > 0.0 and np.isfinite(score):
                    history_row.update({
                        'ticker': ticker,
                        'fundamental_error_code': '',
                        'fundamental_error': '',
                        'fundamental_provider_warning': ' | '.join(errors),
                        'fundamental_provider': 'Yahoo Fundamentals Timeseries Direct',
                        'fundamental_fetched_at': pd.Timestamp.now(tz='Asia/Jakarta').isoformat(),
                    })
                    return history_row
                errors.append(
                    f'timeseries direct insufficient history coverage={coverage:.1f}'
                )
            else:
                errors.append('timeseries direct NO_NORMALIZED_HISTORY')
        except Exception as exc:
            errors.append(f'timeseries direct {type(exc).__name__}: {str(exc)[:100]}')
    if not info and _yfinance_fallback_enabled():
        try:
            import yfinance as yf
            info = yf.Ticker(ticker).get_info() or {}
            provider = 'Yahoo Finance via yfinance (opt-in fallback)'
        except Exception as exc:
            errors.append(f'yfinance fallback {type(exc).__name__}: {str(exc)[:100]}')
    if not info:
        return _fundamental_failure_row(ticker, 'ALL_SNAPSHOT_PROVIDERS_FAILED', ' | '.join(errors))
    ...
    row.update({
        'ticker': ticker, 'latest_statement_date': latest_statement,
        'statement_age_days': statement_age, 'fundamental_error': ' | '.join(errors),
        'fundamental_provider': provider or 'Yahoo public JSON',
        'fundamental_fetched_at': pd.Timestamp.now(tz='Asia/Jakarta').isoformat(),
    })
    return row
...
def _official_idx_reference(value: object) -> bool:
    """Accept current and legacy HTTPS hosts operated by IDX."""
    from urllib.parse import urlparse
    try:
        parsed = urlparse(str(value or '').strip())
        return parsed.scheme == 'https' and _is_official_idx_host(parsed.hostname)
    except Exception:
        return False
...
def normalize_fundamental_history(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    # mixed-offset Series as object dtype, so calling ``.dt`` directly raises
    # ``AttributeError``.  Normalize every scalar first, then materialise a
    # homogeneous tz-naive datetime64 Series.  Period-end facts are date
    ...
    # this canonical public reference allows the narrative engine to retain a
    # verifiable source lineage instead of treating valid structured financial
    # evidence as missing.  This does not upgrade Yahoo to an official source.
    yahoo_missing_url = out['source_family'].eq('YAHOO') & out['source_url'].str.strip().eq('')
    if yahoo_missing_url.any():
        out.loc[yahoo_missing_url, 'source_url'] = out.loc[yahoo_missing_url, 'ticker'].map(
            lambda value: f"https://finance.yahoo.com/quote/{value}/financials"
        )
    twelve_missing_url = out['source_family'].eq('TWELVE_DATA') & out['source_url'].str.strip().eq('')
    if twelve_missing_url.any():
        out.loc[twelve_missing_url, 'source_url'] = 'https://twelvedata.com/fundamentals'
    ...
    # Merge complementary facts from duplicate rows of the same provider and
    # period.  Choosing one whole row can discard a valid cash-flow fact fetched
    # in a different request; field-wise coalescing retains the best verified
    # observation while preserving provider independence for consensus checks.
    out = out.sort_values(['ticker', 'source_family', 'period_end', 'source_verified'])
    keys = ['ticker', 'source_family', 'period_end', 'period_type']
    merged_rows: list[pd.Series] = []
    for _, group in out.groupby(keys, sort=False, dropna=False):
        group = group.sort_values('source_verified')
        base = group.iloc[-1].copy()
        for column in _FUNDAMENTAL_VALUE_ALIASES:
            values = pd.to_numeric(group[column], errors='coerce').dropna()
            if not values.empty:
                base[column] = values.iloc[-1]
        for column in ('source_name', 'source_url', 'currency', 'statement_basis'):
        ...
        base['validation_flags'] = ' • '.join(dict.fromkeys(flags))
        base['source_verified'] = bool(group['source_verified'].fillna(False).astype(bool).any())
        merged_rows.append(base)
    out = pd.DataFrame(merged_rows) if merged_rows else out.iloc[0:0].copy()
...
def _yahoo_statement_rows_yfinance(ticker: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    try:
        import yfinance as yf
        ...
        for period_type, attributes in (
            ...
            for period in sorted(periods):
                row: dict[str, Any] = {
                    ...
                    'currency': 'IDR', 'source_verified': False,
                    'validation_flags': '',
                }
                for canonical, aliases in _FUNDAMENTAL_VALUE_ALIASES.items():
                    source_statement = (
...
def _yahoo_statement_rows(ticker: str, allow_wrapper_fallback: bool | None=None) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    direct_report: dict[str, Any] = {}
    recent = _load_recent_direct_fundamental_history(ticker)
    if recent is not None and not recent.empty:
        rows = recent.to_dict('records')
        return rows, {
            'ticker': ticker, 'provider': 'YAHOO_DIRECT_SAME_RUN_CACHE',
            'status': 'OK', 'rows': len(rows), 'error': '', 'error_code': '',
        }
    try:
        from free_data_providers import yahoo_fundamental_timeseries_direct
        ...
        if direct is not None and not direct.empty:
            ...
            direct_report.update({
                'ticker': ticker, 'provider': 'YAHOO_DIRECT', 'status': 'OK',
                'rows': len(rows), 'error': '', 'error_code': '',
            })
            return rows, direct_report
        direct_report.update({
            'ticker': ticker, 'provider': 'YAHOO_DIRECT', 'status': 'NO_DATA',
            'rows': 0, 'error': 'Yahoo timeseries direct returned no data',
            'error_code': 'NO_SYMBOL_DATA',
        })
    except Exception as exc:
        direct_report = {
            'ticker': ticker, 'provider': 'YAHOO_DIRECT', 'status': 'FAILED',
            'rows': 0, 'error': f'{type(exc).__name__}: {str(exc)[:140]}',
            'error_code': classify_provider_error(exc),
        }
    wrapper_enabled = _yfinance_fallback_enabled() if allow_wrapper_fallback is None else bool(allow_wrapper_fallback)
        ...
        return [], {
            **direct_report,
            ...
            'error': ' | '.join(filter(None, [
                str(direct_report.get('error') or ''),
                _yahoo_cooldown_message(health),
            ])),
            'error_code': 'PROVIDER_COOLDOWN',
        }
    legacy_rows, legacy_report = _yahoo_statement_rows_yfinance(ticker)
    if legacy_rows:
        return legacy_rows, legacy_report
    legacy_report['error'] = ' | '.join(filter(None, [
        str(direct_report.get('error') or ''),
        str(legacy_report.get('error') or ''),
    ]))
...
def fetch_yahoo_fundamental_history(tickers: Iterable[str], max_workers: int=2, max_tickers: int=40, enable_yfinance_fallback: bool=False) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    health = _yahoo_provider_health_status()
    # Prefer complete persistent cache during cooldown.  If one or more requested
    # issuers are missing, continue to the dependency-light direct JSON route; a
    # yfinance-specific breaker must not starve cold-start data completion.
    cooldown_message = _yahoo_cooldown_message(health) if health else ''
    cached_names = set(cached.get('ticker', pd.Series(dtype=str)).astype(str)) if not cached.empty else set()
    if health and set(names).issubset(cached_names):
        reports = pd.DataFrame([{
            'ticker': ticker, 'provider': 'YAHOO', 'status': 'CACHE_FALLBACK',
            'rows': int(len(cached.loc[cached['ticker'].eq(ticker)])),
            'error': cooldown_message, 'error_code': 'PROVIDER_COOLDOWN',
        } for ticker in names])
        return cached, reports

    rows: list[dict[str, Any]] = []
    reports: list[dict[str, Any]] = []
    first_rows, first_report = _yahoo_statement_rows(names[0], allow_wrapper_fallback=enable_yfinance_fallback)
    ...
    if first_rows:
        _clear_yahoo_provider_unavailable()
        first_current = normalize_fundamental_history(pd.DataFrame(rows))
        cached_all = _persist_fundamental_history('yahoo_fundamental_history', cached_all, first_current)
    ...
    for offset in range(0, len(remaining), chunk_size):
        chunk = remaining[offset:offset + chunk_size]
        with ThreadPoolExecutor(max_workers=min(workers, len(chunk))) as pool:
            futures = {pool.submit(_yahoo_statement_rows, ticker, enable_yfinance_fallback): ticker for ticker in chunk}
        ...
        # Persist each completed provider batch.  A long 400-ticker refresh can
        # therefore continue from durable statement history after interruption.
        progressive = normalize_fundamental_history(pd.DataFrame(rows))
        if not progressive.empty:
            cached_all = _persist_fundamental_history('yahoo_fundamental_history', cached_all, progressive)
...
def fetch_twelve_data_fundamental_history(tickers: Iterable[str], api_key: str, max_tickers: int=20, timeout: int=15, max_workers: int=4) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def one_ticker(ticker: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [pool.submit(one_ticker, ticker) for ticker in names]
        for future in as_completed(futures):
...
IDX_FINANCIAL_REPORT_ENDPOINTS: tuple[str, ...] = (
    'https://block.idx.id/primary/ListedCompany/GetFinancialReport',
    'https://block.idx.id/umbraco/Surface/ListedCompany/GetFinancialReport',
    'https://www.idx.id/primary/ListedCompany/GetFinancialReport',
    'https://www.idx.id/umbraco/Surface/ListedCompany/GetFinancialReport',
...
IDX_FINANCIAL_STATEMENTS_PAGE = (
    'https://www.idx.id/id/perusahaan-tercatat/laporan-keuangan-dan-tahunan'
)
...
def _idx_attachment_url(value: object) -> str:
    ...
    if not raw:
        return ''
    url = urljoin('https://www.idx.id/', raw)
    parsed = urlparse(url)
    host = (parsed.hostname or '').lower()
    if parsed.scheme != 'https' or not _is_official_idx_host(host):
        return ''
    return url
...
def _idx_manifest_rows(
    ...
    if request_get is None:
        ...
        for seed_url in _idx_url_candidates(IDX_FINANCIAL_STATEMENTS_PAGE):
            try:
                seed = session.get(seed_url, headers=seed_headers, timeout=timeout, allow_redirects=True)
                if int(getattr(seed, 'status_code', 0)) == 200 and _official_idx_reference(getattr(seed, 'url', seed_url)):
                    seed_error = ''
                    break
                seed_error = f'seed HTTP {getattr(seed, "status_code", "?")}'
            except Exception as exc:
        ...
        for period_code, period_type in IDX_FINANCIAL_REPORT_PERIODS:
            ...
            if circuit_open:
                audit.append({
                    ...
                    'scope': f'{year}/{period_code}', 'status': 'CIRCUIT_OPEN',
                    'rows': 0, 'matched_tickers': 0,
                    'error': 'Provider IDX dihentikan untuk batch ini setelah blocking HTTP response',
            ...
            for candidate_index in range(endpoint_index, len(IDX_FINANCIAL_REPORT_ENDPOINTS)):
                ...
                try:
                    for page in range(max(1, int(max_pages))):
                        ...
                        endpoint_origin = f'{endpoint_parsed.scheme}://{endpoint_parsed.netloc}'
                        request_headers = dict(headers)
                        request_headers['Origin'] = endpoint_origin
                        request_headers['Referer'] = endpoint_origin + '/id/perusahaan-tercatat/laporan-keuangan-dan-tahunan/'
                        response = getter(
                            endpoint,
                            params={
                    ...
                    endpoint_errors.append(last_error)
                    endpoint_rows = []
                    blocking_tokens = ('HTTP 401', 'HTTP 403', 'HTTP 429')
                    if len(endpoint_errors) >= 3 and all(
                        any(token in error for token in blocking_tokens)
                        for error in endpoint_errors
                    ):
                        break
                    continue
            blocking_tokens = ('HTTP 401', 'HTTP 403', 'HTTP 429')
            if not endpoint_used and endpoint_errors and all(any(token in error for token in blocking_tokens) for error in endpoint_errors):
                circuit_open = True
                last_error = (last_error + (' | ' + seed_error if seed_error else ''))[:420]
...
def combine_fundamental_history(*frames: pd.DataFrame) -> pd.DataFrame:
    ...
    for frame in frames:
        if frame is None or frame.empty:
            continue
        clean = frame.dropna(axis=1, how="all")
        if clean.shape[1] > 0:
            records.extend(clean.to_dict(orient="records"))
    if not records:
        return _fundamental_empty_history()
    return normalize_fundamental_history(pd.DataFrame.from_records(records))


def _median_statement_periods(frame: pd.DataFrame, annual: bool=False) -> pd.DataFrame:
    """Build one period row with official verified facts taking precedence.
    ...
    rows: list[dict[str, object]] = []
    for period_end, group in selected.groupby('period_end', sort=True):
        official = group.loc[
            group['source_family'].eq('IDX_OFFICIAL_XBRL')
...
def _window_ratio_change(
    frame: pd.DataFrame,
    numerator: str,
    denominator: str,
    window: int = 4,
) -> tuple[float, float, float]:
    if len(frame) < 2 * int(window):
        return np.nan, np.nan, np.nan
    current_num = pd.to_numeric(frame[numerator].tail(window), errors='coerce')
    current_den = pd.to_numeric(frame[denominator].tail(window), errors='coerce')
    prior_num = pd.to_numeric(
        frame[numerator].iloc[-2 * window:-window], errors='coerce',
    )
    prior_den = pd.to_numeric(
        frame[denominator].iloc[-2 * window:-window], errors='coerce',
    )
    if any(values.notna().sum() != window for values in (
        current_num, current_den, prior_num, prior_den,
    )):
        return np.nan, np.nan, np.nan
    current_ratio = _safe_ratio(current_num.sum(), current_den.sum())
    prior_ratio = _safe_ratio(prior_num.sum(), prior_den.sum())
    change = (
        current_ratio - prior_ratio
        if np.isfinite(current_ratio) and np.isfinite(prior_ratio)
        else np.nan
    )
    return current_ratio, prior_ratio, change
...
def _history_features_for_ticker(ticker: str, history: pd.DataFrame, now: Any | None=None) -> dict[str, Any]:
    ...
    official_rows = local.loc[
        local['source_family'].eq('IDX_OFFICIAL_XBRL')
        & local['source_verified'].fillna(False).astype(bool)
    ].copy()
    official_critical_coverage = 0.0
    official_latest_period = pd.NaT
    if not official_rows.empty:
        official_latest_period = official_rows['period_end'].max()
        latest_official = official_rows.loc[official_rows['period_end'].eq(official_latest_period)]
        critical_presence = [
            pd.to_numeric(latest_official.get('revenue'), errors='coerce').notna().any(),
            pd.to_numeric(latest_official.get('net_income'), errors='coerce').notna().any(),
            pd.to_numeric(latest_official.get('operating_cash_flow'), errors='coerce').notna().any(),
            pd.to_numeric(latest_official.get('total_assets'), errors='coerce').notna().any(),
            pd.to_numeric(latest_official.get('equity'), errors='coerce').notna().any(),
            (
                pd.to_numeric(latest_official.get('total_debt'), errors='coerce').notna().any()
                or pd.to_numeric(latest_official.get('total_liabilities'), errors='coerce').notna().any()
            ),
        ]
        official_critical_coverage = 100.0 * sum(bool(v) for v in critical_presence) / len(critical_presence)
    source_url_rows = local.copy()
    source_url_rows['_url'] = source_url_rows.get(
        'source_url', pd.Series('', index=source_url_rows.index),
    ).fillna('').astype(str).str.strip()
    source_url_rows = source_url_rows[source_url_rows['_url'].str.startswith('https://')]
    if not source_url_rows.empty:
        source_url_rows['_official_rank'] = (
            source_url_rows['source_family'].isin(
                {'IDX_OFFICIAL_REFERENCE', 'IDX_OFFICIAL_XBRL'}
            ).astype(int)
            + source_url_rows.get(
                'source_verified', pd.Series(False, index=source_url_rows.index),
            ).fillna(False).astype(bool).astype(int)
        )
        source_url_rows = source_url_rows.sort_values(
            ['period_end', '_official_rank'],
            ascending=[False, False],
            kind='stable',
        )
        source_urls = list(dict.fromkeys(source_url_rows['_url'].tolist()))[:8]
    ...
    all_validation_flags = sorted({
        flag.strip()
        for value in local['validation_flags'].fillna('').astype(str)
        for flag in re.split(r'\s*[|•]\s*', value)
        if flag.strip()
    })
    informational_prefixes = (
        'OCF_RECONSTRUCTED_FROM_REPORTED_FCF_AND_CAPEX',
    )
    lineage_notes = [
        flag for flag in all_validation_flags
        if flag.startswith(informational_prefixes)
    ]
    identity_flags = [
        flag for flag in all_validation_flags
        if not flag.startswith(informational_prefixes)
    ]
    ...
    earnings_growth_acceleration = (
        earnings_yoy - prior_earnings_yoy
        if np.isfinite(earnings_yoy) and np.isfinite(prior_earnings_yoy)
        else np.nan
    )
    ...
    gross_profit_ttm = _ttm_sum(quarterly, 'gross_profit')
    ...
    _, _, cash_conversion_change_yoy = _window_ratio_change(
        quarterly, 'operating_cash_flow', 'net_income',
    )
    ...
    debt_prior = _num(prior_year.get('total_debt'))
    leverage_latest = _safe_ratio(debt_latest, assets_latest)
    leverage_prior = _safe_ratio(debt_prior, assets_prior)
    feature_values = {
        'history_revenue_growth': revenue_yoy,
        'history_earnings_growth': earnings_yoy,
        'history_prior_revenue_growth': prior_revenue_yoy,
        'history_prior_earnings_growth': prior_earnings_yoy,
        'history_revenue_growth_acceleration': revenue_growth_acceleration,
        'history_earnings_growth_acceleration': earnings_growth_acceleration,
        'history_gross_margin_change_yoy': gross_margin_change_yoy,
        'history_cash_conversion_change_yoy': cash_conversion_change_yoy,
        ...
        'history_gross_margin': _safe_ratio(gross_profit_ttm, revenue_ttm),
        'history_gross_profit_growth': gross_profit_growth,
        'history_gross_profitability': _safe_ratio(gross_profit_ttm, average_assets),
        ...
        'history_accruals_to_assets': accruals_to_assets,
        'history_leverage_change_yoy': (
            leverage_latest - leverage_prior
            if np.isfinite(leverage_latest) and np.isfinite(leverage_prior) else np.nan
        ),
        'history_revenue_ttm': revenue_ttm,
        'history_gross_profit_ttm': gross_profit_ttm,
        ...
        'history_shares_outstanding_latest': shares_latest,
        'history_car': _num(latest.get('car')),
        'history_npl_gross': _num(latest.get('npl_gross')),
        'history_ldr': _num(latest.get('ldr')),
    }
    inflection_items = [
        (_linear_score(revenue_growth_acceleration, -0.10, 0.10), 0.30),
        (_linear_score(earnings_growth_acceleration, -0.20, 0.20), 0.30),
        (_linear_score(gross_margin_change_yoy, -0.03, 0.03), 0.25),
        (_linear_score(cash_conversion_change_yoy, -0.50, 0.50), 0.15),
    ]
    available_inflections = [
        (score, weight)
        for score, weight in inflection_items
        if score is not None and np.isfinite(score)
    ]
    inflection_weight = sum(weight for _, weight in available_inflections)
    feature_values['fundamental_inflection_score'] = (
        sum(score * weight for score, weight in available_inflections)
        / inflection_weight
        if inflection_weight else np.nan
    )
    feature_values['fundamental_inflection_coverage_pct'] = round(
        100.0 * inflection_weight / sum(weight for _, weight in inflection_items),
        1,
    )
    required_inputs = (
    ...
    core_field_values = {
        ...
        'operating_cash_flow_ttm': ocf_ttm, 'free_cash_flow_ttm': fcf_ttm,
        'total_assets': assets_latest, 'equity': equity_latest,
        'total_debt': debt_latest, 'cash': cash_latest,
        'shares_outstanding': shares_latest, 'ebit_ttm': ebit_ttm,
    }
    missing_core_fields = [name for name, value in core_field_values.items() if not np.isfinite(value)]
    income_family = [revenue_ttm, gross_profit_ttm, operating_income_ttm, net_income_ttm]
    balance_family = [assets_latest, equity_latest, debt_latest, cash_latest, shares_latest]
    cashflow_family = [ocf_ttm, capex_ttm, fcf_ttm]
    family_coverages = {
        'INCOME': 100.0 * sum(np.isfinite(v) for v in income_family) / len(income_family),
        'BALANCE_SHEET': 100.0 * sum(np.isfinite(v) for v in balance_family) / len(balance_family),
        'CASH_FLOW': 100.0 * sum(np.isfinite(v) for v in cashflow_family) / len(cashflow_family),
    }
    statement_family_coverage = float(np.mean(list(family_coverages.values())))
    period_coverage = min(100.0, max(100.0 * len(quarterly) / 8.0, 100.0 * len(annual) / 3.0))
    complete_for_multibagger = bool(
        history_coverage >= 70.0 and statement_family_coverage >= 65.0
        and period_coverage >= 75.0 and len(missing_core_fields) <= 2
        and source_count >= 1 and latest_age <= 365 and not severe_conflict
    )
    ...
    return {
        ...
        **feature_values,
        ...
        'fundamental_source_urls': ' | '.join(source_urls),
        'fundamental_official_source_urls': ' | '.join(official_source_urls),
        'fundamental_official_reference': official_referenced,
        'fundamental_official_verified': official_verified,
        'fundamental_official_source_coverage_pct': round(float(official_critical_coverage), 1),
        'fundamental_official_latest_period': official_latest_period,
        'fundamental_reconciliation_state': reconciliation_state,
        ...
        'fundamental_lineage_notes': ' • '.join(lineage_notes),
        'fundamental_data_grade': grade,
        'fundamental_primary_currency': primary_currency,
        'fundamental_missing_core_fields': ' | '.join(missing_core_fields),
        'fundamental_statement_family_coverage_pct': round(statement_family_coverage, 1),
        'fundamental_income_statement_coverage_pct': round(family_coverages['INCOME'], 1),
        'fundamental_balance_sheet_coverage_pct': round(family_coverages['BALANCE_SHEET'], 1),
        'fundamental_cashflow_statement_coverage_pct': round(family_coverages['CASH_FLOW'], 1),
        'fundamental_history_period_coverage_pct': round(period_coverage, 1),
        'fundamental_complete_for_multibagger': complete_for_multibagger,
...
def _history_business_score(row: Mapping[str, Any], is_financial: bool=False) -> tuple[float, float, str]:
    ...
    gross_profitability = _num(row.get('history_gross_profitability'))
    gross_margin = _num(row.get('history_gross_margin'))
    gross_profit_growth = _num(row.get('history_gross_profit_growth'))
    fundamental_inflection = _num(row.get('fundamental_inflection_score'))
    accruals_to_assets = _num(row.get('history_accruals_to_assets'))
    leverage_change = _num(row.get('history_leverage_change_yoy'))
    items: list[tuple[float | None, float]] = [
        (_linear_score(growth, -0.05, 0.18), 12),
        (_linear_score(earnings_growth, -0.10, 0.22), 10),
        (_linear_score(gross_profit_growth, -0.05, 0.18), 5),
        (fundamental_inflection, 7),
        (_linear_score(gross_profitability, 0.03, 0.30), 8),
        (_linear_score(gross_margin, 0.08, 0.40), 4),
        ...
        (_linear_score(accruals_to_assets, 0.10, -0.02, higher_is_better=True), 5),
    ...
    if not is_financial:
        items.extend([
            ...
            (_linear_score(leverage_change, 0.08, -0.04, higher_is_better=True), 3),
        ])
    ...
    if np.isfinite(accruals_to_assets) and accruals_to_assets > 0.08:
        flags.append('Akrual tinggi terhadap aset')
    if np.isfinite(gross_profit_growth) and gross_profit_growth < 0:
        flags.append('Gross profit menyusut')
    if np.isfinite(leverage_change) and leverage_change > 0.08:
        flags.append('Leverage terhadap aset meningkat')
...
def enrich_fundamentals_with_history(fundamentals: pd.DataFrame, history: pd.DataFrame, now: Any | None=None) -> pd.DataFrame:
    ...
    if features.empty:
        ...
        # A point-in-time provider snapshot is useful, but it is not statement
        # history and must not masquerade as one historical source.  Keep both
        # coverages explicit so the UI can distinguish "snapshot available"
        # from "quarterly history verified".
        snapshot_coverage = pd.to_numeric(
            out.get('fundamental_coverage', pd.Series(0.0, index=out.index)),
            errors='coerce',
        ).fillna(0.0)
        out['fundamental_source_count'] = 0
        out['fundamental_history_source_count'] = 0
        out['fundamental_snapshot_source_count'] = snapshot_coverage.gt(0.0).astype(int)
        out['fundamental_source_families'] = ''
        ...
        return out
    # Cached snapshot payloads can already contain columns from an older
    # history enrichment.  A plain merge would create ``*_x``/``*_y`` pairs;
    # downstream code would then miss the canonical names and silently assign
    # grade D/source_count 0 to every issuer.  Recomputed statement features
    # are authoritative for this run, so remove their stale copies first.
    history_feature_columns = [
        column for column in features.columns
        if column != 'ticker' and column in base.columns
    ]
    clean_base = base.drop(columns=history_feature_columns, errors='ignore')
    out = clean_base.merge(features, on='ticker', how='outer')
    current_score = (
    ...
    cached_snapshot_score = (
        pd.to_numeric(out['fundamental_score_snapshot'], errors='coerce')
        if 'fundamental_score_snapshot' in out
        else pd.Series(np.nan, index=out.index, dtype=float)
    )
    # A database row may contain last run's combined history score plus the
    # original provider snapshot. Reusing the combined score as the next
    # snapshot would recursively double-count history on every scan.
    snapshot_score = cached_snapshot_score.where(cached_snapshot_score.notna(), current_score)
    current_coverage = (
    ...
    cached_snapshot_coverage = (
        pd.to_numeric(out['fundamental_coverage_snapshot'], errors='coerce')
        if 'fundamental_coverage_snapshot' in out
        else pd.Series(np.nan, index=out.index, dtype=float)
    )
    snapshot_coverage = cached_snapshot_coverage.where(cached_snapshot_coverage.notna(), current_coverage)
    out['fundamental_score_snapshot'] = snapshot_score
    out['fundamental_coverage_snapshot'] = snapshot_coverage.fillna(0.0)
    # Preserve point-in-time proxy growth before latest normalized statement
    # history overrides the analytical fields.  This makes stale-proxy vs
    # latest-period sign conflicts auditable instead of silently disappearing.
    for _metric in ('revenue_growth', 'earnings_growth'):
        _snapshot_col = f'{_metric}_snapshot'
        if _snapshot_col not in out.columns:
            out[_snapshot_col] = pd.to_numeric(
                out.get(_metric, pd.Series(np.nan, index=out.index)), errors='coerce'
            )
    if 'latest_statement_date_snapshot' not in out.columns:
        out['latest_statement_date_snapshot'] = out.get(
            'latest_statement_date', pd.Series(pd.NaT, index=out.index)
        )
    out['fundamental_data_grade'] = out.get(
        'fundamental_data_grade',
        pd.Series('D', index=out.index),
    ).fillna('D').replace('', 'D')
    history_source_count = pd.to_numeric(
        out.get('fundamental_source_count', pd.Series(0.0, index=out.index)),
        errors='coerce',
    ).fillna(0.0).clip(lower=0.0)
    out['fundamental_source_count'] = history_source_count.astype(int)
    out['fundamental_history_source_count'] = history_source_count.astype(int)
    out['fundamental_snapshot_source_count'] = (
        out['fundamental_coverage_snapshot'].gt(0.0).astype(int)
    )
    out['fundamental_source_families'] = out.get(
    ...
    for index, row in out.iterrows():
        ...
        if np.isfinite(history_score) and np.isfinite(snapshot_score):
            combined_coverage = (
                0.55 * history_coverage + 0.45 * snapshot_coverage
            )
        elif np.isfinite(history_score):
            # A complete normalized statement history is direct financial
            # evidence, not 55% of a missing quoteSummary snapshot. Use its
            # measured field coverage without inventing the absent snapshot.
            combined_coverage = history_coverage
        else:
            combined_coverage = snapshot_coverage
        # Database rows can carry ISO-8601 offsets while history rows are often
        # date-only/tz-naive. Normalize both candidates before comparison and
        # age arithmetic; pandas rejects tz-aware minus tz-naive timestamps.
        latest_statement = _latest_jakarta_naive_timestamp(
            row.get('fundamental_history_latest_period'),
            row.get('latest_statement_date'),
        )
        current = _as_jakarta_naive_timestamp(now if now is not None else pd.Timestamp.now(tz='Asia/Jakarta'))
        age = (
            int((current.normalize() - latest_statement.normalize()).days)
...
def _valuation_financial_model(row: Mapping[str, Any]) -> tuple[bool, str]:
    """Resolve financial-sector valuation rules from explicit or profile data."""
    explicit = _safe_text(row.get('fundamental_model')).upper()
    if explicit == 'FINANCIAL':
        return True, 'EXPLICIT_FUNDAMENTAL_MODEL'
    profile_text = ' '.join(
        _safe_text(row.get(column)).lower()
        for column in (
            'sector', 'industry', 'company_name', 'long_name', 'short_name',
            'longName', 'shortName',
        )
    )
    financial_tokens = (
        'financial', 'bank', 'finance', 'multifinance', 'multiartha',
        'insurance', 'asuransi',
    )
    if any(token in profile_text for token in financial_tokens):
        return True, 'SECTOR_OR_COMPANY_PROFILE'
    prudential_fields = (
        row.get('history_car'), row.get('history_npl_gross'),
        row.get('history_ldr'), row.get('car'), row.get('npl_gross'),
        row.get('ldr'),
    )
    if any(np.isfinite(_num(value)) for value in prudential_fields):
        return True, 'BANK_PRUDENTIAL_FIELDS'
    if explicit in {'GENERAL', 'NON_FINANCIAL'}:
        return False, 'EXPLICIT_FUNDAMENTAL_MODEL'
    return False, 'GENERAL_DEFAULT_NO_SECTOR_EVIDENCE'


def _valuation_split_adjustment(
    frame: pd.DataFrame,
    statement_date: Any,
    price_date: Any,
) -> tuple[float, str, str]:
    """Return the post-statement share multiplier with explicit uncertainty."""
    attrs = dict(getattr(frame, 'attrs', {}) or {}) if isinstance(frame, pd.DataFrame) else {}
    start = pd.to_datetime(statement_date, errors='coerce')
    end = pd.to_datetime(price_date, errors='coerce')
    if pd.isna(start) or pd.isna(end):
        return 1.0, 'DATE_CONTEXT_INCOMPLETE', ''
    start = pd.Timestamp(start).normalize()
    end = pd.Timestamp(end).normalize()
    declared_dates = {
        pd.Timestamp(value).normalize()
        for value in attrs.get('corporate_action_split_dates', []) or []
    ...
    applied: list[str] = []
    ratio_missing = False
    for event in attrs.get('corporate_action_splits', []) or []:
        ...
        if not np.isfinite(ratio) or ratio <= 0:
            ...
            continue
        factor *= float(ratio)
        applied.append(f'{date.date().isoformat()}x{ratio:g}')
    if ratio_missing or declared_dates - covered_dates:
        return np.nan, 'POST_STATEMENT_SPLIT_RATIO_MISSING', ' | '.join(applied)
    if applied:
        return factor, 'POST_STATEMENT_SPLIT_ADJUSTED', ' | '.join(applied)
    return 1.0, 'NO_POST_STATEMENT_SPLIT', ''


def _valuation_fx_record(
    currency: str,
    price_date: Any,
    reference_fx: pd.DataFrame | None,
    *,
    max_age_days: int = 7,
) -> dict[str, Any]:
    code = str(currency or '').strip().upper()
    date = pd.to_datetime(price_date, errors='coerce')
    if code == 'IDR':
        return {
            'rate': 1.0, 'as_of': date, 'source': 'IDR_NATIVE',
            'source_verified': True, 'current': True,
        }
    if not code or not isinstance(reference_fx, pd.DataFrame) or reference_fx.empty:
        return {
            'rate': np.nan, 'as_of': pd.NaT, 'source': '',
            'source_verified': False, 'current': False,
        }
    local = reference_fx.copy()
    if 'currency' not in local or 'idr_per_unit' not in local:
        return {
            'rate': np.nan, 'as_of': pd.NaT, 'source': '',
            'source_verified': False, 'current': False,
        }
    local = local.loc[
        local['currency'].fillna('').astype(str).str.upper().eq(code)
    ].copy()
    local['_asof'] = pd.to_datetime(local.get('as_of'), errors='coerce')
    if pd.notna(date):
        local = local.loc[local['_asof'].le(pd.Timestamp(date))]
    local['_rate'] = pd.to_numeric(local['idr_per_unit'], errors='coerce')
    local = local.loc[local['_rate'].gt(0.0) & local['_asof'].notna()]
    if local.empty:
        return {
            'rate': np.nan, 'as_of': pd.NaT, 'source': '',
            'source_verified': False, 'current': False,
        }
    row = local.sort_values('_asof', kind='stable').iloc[-1]
    age = (
        int((pd.Timestamp(date).normalize() - row['_asof'].normalize()).days)
        if pd.notna(date) else np.nan
    )
    current = bool(np.isfinite(age) and 0 <= age <= max_age_days)
    return {
        'rate': float(row['_rate']),
        'as_of': row['_asof'],
        'source': _safe_text(row.get('source_family')),
...
def enrich_fundamentals_with_valuation(
    fundamentals: pd.DataFrame,
    prepared: Mapping[str, pd.DataFrame],
    *,
    reference_fx: pd.DataFrame | None = None,
    now: Any | None = None,
    market_cap_max_age_days: int = 14,
) -> pd.DataFrame:
    """Build auditable point-in-time valuation without inventing missing data.

    Market capitalisation is cross-checked against a current snapshot when one
    exists. Statement shares are adjusted only when every post-statement split
    ratio is known; rights issues or unexplained share changes are reconciled
    only through a dated market-cap reference. Currency-denominated statement
    metrics require a current IDR conversion rate.
    """
    if fundamentals is None or fundamentals.empty:
        return pd.DataFrame() if fundamentals is None else fundamentals.copy()
    current = _as_jakarta_naive_timestamp(
        now if now is not None else pd.Timestamp.now(tz='Asia/Jakarta')
    )
    records: list[dict[str, Any]] = []
    for _, source_row in fundamentals.iterrows():
        row = source_row.to_dict()
        ticker = _safe_text(row.get('ticker')).upper()
        frame = prepared.get(ticker, pd.DataFrame()) if isinstance(prepared, Mapping) else pd.DataFrame()
        price = np.nan
        price_date = pd.NaT
        if isinstance(frame, pd.DataFrame) and not frame.empty and 'Close' in frame:
            ...
            if not closes.empty:
                ...
                price_date = pd.Timestamp(closes.index[-1]).normalize()
        statement_date = _latest_jakarta_naive_timestamp(
            row.get('latest_statement_date'),
            row.get('fundamental_history_latest_period'),
        )
        statement_age = (
        ...
        if not (10_000_000_000.0 <= derived_market_cap <= 20_000_000_000_000_000.0):
            derived_market_cap = np.nan

        reference_market_cap = _num(row.get('market_cap'))
        # ``market_cap_asof`` is the observation date of the value itself and
        # must take precedence over a later ingestion timestamp. Selecting the
        # latest of both can make a valid EOD reference appear to be dated
        # after the price bar and incorrectly disable reconciliation.
        reference_asof = _as_jakarta_naive_timestamp(
            row.get('market_cap_asof')
        )
        if pd.isna(reference_asof):
            reference_asof = _as_jakarta_naive_timestamp(
                row.get('fundamental_fetched_at')
            )
        reference_age = (
            int((pd.Timestamp(price_date).normalize() - reference_asof.normalize()).days)
            if pd.notna(price_date) and pd.notna(reference_asof) else np.nan
        )
        reference_current = bool(
            reference_market_cap > 0 and np.isfinite(reference_age)
            and 0 <= reference_age <= int(market_cap_max_age_days)
        )
        reference_source = _safe_text(row.get('market_cap_source')) or _safe_text(
            row.get('fundamental_provider')
        )
        reference_verified = _truthy(row.get('market_cap_source_verified', False))
        divergence = (
            abs(derived_market_cap / reference_market_cap - 1.0)
            if derived_market_cap > 0 and reference_market_cap > 0 else np.nan
        )
        flags: list[str] = []
        if reference_current:
            market_cap = reference_market_cap
            market_cap_mode = 'CURRENT_MARKET_CAP_REFERENCE'
            if np.isfinite(divergence) and divergence <= 0.25:
                market_cap_mode = 'CURRENT_REFERENCE_CROSSCHECK_PASS'
            elif np.isfinite(divergence) and divergence > 0.25:
                flags.append('SHARE_COUNT_RECONCILED_TO_CURRENT_MARKET_CAP')
        ...
        if split_state == 'POST_STATEMENT_SPLIT_RATIO_MISSING':
            flags.append('POST_STATEMENT_SPLIT_RATIO_MISSING')
            if reference_current:
                flags.append('CURRENT_MARKET_CAP_REFERENCE_USED_FOR_SPLIT_GAP')
        if np.isfinite(statement_age) and statement_age > 365:
            flags.append('STATEMENT_SHARES_STALE')

        currency = _safe_text(row.get('fundamental_primary_currency')).upper()
        fx = _valuation_fx_record(currency, price_date, reference_fx)
        fx_rate = _num(fx.get('rate'))
        ...
        def in_idr(value: float) -> float:
            return value * fx_rate if np.isfinite(value) and fx_rate > 0 else np.nan

        net_income_idr = in_idr(net_income)
        fcf_idr = in_idr(fcf)
        equity_idr = in_idr(equity)
        debt_idr = in_idr(debt)
        cash_idr = in_idr(cash)
        ebitda_idr = in_idr(ebitda)
        trailing_pe_derived = (
            market_cap / net_income_idr
            if market_cap > 0 and net_income_idr > 0 else np.nan
        )
        earnings_yield_derived = (
            net_income_idr / market_cap
            if market_cap > 0 and np.isfinite(net_income_idr) else np.nan
        )
        price_to_book_derived = (
            market_cap / equity_idr
            if market_cap > 0 and equity_idr > 0 else np.nan
        )
        fcf_yield_derived = (
            fcf_idr / market_cap
            if market_cap > 0 and np.isfinite(fcf_idr) else np.nan
        )
        enterprise_value_derived = (
            market_cap + debt_idr - cash_idr
            if market_cap > 0 and np.isfinite(debt_idr) and np.isfinite(cash_idr)
            else np.nan
        )
        ev_ebitda_derived = (
            enterprise_value_derived / ebitda_idr
            if enterprise_value_derived > 0 and ebitda_idr > 0 else np.nan
        )
        peg_growth_pct_used = (
            min(100.0, 100.0 * growth)
            if np.isfinite(growth) and growth >= 0.10 else np.nan
        )
        if np.isfinite(growth) and growth > 1.0:
            flags.append('PEG_GROWTH_CAPPED_AT_100PCT')
        peg_derived = (
            trailing_pe_derived / peg_growth_pct_used
            if trailing_pe_derived > 0 and growth >= 0.10
            and positive_earnings_ratio >= 0.75
            and peg_growth_pct_used > 0 else np.nan
        )

        snapshot_values = {
            name: _num(row.get(name)) for name in (
                'trailing_pe', 'price_to_book', 'peg_ratio', 'fcf_yield',
                'enterprise_value', 'ev_ebitda', 'earnings_yield',
            )
        }
        derived_values = {
            'trailing_pe': trailing_pe_derived,
            'price_to_book': price_to_book_derived,
            'peg_ratio': peg_derived,
            'fcf_yield': fcf_yield_derived,
            'enterprise_value': enterprise_value_derived,
        ...
        for name, derived in derived_values.items():
            ...
            if np.isfinite(derived):
                row[name] = derived
                metric_lineage.append(f'{name.upper()}:STATEMENT_TTM_FX')
            elif np.isfinite(snapshot_values[name]):
                row[name] = snapshot_values[name]
                metric_lineage.append(f'{name.upper()}:PROVIDER_SNAPSHOT')
            else:
                row[name] = np.nan
        row['market_cap_snapshot'] = reference_market_cap
        row['market_cap_derived_idr'] = derived_market_cap
        row['market_cap'] = market_cap

        is_financial, sector_model_source = _valuation_financial_model(row)
        row['fundamental_model'] = 'FINANCIAL' if is_financial else 'GENERAL'
        weighted_fields = (
            (
                ('market_cap', 15.0), ('earnings_yield', 35.0),
                ('price_to_book', 35.0), ('peg_ratio', 15.0),
            ) if is_financial else (
                ('market_cap', 10.0), ('earnings_yield', 20.0),
                ('fcf_yield', 25.0), ('ev_ebitda', 20.0),
                ('peg_ratio', 15.0), ('price_to_book', 10.0),
            )
        )
        coverage = sum(
            weight for name, weight in weighted_fields
            if np.isfinite(_num(row.get(name)))
        )
        fx_usable = currency == 'IDR' or bool(fx.get('current', False))
        score_eligible = bool(
            market_cap > 0 and coverage >= 45.0 and fx_usable
            and (not np.isfinite(statement_age) or statement_age <= 365)
        )
        production_eligible = bool(
            score_eligible
            and bool(fx.get('source_verified', False))
            and split_state != 'POST_STATEMENT_SPLIT_RATIO_MISSING'
            and (
                market_cap_mode in {
        ...
        statement_official_verified = _truthy(
            row.get('fundamental_official_verified', False)
        )
        if (
            production_eligible and coverage >= 75.0
            and statement_official_verified and reference_verified
        ):
            state = 'FULL_VERIFIED_VALUATION'
            confidence = 'HIGH'
            source_tier = 'OFFICIAL_STATEMENT_AND_MARKET_CAP'
        elif production_eligible and coverage >= 75.0:
            state = 'FULL_AUDITABLE_DERIVED_VALUATION'
            confidence = 'MEDIUM'
            source_tier = 'CROSSCHECKED_DERIVED_RESEARCH_DATA'
        elif score_eligible:
            state = 'RESEARCH_VALUATION_WITH_PROXY_OR_DERIVATION'
            confidence = 'MEDIUM' if coverage >= 65.0 else 'LOW'
            source_tier = 'PARTIAL_OR_PROXY_RESEARCH_DATA'
        elif market_cap > 0:
            state = 'PARTIAL_VALUATION_NOT_SCORED'
            confidence = 'LOW'
            source_tier = 'PARTIAL_NOT_SCORE_ELIGIBLE'
        else:
        ...
        row.update({
            ...
            'valuation_price_asof': price_date,
            ...
            'valuation_shares_adjusted': shares_adjusted,
            'valuation_market_cap_mode': market_cap_mode,
            'valuation_market_cap_reference_asof': reference_asof,
            'valuation_market_cap_reference_source': reference_source,
            'valuation_market_cap_reference_verified': reference_verified,
            'valuation_market_cap_divergence_pct': (
                100.0 * divergence if np.isfinite(divergence) else np.nan
            ),
            'valuation_fx_rate_to_idr': fx_rate,
            'valuation_fx_asof': fx.get('as_of'),
            'valuation_fx_source': fx.get('source', ''),
            ...
            'valuation_source_tier': source_tier,
            'valuation_statement_official_verified': statement_official_verified,
            'valuation_sector_model_source': sector_model_source,
            'valuation_metric_lineage': ' | '.join(metric_lineage),
            'valuation_flags': ' | '.join(dict.fromkeys(flags)),
        })
        records.append(row)
...
def fetch_execution_snapshots(
    ...
    """Fetch bounded completed-EOD snapshots for final candidates.

    The default route uses Yahoo chart JSON with an explicit timeout.  The
    previous ``yfinance.get_info`` route had no reliable wall-clock timeout and
    could stall a Streamlit scan.  Independent-price validation remains a
    separate downstream gate (Twelve Data/iTick/other provider).
    """
    names = list(dict.fromkeys(tickers))
    if not names:
        return pd.DataFrame()
    ...
    expected = _expected_last_completed_daily_date(now_jkt)
    try:
        from free_data_providers import yahoo_chart_direct
    except Exception as exc:
        return pd.DataFrame([
            {
                ...
                'quote_error': f'PROVIDER_DEPENDENCY: {type(exc).__name__}: {str(exc)[:100]}',
            }
            for ticker in names
        ])
    ...
    def one(ticker: str) -> dict[str, Any]:
        try:
            ...
            frame = _completed_daily_frame(raw, now_jkt)
            if frame.empty:
                raise RuntimeError('completed EOD snapshot kosong')
            last = frame.iloc[-1]
            last_date = pd.Timestamp(frame.index[-1]).normalize()
            last_price = _num(last.get('Close'))
            previous_close = _num(frame['Close'].iloc[-2]) if len(frame) >= 2 else np.nan
            volume = _num(last.get('Volume'))
            quote_time = last_date + pd.Timedelta(hours=16, minutes=15)
            verified = bool(
                last_date == expected
                and np.isfinite(last_price) and last_price > 0
                and np.isfinite(volume) and volume > 0
            )
            return {
                'ticker': ticker,
                'quote_checked_at': now,
                ...
                'quote_bid': np.nan,
                'quote_ask': np.nan,
                'quote_spread_pct': np.nan,
                'quote_volume': volume,
                'quote_market_state': 'CLOSED' if verified else 'UNKNOWN',
                'quote_exchange': 'JKT',
                'quote_type': 'EQUITY',
                'quote_verified': verified,
                'quote_source': _safe_text(meta.get('provider')) or 'YAHOO_CHART_DIRECT',
                'quote_error': '' if verified else f'last={last_date.date()}, expected={expected.date()}',
            }
        except Exception as exc:
            return {
                'ticker': ticker,
                'quote_checked_at': now,
                'quote_verified': False,
                'quote_source': 'YAHOO_CHART_DIRECT',
                'quote_error': f'{type(exc).__name__}: {str(exc)[:120]}',
            }

    rows: list[dict[str, Any]] = []
    with ThreadPoolExecutor(max_workers=min(max(1, int(max_workers)), len(names))) as pool:
...
class ScanConfig:
    ...
    validation_max_tickers: int = 400
    min_oos_unique_dates: int = 10
    beta_prior_wins: float = 8.0
    beta_prior_losses: float = 8.0
    fundamental_top_n: int = 400
    fundamental_history_top_n: int = 400
    idx_fundamental_top_n: int = 80
    idx_fundamental_years_back: int = 3
    fundamental_crosscheck_top_n: int = 8
    fundamental_backfill_cohorts: int = 7
    fundamental_backfill_max_per_scan: int = 400
    fundamental_history_priority_top_n: int = 16
    forward_backfill_cohorts: int = 7
    event_aware_refresh_enabled: bool = True
    full_completion_max_tickers: int = 400
    official_fundamental_refresh_max_per_scan: int = 80
    narrative_full_completion_max_tickers: int = 400
    relative_overlay_max_pct: float = 10.0
    fundamental_provider_batch_size: int = 40
    multibagger_min_metric_coverage_pct: float = 65.0
    multibagger_a_min_metric_coverage_pct: float = 80.0
    ...
    ai_mode: str = 'REMOVED_FROM_PRODUCTION'
    ai_max_weight: float = 0.0
    ai_min_training_events: int = 30
    ai_min_strategy_events: int = 18
    ai_knn_k: int = 21
    ai_memory_entry_window_bars: int = 5
    ai_memory_horizon_bars: int = 20
    # Cross-sectional selection is deliberately separated from setup timing.
    # AI remains a challenger until its unseen, after-cost metrics pass every
    # promotion guard in selector_engine.py.
    selector_enabled: bool = True
    selector_training_lookback_bars: int = 620
    selector_anchor_step_bars: int = 5
    selector_max_ai_weight: float = 0.0
    selector_min_training_rows: int = 180
    selector_min_eval_rows: int = 60
    selector_min_eval_dates: int = 12
    selector_min_eval_tickers: int = 25
    selector_max_drawdown_pct: float = 20.0
    selector_roundtrip_cost_pct: float = 0.0065
    selector_market_impact_enabled: bool = True
    selector_impact_very_illiquid_pct: float = 0.0150
    selector_impact_illiquid_pct: float = 0.0075
    selector_impact_medium_pct: float = 0.0035
    selector_impact_liquid_pct: float = 0.0015
    selector_impact_very_liquid_pct: float = 0.0005
    selector_cscv_slices: int = 6
    selector_max_pbo_pct: float = 50.0
    selector_min_feature_coverage_pct: float = 75.0
    selector_min_model_feature_coverage_pct: float = 80.0
    swing_min_component_coverage_pct: float = 65.0
    swing_min_selector_feature_coverage_pct: float = 75.0
    execution_ai_min_expectancy_r: float = 0.05
    execution_ai_max_drawdown_pct: float = 20.0
    execution_ai_min_profit_factor: float = 1.05
    # Point-in-time narrative intelligence. Missing evidence stays neutral with
    # zero production influence; official critical contradictions fail closed.
    narrative_enabled: bool = True
    narrative_event_lookback_days: int = 180
    narrative_refresh_max_per_scan: int = 400
    narrative_backfill_cohorts: int = 7
    narrative_cache_days: int = 3
    narrative_half_life_days: float = 45.0
    narrative_min_conversion_events: int = 20
    narrative_roundtrip_cost_pct: float = 0.0065
    narrative_growth_max_adjustment_points: float = 0.0
    narrative_turnaround_max_adjustment_points: float = 0.0
    narrative_swing_max_adjustment_points: float = 0.0
    # Emir public-framework receives the larger production overlay. Positive
    # points require complete evidence gates; missing evidence contributes zero.
    emir_growth_max_adjustment_points: float = 0.0
    emir_turnaround_max_adjustment_points: float = 0.0
    emir_swing_max_adjustment_points: float = 0.0
    # Objective Astronacci-style time-cycle intelligence. It only affects
    # daily Core Swing ranking and Multibagger timing.
    time_cycle_enabled: bool = False
    time_cycle_core_max_weight: float = 0.0
    time_cycle_multibagger_max_weight: float = 0.0
    # Full per-ticker time-cycle/EOFF reconstruction is expensive and has zero
    # Multibagger production weight by default. Keep it opt-in for diagnostics;
    # Core Swing retains its independent bounded overlay.
    multibagger_time_cycle_full_refresh_enabled: bool = False
    time_cycle_min_confidence: float = 55.0
    time_cycle_lunar_enabled: bool = False
    time_cycle_min_history_bars: int = 180
    # Clean-room Eye-of-Future reconstruction. These controls affect only the
    # existing daily time-cycle overlay for Core Swing and Multibagger.
    eoff_enabled: bool = False
    eoff_ephemeris_enabled: bool = False
    eoff_min_fib_cluster: int = 4
    eoff_min_unique_anchors: int = 3
    eoff_max_dominant_anchor_share: float = 0.55
    eoff_aspect_orb_deg: float = 3.0
    eoff_require_astro_fib_confluence: bool = False
    eoff_astro_shadow_only: bool = True
    idx_trading_holidays: tuple[str, ...] = ()
    idx_official_open_dates: tuple[str, ...] = ()
    idx_official_closed_dates: tuple[str, ...] = ()
    require_official_idx_calendar: bool = False
    silent_accumulation_min_score: float = 62.0
    silent_accumulation_max_distribution_days: int = 3
    max_avg_down_loss_pct: float = 0.1
    max_avg_down_position_pct: float = 0.28
    # Persistent incremental recomputation. The cache is decision-neutral:
    # every key is a deterministic hash of stage inputs and scanner version.
    incremental_cache_enabled: bool = True
    incremental_cache_full_result_enabled: bool = True
    incremental_cache_stage_enabled: bool = True
    incremental_cache_max_age_hours: float = 8.0
    incremental_cache_max_entries: int = 48
    incremental_cache_max_bytes_mb: int = 1024
    incremental_cache_read_only: bool = False
    # Core Engine cache: prepared indicators and local setup analysis are
    # partitioned deterministically so one changed ticker only rebuilds one
    # small bucket. Market breadth/final ranking are always recomputed unless
    # the complete input fingerprint is identical.
    core_incremental_cache_enabled: bool = True
    core_incremental_cache_full_result_enabled: bool = True
    core_incremental_cache_stage_enabled: bool = True
    core_incremental_cache_partitions: int = 16
...
def build_source_quorum_audit(
    ...
    if not price.empty and 'ticker' in price:
        ...
        overlap = pd.to_numeric(price.get('independent_overlap_bars', pd.Series(0.0, index=price.index)), errors='coerce').fillna(0)
        correlation = pd.to_numeric(price.get('independent_return_correlation', pd.Series(np.nan, index=price.index)), errors='coerce')
    ...
    if not validation.empty:
        ...
        events = pd.to_numeric(validation.get('signal_events_oos', pd.Series(0.0, index=validation.index)), errors='coerce').fillna(0)
    ...
    rows = [
        ...
        {
            'data_layer': 'Broker summary', 'policy': 'BROKER_SOURCE_OF_TRUTH',
            'observed_sources': 'STOCKBIT_EXPORT' if (broker_summary is not None and not broker_summary.empty) else 'UNAVAILABLE',
            'verified_tickers': 0, 'universe_tickers': len(names), 'coverage_pct': 0.0,
            'quorum_state': state(0, manual=True),
            'order_effect': 'Broker Summary hanya konfirmasi tambahan; tidak menggantikan fundamental dan struktur harga.',
        },
        {
...
def _concat_cache_frames(frames: Iterable[pd.DataFrame]) -> pd.DataFrame:
    """Concatenate cache frames without pandas empty/all-NA dtype warnings.

    Empty inputs are ignored during dtype inference. All source columns are
    restored after concatenation so cache schema is retained even when a column
    is temporarily all-NA in every live row.
    """
    ...
    for frame in frames:
        ...
        clean = frame.dropna(axis=1, how='all').copy()
        if not clean.empty and len(clean.columns) > 0:
            prepared.append(clean)
    if not prepared:
        return pd.DataFrame(columns=all_columns)
    if len(prepared) == 1:
        merged = prepared[0].reset_index(drop=True)
    else:
        merged = pd.concat(prepared, ignore_index=True, sort=False, copy=False)
    return merged.reindex(columns=all_columns)
...
def _latest_jakarta_naive_timestamp(*values: Any) -> pd.Timestamp:
    """Return the latest valid timestamp after timezone normalization.

    This helper is intentionally scalar and deterministic. It prevents mixed
    Supabase ISO-offset values and date-only provider values from being compared
    or subtracted before both have been converted to Jakarta wall time.
    """
    normalized = [_as_jakarta_naive_timestamp(value) for value in values]
    valid = [stamp for stamp in normalized if pd.notna(stamp)]
...
def read_cached_market_status(tickers: Iterable[str], config: ScanConfig | None=None) -> pd.DataFrame:
    """Read only fresh verified market-status cache rows; never call providers."""
    cfg = config or ScanConfig()
    names = list(dict.fromkeys(tickers))
    ...
    resolved = _merge_resilient_rows(
        pd.DataFrame(), cached, names, 'market_status_asof', usable,
        cfg.market_status_cache_days, 'CACHE_FALLBACK',
    )
    tier = resolved.get(
        'evidence_source_tier', pd.Series('UNRESOLVED', index=resolved.index),
    ).fillna('UNRESOLVED').astype(str).str.upper()
    resolved['market_status_route_state'] = tier.map({
        'CACHE_FALLBACK': 'CACHE_AUTHORITY_FALLBACK',
    }).fillna('UNAVAILABLE_FAIL_CLOSED')
    resolved['market_status_score_eligible'] = resolved.apply(
        lambda row: usable(row.to_dict()), axis=1,
    )
    return resolved


def read_cached_news_review(
    tickers: Iterable[str], lookback_days: int=7,
    config: ScanConfig | None=None,
) -> pd.DataFrame:
    """Read only fresh complete news-review cache rows; never call providers."""
    del lookback_days  # cache freshness is controlled by ScanConfig.news_cache_days
    cfg = config or ScanConfig()
    names = list(dict.fromkeys(tickers))
    ...
    resolved = _merge_resilient_rows(
        pd.DataFrame(), cached, names, 'news_reviewed_at', usable,
        cfg.news_cache_days, 'CACHE_FALLBACK',
    )
    tier = resolved.get(
        'evidence_source_tier', pd.Series('UNRESOLVED', index=resolved.index),
    ).fillna('UNRESOLVED').astype(str).str.upper()
    resolved['news_route_state'] = tier.map({
        'CACHE_FALLBACK': 'CACHE_VERIFIED_FALLBACK',
    }).fillna('UNAVAILABLE_NOT_SCORED')
    resolved['news_score_eligible'] = resolved.apply(
        lambda row: usable(row.to_dict()), axis=1,
    )
    return resolved


def read_cached_fundamentals(
    tickers: Iterable[str], config: ScanConfig | None=None,
) -> pd.DataFrame:
    """Read only fresh usable local fundamental snapshots; never call providers."""
    cfg = config or ScanConfig()
    names = list(dict.fromkeys(tickers))
    ...
    def usable(row: Mapping[str, Any]) -> bool:
        return (
            _finite(row.get('fundamental_coverage'), 0) >= 45
            and not _safe_text(row.get('fundamental_error'))
        )

    resolved = _merge_resilient_rows(
        pd.DataFrame(), cached, names, 'fundamental_fetched_at', usable,
        cfg.fundamental_cache_days, 'CACHE_FALLBACK',
    )
    tier = resolved.get(
        'evidence_source_tier', pd.Series('UNRESOLVED', index=resolved.index),
    ).fillna('UNRESOLVED').astype(str).str.upper()
    resolved['fundamental_route_state'] = tier.map({
        'CACHE_FALLBACK': 'CACHE_VERIFIED_FALLBACK',
    }).fillna('UNAVAILABLE_NOT_SCORED')
    resolved['fundamental_score_eligible'] = resolved.apply(
        lambda row: usable(row.to_dict()), axis=1,
    )
    unresolved_mask = ~resolved['fundamental_score_eligible']
    for column in ('fundamental_score', 'fundamental_score_10'):
        if column in resolved:
            resolved.loc[unresolved_mask, column] = np.nan
    resolved.loc[unresolved_mask, 'fundamental_reliability'] = 'NONE'
    return resolved


def read_cached_fundamental_history(tickers: Iterable[str]) -> pd.DataFrame:
    """Read all local statement caches for the requested tickers without I/O calls."""
    names = set(dict.fromkeys(tickers))
    frames: list[pd.DataFrame] = []
    for cache_name in (
        'idx_fundamental_history', 'yahoo_fundamental_history',
        'twelve_fundamental_history',
    ):
        frame = normalize_fundamental_history(_load_cache(cache_name))
        if frame is not None and not frame.empty and 'ticker' in frame:
            frames.append(frame.loc[frame['ticker'].isin(names)].copy())
    return combine_fundamental_history(*frames)


def fetch_resilient_market_status(tickers: Iterable[str], config: ScanConfig | None=None) -> pd.DataFrame:
    """Cache-first market-status routing.

    Fresh verified cache rows generate zero provider calls.  Only stale or
    missing tickers are sent to the external authority lookup.
    """
    cfg = config or ScanConfig()
    names = list(dict.fromkeys(tickers))
    cached = _load_cache('market_status')
    usable = lambda row: _truthy(row.get('market_status_verified', False))
    cached_map = (
        {} if cached is None or cached.empty else
        {str(row['ticker']): row.to_dict() for _, row in cached.dropna(subset=['ticker']).drop_duplicates('ticker', keep='last').iterrows()}
    )
    now = pd.Timestamp.now(tz='Asia/Jakarta').tz_localize(None)
    refresh_names = [
        ticker for ticker in names
        if not _cache_row_is_fresh(
            cached_map.get(ticker), 'market_status_asof',
            cfg.market_status_cache_days, usable, now=now,
        )
    ]
    attempts: list[pd.DataFrame] = []
    unresolved = set(refresh_names)
    for attempt in range(max(1, cfg.provider_retry_count)):
        if not unresolved:
            break
        frame = fetch_automatic_market_status(sorted(unresolved), timeout=8)
        if frame is None or frame.empty:
            continue
        frame = frame.copy()
        frame['market_status_provider_attempt'] = attempt + 1
        attempts.append(frame)
        verified = frame.get(
            'market_status_verified', pd.Series(False, index=frame.index),
        ).map(_truthy)
        if 'ticker' in frame:
            unresolved -= set(frame.loc[verified, 'ticker'].dropna().astype(str))
    current = _concat_cache_frames(attempts)
    if not current.empty and 'ticker' in current:
        market_verified = current.get(
            'market_status_verified', pd.Series(False, index=current.index),
        ).map(_truthy)
        current['_route_ok'] = market_verified.astype(int)
        current = current.sort_values(
            ['ticker', '_route_ok', 'market_status_provider_attempt'],
            ascending=[True, False, False], kind='stable',
        ).drop_duplicates('ticker', keep='first').drop(columns='_route_ok')
    ...
    if not current.empty:
        ...
        live = current[market_verified].copy()
        if not live.empty:
            prior = cached[
                ~cached.get('ticker', pd.Series(dtype=str)).isin(live['ticker'])
            ] if not cached.empty and 'ticker' in cached else pd.DataFrame()
            _write_cache('market_status', _concat_cache_frames([live, prior]))
    tier = resolved.get(
        'evidence_source_tier', pd.Series('UNRESOLVED', index=resolved.index),
    ).fillna('UNRESOLVED').astype(str).str.upper()
    resolved['market_status_route_state'] = tier.map({
        'LIVE': 'LIVE_AUTHORITY_VERIFIED',
        'CACHE_FALLBACK': 'CACHE_AUTHORITY_FALLBACK',
    }).fillna('UNAVAILABLE_FAIL_CLOSED')
    resolved['market_status_score_eligible'] = resolved.apply(
        lambda row: usable(row.to_dict()), axis=1,
    )
    resolved['market_status_provider_requested'] = resolved['ticker'].isin(refresh_names)
    return resolved

def fetch_resilient_news_review(tickers: Iterable[str], lookback_days: int=7, config: ScanConfig | None=None) -> pd.DataFrame:
    """Cache-first disclosure/news routing.

    Fresh complete reviews are returned immediately.  External queries are
    limited to stale or missing tickers, which is essential for a 400-ticker
    universe.
    """
    cfg = config or ScanConfig()
    names = list(dict.fromkeys(tickers))
    ...
    refresh_names = [
        ticker for ticker in names
        if not _cache_row_is_fresh(
            cached_map.get(ticker), 'news_reviewed_at',
            cfg.news_cache_days, usable, now=now,
        )
    ]
    attempts: list[pd.DataFrame] = []
    unresolved = set(refresh_names)
    for attempt in range(max(1, cfg.provider_retry_count)):
        if not unresolved:
            break
        frame = fetch_automatic_news_review(
            sorted(unresolved), lookback_days=lookback_days,
        )
        if frame is None or frame.empty:
            continue
        frame = frame.copy()
        frame['news_provider_attempt'] = attempt + 1
        attempts.append(frame)
        ok = frame.apply(lambda row: usable(row.to_dict()), axis=1)
        if 'ticker' in frame:
            unresolved -= set(frame.loc[ok, 'ticker'].dropna().astype(str))
    current = _concat_cache_frames(attempts)
    if not current.empty and 'ticker' in current:
        current['_route_ok'] = current.apply(
            lambda row: int(usable(row.to_dict())), axis=1,
        )
        current = current.sort_values(
            ['ticker', '_route_ok', 'news_provider_attempt'],
            ascending=[True, False, False], kind='stable',
        ).drop_duplicates('ticker', keep='first').drop(columns='_route_ok')
    elif not current.empty:
        current = pd.DataFrame()
    ...
    if not current.empty:
        ...
        if not live.empty:
            ...
            _write_cache('news_review', _concat_cache_frames([live, prior]))
    tier = resolved.get(
        'evidence_source_tier', pd.Series('UNRESOLVED', index=resolved.index),
    ).fillna('UNRESOLVED').astype(str).str.upper()
    resolved['news_route_state'] = tier.map({
    ...
    resolved['news_provider_requested'] = resolved['ticker'].isin(refresh_names)
    return resolved
...
def fetch_resilient_fundamentals(tickers: Iterable[str], config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    attempts: list[pd.DataFrame] = []
    unresolved = set(refresh_names)
    batch_size = max(10, int(getattr(cfg, 'fundamental_provider_batch_size', 40)))
    for attempt in range(max(1, cfg.provider_retry_count)):
        if not unresolved:
            break
        ...
        for offset in range(0, len(pending), batch_size):
            ...
            frame = fetch_fundamentals(batch)
            if frame is None or frame.empty:
                continue
            frame = frame.copy()
            frame['fundamental_provider_attempt'] = attempt + 1
            frame['fundamental_provider_batch'] = (offset // batch_size) + 1
            attempts.append(frame)
            ok = frame.apply(lambda row: usable(row.to_dict()), axis=1)
            if 'ticker' in frame:
                resolved_batch = set(frame.loc[ok, 'ticker'].dropna().astype(str))
                unresolved -= resolved_batch
            # Durable progress checkpoint: retain every successful batch so a
            # 400-ticker scan can resume after provider/runtime interruption.
            live_batch = frame[ok].copy()
            if not live_batch.empty:
                latest_cache = _load_cache('fundamentals')
                prior = (
                    latest_cache[~latest_cache.get('ticker', pd.Series(dtype=str)).isin(live_batch['ticker'])]
                    if latest_cache is not None and not latest_cache.empty and 'ticker' in latest_cache
                    else pd.DataFrame()
                )
                _write_cache('fundamentals', _concat_cache_frames([live_batch, prior]))
    current = _concat_cache_frames(attempts)
    if not current.empty and 'ticker' in current:
        current['_route_ok'] = current.apply(lambda row: int(usable(row.to_dict())), axis=1)
        current['_route_coverage'] = pd.to_numeric(
            current.get(
                'fundamental_coverage', pd.Series(np.nan, index=current.index),
            ),
            errors='coerce',
        ).fillna(0.0)
        current = current.sort_values(['ticker', '_route_ok', '_route_coverage', 'fundamental_provider_attempt'], ascending=[True, False, False, False], kind='stable').drop_duplicates('ticker', keep='first').drop(columns=['_route_ok', '_route_coverage'])
    elif not current.empty:
        current = pd.DataFrame()
    ...
    if not current.empty:
        ...
        if not live.empty:
            ...
            _write_cache('fundamentals', _concat_cache_frames([live, prior]))
    tier = resolved.get('evidence_source_tier', pd.Series('UNRESOLVED', index=resolved.index)).fillna('UNRESOLVED').astype(str).str.upper()
    resolved['fundamental_route_state'] = tier.map({'LIVE': 'LIVE_COVERAGE_VERIFIED', 'CACHE_FALLBACK': 'CACHE_VERIFIED_FALLBACK'}).fillna('UNAVAILABLE_NOT_SCORED')
    resolved['fundamental_score_eligible'] = resolved.apply(lambda row: usable(row.to_dict()), axis=1)
    unresolved_mask = ~resolved['fundamental_score_eligible']
    for column in ('fundamental_score', 'fundamental_score_10'):
        if column in resolved:
            resolved.loc[unresolved_mask, column] = np.nan
...
def attach_ohlcv_source_lineage(signals: pd.DataFrame, source_tiers: Mapping[str, str] | None) -> pd.DataFrame:
    """Attach the authoritative OHLCV provider tier to every signal row.

    The downloader returns source lineage in ``DownloadReport.source_tiers``.
    Execution gates must receive that lineage explicitly; otherwise a valid
    downloaded history is indistinguishable from unavailable data and every
    candidate can be hard-blocked. Missing mappings remain fail-closed.
    """
    if signals is None or signals.empty:
        return signals.copy() if isinstance(signals, pd.DataFrame) else pd.DataFrame()
    out = signals.copy()
    normalized = {
        str(ticker).strip().upper(): _safe_text(tier).upper() or 'UNAVAILABLE'
        for ticker, tier in dict(source_tiers or {}).items()
    }
    ticker_key = out['ticker'].astype(str).str.strip().str.upper()
    out['ohlcv_source_tier'] = ticker_key.map(normalized).fillna('UNAVAILABLE')
    out['ohlcv_source_verified'] = ~out['ohlcv_source_tier'].isin(
        {'', 'UNAVAILABLE', 'CACHE_FALLBACK', 'CACHE_STALE_RESEARCH_ONLY'}
    )
    out['ohlcv_source_family'] = out['ohlcv_source_tier'].map(_primary_source_family_from_tier)
    return out
...
def _fetch_official_idx_pages(timeout: int=8) -> tuple[dict[str, str], dict[str, str]]:
    ...
    def one(item: tuple[str, str]) -> tuple[str, str | None, str | None]:
        key, url = item
        errors: list[str] = []
        for candidate in _idx_url_candidates(url):
            try:
        ...
        return (key, None, ' | '.join(errors)[:420])
...
def _fetch_idx_disclosure_page(timeout: int=8) -> tuple[str, bool, str]:
    import requests
    primary = 'https://www.idx.id/id/perusahaan-tercatat/keterbukaan-informasi'
    errors: list[str] = []
    for url in _idx_url_candidates(primary):
        try:
            response = requests.get(
                url, timeout=timeout,
            ...
            ok = len(text) > 600 and ('KETERBUKAAN INFORMASI' in text or 'DISCLOSURE' in text)
            if ok:
                return (text, True, '')
            raise RuntimeError('semantic marker tidak ditemukan')
        except Exception as exc:
            errors.append(f'{url}:{type(exc).__name__}:{str(exc)[:90]}')
    return ('', False, ' | '.join(errors)[:420])
...
def _row_has_current_final_ohlcv(row: Mapping[str, Any], cfg: ScanConfig) -> bool:
    explicit = row.get('daily_session_current')
    if explicit is not None and not (isinstance(explicit, float) and np.isnan(explicit)):
        return _truthy(explicit) and (not _truthy(row.get('current_bar_incomplete', False)))
    # Legacy rows without the new session field remain research-only unless the
    # exact expected session can be proven from ``last_date``.
    last = pd.to_datetime(row.get('last_date'), errors='coerce')
    if pd.isna(last):
        # Backward-compatible for isolated downstream/unit-test rows.  Real scan
        # rows always carry ``daily_session_current`` from _tradeability.
        return (
            _finite(row.get('absolute_data_age_days'), 999.0) == 0.0
            and not _truthy(row.get('current_bar_incomplete', False))
        )
    expected = _expected_last_completed_daily_date(_jakarta_timestamp())
    return idx_session_lag(last, expected) == 0 and (not _truthy(row.get('current_bar_incomplete', False)))
...
def _market_status_data_coverage(row: Mapping[str, Any], cfg: ScanConfig) -> float:
    ...
    final_ohlcv = _row_has_current_final_ohlcv(row, cfg)
...
def fetch_automatic_news_review(tickers: Iterable[str], lookback_days: int=7, max_workers: int=8) -> pd.DataFrame:
    ...
    def one(ticker: str) -> dict[str, Any]:
        ...
        yahoo_ok = False
        yahoo_error = ''
        if not rss_ok and yf is not None:
            try:
                obj = yf.Ticker(ticker)
                try:
        ...
        elif not rss_ok and yf is None:
            yahoo_error = 'PROVIDER_DEPENDENCY: yfinance unavailable'
        ...
        for item in items:
            ...
            if title:
                narrative_items.append({
                    ...
                    'source_url': url,
                    'source_family': (
                        'GOOGLE_NEWS_RSS' if rss_ok
                        else 'YAHOO_FINANCE_NEWS'
                    ),
                    'event_date': (
                        published.isoformat()
                    ...
                    'sentiment': sentiment,
                    'materiality': (
                        'HIGH' if is_negative or is_event
                        else 'MEDIUM' if sentiment == 'POSITIVE'
                        else 'LOW'
                    ),
                    # A news-aggregator URL is source-identified, not an
                    # official issuer/IDX verification.
                    'official_verified': False,
                })
            reviewed.append(item)
        if ticker in disclosure_mentions:
            narrative_items.append({
                'headline': 'Official IDX disclosure mention requires review',
                'summary': (
                    'Ticker terdeteksi pada halaman keterbukaan IDX; detail '
                    'aksi korporasi harus dibaca sebelum keputusan.'
                ),
                'source_url': IDX_AUTOMATION_URLS.get(
                    'corporate_actions', '',
                ),
                'source_family': 'IDX_DISCLOSURE',
                'event_date': now.isoformat(),
                'detected_at': now.isoformat(),
                'event_type': 'CORPORATE_ACTION_DISCLOSURE',
                'sentiment': 'NEUTRAL',
                'materiality': 'HIGH',
                'official_verified': bool(disclosure_ok),
            })
        complete = query_ok
        ...
        return {
            ...
            'narrative_events_json': json.dumps(
                narrative_items, ensure_ascii=False,
            ),
            'news_error': errors,
...
def apply_independent_price_gate(signals: pd.DataFrame, validation: pd.DataFrame | None, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for column, default in defaults.items():
        if column not in out:
            out[column] = default
            continue
        if column == 'independent_price_verified':
            out[column] = out[column].map(lambda value: _truthy(value) if not pd.isna(value) else False).astype(bool)
        elif isinstance(default, str):
            out[column] = out[column].astype('string').fillna(default).astype(str)
        else:
            out[column] = pd.to_numeric(out[column], errors='coerce').fillna(float(default))
...
def _write_independent_price_cache(frame: pd.DataFrame) -> None:
    ...
    cutoff = pd.Timestamp(newest).normalize() - timedelta(days=45)
...
def fetch_itick_independent_eod(
    ...
    if raw_report is None or raw_report.empty:
        ...
        normalized = pd.DataFrame({
            ...
            'rows': pd.to_numeric(raw_report.get('bars', pd.Series(0.0, index=raw_report.index)), errors='coerce').fillna(0).astype(int),
            'asof': pd.NaT,
...
def _candidate_idx_summary_dates(reference_date: Any | None, lookback_days: int) -> list[pd.Timestamp]:
    ...
    for offset in range(max(1, int(lookback_days))):
        candidate = anchor - timedelta(days=int(offset))
...
def fetch_idx_official_eod_quotes(tickers: Iterable[str], reference_date: Any | None=None, lookback_days: int=7, timeout: int=10) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not names:
        return (_empty_automatic_independent_data(), _automatic_provider_report([]))
    import requests
    page_url = 'https://www.idx.id/id/data-pasar/ringkasan-perdagangan/ringkasan-saham'
...
def fetch_automatic_independent_prices(tickers: Iterable[str], reference_date: Any | None=None, twelve_data_api_key: str='', itick_api_token: str='', primary_reference: Mapping[str, tuple[Any, float]] | None=None, primary_source_tiers: Mapping[str, str] | None=None, config: ScanConfig | None=None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if remaining and _safe_text(twelve_data_api_key):
        ...
        if not twelve_report.empty:
            normalized_report = pd.DataFrame({'provider': 'TWELVE_DATA', 'scope': twelve_report.get('ticker', ''), 'status': twelve_report.get('status', 'FAILED'), 'rows': pd.to_numeric(twelve_report.get('bars', pd.Series(0.0, index=twelve_report.index)), errors='coerce').fillna(0).astype(int), 'asof': pd.NaT, 'error': twelve_report.get('error', '')})
...
def _expected_last_completed_daily_date(now: Any | None=None) -> pd.Timestamp:
    """Return the exact latest completed IDX trading session.

    Weekends and official exchange closures are skipped.  A cache is current
    only when its last bar equals this date; calendar-day tolerance is not used.
    """
    stamp = _jakarta_timestamp(now)
    local_day = stamp.tz_localize(None).normalize()
    include_today = idx_daily_bar_is_final(stamp)
    return previous_idx_session(local_day, include_date=include_today)
...
def _completed_daily_frame(frame: pd.DataFrame, now: Any | None=None) -> pd.DataFrame:
    ...
    if clean.empty:
        return clean
    clean, removed_nontrading = filter_known_nontrading_dates(clean)
    if clean.empty:
        return clean
    ...
    clean.attrs['idx_calendar_version'] = IDX_CALENDAR_VERSION
    clean.attrs['removed_known_nontrading_bars'] = len(removed_nontrading)
    return clean
...
def _cache_meta_proves_final(frame: pd.DataFrame, now: Any | None=None) -> bool:
    ...
    if str(attrs.get('bar_state', '')).upper() == 'FINAL_EOD':
        finalized = pd.to_datetime(attrs.get('finalized_for_date'), errors='coerce')
        return bool(pd.isna(finalized) or pd.Timestamp(finalized).normalize() == expected)
...
def _daily_cache_is_current(frame: pd.DataFrame, now: Any | None=None) -> bool:
    ...
    if int(_finite(attrs.get('schema_version'), 0.0)) >= 4:
        # Schema v4 is written only after strict cleaning/completed-EOD
        # filtering. The final-date metadata is sufficient for a warm hit and
        # avoids validating the same 700-bar frame twice.
        return _cache_meta_proves_final(frame, now)
    clean = _clean_ohlcv(frame, strict=True)
    if clean.empty:
        return False
...
def _write_daily_ohlcv_cache(ticker: str, frame: pd.DataFrame, source_family: str='UNKNOWN', now: Any | None=None) -> None:
    ...
    try:
        ...
        tmp.replace(path)
        _write_daily_ohlcv_binary_cache(ticker, final_frame)
        meta_path = _daily_ohlcv_cache_meta_path(ticker)
        meta_tmp = meta_path.with_suffix('.tmp')
        source_attrs = dict(getattr(final_frame, 'attrs', {}) or {})
        split_dates = [
            str(value) for value in
            source_attrs.get('corporate_action_split_dates', []) or []
            if str(value).strip()
        ]
        dividend_dates = [
            str(value) for value in
            source_attrs.get('corporate_action_dividend_dates', []) or []
            if str(value).strip()
        ]
        split_events: list[dict[str, Any]] = []
        for raw_event in source_attrs.get('corporate_action_splits', []) or []:
            if not isinstance(raw_event, Mapping):
                continue
            event_date = str(raw_event.get('date') or '').strip()
            ratio = _num(raw_event.get('ratio'))
            if not event_date:
                continue
            split_events.append({
                'date': event_date,
                'numerator': (
                    _num(raw_event.get('numerator'))
                    if np.isfinite(_num(raw_event.get('numerator'))) else None
                ),
                'denominator': (
                    _num(raw_event.get('denominator'))
                    if np.isfinite(_num(raw_event.get('denominator'))) else None
                ),
                'ratio': ratio if np.isfinite(ratio) and ratio > 0 else None,
                'source': str(raw_event.get('source') or ''),
            })
        payload = {
            'schema_version': IDX_CACHE_SCHEMA_VERSION,
            'source_family': str(source_family or 'UNKNOWN').upper(),
            'written_at': stamp.isoformat(),
            'last_bar_date': pd.Timestamp(final_frame.index[-1]).date().isoformat(),
            'bar_state': 'FINAL_EOD',
            'finalized_for_date': _expected_last_completed_daily_date(stamp).date().isoformat(),
            'finalization_cutoff_wib': f'{IDX_DAILY_FINAL_HOUR:02d}:{IDX_DAILY_FINAL_MINUTE:02d}',
            ...
            'corporate_action_splits': sorted(
                split_events, key=lambda event: event['date'],
            ),
        }
...
def download_ohlcv(tickers: Iterable[str], period: str='5y', batch_size: int=80, itick_api_token: str='') -> tuple[dict[str, pd.DataFrame], DownloadReport]:
    histories, report = _download_ohlcv_v431(tickers, period, batch_size, itick_api_token)
    completed_map: dict[str, pd.DataFrame] = {}
    for ticker, frame in histories.items():
        ...
        if completed.empty:
            ...
            continue
        completed_map[ticker] = completed
    # Remove official closures and only market-wide consensus holiday rows.
    sanitized, calendar_audit = sanitize_idx_histories(completed_map)
    expected = _expected_last_completed_daily_date()
    filtered: dict[str, pd.DataFrame] = {}
    for ticker, completed in sanitized.items():
        if completed.empty:
            report.failed[ticker] = 'Seluruh bar terhapus oleh validasi sesi IDX'
            report.source_tiers[ticker] = 'UNAVAILABLE'
            continue
        latest = pd.Timestamp(completed.index[-1]).normalize()
        removed_count = int((calendar_audit.get('ticker', pd.Series(dtype=str)) == ticker).sum()) if not calendar_audit.empty else 0
        if removed_count:
            prior = report.warnings.get(ticker, '')
            note = f'{removed_count} bar non-trading IDX dibuang sebelum pengukuran likuiditas'
            report.warnings[ticker] = ' • '.join((x for x in (prior, note) if x))
        if latest != expected:
            report.failed[ticker] = f'Data stale: last={latest.date().isoformat()}, expected_session={expected.date().isoformat()}'
            report.source_tiers[ticker] = 'CACHE_STALE_RESEARCH_ONLY'
            prior = report.warnings.get(ticker, '')
            report.warnings[ticker] = ' • '.join((x for x in (prior, 'Tidak score-eligible dan tidak execution-eligible') if x))
            continue
        completed.attrs['current_data_state'] = 'CURRENT_LAST_COMPLETED_SESSION'
        completed.attrs['current_data_score_eligible'] = True
...
def _entry_plan_contract_evaluation(
    row: Mapping[str, Any],
    cfg: ScanConfig,
) -> tuple[bool, str, dict[str, float]]:
    """Validate one non-confirmed entry plan before exposing ENTRY_PLAN_READY.

    A detector's ``trigger`` can be a future reclaim/confirmation level rather
    than the order price.  It must therefore remain below TP1 for a limit/retest
    plan.  RR is always recomputed from the actual planned fill (``entry``),
    never from that confirmation level.
    """
    entry = _finite(row.get('entry'), np.nan)
    low = _finite(row.get('entry_low'), entry)
    high = _finite(row.get('entry_high'), entry)
    stop = _finite(row.get('stop_loss'), np.nan)
    tp1 = _finite(row.get('tp1'), np.nan)
    tp2 = _finite(row.get('tp2'), np.nan)
    confirmation = _finite(row.get('trigger'), np.nan)
    values = {
        'execution_price_reference': entry,
        'confirmation_level': confirmation,
        'entry_plan_rr1_recomputed': np.nan,
        'entry_plan_rr2_recomputed': np.nan,
    }
    if not all(np.isfinite(value) and value > 0 for value in (entry, stop, tp1, tp2)):
        return False, 'INCOMPLETE_ENTRY_PLAN', values
    if np.isfinite(low) and np.isfinite(high):
        if low > high or not (low - 1e-9 <= entry <= high + idx_tick_size(entry)):
            return False, 'INVALID_ENTRY_ZONE_GEOMETRY', values
    if not (stop < entry < tp1 < tp2):
        return False, 'INVALID_TARGET_GEOMETRY', values
    risk = entry - stop
    rr1 = (tp1 - entry) / risk
    rr2 = (tp2 - entry) / risk
    values['entry_plan_rr1_recomputed'] = float(rr1)
    values['entry_plan_rr2_recomputed'] = float(rr2)
    if rr1 + 1e-9 < cfg.min_rr1 or rr2 + 1e-9 < cfg.min_rr2:
        return False, 'ENTRY_PLAN_RR_BELOW_MINIMUM', values
    entry_type = _safe_text(row.get('entry_type')).upper()
    action = _safe_text(row.get('action')).upper()
    limit_retest_style = not ('BUY_STOP' in entry_type or action in {'READY_TRIGGER', 'BUY_ON_CONFIRMED_TRIGGER'})
    if limit_retest_style and np.isfinite(confirmation):
        if not (stop < confirmation < tp1):
            return False, 'CONFIRMATION_NOT_BELOW_TP1', values
    return True, 'VALID_ATOMIC_ENTRY_PLAN', values
...
def _autopilot_gate_evaluation(
    ...
    checks = {
        ...
        'DAILY_SOURCE_NOT_LIVE': bool(
            source_tier not in {'', 'CACHE_FALLBACK', 'CACHE_STALE_RESEARCH_ONLY', 'UNAVAILABLE'}
            and _row_has_current_final_ohlcv(row, cfg)
        ),
...
def finalize_execution_integrity(signals: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for column, default in {
        ...
        'entry_plan_contract_valid': False,
        'entry_plan_contract_state': 'NOT_EVALUATED',
        'entry_plan_min_rr1': float(cfg.min_rr1),
        'entry_plan_min_rr2': float(cfg.min_rr2),
    ...
    for idx, row in out.iterrows():
        ...
        if pending_close:
            ...
            continue
        plan_contract_valid, plan_contract_state, plan_contract_values = _entry_plan_contract_evaluation(row, cfg)
        out.at[idx, 'entry_plan_contract_valid'] = plan_contract_valid
        out.at[idx, 'entry_plan_contract_state'] = plan_contract_state
        for key, value in plan_contract_values.items():
            out.at[idx, key] = value
        if candidate and not hard and not plan_contract_valid:
            out.at[idx, 'status'] = 'WAIT_FOR_VALID_PLAN'
            out.at[idx, 'execution_mode'] = 'INVALID_ENTRY_PLAN'
            out.at[idx, 'setup_state'] = 'SETUP_READY_PLAN_INVALID'
            ...
            out.at[idx, 'stockbit_order_price'] = np.nan
            out.at[idx, 'stockbit_order_lots'] = 0
            out.at[idx, 'execution_timing'] = 'WAIT_REBUILD_ATOMIC_PLAN'
            out.at[idx, 'automation_decision'] = 'INVALID_ENTRY_PLAN'
            out.at[idx, 'execution_gate_failures'] = plan_contract_state
            out.at[idx, 'signal_execution_blockers'] = plan_contract_state
            out.at[idx, 'primary_execution_blocker'] = plan_contract_state
            out.at[idx, 'evidence_state'] = 'PLAN_LEVELS_DIAGNOSTIC_ONLY'
            out.at[idx, 'manual_execution_candidate'] = False
            out.at[idx, 'autopilot_verified'] = False
            out.at[idx, 'strict_execution_ready'] = False
            out.at[idx, 'analyst_pre_budget_ready'] = False
            out.at[idx, 'setup_valid_signal'] = False
            out.at[idx, 'analyst_candidate_reason'] = 'INVALID_ENTRY_PLAN'
            continue
        policy = _safe_text(getattr(cfg, 'execution_policy', 'SIGNAL_FIRST')).upper() or 'SIGNAL_FIRST'
        if policy != 'ACCOUNT_GUARDED':
...
def apply_analyst_fusion_gate(signals: pd.DataFrame, config: ScanConfig | None=None, minimum_score: float=68.0) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        out.at[idx, 'requires_stockbit_price_check'] = True
    return out
# Stable public helper API for scanner_focus. The focused module imports only
# explicit primitives, keeping the core engine independent from UI concerns.
def seed_daily_ohlcv_cache(ticker: str, frame: pd.DataFrame, source_family: str = "DATABASE") -> None:
    """Seed the local incremental cache from the persistent database.

    Streamlit's filesystem is ephemeral, while Supabase is durable. Seeding the
    local cache before a provider refresh lets the existing downloader request
    only bars after the last persisted session instead of re-downloading years.
    """
    _write_daily_ohlcv_cache(str(ticker), frame, source_family)
...
silent_accumulation_profile = _silent_accumulation_profile
truthy = _truthy

__version__ = '8.0.2-execution-integrity-source-lineage-hotfix'

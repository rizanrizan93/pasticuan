...
def build_issuer_map(
    ...
    base = _latest_by_ticker(normalize_fundamental_classification(fundamentals))
    if base.empty:
    ...
    for _, row in base.iterrows():
        ...
        sector_score = _finite(sector_scores.get(sector), np.nan) if sector != "UNKNOWN" else np.nan
        ...
        rows.append({
            ...
            "issuer_macro_alignment_state": "SCORED" if coverage >= 60 and sector != "UNKNOWN" else "PARTIAL" if coverage > 0 else "SECTOR_MISSING" if sector == "UNKNOWN" else "MISSING",

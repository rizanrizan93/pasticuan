...
NARRATIVE_ENGINE_VERSION = "1.3.0"
...
def _structured_financial_lineage(
    ...
    lineage_coverage = min(82.0, 0.55 * proxy_coverage + 0.20 * history_coverage + source_bonus + grade_bonus + official_bonus)
    missing: list[str] = []
    ...
    return {
        "eligible": eligible,
        "score": score,
        "coverage_pct": round(lineage_coverage if eligible else 0.0, 1),
        "state": (
            "OFFICIAL_STRUCTURED_FINANCIAL_EVIDENCE" if eligible and official
            else "IDENTIFIED_STRUCTURED_FINANCIAL_EVIDENCE" if eligible
...
def _capital_allocation_alignment_proxy(
    ...
    fund = dict(fundamental or {})
    components: list[tuple[str, float, float]] = []
    roic = _finite(fund.get("history_roic_proxy"), np.nan)
    cash_conversion = _finite(
        fund.get("history_cash_conversion", fund.get("cash_conversion_ttm")),
        np.nan,
    )
    ...
    if np.isfinite(roic):
        components.append(("ROIC", _linear_component(roic, 0.02, 0.20), 0.35))
    if np.isfinite(cash_conversion):
        score = (
            85.0 if 0.80 <= cash_conversion <= 1.80
            else 65.0 if 0.50 <= cash_conversion < 0.80
        ...
        components.append(("CASH_CONVERSION", score, 0.25))
    if np.isfinite(fcf):
        components.append(("FCF_DISCIPLINE", 72.0 if fcf > 0 else 38.0, 0.15))
    if np.isfinite(dilution):
        score = (
            85.0 if dilution <= 0.0
            else 65.0 if dilution <= 0.05
            else 35.0 if dilution <= 0.12
            else 12.0
        )
        components.append(("DILUTION_DISCIPLINE", score, 0.25))
    if not components:
        return {"score": np.nan, "coverage_pct": 0.0, "basis": ""}
    total_weight = sum(weight for _, _, weight in components)
    score = sum(score * weight for _, score, weight in components) / max(total_weight, 1e-9)
    return {
        "score": round(max(0.0, min(100.0, score)), 1),
        "coverage_pct": round(min(70.0, 100.0 * total_weight), 1),
        "basis": " | ".join(name for name, _, _ in components),
    }
    ...
    *,
    ...
    for ticker in names:
        ...
        row = {
            ...
            "narrative_state": (
                "SOURCE_GROUNDED"
                if narrative_evidence_mode in {"SOURCE_EVENT", "EVENT_PLUS_STRUCTURED_FINANCIAL"}
            ...
            "narrative_rank_neutral_score": round(narrative_effective, 1),
            ...
            "evidence_acquisition_missing": _text(structured_financial.get("missing")),
            ...
            "issuer_alignment_rank_neutral_score": round(

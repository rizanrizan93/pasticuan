...
NARRATIVE_ENGINE_VERSION = "1.1.0"
...
class NarrativeConfig:
    ...
    growth_max_adjustment_points: float = 8.0
    turnaround_max_adjustment_points: float = 10.0
    swing_max_adjustment_points: float = 8.0
    ...
    def from_scan_config(cls, config: Any | None) -> "NarrativeConfig":
        ...
        return cls(
            ...
            growth_max_adjustment_points=max(
                0.0, min(
                    12.0,
                    number("narrative_growth_max_adjustment_points", 8.0),
                ),
            ),
            turnaround_max_adjustment_points=max(
                0.0, min(
                    15.0,
                    number(
                        "narrative_turnaround_max_adjustment_points", 10.0,
                    ),
                ),
            ),
            swing_max_adjustment_points=max(
                0.0, min(
                    12.0,
                    number("narrative_swing_max_adjustment_points", 8.0),
                ),
            ),
        )
...
def _events_from_projects(
    ...
    for raw in project_management.to_dict("records"):
        ...
        source_urls = [
            value.strip()
            for value in (
                _text(raw.get("project_source_urls"))
        ...
        record = _event_record(
            ...
            source_family=(
                raw.get("project_source_families") or "PROJECT_REVIEW"
            ),
            event_date=raw.get("event_date") or raw.get("last_verified_at"),
            detected_at=raw.get("last_verified_at") or raw.get("as_of"),
...
def _events_from_fundamentals(
    ...
    for raw in fundamentals.to_dict("records"):
        ...
        headline = (
            ...
            f"pada laporan {_text(raw.get('statement_date')) or _text(raw.get('period_end'))}"
        )
        summary = " | ".join(
            part for part in (
        ...
        record = _event_record(
            ...
            source_family=(
                ...
                or "FUNDAMENTAL_SNAPSHOT"
            ),
            event_date=raw.get("statement_date") or raw.get("period_end"),
            detected_at=raw.get("fundamental_fetched_at"),
...
def _conversion_statistics(
    ...
    for ticker, group in local.groupby("ticker", sort=False):
        ...
        for horizon in (5, 20, 60):
            ...
            profile.update({
                f"narrative_conversion_rate_{horizon}d_pct": round(
                    posterior, 1,
                ),
                f"narrative_conversion_effective_{horizon}d_score": round(
                    effective, 1,
                ),
                f"narrative_conversion_expectancy_{horizon}d_pct": round(
                    expectancy, 3,
                ),
                ...
                f"narrative_conversion_cohort_prior_{horizon}d_pct": round(
                    prior_rate, 1,
                ),
            })
...
def build_narrative_profiles(
    ...
    *,
    ...
    for ticker in names:
        ...
        narrative_raw = max(0.0, min(100.0, 50.0 + 0.5 * signed_mean))
        cluster_count = int(
        ...
        for pm in pm_evidence:
            ...
            if governance or related == "CRITICAL":
                ...
                pm_coverage += 25.0
            pm_coverage += min(20.0, 0.20 * coverage)
        ...
        alignment_state = (
            ...
            else "ALIGNED" if alignment_effective >= 62.0
            else "MISALIGNED_OR_ENTRENCHMENT_RISK"
            if alignment_effective <= 38.0 else "MIXED"
        )
        ...
        primary_reason = (
            f"{convergence_state}; narrative {narrative_effective:.1f}, "
            f"effective Silent Accumulation {flow['effective_silent']:.1f}, "
            f"alignment {alignment_effective:.1f}"
        )
        primary_risk = (
        ...
        row = {
            ...
            "narrative_state": (
                ...
                if len(active) else "NO_EVENT_EVIDENCE"
            ),
            "narrative_score": round(narrative_raw, 1),
            "narrative_effective_score": round(narrative_effective, 1),
            ...
            "narrative_source_quality_score": round(
                float(pd.to_numeric(
                    active.get("source_quality_score"), errors="coerce",
                ).mean()) if not active.empty else 0.0,
                1,
            ),
            "narrative_novelty_score": round(
                float(pd.to_numeric(
                    active.get("novelty_score"), errors="coerce",
                ).mean()) if not active.empty else 0.0,
                1,
            ),
            "narrative_financial_bridge_score": round(bridge_mean, 1),
            "issuer_alignment_score": round(alignment_raw, 1),
            "issuer_alignment_effective_score": round(
                alignment_effective, 1,
            ),
            ...
            "narrative_flow_convergence_score": round(
                narrative_flow_raw, 1,
            ),
            "narrative_flow_effective_score": round(
            ...
            "narrative_production_policy": (
                "POSITIVE_NARRATIVE_ACTIVE_SOURCE_WEIGHTED_"
        ...
        row.update({
            f"narrative_conversion_rate_{horizon}d_pct":
                conversion_profile.get(
                    f"narrative_conversion_rate_{horizon}d_pct", 50.0,
                )
            for horizon in (5, 20, 60)
        })
        row.update({
    ...
    *,
    ...
    if not cfg.enabled:
        profiles = pd.DataFrame({
            ...
            "narrative_effective_score": 50.0,
            "issuer_alignment_effective_score": 50.0,

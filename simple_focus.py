...
SIMPLE_FOCUS_VERSION = "9.1.0-database-first"
...
def build_next_leaders(universe: pd.DataFrame, config: ScanConfig | None = None) -> pd.DataFrame:
    ...
    if out.empty:
        return out
    out = out.sort_values(["v9_next_leader_score", "score_coverage_pct", "ticker"], ascending=[False, False, True], na_position="last", kind="stable").reset_index(drop=True)
...
def build_swing_ready(universe: pd.DataFrame, next_leaders: pd.DataFrame) -> pd.DataFrame:
    ...
    if out.empty:
        return out
    out = out.sort_values(["v9_swing_score", "score_coverage_pct", "ticker"], ascending=[False, False, True], na_position="last", kind="stable").reset_index(drop=True)
    out["rank"] = np.arange(1, len(out) + 1)
...
def build_simple_focus(
    ...
    *,
    ...
    cfg = config or ScanConfig()
    silent_profiles, silent_frame = _build_silent_profiles(prepared)
    ...
    next_leaders = build_next_leaders(universe, cfg)
    swing_ready = build_swing_ready(universe, next_leaders)
    evidence = build_evidence_audit(next_leaders, swing_ready)
    return {
        "next_leaders": next_leaders,
        "swing_ready": swing_ready,
        "multibagger": next_leaders,
        "multibagger_research": next_leaders,

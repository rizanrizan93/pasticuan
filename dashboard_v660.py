...
DASHBOARD_VERSION = "6.6.3-public-prior-eoff-weighting"
CORE_STRATEGIES = {
...
def _build_swing_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if isinstance(builder, pd.DataFrame) and not builder.empty:
        source = builder.copy()
        strategy = source.get("strategy", pd.Series(index=source.index, dtype=str)).astype(str).str.upper()
        source = source[strategy.isin(CORE_STRATEGIES)].copy()
    else:
        source = result.get("signals", pd.DataFrame())
        if not isinstance(source, pd.DataFrame):
            source = pd.DataFrame()
    for _, row in source.iterrows():
        strategy = _text(row.get("strategy")) or _text(row.get("setup")) or "CORE"
        if strategy.upper() not in CORE_STRATEGIES and "SNIPER" not in strategy.upper():
            continue
...
def build_top15_ranking(result: Mapping[str, Any], limit: int = 15) -> pd.DataFrame:
    ...
    return out.drop(columns="_action_rank")
...
def render_top15_dashboard(
    ...
    with top_tab:
        ...
        if ranking.empty:
            ...
            m1.metric("Kandidat", len(ranking))
            m2.metric("Buy/prepare", int(ranking["decision"].isin(["BUY_ON_CONFIRMED_TRIGGER", "PREPARE_BUY_WAIT_TRIGGER"]).sum()))
            m3.metric("Multibagger", int(ranking["category"].str.contains("MULTIBAGGER", na=False).sum()))
            m4.metric("Swing/Core", int(ranking["category"].str.contains("SWING/CORE", na=False).sum()))

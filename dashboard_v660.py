...
def _build_swing_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for _, row in source.iterrows():
        ...
        cycle_weight = min(0.04, max(0.0, _num(row.get("time_cycle_effective_weight_pct"), 0.0) / 100.0)) if cycle_state == "VALIDATED" else 0.0
        ...
        rows.append({
            ...
            "emir_method_score": _num(row.get("emir_method_score"), np.nan),
            "emir_method_coverage_pct": _num(row.get("emir_method_coverage_pct"), np.nan),
            "emir_method_score_state": _text(row.get("emir_method_score_state")),
            "emir_method_state": _text(row.get("emir_method_state")),
            "emir_method_production_eligible": bool(row.get("emir_method_production_eligible", False)),
            "emir_method_reliability_pct": _num(row.get("emir_method_reliability_pct"), np.nan),
            "core_priority_score_pre_overlay": _num(row.get("core_priority_score_pre_overlay"), np.nan),
            "core_narrative_contribution_points": _num(row.get("core_narrative_contribution_points"), 0.0),
            "core_emir_contribution_points": _num(row.get("core_emir_contribution_points"), 0.0),
            "core_priority_score_formula": _text(row.get("core_priority_score_formula")),
...
def _build_multibagger_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for _, row in frame.iterrows():
        ...
        rows.append({
            ...
            "emir_method_production_eligible": bool(row.get("emir_method_production_eligible", False)),
            "emir_method_reliability_pct": _num(row.get("emir_method_reliability_pct"), np.nan),
            "growth_narrative_contribution_points": _num(row.get("growth_narrative_contribution_points"), 0.0),
            "growth_emir_contribution_points": _num(row.get("growth_emir_contribution_points"), 0.0),
            "turnaround_narrative_contribution_points": _num(row.get("turnaround_narrative_contribution_points"), 0.0),
            "turnaround_emir_contribution_points": _num(row.get("turnaround_emir_contribution_points"), 0.0),
            "multibagger_final_score_formula": _text(row.get("multibagger_final_score_formula")),

...
DASHBOARD_VERSION = "6.6.4-balanced-candidate-pool"
CORE_STRATEGIES = {
...
def build_top15_ranking(result: Mapping[str, Any], limit: int = 15) -> pd.DataFrame:
    raw = _build_swing_rows(result) + _build_multibagger_rows(result)
    if not raw:
        return pd.DataFrame()
    frame = pd.DataFrame(raw)
    frame = frame[frame["ticker"].astype(str).str.len().gt(0)].copy()
    # Keep setup identities separate.  Merging by ticker previously allowed a
    # Multibagger score to inherit a Swing entry/SL/TP from a different row.
    frame = frame.sort_values("combined_score", ascending=False, na_position="last")
    out = frame.drop_duplicates(["ticker", "category", "strategy"], keep="first").copy()
    out["candidate_id"] = out.apply(
        lambda row: f"{_text(row.get('ticker'))}|{_text(row.get('category'))}|{_text(row.get('strategy'))}",
        axis=1,
    )
    ticker_details = {
        ticker: group.sort_values("combined_score", ascending=False, na_position="last").to_dict("records")
        for ticker, group in out.groupby("ticker", sort=False)
    }
    out["all_rows"] = out["ticker"].map(ticker_details)
    out["_action_rank"] = out["decision"].map(ACTION_PRIORITY).fillna(8)
    # Execution semantics outrank score: an AVOID row can never sit above BUY.
    out = out.sort_values(["_action_rank", "combined_score", "base_conviction"], ascending=[True, False, False], na_position="last")
    pool = out.copy()
    eligible_multibagger = int(pool["category"].eq("MULTIBAGGER").sum())
    eligible_swing = int(pool["category"].eq("SWING/CORE").sum())
    best_multibagger = _num(pool.loc[pool["category"].eq("MULTIBAGGER"), "combined_score"].max(), np.nan)
    best_swing = _num(pool.loc[pool["category"].eq("SWING/CORE"), "combined_score"].max(), np.nan)
    out = out.head(max(1, int(limit))).reset_index(drop=True)
    out.insert(0, "rank", np.arange(1, len(out) + 1))
    final = out.drop(columns="_action_rank")
    ...
    final.attrs["candidate_pool_audit"] = {
        ...
        "top15_multibagger": int(final["category"].eq("MULTIBAGGER").sum()),
        "top15_swing_core": int(final["category"].eq("SWING/CORE").sum()),
        "best_multibagger_score": best_multibagger,
        "best_swing_core_score": best_swing,
        "top15_cutoff_score": cutoff,
...
def _ranking_display(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    columns = [
        "rank", "ticker", "category", "strategy", "combined_score", "decision",
...
def render_ranked_detail(result: Mapping[str, Any], ranking: pd.DataFrame, candidate_id: str) -> None:
    ...
    if isinstance(signals, pd.DataFrame) and not signals.empty and ticker in set(signals.get("ticker", pd.Series(dtype=str)).astype(str)):
        ...
        if price_frame is not None and not price_frame.empty:
            try:
                st.plotly_chart(make_signal_chart(price_frame, signal), width="stretch", key=f"top15_signal_{ticker}")
        ...
        if price_frame is not None and not price_frame.empty:
            try:
                ...
                if chart is not None:
                    st.plotly_chart(chart, width="stretch", key=f"top15_cycle_{ticker}")
            except Exception as exc:
...
def render_top15_dashboard(
    ...
    st.subheader("Top 15 Saham Terbaik — Multibagger & Swing/Core")
    st.caption(
        "Ranking menggabungkan conviction utama, AI tervalidasi bila tersedia, kualitas fundamental/forward, "
        "struktur setup, target, data, Time-Cycle, dan kesimpulan tanggal beli. BPJS, BSJP, ARA, dan intraday tidak masuk."
    )
    top_tab, detail_tab = st.tabs(["Top 15 Ranking", "Bedah Ticker Tanpa CSV"])
    with top_tab:
        ...
        if ranking.empty:
            ...
            m1.metric("Top 15", len(ranking))
            m2.metric("Buy/prepare", int(ranking["decision"].isin(["BUY_ON_CONFIRMED_TRIGGER", "PREPARE_BUY_WAIT_TRIGGER"]).sum()))
            m3.metric("Top Multibagger", int(ranking["category"].str.contains("MULTIBAGGER", na=False).sum()))
            m4.metric("Top Swing/Core", int(ranking["category"].str.contains("SWING/CORE", na=False).sum()))
            m5.metric("Pool Multibagger", int(audit.get("eligible_multibagger", 0)))
            m6.metric("Pool Swing/Core", int(audit.get("eligible_swing_core", 0)))
            swing_pool = int(audit.get("eligible_swing_core", 0))
            swing_top = int(audit.get("top15_swing_core", 0))
            if swing_pool == 0:
                st.warning(
                ...
                st.info(
                    f"Ada {swing_pool} kandidat Swing/Core di pool, tetapi tidak masuk Top 15 murni. "
                    f"Skor Swing/Core terbaik {_num(audit.get('best_swing_core_score'), 0.0):.1f}; "
                    f"cutoff Top 15 {_num(audit.get('top15_cutoff_score'), 0.0):.1f}. Tidak ada kuota kategori paksa."
                )
            ...
            try:
                event = st.dataframe(
                    ...
                    key="top15_clickable_ranking_v660",
                    column_config={
            ...
            if selected_index is not None and 0 <= selected_index < len(ranking):
                st.session_state["top15_selected_candidate_v661"] = ranking.iloc[selected_index]["candidate_id"]
            labels = {
                f"#{int(row['rank'])} {row['ticker']} | {row['category']} | {row['strategy']}": row["candidate_id"]
                for _, row in ranking.iterrows()
            }
            prior_id = st.session_state.get("top15_selected_candidate_v661")
            ...
            selected_label = st.selectbox(
                ...
                key="top15_detail_selector_v661",
            )
            candidate_id = labels[selected_label]
            st.session_state["top15_selected_candidate_v661"] = candidate_id
            render_ranked_detail(result, ranking, candidate_id)
            st.download_button(
                "Download Top 15 ranking",
                _ranking_display(ranking).to_csv(index=False).encode("utf-8"),
                "top15_multibagger_swing_core.csv",
                "text/csv",
                width="stretch",
            )

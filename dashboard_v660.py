...
DASHBOARD_VERSION = "6.8.1-research-maintenance"
CORE_STRATEGIES = {
...
def _build_swing_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for _, row in source.iterrows():
        ...
        rows.append({
            ...
            "silent_accumulation_score": round(_clip(row.get("silent_accumulation_score")), 1),
...
def render_ranked_detail(result: Mapping[str, Any], ranking: pd.DataFrame, candidate_id: str) -> None:
    ...
    if all_rows:
        ...
        detail_columns = [
            "category", "strategy", "combined_score", "base_conviction",
            "multibagger_quality_score", "execution_readiness_score", "multibagger_candidate_type",
            "research_recommendation_status", "economic_earnings_score", "economic_earnings_state",
            "minority_leakage_pct", "silent_accumulation_score", "silent_accumulation_state",
...
def render_top20_dashboard(
    ...
    st.caption(
        ...
        "Satu ticker hanya memakai satu slot. Ranking hanya berisi Multibagger dan Swing/Core."
    )

...
DASHBOARD_VERSION = "7.1.2-mixed-datetime-normalization"
CORE_STRATEGIES = {
...
def _best_buy_date_profile(row: Mapping[str, Any], as_of: pd.Timestamp, max_horizon_days: int = 120) -> tuple[str, str]:
    ...
    if date > as_of + pd.Timedelta(days=max(1, int(max_horizon_days))):
...
def _core_candidate_source(result: Mapping[str, Any]) -> pd.DataFrame:
    ...
    normalized_strategy = pd.Series(
        [
            _normalize_core_strategy(_text(row.get("strategy")) or _text(row.get("setup")))
...
def _ranking_display(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    columns = [
        "rank", "ticker", "category", "strategy", "ranking_priority", "final_score",
        "silent_accumulation_score", "decision_precedence_state", "decision",
        "best_buy_date", "best_buy_date_state", "buy_window", "entry_zone",
        "trigger", "stop_loss", "tp1", "tp2", "time_cycle_score",
        "time_cycle_confidence", "phase", "eoff_strength", "status",
    ]
...
def render_ranked_detail(result: Mapping[str, Any], ranking: pd.DataFrame, candidate_id: str) -> None:
    ...
    if all_rows:
        ...
        detail_columns = [
            ...
            "silent_accumulation_score", "silent_accumulation_state", "accumulation_persistence_score",
...
def render_top20_dashboard(
    ...
    st.subheader("Top 20 Saham Terbaik — Final Score & Silent Accumulation")
    st.caption(
        "Ranking P0 hanya berisi kandidat eligible. Urutan utama: Final Score tertinggi, lalu Silent Accumulation. "
        "REJECT, BLOCKED, NOT_QUALIFIED, NO_ALLOCATION, data tidak terskor/kedaluwarsa, dan non-syariah dikeluarkan. "
        "Best Buy Date dan EOFF hanya menjadi informasi timing setelah cycle VALIDATED."
    )
    ...
    with top_tab:
        ...
        if ranking.empty:
            ...
            m3.metric("Excluded P0", int(audit.get("excluded_total", 0)))
            ...
            try:
                event = st.dataframe(
                    ...
                    column_config={
                        "final_score": st.column_config.NumberColumn("Final", format="%.1f"),
                        "silent_accumulation_score": st.column_config.NumberColumn("Silent Accum", format="%.1f"),

...
def _score_cross_section(
    ...
    for _, local in groups:
        local = local.copy()
        feature_values = local.reindex(columns=FEATURE_COLUMNS).apply(
            pd.to_numeric, errors="coerce",
        )
        local["technical_feature_coverage_pct"] = (
            100.0 * feature_values.notna().mean(axis=1)
        ).round(1)
        coverage_weight = (
            local["technical_feature_coverage_pct"] / 100.0
        ).clip(0.0, 1.0)
        trend = (
        ...
        roc_ranks = pd.concat(
            [
                _percentile(local["roc20"]).rename("20D"),
                _percentile(local["roc60"]).rename("60D"),
                _percentile(local["roc120"]).rename("120D"),
            ],
            axis=1,
        )
        ...
        momentum_continuity = (
            0.30 * _percentile(local["positive_day_ratio20"])
            + 0.25 * _percentile(local["trend_efficiency20"])
            + 0.25 * _percentile(local["trend_efficiency60"])
            + 0.10 * _percentile(-pd.to_numeric(local["jump_concentration20"], errors="coerce"))
            + 0.10 * _percentile(local["drawdown60"])
        )
        high52 = _percentile(local["distance_52w_high"])
        momentum_quality = (
            0.30 * momentum
        ...
        sector = local.get(
            "sector", pd.Series("", index=local.index, dtype="object"),
        ).fillna("").astype(str).str.strip()
        sector_peer_count = sector.map(sector.value_counts()).where(sector.ne(""), 0)
        sector_raw = (
            0.55 * pd.to_numeric(local["relative_strength60"], errors="coerce")
            + 0.30 * pd.to_numeric(local["roc60"], errors="coerce")
            + 0.15 * pd.to_numeric(local["roc20"], errors="coerce")
        )
        sector_relative = pd.Series(50.0, index=local.index, dtype=float)
        ...
        if eligible_sector.any():
            sector_relative.loc[eligible_sector] = (
                ...
                .groupby(sector.loc[eligible_sector])
        ...
        def coverage_adjust(score: pd.Series) -> pd.Series:
            numeric = pd.to_numeric(score, errors="coerce").fillna(50.0)
        ...
        entry = coverage_adjust(pd.Series(entry, index=local.index))
        ...
        local["momentum_consistency_score"] = momentum_consistency.round(2)
        local["momentum_continuity_score"] = momentum_continuity.round(2)
        local["high_52w_proximity_score"] = high52.round(2)
        local["momentum_quality_score"] = momentum_quality.round(2)
        local["relative_strength_score"] = relative.round(2)
        local["sector_peer_count"] = sector_peer_count.fillna(0).astype(int)
        local["sector_relative_strength_score"] = sector_relative.round(2)
        local["sector_relative_state"] = np.where(
            eligible_sector, "SECTOR_NEUTRAL_ACTIVE", "INSUFFICIENT_SECTOR_PEERS",
        )
        local["flow_score"] = flow.round(2)
        local["entry_geometry_score"] = entry.round(2)
        local["liquidity_score"] = liquidity.round(2)
        local["rule_engine_score"] = absolute_rule.round(2)
        # Absolute scores are invariant to the number or quality of peers.
        # Cross-sectional ranks remain a bounded overlay, never the base score.
        roc20 = pd.to_numeric(local["roc20"], errors="coerce")
        roc60 = pd.to_numeric(local["roc60"], errors="coerce")
        roc120 = pd.to_numeric(local["roc120"], errors="coerce")
        absolute_momentum = (
            0.45 * (50.0 + 250.0 * roc20).clip(0.0, 100.0)
            + 0.35 * (50.0 + 150.0 * roc60).clip(0.0, 100.0)
            + 0.20 * (50.0 + 100.0 * roc120).clip(0.0, 100.0)
        )
        cmf20 = pd.to_numeric(local["cmf20"], errors="coerce")
        cmf60 = pd.to_numeric(local["cmf60"], errors="coerce")
        accumulation = pd.to_numeric(local["accumulation_proxy"], errors="coerce")
        ...
        absolute_flow = (
            ...
            + 0.25 * pd.Series(accumulation_score, index=local.index).clip(0.0, 100.0)
        )
        log_adtv = pd.to_numeric(local["log_adtv20"], errors="coerce")
        absolute_liquidity = (50.0 + 20.0 * (log_adtv - np.log(2_000_000_000.0))).clip(0.0, 100.0)
        relative_abs = (50.0 + 200.0 * pd.to_numeric(local["relative_strength60"], errors="coerce")).clip(0.0, 100.0)
        absolute_selector = (
            0.38 * absolute_rule
        ...
        peer_count = int(local["ticker"].nunique()) if "ticker" in local else int(len(local))
        if peer_count < 20:
            relative_weight = 0.0
            universe_state = "ABSOLUTE_ONLY_SMALL_UNIVERSE"
        elif peer_count < 50:
            relative_weight = 0.05
            universe_state = "LIMITED_RELATIVE_OVERLAY"
        elif peer_count < 100:
            relative_weight = 0.10
            universe_state = "MODERATE_RELATIVE_OVERLAY"
        else:
            relative_weight = 0.10
            universe_state = "ABSOLUTE_DOMINANT_RELATIVE_OVERLAY"
        relative_selector = (
            0.28 * trend
        ...
        local["absolute_momentum_score"] = coverage_adjust(absolute_momentum).round(2)
        local["absolute_flow_score"] = coverage_adjust(absolute_flow).round(2)
        local["absolute_liquidity_score"] = coverage_adjust(absolute_liquidity).round(2)
        local["absolute_selector_score"] = coverage_adjust(absolute_selector).round(2)
        local["relative_selector_overlay_score"] = coverage_adjust(relative_selector).round(2)
        local["cross_sectional_peer_count"] = peer_count
        local["relative_overlay_weight_pct"] = round(100.0 * relative_weight, 1)
        local["selector_universe_state"] = universe_state
        local["score_inflation_guard_active"] = bool(relative_weight < 0.10)
        local["independent_selector_score"] = (
            (1.0 - relative_weight) * local["absolute_selector_score"]
            + relative_weight * local["relative_selector_overlay_score"]
        ).round(2)
        local["selector_rank_eligible"] = local[
            "technical_feature_coverage_pct"
        ].ge(float(min_feature_coverage_pct))
        local["selector_data_state"] = np.select(
            [
                local["technical_feature_coverage_pct"].ge(90.0),
                local["selector_rank_eligible"],
            ],
            ["FEATURES_COMPLETE", "FEATURES_SUFFICIENT"],
            default="DATA_PENDING_FEATURES",
        )
        local["selector_missing_feature_count"] = (
            len(FEATURE_COLUMNS) - feature_values.notna().sum(axis=1)
        ).astype(int)
        local["selector_missing_features"] = [
            " | ".join(feature_values.columns[~mask].tolist())
            for mask in feature_values.notna().to_numpy()
        ]
        scored.append(local)
...
def build_selector_panel(
    ...
    for ticker, raw in prepared.items():
        ...
        for position in range(start, stop, max(1, int(cfg.anchor_step_bars))):
            ...
            liquidity_bucket, market_impact = _liquidity_cost_from_log_adtv(
                current.get("log_adtv20"), cfg,
            )
            total_cost = max(0.0, float(cfg.roundtrip_cost_pct)) + market_impact
            record["liquidity_bucket"] = liquidity_bucket
            record["estimated_market_impact_cost_pct"] = market_impact
            record["estimated_total_cost_pct"] = total_cost
            ...
            for horizon in cfg.horizons:
                ...
                net_stock = stock_return - total_cost if np.isfinite(stock_return) else np.nan
                net_excess = (
...
def build_cross_sectional_selector(
    ...
    cfg = config or SelectorConfig()
    current = _current_feature_rows(prepared, cfg, sector_map)
    panel = build_selector_panel(prepared, cfg, sector_map)

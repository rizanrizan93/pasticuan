...
SELECTOR_VERSION = "8.0.0-slim-selector-ai-shadow-zero-weight"
...
def _reason_fields(row: Mapping[str, Any]) -> dict[str, str]:
    ...
    ai_weights = [
        _finite(row.get(name), 0.0)
        for name in (
            "selector_ai_weight_5d_pct",
            "selector_ai_weight_20d_pct",
            "selector_ai_weight_60d_pct",
        )
    ]
    ai_active = max(ai_weights, default=0.0) > 0.0
    if ai_active and state == "PROMOTED_AI_CHAMPION":
        ...
        codes.append("RULE_BASED_SELECTOR_ONLY")
    if not positives:
...
def build_cross_sectional_selector(
    ...
    for horizon in cfg.horizons:
        ...
        if float(cfg.max_ai_weight) <= 0.0:
            current[f"selector_model_state_{horizon}d"] = "RULE_BASED_ONLY"
            current[f"selector_champion_{horizon}d"] = "INDEPENDENT_SELECTOR"
        else:
...
def update_selector_outcomes(
    ...
    # Supabase/PostgREST commonly returns date columns as ISO strings, while
    # newly generated in-memory records use pandas.Timestamp.  A mixed object
    # column cannot be ordered by pandas (Timestamp < str), so normalise every
    # persisted temporal field at the boundary before any merge or sort.
    for date_column, normalise_day in (("signal_date", True), ("resolved_at", False)):
        if date_column not in memory.columns:
            continue
        parsed_dates = pd.to_datetime(memory[date_column], errors="coerce", utc=True)
        parsed_dates = parsed_dates.dt.tz_convert(None)
        memory[date_column] = parsed_dates.dt.normalize() if normalise_day else parsed_dates
    ...
    if memory.empty:
        ...
        if not additions_frame.empty:
            # Avoid pandas' deprecated dtype inference path when one side has
            # columns that are entirely NA.  Preserve the full schema, but only
            # concatenate columns carrying evidence and restore empty columns
            # afterwards.
            ordered_columns = list(dict.fromkeys([
                *memory.columns.tolist(), *additions_frame.columns.tolist(),
            ]))
            memory_nonempty = memory.dropna(axis=1, how="all")
            additions_nonempty = additions_frame.dropna(axis=1, how="all")
            memory = pd.concat(
                [memory_nonempty, additions_nonempty], ignore_index=True, sort=False
    ...
    for _, raw_row in memory.iterrows():
        ...
        resolved.append(record)
    outcome_frame = pd.DataFrame(resolved)
    if outcome_frame.empty:
        return outcome_frame
    if "signal_date" in outcome_frame.columns:
        signal_dates = pd.to_datetime(
            outcome_frame["signal_date"], errors="coerce", utc=True,
        ).dt.tz_convert(None).dt.normalize()
        outcome_frame["signal_date"] = signal_dates
    if "resolved_at" in outcome_frame.columns:
        outcome_frame["resolved_at"] = pd.to_datetime(
            outcome_frame["resolved_at"], errors="coerce", utc=True,
        ).dt.tz_convert(None)
    return outcome_frame.drop_duplicates(

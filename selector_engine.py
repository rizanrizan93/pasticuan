...
SELECTOR_VERSION = "1.2.0-sector-neutral-impact-pbo"
...
def build_cross_sectional_selector(
    ...
    current["swing_selection_score"] = np.round(
        0.35 * horizon_swing + 0.35 * technical + 0.20 * silent + 0.10 * swing_quality,
        1,
    )
    current["multibagger_timing_selector_score"] = np.round(
        0.45 * horizon_long + 0.30 * silent + 0.25 * technical,
        1,
    )
    ...
    current = current.sort_values(
        [
            "swing_selection_score", "silent_accumulation_score",

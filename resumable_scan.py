...
RESUMABLE_SCAN_VERSION = "9.8.1-legacy-compat"
            ...
            if not claimed.empty:
                ...
                for _, item_row in claimed.iterrows():
                    ...
                    checkpoints.append({
                        ...
                        "payload": json_safe(outcome.payload or {}),
                        "error": outcome.error or "ITEM_FAILED",
                        "retry_delay_seconds": int(outcome.retry_delay_seconds),
                    })
                try:
                    # Healthy path: one UPSERT for the whole chunk instead of one
                    # network round trip per ticker. Durability is unchanged.
                    bridge.checkpoint_scan_job_items_batch(checkpoints)
                except Exception:
                    # Preserve legacy row-level fallback, but never multiply a
                    # systemic Supabase/auth/schema outage into 20 individual
                    # requests. The outer worker loop will pause/retry safely.
                    detail = str(getattr(bridge, "_write_details", {}).get("scan_job_items", ""))
                    checker = getattr(bridge, "_failure_text_is_systemic", None)
                    if callable(checker) and checker(detail):
                        raise
                    for checkpoint in checkpoints:
                        item = dict(checkpoint["item"])
                        if checkpoint["success"]:
                            bridge.complete_scan_job_item(item, result_payload=checkpoint["payload"])
                        else:
                            bridge.fail_scan_job_item(
                                item,
                                checkpoint["error"],
                                retry_delay_seconds=checkpoint["retry_delay_seconds"],
                                result_payload=checkpoint["payload"],
                            )

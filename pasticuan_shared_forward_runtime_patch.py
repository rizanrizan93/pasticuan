...
def _attach_profiles(frame: pd.DataFrame, rows: list[dict[str, Any]], universe: Sequence[str]) -> pd.DataFrame:
    ...
    if frame.empty or profile_frame.empty:
        return frame
...
def install() -> dict[str, str]:
    ...
    def shared_job_forward_quality(job_id: str, bridge: Any, universe: Sequence[str]) -> pd.DataFrame:
        ...
        # Before v36 is deployed or during a Shared Hub outage, retain the old
        # fail-soft cache path. Once the canonical table is readable, even an
        # empty result is authoritative and must not silently diverge locally.

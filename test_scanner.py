...
class SpecialtyScannerV42Tests(unittest.TestCase):
    ...
    def test_bsjp_ready_requires_late_session_intraday(self):
        ...
        self.assertEqual(result.loc[0, "order_instruction"], "MANUAL_REVIEW_ONLY")
    ...
    def test_risk_off_regime_blocks_bsjp_ready(self):
        frame = self._strong_daily_frame()
        result = scan_bsjp_candidates(
            {"TEST.JK": frame}, {"TEST.JK": self._intraday("bsjp")},
            now="2026-07-13 15:30:00",
        ...
        self.assertNotEqual(result.loc[0, "bsjp_status"], "BSJP_READY")
        self.assertIn("RISK_OFF", result.loc[0, "blockers"])
    ...
    def test_ara_hunter_never_becomes_direct_execution(self):
        frame = self._strong_daily_frame()
        result = scan_ara_hunter_candidates(
            {"TEST.JK": frame}, {"TEST.JK": self._intraday("bsjp")}
        )
        self.assertFalse(result.empty)
        self.assertTrue((result["order_instruction"] == "SPECULATIVE_REVIEW_ONLY").all())
        self.assertTrue((result["max_position_pct_equity"] <= 0.05).all())
...
class ZeroUploadAutomationV426Tests(unittest.TestCase):
    ...
    def test_streamlit_ui_only_keeps_universe_and_optional_portfolio_uploaders(self):
        app_source = (ROOT / "app.py").read_text(encoding="utf-8")
        self.assertEqual(app_source.count("st.file_uploader("), 2)
        self.assertNotIn("independent_price_upload", app_source)
        self.assertNotIn("broker_summary_upload", app_source)
    ...
    def test_effective_release_version_and_price_verification_capacity(self):
        self.assertEqual(scanner_module.__version__, "4.3.3-premarket-session-fix")

...
class SpecialtyScannerV42Tests(unittest.TestCase):
    ...
    def _intraday(mode: str) -> pd.DataFrame:
        index = pd.date_range("2026-07-13 09:00", periods=20, freq="15min")
        close = np.linspace(1_000, 1_030, 20)
        open_ = np.r_[998.0, close[:-1]]
        high = np.maximum(open_, close) + 2
        low = np.minimum(open_, close) - 2
        if mode == "bsjp":
            volume = np.r_[np.full(16, 500_000.0), np.full(4, 2_000_000.0)]
        else:
            volume = np.r_[np.full(4, 2_000_000.0), np.full(16, 500_000.0)]
        return pd.DataFrame(
            {"Open": open_, "High": high, "Low": low, "Close": close, "Volume": volume},
            index=index,
        )
...
class NewsFallbackV422Tests(unittest.TestCase):
    @patch("requests.get")
    def test_google_news_rss_parser_returns_dated_items(self, mocked_get):

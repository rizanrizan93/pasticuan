...
DATABASE_BRIDGE_VERSION = "3.0-v7.0.0"
DATABASE_SCHEMA_VERSION = "scanner_schema_v3"
...
TABLE_FIELD_TYPES: dict[str, dict[str, set[str]]] = {
    ...
    "multibagger_snapshots": {
        ...
        "numeric": {
            ...
            "silent_accumulation_score", "silent_accumulation_confidence",
    ...
    "eoff_predictions": {
        "text": {
            "snapshot_id", "scan_id", "ticker", "best_buy_date",
        ...
        "integer": {"eoff_public_directional_events"},
        "boolean": set(), "json": set(),
        "required": {"snapshot_id", "ticker", "as_of", "model_version", "schema_version"},
    },
    "project_events": {
    ...
    "fundamental_history_cache": {
        "text": {
            ...
            "content_hash", "refresh_state", "model_version", "schema_version",
        },
    ...
    "refresh_state": {
        "text": {
            ...
            "detail", "model_version", "schema_version",
        },
...
class ScannerDatabaseBridge:
    ...
    def _read_cache_table(
        ...
        try:
            ...
            state = "MIGRATION_REQUIRED_V3" if migration_missing else "DATABASE_READ_FAIL_SOFT"
            audit = pd.DataFrame([{
                "ticker": ticker, "provider": "SUPABASE_DATABASE_FIRST", "scope": payload_kind,
                "status": state, "database_read_state": state, "age_days": np.nan,
        ...
        for ticker in names:
            ...
            audits.append({
                ...
                "refresh_required": state != "DATABASE_CURRENT", "rows": loaded,
    ...
    def build_payloads(self, result: Mapping[str, Any]) -> dict[str, list[dict[str, Any]]]:
        ...
        model_version = _clean_text(result.get("scanner_version")) or "7.0.0"
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise
            return self.status_row("HEALTHY_V3_DATABASE_FIRST", f"HTTP 200; schema {self.settings.schema}; read-through cache ready")
        except Exception as exc:

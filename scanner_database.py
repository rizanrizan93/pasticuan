        ...
        *,
        ...
        for raw_ticker, raw_payload in payloads.items():
            ...
            records.append({
                ...
                "payload": payload, "content_hash": _semantic_hash({"compact_hash": _semantic_hash(compact_payload) if compact_payload else None, "bars": int(compact_bars), "last": pd.Timestamp(frame.index[-1]).date().isoformat() if not frame.empty else None}), "updated_at": now,
            })
        if not records:
            return pd.DataFrame()
        try:

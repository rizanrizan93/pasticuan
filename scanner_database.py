...
DATABASE_BRIDGE_VERSION = "14.3-bounded-write-isolation"
DATABASE_SCHEMA_VERSION = "scanner_schema_v14"
...
class ScannerDatabaseBridge:
    ...
    def write_ohlcv_cache(
        ...
        *,
        ...
        for ticker in names:
            frame = self._normalise_ohlcv_frame(histories.get(ticker))
            payload = self._ohlcv_payload(frame, max_bars=max_bars)
            ...
            state = states.get(ticker) or ("CURRENT" if payload else "MISSING")
            error = failures.get(ticker, "")
            record = {
                "ticker": ticker,
                "payload": payload,
                "bar_count": len(payload),
                "first_bar_date": payload[0]["Date"] if payload else None,
                "last_bar_date": payload[-1]["Date"] if payload else None,
            ...
            audit_rows.append({
                "ticker": ticker, "provider": "SUPABASE_OHLCV",
                "status": "WRITE_PENDING", "bars": len(payload),
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise
            return self.status_row(
                "HEALTHY_V14_GUARDED_REAL_MONEY",
                f"HTTP 200; schema {self.settings.schema}; resumable jobs, persistent OHLCV, official reconciliation, and guarded real-money fields ready",
            )
        except Exception as exc:

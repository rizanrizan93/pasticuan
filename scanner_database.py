...
DATABASE_BRIDGE_VERSION = "4.0-v7.1.0"
DATABASE_SCHEMA_VERSION = "scanner_schema_v4"
...
TABLE_FIELD_TYPES: dict[str, dict[str, set[str]]] = {
    ...
    "research_outcomes": {
        ...
        "numeric": {"signal_score", "signal_confidence", "entry_reference", "forward_return_5d", "forward_return_10d", "forward_return_20d", "maximum_favourable_excursion", "maximum_adverse_excursion"},
...
class ScannerDatabaseBridge:
    ...
    def _read_cache_table(
        ...
        for ticker in names:
            ...
            if stored_refresh_state in {"EVENT_DUE", "FORCE_REFRESH", "REVISION_DETECTED"}:
            ...
            model_reason = semantic_refresh_reason(row.get("model_version"), MODEL_VERSIONS["scanner"])
            if parser_reason:
                state = "DATABASE_MODEL_STALE"
                refresh_reason = parser_reason
            elif model_reason and model_reason in {"MODEL_MAJOR_CHANGED", "MODEL_MINOR_CHANGED"}:
                state = "DATABASE_MODEL_STALE"
            ...
            audits.append({
                ...
                "asof": _clean_text(checked_at), "error": "" if loaded else "Payload kosong/tidak valid",
                "source_family": "DATABASE",
            })
    ...
    def build_payloads(self, result: Mapping[str, Any]) -> dict[str, list[dict[str, Any]]]:
        ...
        model_version = _clean_text(result.get("scanner_version")) or "7.1.0"
        ...
        if isinstance(fundamentals, pd.DataFrame) and not fundamentals.empty and "ticker" in fundamentals:
            for _, row in fundamentals.dropna(subset=["ticker"]).drop_duplicates("ticker", keep="last").iterrows():
                ...
                content_hash = _stable_hash({key: value for key, value in payload.items() if key not in {"database_source_checked_at", "database_source_state"}})
                ...
                next_check_at = (pd.to_datetime(checked_at, errors="coerce", utc=True) + pd.Timedelta(days=self.settings.fundamental_max_age_days))
        ...
        if isinstance(fundamental_history, pd.DataFrame) and not fundamental_history.empty and "ticker" in fundamental_history:
            for ticker, group in fundamental_history.groupby("ticker", sort=False):
                ...
                payloads["fundamental_history_cache"].append({
                    ...
                    "next_check_at": (pd.to_datetime(checked_at, errors="coerce", utc=True) + pd.Timedelta(days=self.settings.history_max_age_days)).isoformat(),
        ...
        if isinstance(projects, pd.DataFrame) and not projects.empty and "ticker" in projects:
            for ticker, group in projects.groupby("ticker", sort=False):
                ...
                payloads["forward_quality_cache"].append({
                    ...
                    "next_check_at": (pd.to_datetime(checked_at, errors="coerce", utc=True) + pd.Timedelta(days=self.settings.forward_max_age_days)).isoformat(),
            ...
            if isinstance(report, pd.DataFrame) and not report.empty and "provider" in report:
                ...
                for _, row in scheduled.iterrows():
                    ...
                    payloads["backfill_state"].append({
                        ...
                        "next_due_at": (pd.Timestamp(as_of) + pd.Timedelta(days=1)).isoformat(),
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise
            return self.status_row("HEALTHY_V4_RESEARCH_MEMORY", f"HTTP 200; schema {self.settings.schema}; database-first, research memory, and calendar hooks ready")
        except Exception as exc:
...
__all__ = [
    ...
    "_snapshot_id", "_provider_health_frame", "_freshness_state",
]

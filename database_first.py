...
def _ticker_set(frame: pd.DataFrame | None, *, eligible_column: str | None = None) -> set[str]:
    ...
    if eligible_column and eligible_column in local.columns:
        local = local.loc[local[eligible_column].fillna(False).astype(bool)]
...
def _audit_refresh_map(report: pd.DataFrame | None, scope: str) -> dict[str, bool]:
    ...
    if local.empty:
        return {}
    refresh = local.get("refresh_required", pd.Series(True, index=local.index)).fillna(True).astype(bool)
...
def readiness_summary(
    ...
    *,
    ...
    def pct(column: str) -> float:
        return 100.0 * float(coverage.get(column, pd.Series(False, index=coverage.index)).fillna(False).astype(bool).sum()) / total
    ...
    ready_count = int(coverage.get("database_ticker_ready", pd.Series(False, index=coverage.index)).fillna(False).astype(bool).sum())
    ...
    *,
    ...
    for column in (
        ...
        if column not in local.columns:
            local[column] = False
        local[column] = local[column].fillna(False).astype(bool)
...
def estimate_remaining_passes(coverage: pd.DataFrame, batch_size: int) -> int:
    if coverage is None or coverage.empty:
        return 0
    pending = int((~coverage.get("database_ticker_ready", pd.Series(False, index=coverage.index)).fillna(False).astype(bool)).sum())

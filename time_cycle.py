...
from research_maintenance import adjust_to_idx_trading_day
...
TIME_CYCLE_VERSION = "7.1.0-idx-calendar-unique-anchor"
...
def _build_quick_buy_decision(
    ...
    *,
    ...
    unavailable = {
        ...
        "best_buy_date": "",
        "best_buy_raw_date": "",
        "best_buy_calendar_state": "INVALID_DATE",
        "best_buy_calendar_verified": False,
        "best_buy_date_adjustment_days": 0,
        "best_buy_date_basis": "",
    ...
    raw_candidate_date = candidate_date
    calendar = adjust_to_idx_trading_day(
        candidate_date,
        holidays=idx_trading_holidays,
        official_open_dates=idx_official_open_dates,
        official_closed_dates=idx_official_closed_dates,
        direction="next",
    )
    candidate_date = _parse_business_date(calendar.get("trading_date"))
    if candidate_date is not None and raw_candidate_date is not None and candidate_date != raw_candidate_date:
        date_basis = f"{date_basis}_ADJUSTED_TO_IDX_SESSION"
    calendar_verified = bool(calendar.get("calendar_verified"))
    if require_official_idx_calendar and not calendar_verified:
        candidate_date = None
    ...
    if not plan_valid:
        ...
        result.update({
            ...
            "quick_buy_action": "WAIT_FOR_EVIDENCE",
            "best_buy_date": "",
            "best_buy_raw_date": calendar.get("raw_date", ""),
            "best_buy_calendar_state": calendar.get("calendar_state", ""),
            "best_buy_calendar_verified": calendar_verified,
            "best_buy_date_adjustment_days": int(calendar.get("adjustment_days", 0) or 0),
...
def analyze_time_cycle(frame: pd.DataFrame | None, config: TimeCycleConfig | None = None) -> dict[str, Any]:
    ...
    unavailable = {
        ...
        "eoff_fib_unique_anchor_count": 0,
        "eoff_fib_unique_anchor_ratio": 0.0,
        "eoff_fib_dominant_anchor_share": 1.0,
        "eoff_unique_anchor_gate": False,
        "eoff_unique_anchor_signature": "",
    ...
    if not np.isfinite(dominant):
        ...
        result.update(analyze_eoff_reconstruction(
            df, config=EOFFConfig(
                ...
                min_unique_anchors=cfg.eoff_min_unique_anchors,
                max_dominant_anchor_share=cfg.eoff_max_dominant_anchor_share,
            ),
        ))
    ...
    # capped separately at 4% for core/swing and 0% for Multibagger.
    eoff_active = bool(eoff.get("eoff_signal_active"))
    eoff_history = int(_finite(eoff.get("eoff_public_directional_events"), 0.0))
    eoff_edge = _finite(eoff.get("eoff_public_reversal_hit_rate"), np.nan) - _finite(eoff.get("eoff_public_baseline_rate"), np.nan)
    strength_multiplier = {
        "VERY_STRONG": 1.0, "STRONG": 0.72, "MEDIUM": 0.25, "LOW": 0.0,
    }.get(str(eoff.get("eoff_strength_label") or "LOW"), 0.0)
    objective_gate = bool(
        eoff_active and eoff_history >= 20 and np.isfinite(eoff_edge) and eoff_edge >= 5.0
        and _finite(eoff.get("eoff_public_forward_hit_rate"), 0.0) >= 52.0
        and _finite(eoff.get("eoff_public_median_directional_return_pct"), 0.0) > 0.0
    )
    eoff_internal_weight = min(0.08, 0.08 * strength_multiplier * min(1.0, eoff_history / 40.0)) if objective_gate else 0.0
    ...
    quick_buy = _build_quick_buy_decision(
        ...
        idx_trading_holidays=cfg.idx_trading_holidays,
        idx_official_open_dates=cfg.idx_official_open_dates,
        idx_official_closed_dates=cfg.idx_official_closed_dates,
        require_official_idx_calendar=cfg.require_official_idx_calendar,
    )
    ...
    explanation = (
        ...
        f"EOFF {eoff.get('eoff_strength_label', 'LOW')} active={eoff_active} objective-gate={objective_gate} internal-weight={eoff_internal_weight*100:.1f}%; astro remains shadow-only."
    )
    return {
        "time_cycle_version": TIME_CYCLE_VERSION,
...
def enrich_core_signals_with_time_cycle(
    ...
    *,
    ...
    cap = max(0.0, min(0.04, float(max_weight)))

...
EOFF_VERSION = "7.1.0-unique-anchor-outcome-memory"
...
class EOFFConfig:
    ...
    min_unique_anchors: int = 3
    max_dominant_anchor_share: float = 0.55
    future_horizon_bars: int = 34
    require_astro_fib_confluence: bool = False
...
def _best_projection_cluster(candidates: Sequence[Mapping[str, Any]], current_position: int, config: EOFFConfig) -> dict[str, Any]:
    if not candidates:
        return {
            ...
            "fib_unique_anchor_ratio": 0.0, "fib_dominant_anchor_share": 1.0,
            "fib_unique_anchor_gate": False, "fib_unique_anchor_signature": "",
    ...
    anchors = [int(row["anchor"]) for row in deduped]
    anchor_count = len(set(anchors))
    count = len(deduped)
    anchor_frequency = {anchor: anchors.count(anchor) for anchor in set(anchors)}
    dominant_share = max(anchor_frequency.values()) / count if count else 1.0
    unique_ratio = anchor_count / count if count else 0.0
    unique_gate = bool(
        anchor_count >= max(1, int(config.min_unique_anchors))
        and dominant_share <= max(0.0, min(1.0, float(config.max_dominant_anchor_share)))
    )
    signature_payload = sorted((int(row["anchor"]), str(row["basis"])) for row in deduped)
    signature = sha256(json.dumps(signature_payload, separators=(",", ":")).encode("utf-8")).hexdigest()[:20] if signature_payload else ""
    diversity_multiplier = 1.0 if unique_gate else max(0.35, unique_ratio * (1.0 - dominant_share + 0.25))
    score = _clip((12.0 * count + 5.0 * min(6, anchor_count)) * diversity_multiplier)
    return {
        "fib_cluster_count": count,
        "fib_unique_anchor_count": anchor_count,
        "fib_unique_anchor_ratio": round(unique_ratio, 4),
        "fib_dominant_anchor_share": round(dominant_share, 4),
        "fib_unique_anchor_gate": unique_gate,
        "fib_unique_anchor_signature": signature,
...
def analyze_eoff_reconstruction(
    ...
    *,
    ...
    unavailable = {
        ...
        "eoff_adaptive_total_weight_pct": 0.0,
        ...
        "eoff_astro_weight_policy": "SHADOW_DIAGNOSTIC_ZERO_PRODUCTION_WEIGHT",
    ...
    time_power = _clip(
        0.70 * fib_score + 0.30 * cycle_evidence
    )
    if direction == "BULLISH":
    ...
    unique_anchor_gate = bool(cluster.get(
        "fib_unique_anchor_gate",
        int(cluster.get("fib_unique_anchor_count", 0)) >= max(1, cfg.min_unique_anchors),
    ))
    fib_active = int(cluster.get("fib_cluster_count", 0)) >= max(1, cfg.min_fib_cluster) and unique_anchor_gate
    ...
    # Research policy v6.9.0: astronomy remains a shadow diagnostic.
    # Production direction/timing is driven by Fibonacci clusters, causal
    # price-pattern-momentum context and chronological forward validation.
    core_path = bool(core_historical_ok)
    adaptive_path = bool(validated_active_factors)
    secondary_prior_path = False
    validation_path = (
        "OBJECTIVE_FIB_PRICE_FORWARD_VALIDATION_UNIQUE_ANCHORS" if core_path and unique_anchor_gate
        else "RESEARCH_ONLY_ANCHOR_CONCENTRATION" if not unique_anchor_gate
        else "RESEARCH_ONLY_INSUFFICIENT_FORWARD_EVIDENCE"
    )
    astro_event_active = bool(core_astro_active or adaptive_event_active)
    historical_ok = core_path
    signal_active = bool(
        fib_active
    ...
    if signal_active and reconstruction >= 82.0 and int(cluster.get("fib_cluster_count", 0)) >= cfg.min_fib_cluster + 3:
        ...
        strength = "LOW"
    state = "ACTIVE_OBJECTIVE_TIME_VALIDATED" if signal_active else "RESEARCH_ONLY_NO_DIRECTIONAL_VALIDATION" if ephemeris.get("ephemeris_available") else "EPHEMERIS_UNAVAILABLE"
    ...
    explanation = (
        ...
        f"{int(cluster.get('fib_unique_anchor_count', 0))} anchor; unique gate {bool(cluster.get('fib_unique_anchor_gate'))}; time power {time_power:.1f}; "
        f"astro shadow {astro_score:.1f} (production weight 0%); price {price_score:.1f}; pattern {pattern_score:.1f}; "
    ...
    return {
        ...
        "eoff_adaptive_total_weight_pct": 0.0,
        "eoff_secondary_prior_share_pct": 0.0,
        ...
        "eoff_phase_weight_pct": 0.0,
        "eoff_aspect_weight_pct": 0.0,
        ...
        "eoff_fib_unique_anchor_ratio": round(_finite(cluster.get("fib_unique_anchor_ratio"), 0.0), 4),
        "eoff_fib_dominant_anchor_share": round(_finite(cluster.get("fib_dominant_anchor_share"), 1.0), 4),
        "eoff_unique_anchor_gate": unique_anchor_gate,
        "eoff_unique_anchor_signature": str(cluster.get("fib_unique_anchor_signature") or ""),
        ...
        "eoff_declination_weight_pct": 0.0,
        ...
        "eoff_retrograde_weight_pct": 0.0,

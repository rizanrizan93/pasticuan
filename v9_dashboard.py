...
V9_DASHBOARD_VERSION = "1.0.0-emir-style-modular"
...
LEADER_STATUS_PRIORITY = {"BUY_ZONE": 0, "WATCH": 1, "WAIT": 2}
SWING_STATUS_PRIORITY = {"EXECUTION_READY": 0, "ENTRY_PLAN_READY": 1, "WATCHLIST": 2, "WAIT": 3}
...
def _final_score(row: Mapping[str, Any], model: str) -> float:
    ...
    return _score(row.get("final_score", row.get(field, 0.0)))
...
def select_top_candidates(frame: pd.DataFrame, *, model: str, limit: int = 3) -> pd.DataFrame:
    ...
    score_source = local[score_field] if score_field in local.columns else local["final_score"] if "final_score" in local.columns else pd.Series(np.nan, index=local.index)
...
def _card_html(row: Mapping[str, Any], rank: int, model: str) -> str:
    ...
    model_label = "THE NEXT LEADER" if model == "NEXT_LEADER" else "SWING READY"
    ...
    <section class="v9-card {top_color}">
      <div class="v9-card-head">
        ...
        <div class="v9-score"><span>FINAL SCORE</span><strong>{score:.0f}</strong><small>/100 • cov {_fmt_pct(row.get('score_coverage_pct'))}</small></div>
...
def render_dashboard_html(
    ...
    *,
    ...
    @media(max-width:900px){{.v9-card-head{{grid-template-columns:56px 1fr 105px}} .v9-grid-main,.v9-grid-bottom{{grid-template-columns:1fr}} .v9-identity h2{{font-size:28px}}}}
    ...
    <div class="v9-wrap">
      ...
      <div class="v9-method"><strong>v9 Macro-First + Inventory Guardrails</strong><br>{_esc(method)}<br><small>Valuation dan fundamental score tetap berasal dari core v9; lifecycle hanya mengatur timing/anti-chasing.</small></div>

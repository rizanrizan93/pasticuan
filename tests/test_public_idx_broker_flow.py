...
def test_daily_trim_keeps_top_buyers_and_sellers():
    ...
    for i in range(12):
        rows.append({
            ...
            "ticker": "TEST",
            "broker_code": f"B{i:02d}",
            "buy_value": (100-i) * 1000,
            "sell_value": 0,
            "buy_volume": (100-i),
            "sell_volume": 0,
            "buy_avg": 1000,
            "sell_avg": np.nan,
            "net_value": (100-i) * 1000,
            "net_volume": (100-i),
            "gross_value": (100-i) * 1000,
            "source": broker.SOURCE_NAME,
    ...
    for day in pd.bdate_range("2026-07-20", periods=20):
        for rank, value in enumerate((100, 80, 60)):
            rows.append({
                ...
                "net_value": value * 1000000,
                "net_volume": value * 1000,
                "side": "TOP_NET_BUYER",
                "net_rank": rank + 1,
                "buy_value": value * 1000000,
                "sell_value": 0,
                "buy_volume": value * 1000,
                "sell_volume": 0,
                "buy_avg": 1000,
                "sell_avg": np.nan,
...
def test_live_fallback_fail_soft(monkeypatch):
    monkeypatch.setattr(broker, "load_public_cache", lambda: pd.DataFrame())
    def fail(*args, **kwargs):
        raise RuntimeError("source unavailable")
    monkeypatch.setattr(broker, "download_trade_detail", fail)
    features, meta = broker.get_broker_features(["TEST"], live_fallback=True)
    assert meta["state"] == "NO_CACHE"
    assert features.empty or "broker_flow_coverage_pct" in features.columns

...
def _extract_forward_rows(ticker: str, text: str, source_url: str, source_family: str, verified: bool, fund: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for position, sentence in enumerate(sentences):
        if position in consumed:
            continue
        lower = sentence.lower()
        if len(sentence) < 35 or not any(term in lower for term in _FORWARD_PROJECT_TERMS):
            continue
    ...
    ceo_name = ceo_name or safe_text(fund.get('ceo_name'))
...
def allocate_multibagger_capital(candidates: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        base_scale = (1.0 - project_weight - management_weight - future_impact_weight) / base_total_weight
        ...
        strongest = sorted({**normalized, 'Data integrity': data_integrity, 'Solvency': solvency, 'Project pipeline': project_score, 'Management': management_score, 'Future impact': future_impact_score}.items(), key=lambda item: item[1], reverse=True)[:3]
        ...
        out.at[idx, 'allocation_reason'] = f'{tier}; conviction {conviction:.1f}/100 (rule {rule_conviction:.1f}){ai_note}; strongest pillars: {strongest_text}'
        if eligible:

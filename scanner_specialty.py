...
def build_profit_order_builder(
    ...
    result = above.drop_duplicates('ticker', keep='first').copy()
    ...
    for strategy in expected_strategies:
        ...
        if local_raw.empty:
            ...
            state = 'DEDUPED_OR_OUTSIDE_TOP_LIMIT'
        else:
            state = 'INCLUDED'
        audit_rows.append({
            'strategy': strategy,
...
def allocate_multibagger_capital(candidates: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        cycle_weight = (
            ...
            if bool(getattr(cfg, 'time_cycle_enabled', True)) and cycle_confidence >= safe_number(getattr(cfg, 'time_cycle_min_confidence', 55.0), 55.0)
            else 0.0
        )
        base_weights = {

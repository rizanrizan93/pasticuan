...
DEFAULT_HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; IDX-Super-Scanner/7.13.0; +research)",
...
def yahoo_fundamental_timeseries_direct(
    ...
    *,
    ...
    """Fetch annual and quarterly fundamentals from Yahoo timeseries JSON."""
    client = session or requests.Session()
    types = []
    for field in dict.fromkeys((*TIMESERIES_MAP.values(), *TIMESERIES_ALIASES)):
        types.extend((f"quarterly{field}", f"annual{field}"))
    ...
    params = {
        ...
        "merge": "false",
        # ``true`` can collapse historical values into padded null series for
        # IDX symbols.  Sparse real observations are safer than fabricated pads.
        "padTimeSeries": "false",
    }
    payload, attempts = _request_json(
        client, url, params=params, timeout=timeout,
    ...
    if timeseries.get("error"):
        raise RuntimeError(f"Yahoo timeseries error: {timeseries.get('error')}")
    ...
    for series in result:
        ...
        series_type = _series_type(meta.get("type") or series.get("type"))
        prefix = "quarterly" if series_type.startswith("quarterly") else "annual" if series_type.startswith("annual") else ""
        base = series_type[len(prefix):] if prefix else series_type
        canonical = reverse.get(base)
        if not canonical:
            continue
        for point in series.get(series_type, []) or series.get("reportedValue", []) or []:
            date_text = point.get("asOfDate") or point.get("periodType")
            ...
            if isinstance(reported, dict):
                currency = str(reported.get("currencyCode") or "")
            if currency:
    ...
    for key, row in by_period.items():
        ...
        # Yahoo frequently omits OperatingCashFlow while publishing FCF and
        # CapitalExpenditure. Reconstruct the accounting identity only when
        # both reported inputs exist; missing inputs remain missing.
        ...
        if not np.isfinite(ocf_number):
            ...
            if np.isfinite(fcf_number) and np.isfinite(capex_number):
                row["operating_cash_flow"] = (
                    fcf_number - capex_number
                    if capex_number < 0 else fcf_number + capex_number
                )
        ...
        for hidden in [key for key in row if key.startswith("_source_priority_")]:
            row.pop(hidden, None)

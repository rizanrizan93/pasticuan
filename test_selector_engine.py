...
from selector_engine import (
    SelectorConfig,
    _bootstrap_mean_ci,
    _cscv_backtest_overfit_probability,
    _feature_matrix,
    _liquidity_cost_from_log_adtv,
    _spearman,
...
class CrossSectionalSelectorTests(unittest.TestCase):
    def test_model_matrix_appends_missing_indicators(self):
        frame = pd.DataFrame([{
            "close_ema20": 0.05,
            "roc20": np.nan,
        }])
        matrix = _feature_matrix(frame)
        self.assertEqual(matrix.shape[1] % 2, 0)
        midpoint = matrix.shape[1] // 2
        self.assertEqual(float(matrix[0, midpoint]), 0.0)
        # roc20 is the sixth declared feature.
        self.assertEqual(float(matrix[0, midpoint + 5]), 1.0)

    def test_low_feature_coverage_stays_diagnostic_not_production_rank(self):
        feature_names = (
            "close_ema20", "ema20_ema50", "ema50_ema200",
            "ema20_slope10", "adx14_scaled", "roc20", "roc60", "roc120",
            "relative_strength60", "cmf20", "cmf60", "obv_slope20",
            "adl_slope20", "volume_ratio", "atr_pct",
            "distance_ema20_atr", "distance_52w_high",
            "positive_day_ratio20", "trend_efficiency20",
            "trend_efficiency60", "jump_concentration20", "drawdown60",
            "log_adtv20", "accumulation_proxy",
        )
        row = {
            "ticker": "MISSING.JK",
        ...
        scored = _score_cross_section(
            pd.DataFrame([row]),
            min_feature_coverage_pct=75.0,
        )
        self.assertFalse(bool(scored.iloc[0]["selector_rank_eligible"]))
        self.assertEqual(
            scored.iloc[0]["selector_data_state"],
            "DATA_PENDING_FEATURES",
        )
        self.assertGreater(
            int(scored.iloc[0]["selector_missing_feature_count"]),
            15,
        )

    def test_liquidity_impact_cost_is_higher_for_illiquid_names(self):
        cfg = SelectorConfig()
        illiquid_bucket, illiquid_cost = _liquidity_cost_from_log_adtv(
            np.log1p(1_000_000_000.0), cfg,
        )
        liquid_bucket, liquid_cost = _liquidity_cost_from_log_adtv(
            np.log1p(300_000_000_000.0), cfg,
        )
        self.assertEqual(illiquid_bucket, "VERY_ILLIQUID")
        self.assertEqual(liquid_bucket, "VERY_LIQUID")
        self.assertGreater(illiquid_cost, liquid_cost)

    def test_cscv_pbo_detects_stable_champion_without_overfit(self):
        dates = [f"2026-01-{day:02d}" for day in range(1, 25)]
        model_returns = {
            model: {
                date: base + 0.001 * ((index % 3) - 1)
                for index, date in enumerate(dates)
            }
            for model, base in {
                "RULE_ENGINE": 1.0,
                "INDEPENDENT_SELECTOR": 0.8,
                "AI_CHALLENGER": 0.6,
                "RELATIVE_STRENGTH": 0.4,
            }.items()
        }
        audit = _cscv_backtest_overfit_probability(
            model_returns, SelectorConfig(cscv_slices=6),
        )
        self.assertEqual(audit["cscv_state"], "PBO_GUARD_PASSED")
        self.assertEqual(float(audit["cscv_pbo_pct"]), 0.0)
        self.assertGreater(int(audit["cscv_trials"]), 0)
    ...
    def test_python312_runtime_uses_scipy_118_bca(self):
        if _statistical_backend() != "SCIPY_1.18.0":
            self.skipTest(
                "Release pins SciPy 1.18.0; local Codex runtime provides "
                f"{_statistical_backend()}."
            )
    ...
    def test_sector_neutral_relative_strength_activates_with_five_peers(self):
        prepared = _prepared_cross_section(count=10)
        sectors = {
            ticker: "ENERGY" if index < 5 else "MATERIALS"
            for index, ticker in enumerate(prepared)
        }
        selector, _, _ = build_cross_sectional_selector(
            prepared,
            SelectorConfig(
                min_history_bars=120,
                training_lookback_bars=180,
                anchor_step_bars=10,
                min_cross_section=4,
                min_training_rows=9_999,
                min_evaluation_rows=9_999,
                min_evaluation_tickers=25,
                statistical_bootstrap_resamples=199,
                statistical_permutation_resamples=199,
            ),
            sector_map=sectors,
        )
        self.assertTrue(
            selector["sector_relative_state"].eq("SECTOR_NEUTRAL_ACTIVE").all()
        )
        self.assertTrue(selector["sector_peer_count"].eq(5).all())
...
class FundamentalQualityResearchTests(unittest.TestCase):
    def test_history_features_include_gross_profitability_and_accruals(self):
        periods = pd.date_range("2023-03-31", periods=12, freq="QE")
        rows = []
        ...
        self.assertIn("fundamental_inflection_score", features)
        self.assertGreaterEqual(
            float(row["fundamental_inflection_coverage_pct"]), 75.0,
        )
    ...
    def test_selector_outcomes_normalise_mixed_persisted_dates_before_sort(self):
        existing = pd.DataFrame([
            {
                "outcome_id": "mixed-a", "snapshot_id": "snapshot-a",
                "ticker": "AAA.JK", "signal_date": "2026-07-01",
                "horizon": "5D", "horizon_bars": 5,
                "outcome_status": "OPEN",
            },
            {
                "outcome_id": "mixed-b", "snapshot_id": "snapshot-b",
                "ticker": "BBB.JK",
            ...
            {
                "outcome_id": "mixed-invalid", "snapshot_id": "snapshot-c",
                "ticker": "CCC.JK", "signal_date": "not-a-date",
                "horizon": "5D", "horizon_bars": 5,
                "outcome_status": "OPEN",
            },
        ])
        result = update_selector_outcomes(
            existing, pd.DataFrame(), {}, SelectorConfig(horizons=(5,)),
        )
        self.assertEqual(result["outcome_id"].tolist(), [
            "mixed-b", "mixed-a", "mixed-invalid",
        ])
        self.assertTrue(pd.api.types.is_datetime64_ns_dtype(result["signal_date"]))
        self.assertTrue(pd.isna(result.iloc[-1]["signal_date"]))
...
class ExecutionOutcomePersistenceTests(unittest.TestCase):
    def test_ai_outcome_memory_normalises_mixed_persisted_dates(self):
        existing = pd.DataFrame([
            {
                "signal_id": "ai-a", "ticker": "AAA.JK",
                "signal_date": "2026-07-01", "memory_state": "RESOLVED",
            },
            {
                "signal_id": "ai-b", "ticker": "BBB.JK",
                "signal_date": pd.Timestamp("2026-06-30"),
                "memory_state": "RESOLVED",
            },
            {
                "signal_id": "ai-invalid", "ticker": "CCC.JK",
                "signal_date": "invalid-date", "memory_state": "RESOLVED",
            },
        ])
        with mock.patch("ai_engine.save_memory"):
            result = update_outcome_memory(
                pd.DataFrame(), {}, existing, LocalAIConfig(memory_max_rows=10),
            )
        self.assertEqual(result["signal_id"].tolist(), [
            "ai-b", "ai-a", "ai-invalid",
        ])
        self.assertTrue(pd.api.types.is_datetime64_ns_dtype(result["signal_date"]))
        self.assertTrue(pd.isna(result.iloc[-1]["signal_date"]))
    ...
    def test_execution_outcome_records_fill_mfe_mae_and_net_cost(self):
        ...
        self.assertEqual(row["competing_risk_event"], "TP1_FIRST")
        self.assertGreaterEqual(int(row["bars_to_exit"]), 1)

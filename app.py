...
APP_VERSION = "9.4.0-resumable-chunked-scan"
...
def _job_result_from_artifacts(job: Mapping[str, object], artifacts: pd.DataFrame) -> dict[str, object]:
    ...
    if not audit.empty and "completed_ticker_list" in audit.columns:
        raw_completed = audit.iloc[0].get("completed_ticker_list")
...
def _render_resumable_job(job: Mapping[str, object], bridge: ScannerDatabaseBridge) -> None:
    ...
    cols[1].metric("Selesai", f"{completed}/{total}")
    cols[2].metric("Gagal dilewati", failed)
    cols[3].metric("Menunggu retry", retries)
    cols[4].metric("Progress", f"{progress_pct:.1f}%")
    ...
    with st.expander("Status ticker / retry", expanded=status in {"PAUSED", "COMPLETE_WITH_FAILURES", "FAILED"}):
        try:
            items = bridge.read_scan_job_items(str(job.get("job_id")), include_payload=False, limit=5000)
        except Exception as exc:
...
if run_scan:
    ...
    if ticker_file is not None:
        ...
        try:
            ...
            st.error(
                "Job resumable tidak dapat dibuat. Terapkan database/migration_v12_resumable_scan_jobs.sql "
                f"dan pastikan Supabase memakai service-role/secret key. Detail: {type(exc).__name__}: {str(exc)[:500]}"
            )
            st.stop()
...
if job_error and not active_job:
    st.error(
        "Repository job resumable belum dapat dibaca. Jalankan migration_v12_resumable_scan_jobs.sql. "
        f"Detail: {job_error}"
    )

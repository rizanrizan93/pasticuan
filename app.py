...
def render_table_download(df: pd.DataFrame, filename: str, label: str, key: str | None = None):
    csv_bytes = df.to_csv(index=False).encode("utf-8")
    if key is None:
        key = f"download_{filename}_{abs(hash(label))}"
...
def main():
    ...
    with tab1:
        ...
        if isinstance(results, pd.DataFrame) and not results.empty:
            ...
            render_table_download(top, "idx_profit_scanner_results.csv", "Download hasil scan CSV", key="download_results_csv")
        else:
    ...
    with tab2:
        ...
        render_table_download(universe_df, "idx_starter_universe_200.csv", "Download starter universe CSV", key="download_starter_universe_table")
    ...
    with tab3:
        ...
        render_table_download(default_universe_df(), "idx_starter_universe_200.csv", "Download starter universe CSV", key="download_starter_universe_format")

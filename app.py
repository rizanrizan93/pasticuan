...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.3.0", page_icon="🛡️", layout="wide")
...
def render_ihsg_direction_panel(
    ...
    if isinstance(horizons, pd.DataFrame) and not horizons.empty:
        display_columns = [
            "horizon", "prediction_state", "raw_direction", "prob_up_pct",
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.3.0")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.3.0 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "7.3.0-cross-sectional-execution-ai",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.3.0 — Selector relatif, Execution AI, Multibagger, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_orders:
        ...
        if core_radar.empty:
            ...
            radar_columns = [
                column for column in (
                    ...
                    "technical_selection_score", "silent_accumulation_score",
                    "relative_strength_score", "trend_score",
        ...
        if not selector_audit_view.empty:
            with st.expander("Walk-forward selector: rule vs independen vs AI vs relative strength", expanded=False):
                ...
                st.caption(
                    "AI selector hanya dipromosikan bila expectancy net setelah biaya mengalahkan seluruh baseline, "
                    "Brier skill positif, dan maximum drawdown tetap di bawah guard."
                )
        st.divider()
    ...
    with tab_multibagger:
        ...
        if not multibagger.empty:
            top_destination = str(multibagger.iloc[0].get("ticker", "CASH_RESERVE"))
            top_amount = float(multibagger.iloc[0].get("strategic_target_amount_idr", 0.0) or 0.0)
        c1, c2, c3, c4 = st.columns(4)
        ...
        if not multibagger.empty:
            allocation_columns = [
                column for column in [
                    ...
                    "silent_accumulation_score", "selector_trend_score",
    ...
    with tab_audit:
        ...
        st.caption(
            "Bridge v6.0 membaca cache fundamental, histori laporan, forward quality, kalender IDX, selector outcomes, execution-AI outcomes, dan snapshot arah IHSG dari Supabase. "
        ...
        if not database_read_report.empty:
            ...
            misses = int(read_state.isin(["DATABASE_MISS", "DATABASE_EXPIRED", "DATABASE_MODEL_STALE", "DATABASE_EVENT_DUE", "DATABASE_CHECK_DUE", "MIGRATION_REQUIRED_V3", "MIGRATION_REQUIRED_V4", "MIGRATION_REQUIRED_V5", "MIGRATION_REQUIRED_V6"]).sum())
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.3.0 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.3.0 — Selector relatif, IHSG, Multibagger & Core Swing

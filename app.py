...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.2.0", page_icon="🛡️", layout="wide")
...
REQUIRED_SCANNER_FILES = (
    ...
    "ai_engine.py", "time_cycle.py", "eoff_reconstruction.py", "dashboard_v660.py",
    "ihsg_direction.py",
)
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.2.0")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.2.0 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    try:
        calendar_start = pd.Timestamp.now(tz="Asia/Jakarta").normalize() - pd.Timedelta("45D")
        calendar_end = calendar_start + pd.Timedelta("240D")
    ...
    result.update({
        ...
        "scanner_version": "7.2.0-ihsg-direction-risk-overlay",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.2.0 — Arah IHSG, Multibagger & Core Swing</b><br>
...
with main_existing:
    ...
    with tab_audit:
        ...
        st.caption(
            "Bridge v5.0 membaca cache fundamental, histori laporan, forward quality, kalender IDX, outcome memory, dan snapshot arah IHSG dari Supabase. "
        ...
        if not database_read_report.empty:
            ...
            misses = int(read_state.isin(["DATABASE_MISS", "DATABASE_EXPIRED", "DATABASE_MODEL_STALE", "DATABASE_EVENT_DUE", "DATABASE_CHECK_DUE", "MIGRATION_REQUIRED_V3", "MIGRATION_REQUIRED_V4", "MIGRATION_REQUIRED_V5"]).sum())
        ...
        if isinstance(research_summary_view, pd.DataFrame) and not research_summary_view.empty:
            ...
            with st.expander("Detail research outcomes", expanded=False):
                ...
                st.download_button(
                    ...
                    "research_outcomes_v7_2.csv", "text/csv", key="download_research_outcomes_v720",
                )
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.2.0 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.2.0 — IHSG, Multibagger & Core Swing
            ...
            3. **Dua fokus saham:** scanner hanya meranking Multibagger dan Core Swing harian.
            4. **Core Swing:** Pullback Continuation, Breakout Retest, Reversal Accumulation, dan Unicorn/ICT harian tetap independen dengan entry, trigger, stop, TP, RR, dan expiry masing-masing.
            ...
            6. **Timing:** EOFF dan Time-Cycle hanya menjadi overlay terjaga untuk Best Buy Date dan jendela akumulasi; tidak menggantikan struktur harga atau fundamental.

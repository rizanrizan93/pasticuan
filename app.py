...
st.set_page_config(page_title="IDX Super Scanner Profit Conviction Engine v5.8.0", page_icon="🛡️", layout="wide")
...
st.title("IDX Super Scanner — Profit Conviction Engine v5.8.0")
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v5.8.0 mengambil Yahoo dan IDX/XBRL otomatis. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    specialty_screens["profit_order_builder"] = build_profit_order_builder(signals, specialty_screens, cfg)
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v5.8.0 Profit Conviction Engine</b><br>
...
with tab_orders:
    ...
    st.caption(
        "Urutan lintas Pullback Continuation, Sniper, BPJS, BSJP, Pre-ARA, dan ARA Continuation. "
        "Conviction menggabungkan struktur, timing, demand/flow, likuiditas, target harga, kualitas data, dan validasi; bukan jaminan profit."
    )
    if profit_orders.empty:
        ...
        p2.metric("Conviction tertinggi", f"{float(profit_orders.iloc[0]['profit_conviction_score']):.1f}")
        p3.metric("Prioritas #1", str(profit_orders.iloc[0].get('ticker', '-')))
        p4.metric("Strategi #1", str(profit_orders.iloc[0].get('strategy', '-')))
...
with tab_multibagger:
    ...
    st.caption(
        ...
        "integritas data, momentum, dan smart-money proxy. Target weight adalah bobot strategis sleeve; "
    ...
    if not multibagger.empty:
        allocation_columns = [
            column for column in [
                ...
                "multibagger_status", "compounding_state", "strategic_target_weight_pct",
                "deploy_now_weight_pct", "strategic_target_amount_idr", "recommended_allocation_idr", "recommended_lots",
                "allocation_action", "allocation_reason", "red_flags",
...
with tab_method:
    st.markdown(
        """
        ### Hirarki sinyal v5.8.0 Profit Conviction Engine

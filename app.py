...
def load_price_history(symbol: str, months: int = 12) -> pd.DataFrame:
    ...
    periods = [f"{months}mo", f"{max(3, months // 2)}mo"]
    last_error = None
...
def analyze_symbol(symbol: str, months: int, benchmark_df: pd.DataFrame | None = None) -> dict[str, Any]:
    ...
    fund = _fundamental_quality(load_yf_info(symbol))
...
def scan_universe(tickers: list[str], months: int, benchmark_symbol: str, workers: int) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(analyze_symbol, ticker, months, benchmark_df): ticker for ticker in tickers}
...
def main():
    ...
    with tab1:
        ...
        if st.button("Run scan", type="primary"):
            with st.spinner("Scanning universe..."):
                ...
                results = scan_universe(universe_df["Ticker"].tolist(), months=months, benchmark_symbol=benchmark_symbol, workers=workers)
                if results.empty:
    ...
    with tab4:
        ...
        st.dataframe(multibagger_df, use_container_width=True, hide_index=True)

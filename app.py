# Free-tier / Streamlit Cloud fix:
# - joblib is optional via a pickle fallback
# - still add scikit-learn in requirements.txt
# - keep main module as app.py in repo root
...
def render_dashboard(context: Dict):
    ...
    with tabs[0]:
        ...
        st.caption("Jika market risk-off, fokus ke watchlist. Jika risk-on, ambil kandidat dengan score terbaik.")
        if benchmark_detail:
            st.dataframe(pd.DataFrame([benchmark_detail]), width="stretch", hide_index=True)

        st.markdown("### Breadth")
        b1, b2 = st.columns(2)
        b1.metric("Above SMA50", f"{breadth_above_sma50:.1f}%" if np.isfinite(breadth_above_sma50) else "N/A")
        b2.metric("Above SMA200", f"{breadth_above_sma200:.1f}%" if np.isfinite(breadth_above_sma200) else "N/A")
    ...
    with tabs[1]:
        ...
        st.dataframe(filtered[show_cols].head(context["top_n"]).reset_index(drop=True), width="stretch", hide_index=True)

        st.markdown("**Interpretasi:** A+ / A = kandidat utama, B = monitor, C = abaikan.")
        st.caption("Pilih ticker lain di bawah untuk melihat detail tanpa mengulang scan.")
        options = df["ticker"].tolist()
        selected = ensure_selectbox_value("scanner_selected_ticker", options, 0)
    ...
    with tabs[2]:
        ...
        options = df["ticker"].tolist()
        selected = ensure_selectbox_value("entry_plan_ticker", options, 0)
        picked = df[df["ticker"] == selected].iloc[0]
        hist = picked.get("_df")
        if isinstance(hist, pd.DataFrame) and not hist.empty:
            plan = build_entry_plan(hist, picked["setup_label"], account_size=context["account_size"], risk_pct=context["risk_pct"], lot_rounding=context["lot_rounding"])
    ...
    with tabs[3]:
        ...
        if st.session_state.get("ml_artifact") is None:
            ...
            if wf_summary:
                ...
                st.dataframe(pd.DataFrame([wf_summary]), width="stretch", hide_index=True)
    ...
    with tabs[4]:
        ...
        if not backtest_results:
            ...
            if isinstance(backtest_trades, pd.DataFrame) and not backtest_trades.empty:
                ...
                st.dataframe(backtest_trades[trade_view_cols].head(200), width="stretch", hide_index=True)
                st.download_button(
            ...
            if isinstance(backtest_signals, pd.DataFrame) and not backtest_signals.empty:
                ...
                st.dataframe(backtest_signals[signal_view_cols].head(200), width="stretch", hide_index=True)
                st.download_button(
    ...
    with tabs[5]:
        ...
        if sector_rank.empty:
            ...
            st.dataframe(sector_rank, width="stretch", hide_index=True)
        st.caption("Ranking sektor adalah proxy berbasis score rata-rata kandidat yang berhasil dibaca sektornya.")
...
if "scanner_context" not in st.session_state:
    st.session_state["scanner_context"] = None
if "scanner_selected_ticker" not in st.session_state:
    st.session_state["scanner_selected_ticker"] = ""
if "entry_plan_ticker" not in st.session_state:
    st.session_state["entry_plan_ticker"] = ""
...
with st.sidebar:
    st.markdown("### Status")
    if st.session_state.get("scanner_context"):
        ctx = st.session_state["scanner_context"]
    ...
    st.button("Reset hasil", use_container_width=True, on_click=reset_scanner_state)

    with st.expander("Universe", expanded=True):
        source_mode = st.radio("Sumber ticker", ["Paste ticker", "Upload CSV"], index=0, key="source_mode")
        auto_suffix = st.checkbox("Auto tambah .JK", value=True, key="auto_suffix")
        manual_text = st.text_area("Ticker (pisahkan koma / baris baru)", value="BBRI, BMRI, BBCA, ASII, ADRO", height=130, disabled=(source_mode != "Paste ticker"), key="manual_text")
        csv_file = st.file_uploader("Upload CSV universe", type=["csv"], disabled=(source_mode != "Upload CSV"), key="csv_file")

    with st.expander("Scanner settings", expanded=True):
        benchmark_ticker = st.text_input("Benchmark IHSG", value="^JKSE", key="benchmark_ticker")
        period = st.selectbox("History period", ["1y", "2y", "3y", "5y"], index=1, key="period")
        min_price = st.number_input("Min harga", min_value=0.0, value=500.0, step=50.0, key="min_price")
        min_avg_dollar_vol = st.number_input("Min avg dollar volume 20D", min_value=0.0, value=5_000_000_000.0, step=500_000_000.0, format="%.0f", key="min_avg_dollar_vol")
        use_fundamentals = st.checkbox("Pakai fundamentals jika tersedia", value=False, key="use_fundamentals")
        max_workers = st.slider("Parallel workers", 1, 4, 2, key="max_workers")
        min_final_score = st.slider("Min final score", 0, 100, 65, key="min_final_score")
        min_regime_score = st.slider("Min regime score", 0, 100, 50, key="min_regime_score")
        min_trend_score = st.slider("Min trend score", 0, 100, 45, key="min_trend_score")
        min_flow_score = st.slider("Min flow score", 0, 100, 40, key="min_flow_score")
        min_liquidity_score = st.slider("Min liquidity score", 0, 100, 45, key="min_liquidity_score")
        top_n = st.slider("Top N hasil", 5, 100, 25, key="top_n")

    with st.expander("Entry plan", expanded=False):
        account_size = st.number_input("Account size", min_value=0.0, value=100_000_000.0, step=5_000_000.0, format="%.0f", key="account_size")
        risk_pct = st.slider("Risk per trade (%)", 0.1, 5.0, 1.0, 0.1, key="risk_pct")
        lot_rounding = st.checkbox("Round to lots (100 shares)", value=True, key="lot_rounding")

    with st.expander("Machine Learning", expanded=False):
        ml_enabled = st.checkbox("Enable ML ranking", value=True, key="ml_enabled")
        train_period = st.selectbox("ML training period", ["2y", "3y", "5y"], index=1, key="train_period")
        horizon = st.slider("ML horizon (days)", 5, 20, 10, key="horizon")
        target_pct = st.slider("ML target (%)", 3.0, 20.0, 8.0, 0.5, key="target_pct") / 100.0
        stop_pct = st.slider("ML stop (%)", 1.0, 15.0, 4.0, 0.5, key="stop_pct") / 100.0
        sample_stride = st.slider("Sample stride", 1, 5, 2, key="sample_stride")
        max_samples_total = st.slider("Max training samples", 1000, 20000, 6000, 500, key="max_samples_total")
        max_train_tickers = st.slider("Max training tickers", 10, 200, 60, 5, key="max_train_tickers")
        train_btn = st.button("Train ML model", type="secondary", use_container_width=True)

    with st.expander("Backtest & validation", expanded=False):
        backtest_period = st.selectbox("Backtest period", ["5y", "10y"], index=0, key="backtest_period")
        backtest_use_ml = st.checkbox("Use ML in backtest", value=False, key="backtest_use_ml")
        max_backtest_tickers = st.slider("Max backtest tickers", 5, 100, 20, 5, key="max_backtest_tickers")
        max_hold_days = st.slider("Max hold days", 5, 40, 20, 1, key="max_hold_days")
        entry_window_days = st.slider("Entry window days", 1, 10, 3, 1, key="entry_window_days")
        cooldown_days = st.slider("Cooldown after exit (days)", 0, 10, 2, 1, key="cooldown_days")
        backtest_risk_pct = st.slider("Backtest risk per trade (%)", 0.1, 5.0, 1.0, 0.1, key="backtest_risk_pct")
        wf_splits = st.slider("Walk-forward folds", 2, 8, 5, 1, key="wf_splits")
        backtest_btn = st.button("Run backtest", type="secondary", use_container_width=True)
...
if scan_btn:
    ...
    benchmark_df = build_benchmark(benchmark_ticker, period)
    if not benchmark_df.empty:
        benchmark_score, benchmark_label, benchmark_detail = score_market(benchmark_df)
    else:
    ...
    df = df.sort_values(["final_score", "regime_score", "trend_score", "liquidity_score"], ascending=False, na_position="last")
    ...
    # Keep selections valid after a fresh scan
    tickers_list = df["ticker"].tolist()
    if tickers_list:
        if st.session_state.get("scanner_selected_ticker") not in tickers_list:
            st.session_state["scanner_selected_ticker"] = tickers_list[0]
        if st.session_state.get("entry_plan_ticker") not in tickers_list:
            st.session_state["entry_plan_ticker"] = tickers_list[0]

context = st.session_state.get("scanner_context")
if context:

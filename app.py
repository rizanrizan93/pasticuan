...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.13.0", page_icon="🛡️", layout="wide")
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.13.0")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.13.0 mengambil IDX/XBRL official-first, lalu Yahoo timeseries direct bounded sebagai fallback/cross-check. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "7.13.0-currency-split-aware-valuation-lineage",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.13.0 — Currency/Split-Aware Valuation, Complete Universe, Provenance-Gated Evidence, Statistical Selector, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_audit:
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.13.0 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.13.0 — Currency/Split-Aware Valuation, Direct-vs-Proxy Multibagger, IHSG & Core Swing

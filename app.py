...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.16.2", page_icon="🛡️", layout="wide")
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.16.2")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.16.2 mengambil IDX/XBRL official-first, lalu Yahoo timeseries direct bounded sebagai fallback/cross-check. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    if not incremental_cache_report.empty:
        ...
        st.caption(
            f"Incremental cache v7.16.2: {cache_hits} stage hit, "
    ...
    result.update({
        ...
        "scanner_version": "7.16.2-persisted-date-normalisation-hotfix",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.16.2 — Currency/Split-Aware Valuation, Complete Universe, Provenance-Gated Evidence, Statistical Selector, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_audit:
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.16.2 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.16.2 — Currency/Split-Aware Valuation, Direct-vs-Proxy Multibagger, IHSG & Core Swing

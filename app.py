...
st.set_page_config(page_title="IDX Multibagger & Core Swing Scanner v7.1.2", page_icon="🛡️", layout="wide")
...
REQUIRED_SCANNER_FILES = ("scanner.py", "scanner_focus.py", "scanner_database.py", "research_maintenance.py", "ai_engine.py", "time_cycle.py", "eoff_reconstruction.py", "dashboard_v660.py")
...
st.title("IDX Scanner — Multibagger & Core Swing v7.1.2")
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.1.0 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    try:
        calendar_start = pd.Timestamp.now(tz="Asia/Jakarta").normalize() - pd.Timedelta(days=45)
        calendar_end = calendar_start + pd.Timedelta(days=240)
    ...
    try:
        existing_research_outcomes = ScannerDatabaseBridge().read_research_outcomes(all_tickers)
    except Exception:
    ...
    result.update({
        ...
        "scanner_version": "7.1.2-mixed-datetime-normalization",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.1.0 Multibagger & Core Swing Focus</b><br>
...
with main_existing:
    ...
    with tab_audit:
        ...
        st.caption(
            "Bridge v4.0 membaca cache fundamental, histori laporan, forward quality, kalender IDX, dan outcome memory dari Supabase. "
        ...
        if not database_read_report.empty:
            ...
            misses = int(read_state.isin(["DATABASE_MISS", "DATABASE_EXPIRED", "DATABASE_MODEL_STALE", "DATABASE_EVENT_DUE", "DATABASE_CHECK_DUE", "MIGRATION_REQUIRED_V3", "MIGRATION_REQUIRED_V4"]).sum())
        ...
        if isinstance(research_summary_view, pd.DataFrame) and not research_summary_view.empty:
            st.subheader("EOFF & Silent Accumulation outcome memory")
            st.caption(
            ...
            with st.expander("Detail research outcomes", expanded=False):
                ...
                st.download_button(
                    ...
                    "research_outcomes_v7_1.csv", "text/csv", key="download_research_outcomes_v710",
                )
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.1.0 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.1.0 Multibagger & Core Swing

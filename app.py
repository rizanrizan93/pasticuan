...
APP_VERSION = "9.2.0-parallel-database-backfill"
...
def _run_database_backfill(
    *,
    ...
    # Independent acquisition queues run concurrently. This changes backfill
    # wall-clock time from approximately the sum of snapshot + history + market
    # + news latency to the slowest queue, while preserving official-first
    # ordering inside the history family.
    def snapshot_job():
        live = fetch_resilient_fundamentals(snapshot_targets, cfg) if snapshot_targets else pd.DataFrame()
        if not live.empty and "fundamental_score_eligible" in live.columns:
            live = live.loc[_coerce_bool_series(live["fundamental_score_eligible"], default=False)].copy()
        return _merge_prefer_primary(live, baseline_fundamentals)
    ...
    def history_job():
        ...
        # Twelve Data performs three statement calls per issuer. Keep the deep
        # fallback bounded even when a user selects a 150/160-ticker batch.
        twelve_cap = max(0, min(int(yahoo_limit), 24, len(twelve_targets)))
        twelve_targets = twelve_targets[:twelve_cap]
        ...
        return {
            ...
            "yahoo_targets": yahoo_targets,
            "twelve_targets": twelve_targets,
        }
    ...
    def news_job():
        live = fetch_resilient_news_review(news_targets, lookback_days=45, config=cfg) if news_targets else pd.DataFrame()
        return _merge_prefer_primary(live, cached_news_review)

    parallel_results, stage_timing = run_parallel_backfill_jobs({
        "FUNDAMENTAL_SNAPSHOT": snapshot_job,
        "FUNDAMENTAL_HISTORY": history_job,
        "MARKET_STATUS": market_job,
        "NEWS_REVIEW": news_job,
    }, max_workers=4)

    snapshot_result = parallel_results.get("FUNDAMENTAL_SNAPSHOT", baseline_fundamentals)
    snapshot = snapshot_result if isinstance(snapshot_result, pd.DataFrame) else baseline_fundamentals
    history_result = parallel_results.get("FUNDAMENTAL_HISTORY", {})
    if not isinstance(history_result, Mapping):
        history_result = {}
    history = history_result.get("history", baseline_history)
    idx_report = history_result.get("idx_report", pd.DataFrame())
    yahoo_report = history_result.get("yahoo_report", pd.DataFrame())
    twelve_report = history_result.get("twelve_report", pd.DataFrame())
    yahoo_targets = list(history_result.get("yahoo_targets", []))
    twelve_targets = list(history_result.get("twelve_targets", []))
...
with st.sidebar:
    ...
    with st.expander("Pengaturan lanjutan"):
        ...
        if operation_mode == "Isi Database":
            backfill_batch_size = st.slider("Snapshot & history per proses", 20, 160, 60, 20)
            market_backfill_size = st.slider("Market status per proses", 40, 400, 400, 40)
            news_backfill_size = st.slider("News/disclosure per proses", 20, 200, 40, 20)
        ...
        yahoo_history_limit = st.slider("Deep history fallback per proses", 0, 40, 12, 4)

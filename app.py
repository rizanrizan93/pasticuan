...
def load_csv_universe_from_bytes(csv_bytes: bytes, auto_suffix: bool = True) -> List[str]:
    if not csv_bytes:
        return []
    ...
    raw = []
    try:
        series = df[ticker_col].astype(str).fillna("")
        for val in series.tolist():
            t = str(val).strip().upper()
            if t and t not in {"NAN", "NONE", "NULL"}:
                raw.append(normalize_ticker(t, auto_suffix=auto_suffix))
    except Exception:
        return []

    return dedupe_tickers(raw)
...
with st.sidebar:
    ...
    with st.expander("Universe", expanded=True):
        source_mode = st.radio("Sumber universe", ["Upload CSV", "Paste ticker"], index=0)
        auto_suffix = st.checkbox("Auto tambah .JK", value=True)

        uploaded_csv = None
        if source_mode == "Upload CSV":
            uploaded_csv = st.file_uploader(
                "Upload file universe CSV",
                type=["csv"],
                help="CSV minimal punya kolom ticker/symbol/code/kode. Kalau tidak ada, kolom pertama akan dipakai.",
            )
        ...
        st.caption("CSV upload adalah sumber utama. Jika kosong, pakai ticker manual di sidebar.")

    with st.expander("Scanner settings", expanded=True):
        benchmark_ticker = st.text_input("Benchmark (opsional)", value="^JKSE")
        ...
        min_avg_dollar_vol = st.number_input(
            "Min avg value 20D",
            min_value=0.0,
            value=1_000_000_000.0,
            step=100_000_000.0,
            format="%.0f",
            help="Gunakan nilai transaksi rata-rata 20 hari, bukan dollar volume literal.",
        )
...
if scan_btn:
    if source_mode == "Upload CSV":
        if uploaded_csv is not None:
            tickers = load_csv_universe_from_bytes(uploaded_csv.getvalue(), auto_suffix=auto_suffix)
        else:
            tickers = []
    else:
    ...
    if not tickers:
        st.warning("Daftar ticker kosong. Upload CSV universe atau isi ticker manual.")
        st.stop()

    benchmark_df = build_benchmark(benchmark_ticker, period) if benchmark_ticker.strip() else pd.DataFrame()
    if not benchmark_df.empty:
        ...
        st.warning("Benchmark tidak berhasil dimuat. Scanner tetap jalan, tetapi market filter akan bersifat fallback.")
    ...
    for col in ["tech_score", "fundamental_score", "vol_rvol20", "avg_dollar_vol_20", "final_score"]:
...
if context:
    ...
    st.info("Upload CSV universe atau pilih ticker manual, lalu klik **Scan sekarang**.")
    st.markdown(
        """

...
def load_fundamental_snapshot(symbol: str) -> dict:
    ...
    if ticker is not None:
        ...
        if np.isfinite(revenue):
            if np.isfinite(prev_revenue_growth):
                out["revenue_growth"] = prev_revenue_growth
        ...
        if np.isfinite(net_income):
            if np.isfinite(prev_net_income_growth):
                out["earnings_growth"] = prev_net_income_growth
...
with tab2:
        ...
        if analyze_btn:
            ...
            if stock_df.empty or len(stock_df) < min_history_bars:
                ...
                if pd.notna(fundamental.get("fundamental_score", np.nan)) and pd.notna(stock_res.get("future_fundamental_score", np.nan)):
                    ...
                    detail_cols[2].write(f"**Divergence:** `{divergence:+.2f}`")
    ...
    with sub_forward:
        ...
        if ifs_context and fundamental:
            ...
            cols = st.columns(4)
            cols[0].metric("Current Reality", f'{current_score:.2f}' if pd.notna(current_score) else "n/a", fundamental.get("fundamental_grade", "n/a"))
            cols[1].metric("Predicted Future", f'{future_score:.2f}' if pd.notna(future_score) else "n/a", future_context.get("future_fundamental_grade", "n/a"))
            cols[2].metric("Divergence", format_score_delta(divergence), "Future - Current")
            cols[3].metric("Confidence", f'{future_context.get("future_fundamental_confidence", np.nan):.0f}%' if pd.notna(future_context.get("future_fundamental_confidence", np.nan)) else "n/a")

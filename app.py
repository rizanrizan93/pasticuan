...
def load_fundamental_snapshot(symbol: str) -> dict:
    ...
    if ticker is not None:
        ...
        revenue_growth_quarterly = _statement_growth(income_quarterly, ["Total Revenue", "Operating Revenue", "Revenue"])
        revenue_growth_annual = _statement_growth(income_annual, ["Total Revenue", "Operating Revenue", "Revenue"])
        net_income = first_scalar(income_frames, ["Net Income", "Net Income Common Stockholders", "Net Income Applicable To Common Shares"])
        earnings_growth_quarterly = _statement_growth(income_quarterly, ["Net Income", "Net Income Common Stockholders", "Net Income Applicable To Common Shares"])
        earnings_growth_annual = _statement_growth(income_annual, ["Net Income", "Net Income Common Stockholders", "Net Income Applicable To Common Shares"])
        ...
        if np.isfinite(revenue_growth_quarterly) or np.isfinite(revenue_growth_annual):
            if np.isfinite(revenue_growth_quarterly):
                out["revenue_growth"] = revenue_growth_quarterly
                out["revenue_growth_period"] = "Quarterly YoY"
                out["revenue_growth_source"] = "quarterly_income_stmt"
            else:
                out["revenue_growth"] = revenue_growth_annual
        ...
        if np.isfinite(earnings_growth_quarterly) or np.isfinite(earnings_growth_annual):
            if np.isfinite(earnings_growth_quarterly):
                out["earnings_growth"] = earnings_growth_quarterly
        ...
        if any(np.isfinite(v) for v in [
            ...
            out["data_quality_flag"] = "quarterly" if (out["revenue_growth_source"] == "quarterly_income_stmt" or out["earnings_growth_source"] == "quarterly_income_stmt") else ("annual" if (out["revenue_growth_source"] == "income_stmt" or out["earnings_growth_source"] == "income_stmt") else ("fallback" if info else "statement-fallback"))
...
def build_entry_plan(
    ...
    The goal is to keep the plan actionable:
    - entry zone is a range, not a single noisy price
    - stop loss respects structure first, ATR second
    - targets are anchored to recent resistance and ATR extension
    """
    d = stock_res.get("df")
    ...
    try:
        ...
        recent_breakout_high = float(d["High"].tail(5).max())
        ...
        if not np.isfinite(entry_price) or entry_price <= 0:
            candidate_levels = [close, ema20, recent_swing_low + atr_v * 0.25]
            if np.isfinite(ema50):
                candidate_levels.append(ema50)
            candidate_levels = [v for v in candidate_levels if np.isfinite(v) and v > 0]
            entry_price = float(np.median(candidate_levels)) if candidate_levels else float(close)

        # Build a zone around the setup instead of a single point.
        if np.isfinite(stock_res.get("entry_zone_low", np.nan)) and np.isfinite(stock_res.get("entry_zone_high", np.nan)):
            entry_zone_low = float(stock_res["entry_zone_low"])
            entry_zone_high = float(stock_res["entry_zone_high"])
        else:
            lower_anchor = min(entry_price, recent_support, close - atr_v * entry_buffer_atr)
            upper_anchor = max(entry_price, close + atr_v * max(0.10, entry_buffer_atr * 0.50))
            entry_zone_low = max(0.0, float(lower_anchor - atr_v * 0.10))
            entry_zone_high = max(entry_zone_low, float(upper_anchor + atr_v * 0.10))

        if not np.isfinite(stop_price) or stop_price <= 0:
            structural_stop = min(
                recent_swing_low - atr_v * 0.15,
                close - atr_v * stop_loss_atr,
                recent_support - atr_v * 0.35,
            )
            stop_price = max(min(structural_stop, entry_price - atr_v * 0.75), 0.0)

        # Avoid too-tight stops that are easily hit by noise.
        if stop_price >= entry_price:
            stop_price = max(entry_price - atr_v * 0.90, 0.0)

        entry_trigger = str(stock_res.get("entry_trigger", "Pullback_to_support" if entry_price <= close else "Breakout_confirmation"))

        risk_per_share = float(max(entry_price - stop_price, 1e-9))
        target_1 = float(max(recent_swing_high, recent_breakout_high, entry_price + atr_v * target_1_atr))
        ...
        if entry_trigger == "Breakout_confirmation":
            plan_reason = "Breakout-confirmation plan with ATR-anchored stop"
        elif entry_trigger == "Pullback_to_support":
            plan_reason = "Pullback-to-support plan with structural stop"

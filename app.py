...
st.set_page_config(page_title="IDX Multibagger & Core Swing Scanner v7.0.0", page_icon="🛡️", layout="wide")
...
REQUIRED_SCANNER_FILES = ("scanner.py", "scanner_focus.py", "scanner_database.py", "ai_engine.py", "time_cycle.py", "eoff_reconstruction.py", "dashboard_v660.py")
...
def database_first_fundamentals(
    ...
    refresh_names = bridge.refresh_tickers(read_audit, names)
    ...
    audit_frames = [frame for frame in (read_audit, pd.DataFrame(provider_audit)) if frame is not None and not frame.empty]
...
def enrich_fundamental_shortlist(
    ...
    refresh_names = tuple(bridge.refresh_tickers(database_audit, tickers))

    idx_history, idx_report = cached_idx_fundamental_history(
        refresh_names,
        max_tickers=min(int(config.idx_fundamental_top_n), len(refresh_names)),
    ...
    yahoo_names = select_yahoo_fundamental_tickers(
        ...
        max_tickers=min(int(config.fundamental_history_top_n), len(refresh_names)),
    ...
    yahoo_history, yahoo_report = cached_yahoo_fundamental_history(
        tuple(yahoo_names), max_tickers=min(int(config.fundamental_history_top_n), len(yahoo_names)),
    ...
    reports = [
        frame for frame in (database_audit, yahoo_report, idx_report, twelve_report)
...
def database_first_forward_quality(
    ...
    refresh_names = tuple(bridge.refresh_tickers(database_audit, names))
    ...
    reports = [frame for frame in (database_audit, provider_report) if frame is not None and not frame.empty]
...
def run_single_ticker_deep_dive(
    ...
    *,
    ...
    fundamentals, database_fundamental_audit = database_first_fundamentals((normalized,))
...
st.title("IDX Scanner — Multibagger & Core Swing v7.0.0")
...
with st.sidebar:
    ...
    with st.expander("Time-Cycle Intelligence — Swing/Core & Multibagger", expanded=True):
        ...
        st.caption(
            "Implementasi clean-room ini tidak mengklaim formula proprietary asli. Mulai v7.0.0, astro tidak memiliki bobot produksi. EOFF memengaruhi timing hanya setelah cluster waktu, arah harga, dan chronological forward evidence lolos."
        )
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.0.0 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
with st.expander("Konfirmasi opsional: Broker Summary"):
    st.caption(
        ...
        "tetapi tidak menggantikan fundamental, struktur harga, atau verifikasi Stockbit."
    )
...
scan_signature = upload_fingerprint(
    ...
    broksum_uploaded, ai_memory_uploaded,
...
if run_portfolio and portfolio_uploaded is not None:
    ...
    fundamentals, database_fundamental_audit = database_first_fundamentals(portfolio_tickers) if use_fundamentals else (pd.DataFrame(), pd.DataFrame())
    fundamental_history = pd.DataFrame()
...
if run_scan and uploaded is not None:
    ...
    if use_fundamentals and (not signals.empty or portfolio_tickers):
        ...
        fundamentals, database_fundamental_audit = database_first_fundamentals(tuple(top_names))
    ...
    context_names = list(dict.fromkeys(visible_context_names + portfolio_tickers))
    if context_names:
    ...
    for frame in (database_fundamental_audit, fundamental_history_report, automatic_forward_report):
        if frame is None or frame.empty:
            continue
        local = frame.copy()
        if "database_read_state" in local.columns:
            local = local[local["database_read_state"].fillna("").astype(str).str.len().gt(0)]
        elif "provider" in local.columns:
            local = local[local["provider"].fillna("").astype(str).str.contains("SUPABASE_DATABASE_FIRST", na=False)]
        else:
            local = pd.DataFrame()
        if not local.empty:
    ...
    result.update({
        ...
        "scanner_version": "7.0.0-database-first-incremental",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.0.0 Multibagger & Core Swing Focus</b><br>
...
with main_existing:
    ...
    with tab_orders:
        ...
        st.subheader("Core Swing Conviction Ranking")
        st.caption(
    ...
    with tab_multibagger:
        ...
        st.caption(
            "Multibagger Quality menilai growth, profitability, OCF/FCF, solvabilitas, valuasi, likuiditas, proyek, manajemen, ownership, dan future fundamental. "
        ...
        render_focus_download("Download Multibagger Capital Plan", multibagger, "multibagger_capital_plan.csv")
    ...
    with tab_audit:
        ...
        st.caption(
            "Bridge v3.0 membaca cache fundamental, histori laporan, dan forward quality dari Supabase sebelum memanggil provider. "
            "Hanya ticker DATABASE_MISS, DATABASE_STALE_USABLE, atau DATABASE_EXPIRED yang masuk antrean refresh. "
            "Semua kegagalan tetap fail-soft."
        )
        db_col1, db_col2 = st.columns([1, 3])
        with db_col1:
            if st.button("Test koneksi database", key="database_health_check_v700"):
                st.session_state["database_health_report_v700"] = ScannerDatabaseBridge().health_check()
        with db_col2:
            health_report = st.session_state.get("database_health_report_v700")
            if isinstance(health_report, dict) and health_report:
                ...
                if str(health_report.get("state")) == "MIGRATION_REQUIRED_V3":
                    st.error("Jalankan database/migration_v3_database_first.sql di Supabase SQL Editor, lalu reboot aplikasi.")
        if not database_read_report.empty:
            ...
            misses = int(read_state.isin(["DATABASE_MISS", "DATABASE_EXPIRED", "MIGRATION_REQUIRED_V3"]).sum())
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.0.0 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.0.0 Multibagger & Core Swing

            1. **Dua fokus produksi:** scanner hanya meranking Multibagger dan Core Swing harian.
            2. **Core Swing:** Pullback Continuation, Breakout Retest, Reversal Accumulation, dan Unicorn/ICT harian tetap independen dengan entry, trigger, stop, TP, RR, dan expiry masing-masing.
            3. **Multibagger:** kualitas bisnis, growth, profitability, OCF/FCF, solvabilitas, valuasi, proyek, manajemen, ownership, dan future fundamental menjadi fondasi utama.
            4. **Timing:** EOFF dan Time-Cycle hanya menjadi overlay terjaga untuk Best Buy Date dan jendela akumulasi; tidak menggantikan struktur harga atau fundamental.
            5. **Validitas data:** candle EOD harus final, OHLCV tidak boleh unavailable atau terlalu lama, dan suspensi/FCA tetap memblokir.
            6. **Kandidat tetap lengkap:** satu ticker boleh muncul pada beberapa setup Core Swing; Top-20 memakai satu slot per ticker agar ranking tidak didominasi duplikasi.
            7. **Dua kebijakan eksekusi:** `SIGNAL_FIRST` untuk radar dan `ACCOUNT_GUARDED` untuk sizing, cash, slot posisi, serta portfolio heat.
            8. **Verifikasi harga:** IDX EOD → Google Finance → iTick opsional → Twelve Data opsional, dengan cache bukti provider yang masih sama-session.
            ...
            10. **Submit manual:** semua order tetap direvalidasi dan dikirim manual di Stockbit.

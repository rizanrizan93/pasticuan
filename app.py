...
st.set_page_config(page_title="IDX Multibagger & Core Swing Scanner v6.7.1", page_icon="🛡️", layout="wide")
...
st.title("IDX Scanner — Multibagger & Core Swing v6.7.1")
...
with st.sidebar:
    ...
    with st.expander("Time-Cycle Intelligence — Swing/Core & Multibagger", expanded=True):
        ...
        st.caption(
            ...
            "dan timing Multibagger setelah confluence serta historical evidence lolos."
        )
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v6.7.1 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v6.7.1 Multibagger & Core Swing Focus</b><br>
...
with main_existing:
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v6.7.1 Multibagger & Core Swing

...
st.set_page_config(page_title="IDX Super Scanner Resilient v4.2.5", page_icon="🎯", layout="wide")
...
@st.cache_data(ttl=900, show_spinner=False)
def cached_twelve_data_eod(tickers: tuple[str, ...], api_key: str, outputsize: int = 30):
    return fetch_twelve_data_eod(tickers, api_key=api_key, outputsize=outputsize, max_tickers=len(tickers))
...
st.title("IDX Super Scanner — Multi-Source Evidence v4.2.5")
st.caption("Core setup, Sniper Entry, BSJP/BPJS, Multibagger Radar, ARA Hunter, dan portfolio Stockbit")
...
with st.sidebar:
    ...
    st.subheader("Validasi harga independen")
    require_independent_price = st.checkbox(
        "Wajibkan sumber harga kedua untuk BUY_LIMIT",
        value=True,
        help="Tanpa sumber kedua, kandidat terbaik berhenti di READY_FOR_PRICE_VERIFY—bukan hilang dari hasil.",
    )
    if real_money_mode and not require_independent_price:
        st.warning("Single-provider mode aktif: status dapat READY tanpa cross-validation harga independen.")
    use_twelve_data = st.checkbox(
        "Gunakan Twelve Data XIDX EOD",
        value=False,
        help="Opsional dan membutuhkan API key dengan plan yang mencakup XIDX. Hanya shortlist kandidat yang diminta.",
    )
    twelve_data_api_key = st.text_input(
        "Twelve Data API key",
        value=os.environ.get("TWELVE_DATA_API_KEY", ""),
        type="password",
        disabled=not use_twelve_data,
    )
    twelve_data_candidates = st.slider(
        "Maksimum kandidat Twelve Data", 3, 20, 10, 1,
        disabled=not use_twelve_data,
    )
...
broker_sample_csv = (
    ...
    b"ADRO,2026-07-14,XX,1500000000,500000000\n"
)
price_verification_sample_csv = (
    b"ticker,asof,last_price,source\n"
    b"ADRO,2026-07-14 16:20:00,2150,STOCKBIT_MANUAL_QUOTE\n"
)
...
b1_upload, b2_upload = st.columns([3, 1])
with b1_upload:
    broker_uploaded = st.file_uploader(
        "Upload broker summary Stockbit (opsional)",
        type=["csv", "txt"],
        help="Memvalidasi accumulation/distribution proxy 10 hari. Kode broker bukan identitas beneficial owner.",
...
with b2_upload:
    ...
    st.write("")
    st.download_button(
        "Unduh template broksum", broker_sample_csv, "broker_summary_template.csv", "text/csv", width="stretch"
    )

v1_upload, v2_upload = st.columns([3, 1])
with v1_upload:
    verification_source_type = st.selectbox(
        "Jenis sumber file harga independen",
        ["Stockbit quote manual", "IDX official Stock Summary", "Provider eksternal lain"],
        help="Untuk IDX, unduh Ringkasan Saham terbaru lalu upload CSV/XLSX. Untuk Stockbit, cukup isi ticker shortlist.",
    )
    independent_price_uploaded = st.file_uploader(
        "Upload verifikasi harga independen (opsional)",
        type=["csv", "txt", "xlsx", "xlsm"],
        help="Minimal: ticker/kode saham, date/asof/tanggal perdagangan, dan close/last_price/penutupan.",
        key="independent_price_upload",
    )
    st.markdown("[Buka IDX — Ringkasan Saham](https://www.idx.co.id/id/data-pasar/ringkasan-perdagangan/ringkasan-saham)")
with v2_upload:
    st.write("")
    st.write("")
    st.download_button(
        "Unduh template harga", price_verification_sample_csv,
        "price_verification_template.csv", "text/csv", width="stretch",
    )
...
st.info(
    "Pilih Jalankan scanner untuk scan universe. Pilih Analisis portfolio saja untuk mengevaluasi posisi Stockbit "
    "tanpa upload ticker universe dan tanpa menjalankan backtest seluruh pasar."
)

b1, b2 = st.columns(2)
run_scan = b1.button("Jalankan scanner", type="primary", width="stretch", disabled=uploaded is None)
...
cfg = ScanConfig().replace(
    ...
    require_independent_price_verification=bool(require_independent_price),
...
if run_scan and uploaded is not None:
    ...
    independent_upload_data = pd.DataFrame()
    if broker_uploaded is not None:
        try:
            broksum = parse_broker_summary_csv(broker_uploaded)
        except Exception as exc:
            st.error(f"Broker-summary CSV tidak dapat dibaca: {exc}")
            st.stop()
    if independent_price_uploaded is not None:
        source_defaults = {
            "Stockbit quote manual": "STOCKBIT_MANUAL_QUOTE",
            "IDX official Stock Summary": "IDX_OFFICIAL_EOD_UPLOAD",
            "Provider eksternal lain": "MANUAL_EXTERNAL_UPLOAD",
        }
        try:
            independent_upload_data = parse_independent_price_file(
                independent_price_uploaded,
                filename=getattr(independent_price_uploaded, "name", None),
                default_source=source_defaults[verification_source_type],
            )
        except Exception as exc:
            st.error(f"File verifikasi harga independen tidak dapat dibaca: {exc}")
            st.stop()
    ...
    twelve_data = pd.DataFrame()
    twelve_data_report = pd.DataFrame()
    if use_twelve_data and quote_candidates:
        if twelve_data_api_key.strip():
            td_names = tuple(quote_candidates[: int(twelve_data_candidates)])
            progress.progress(84, text=f"Cross-check Twelve Data XIDX untuk {len(td_names)} kandidat…")
            twelve_data, twelve_data_report = cached_twelve_data_eod(td_names, twelve_data_api_key.strip(), 30)
        else:
            twelve_data_report = pd.DataFrame([{
                "ticker": "—", "status": "SKIPPED", "bars": 0,
                "error": "Twelve Data diaktifkan tetapi API key kosong",
            }])
    independent_price_data = pd.concat(
        [frame for frame in (independent_upload_data, twelve_data) if frame is not None and not frame.empty],
        ignore_index=True,
    ) if (not independent_upload_data.empty or not twelve_data.empty) else pd.DataFrame()
    ...
    result.update({
        ...
        "twelve_data_report": twelve_data_report,
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Dua jalur independen</b><br>
          1) Upload universe ticker lalu jalankan scanner.<br>
          2) Upload portfolio Stockbit lalu pilih <b>Analisis portfolio saja</b>—tanpa universe ticker.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with tab_orders:
    if signals.empty:
        ...
        if not price_verify.empty:
            ...
            st.warning(
                "Kandidat ini sudah lolos gate utama tetapi belum memiliki sumber harga independen. "
                "Upload IDX Stock Summary/quote Stockbit atau aktifkan Twelve Data, lalu scan ulang."
            )
...
with tab_audit:
    ...
    broker_audit = result.get("broker_summary", pd.DataFrame())
    independent_audit = result.get("price_validation", pd.DataFrame())
    twelve_audit = result.get("twelve_data_report", pd.DataFrame())
    ...
    a6.metric("Broksum ticker", len(broker_audit))
    a7.metric("Harga independen", independent_ok)
    ...
    st.dataframe(
        pd.DataFrame(
            [
                ...
                ["Twelve Data XIDX EOD", "Cross-check recent close dan return path kandidat", "Optional API key / eligible plan"],
                ["IDX Stock Summary upload", "Cross-check harga EOD resmi", "Optional manual upload"],
                ["Stockbit quote upload", "Cross-check harga shortlist sebelum order", "Optional manual upload"],
                ["Stockbit broker-summary CSV", "Proxy akumulasi/distribusi kode broker", "Optional manual"],
            ],
    ...
    if independent_ok:
        ...
        st.warning(
            "Belum ada harga independen yang terverifikasi. Kandidat yang selain itu sudah lengkap akan berhenti di "
            "READY_FOR_PRICE_VERIFY sampai file IDX/Stockbit atau Twelve Data tersedia."
        )
    if not independent_audit.empty:
        st.dataframe(independent_audit, hide_index=True, width="stretch")
    if not twelve_audit.empty and bool(twelve_audit["status"].ne("OK").any()):
        with st.expander("Audit Twelve Data"):
            st.dataframe(twelve_audit, hide_index=True, width="stretch")
...
with tab_method:
    st.markdown(
        ...
        6. **Independent price:** IDX Stock Summary, quote Stockbit manual, atau Twelve Data XIDX memvalidasi harga Yahoo. Tanpa sumber kedua, kandidat terbaik berhenti di `READY_FOR_PRICE_VERIFY`.
        ...
        `READY_FOR_PRICE_VERIFY` berarti seluruh gate utama hampir selesai dan hanya menunggu sumber harga kedua. `PENDING_DATA` berarti ada gate data/confidence lain yang belum lulus. `EXECUTION_READY` hanya menghasilkan `BUY_LIMIT`; scanner tidak mengirim order otomatis ke Stockbit. `ARA_HUNTER_STRICT` bukan izin direct execution.
        """
    )

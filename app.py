...
st.set_page_config(page_title="IDX Super Scanner Price Structure v5.2.0", page_icon="🛡️", layout="wide")
...
REQUIRED_SCANNER_FILES = ("scanner.py", "scanner_specialty.py")
...
def sort_signals(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    out["status_rank"] = out["status"].map({
        "EXECUTION_READY": 0,
        "ENTRY_PLAN_READY": 1,
        "READY_NOT_SELECTED": 2,
        "READY_FOR_PRICE_VERIFY": 3,
        "PENDING_CLOSE": 4,
        "PENDING_DATA": 5,
        "WATCHLIST_ENTRY": 6,
        "BLOCKED_CONTEXT": 7,
...
def execution_funnel_summary(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        ...
        {"Tahap": "AUTOPILOT_VERIFIED", "Ticker unik": unique_count(signals.get("autopilot_verified", pd.Series(False, index=signals.index)).fillna(False).astype(bool))},
...
st.title("IDX Super Scanner — Price Structure Targets v5.2.0")
st.caption("Semua setup tetap terlihat. Tiket Stockbit hanya diterbitkan ketika trigger, harga independen, validasi, likuiditas, risiko akun, regime, dan mekanik order telah lolos.")

with st.sidebar:
    st.header("Autopilot Guard")
    st.info("Account-risk gate AKTIF untuk tiket order. Setup yang belum lolos tetap muncul sebagai plan/watchlist.")
    real_money_mode = True
    with st.expander("Akun dan risiko — dipakai Autopilot", expanded=True):
    ...
    enable_intraday_specialty = st.checkbox(
        ...
        value=True,
        help="Shortlist dibagi khusus untuk BPJS, BSJP, ARA, dan core. Naikkan hingga 300 bila koneksi/provider memadai.",
    )
    intraday_shortlist_n = st.slider(
        "Maksimum shortlist intraday", 20, 300, 120, 10,
        disabled=not enable_intraday_specialty,
    )
    st.divider()
    st.caption("Semua setup tetap ditampilkan, tetapi hanya AUTOPILOT_VERIFIED membawa lot dan tiket order.")
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v5.2.0 Price Structure + Autopilot Guard</b><br>
...
m5.metric("Autopilot verified", int(signals.loc[signals.get("autopilot_verified", pd.Series(False, index=signals.index)).fillna(False).astype(bool), "ticker"].nunique()) if not signals.empty else 0)
m6, m7, m8, m9, m10 = st.columns(5)
m6.metric("Account gate", "ON")
...
with tab_orders:
    if signals.empty:
        ...
        st.subheader("Order Builder — AUTOPILOT_VERIFIED")
        st.caption("Tabel ini saja yang boleh disalin ke Stockbit. BUY_LIT memakai harga trigger terpisah dari limit; buy-limit biasa di atas pasar bukan pengganti trigger.")
        if execution.empty:
            st.info("Tidak ada order yang lolos seluruh Autopilot Guard saat ini. Tidak melakukan transaksi adalah hasil yang valid.")
        else:
...
with tab_sniper:
    ...
    st.caption("Sweep–BOS–FVG dinilai independen dari status core. Volume retracement dan volume reclaim dibaca berbeda; SL/RR/flow menjadi warning, bukan account blocker.")
...
with tab_fast:
    ...
    with fast_bsjp:
        ...
        st.warning("BSJP_READY memerlukan data intraday fresh, harga di atas VWAP, dan kombinasi valid momentum harian serta late-session pada 14:30–15:49 WIB. Regime, likuiditas, dan stop lebar ditampilkan sebagai warning—bukan blocker.")
    ...
    with fast_bpjs:
        ...
        st.warning("BPJS_READY hanya dapat muncul pada 09:20–10:45 WIB setelah data fresh mengonfirmasi harga di atas VWAP dan breakout opening range 15 menit. Opening volume, gap, regime, likuiditas, dan stop menjadi warning. Posisi tetap wajib ditutup sebelum market close.")
...
with tab_audit:
    ...
    if independent_ok:
        ...
        st.warning(
            "Belum ada harga independen yang terverifikasi. Setup tetap terlihat, tetapi Autopilot Guard "
            "tidak akan menerbitkan tiket Stockbit."
        )
...
with tab_method:
    st.markdown(
        """
        ### Hirarki sinyal v5.2.0 Price Structure + Autopilot Guard
        ...
        3. **Kandidat tetap lengkap:** satu ticker boleh muncul pada beberapa setup dan seluruhnya tetap ditampilkan.
        4. **Autopilot fail-closed:** tiket order memerlukan harga independen satu sesi, trigger, validasi, fundamental minimum, likuiditas, regime, completeness, confidence, dan account-risk gate.
        ...
        7. **Verifikasi harga:** IDX EOD tanggal referensi → Google Finance → iTick opsional → Twelve Data opsional, dengan cache bukti provider yang masih sama-session.
        ...
        9. **Sizing:** `EXECUTION_READY` membawa lot berbasis risiko akun. `BUY_LIT_USER_SIZE` memakai trigger terpisah; `BUY_LIMIT_USER_SIZE` hanya untuk limit yang tidak marketable di atas harga referensi.
        10. **Chart state:** pilihan ticker chart memakai fragment sehingga tidak mereset dashboard atau kembali ke tab pertama.

        `EXECUTION_READY` berarti `AUTOPILOT_VERIFIED`; tetap bukan jaminan profit. Jika tidak ada baris, keputusan sistem adalah tidak bertransaksi.
        """
    )

...
APP_VERSION = "9.7.2-single-scan-database-first"
...
def _database_job_error_message(error: object, *, action: str = "membaca") -> str:
    ...
    if any(token in lowered for token in ("transport failure", "readtimeout", "read timed out", "connecttimeout", "connectionerror", "connection aborted", "temporarily unavailable")):
        return (
            ...
            "Scanner v9.7.2 mencoba ulang GET/idempotent write otomatis dengan backoff dan membatch checkpoint ticker. "
...
st.title("IDX Super Scanner v9.7.2 — Macro-First")
st.caption(
...
if run_scan:
    ...
    if ticker_file is not None:
        ...
        safe_config = {
            ...
            # Single-scan database maintenance: each chunk may repair every
            # missing/stale ticker in that chunk, then persist only the delta.
            "daily_fundamental_refresh_limit": int(job_chunk_size),
            "daily_snapshot_refresh_limit": int(job_chunk_size),
            "daily_news_refresh_limit": int(job_chunk_size),
            "daily_official_fundamental_refresh_limit": int(job_chunk_size),
        }
        try:
...
if active_job and str(active_job.get("job_type", "")).upper() == job_type:
    ...
    if status in {"COMPLETE_WITH_FAILURES", "FAILED"}:
        ...
        if not failed_rows.empty:
            st.error(
                "Job lama selesai dengan ticker gagal. v9.7.2 mengulang ticker tersebut di job yang sama, "

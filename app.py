...
st.set_page_config(page_title="IDX Super Scanner Resilient v4.2.4", page_icon="🎯", layout="wide")
...
st.title("IDX Super Scanner — Resilient Evidence v4.2.4")
...
with tab_audit:
    ...
    st.warning(
        "Harga dan quote otomatis masih berasal dari satu keluarga provider (Yahoo). "
        "Sebelum menaruh order, cocokkan last price, bid–ask, status FCA/suspensi, dan antrean di Stockbit."
    )
...
with tab_method:
    st.markdown(
        ...
        9. **Specialty screens:** core setup ditampilkan terpisah; Sniper memakai strict ICT confluence; BSJP/BPJS wajib candle selesai, sesi hari ini, freshness, regime, dan session window; Multibagger tidak memberi poin pada solvabilitas yang hilang; ARA Hunter tetap speculative-review-only.

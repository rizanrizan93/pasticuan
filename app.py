...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.6.4", page_icon="🛡️", layout="wide")
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.6.4")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.6.4 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "7.6.4-evidence-acquisition",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.6.4 — Narrative Intelligence, Coverage-aware Multibagger, Statistical Selector, Execution AI, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_multibagger:
        ...
        with st.expander(
            ...
            st.caption(
                ...
                "Silent Accumulation tetap menjadi ranking key terpisah agar tidak dihitung berulang."
            )
    ...
    with tab_audit:
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.6.4 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.6.4 — Narrative Intelligence, Coverage-aware Multibagger, IHSG & Core Swing

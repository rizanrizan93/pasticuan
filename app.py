...
st.set_page_config(page_title="IDX Super Scanner Resilient v4.2.2", page_icon="🎯", layout="wide")
...
def cached_intraday_data(tickers: tuple[str, ...], period: str = "5d", interval: str = "15m"):
...
st.title("IDX Super Scanner — Resilient Evidence v4.2.2")
...
with st.sidebar:
    ...
    max_positions = st.slider("Maksimum posisi bersamaan", 1, 20, 8)
    max_position_pct = st.slider("Maksimum modal per posisi", 5, 60, 20, 5) / 100
    ...
    multibagger_full_universe = st.checkbox(
        ...
        help="Untuk universe 300 ticker proses lebih lama, tetapi Multibagger Radar tidak bias hanya ke kandidat teknikal harian.",
    )
    fundamental_n = st.slider("Jumlah kandidat fundamental", 10, 300, 120, disabled=not use_fundamentals or multibagger_full_universe)
    enable_intraday_specialty = st.checkbox(
        "Ambil intraday 15m untuk BSJP/BPJS & ARA Hunter",
        value=True,
    ...
    st.divider()
    st.caption("Pastikan Equity akun dan Cash on hand sesuai snapshot Stockbit terbaru.")
...
if run_scan and uploaded is not None:
    ...
    if enable_intraday_specialty and specialty_shortlist:
        progress.progress(93, text=f"Mengunduh intraday 15m untuk {len(specialty_shortlist)} kandidat specialty…")
        intraday_histories, intraday_report = cached_intraday_data(tuple(specialty_shortlist), "5d", "15m")
...
with tab_orders:
    if signals.empty:
        ...
        if execution.empty:
            st.info("Tidak ada direct-execution order dari core setup saat ini.")
        else:
...
with tab_fast:
    ...
    with fast_bsjp:
        ...
        st.warning("Strategi overnight berisiko gap. BSJP_READY hanya dapat muncul pada 14:30–15:49 WIB dengan data intraday 15 menit; di luar waktu itu hasil bersifat watchlist.")
    ...
    with fast_bpjs:
        ...
        st.warning("BPJS adalah day trade. BPJS_READY hanya dapat muncul pada 09:15–10:45 WIB setelah ORB, VWAP, opening volume, dan batas gap lulus. Posisi wajib ditutup sebelum market close.")
...
with tab_method:
    st.markdown(
        ...
        6. **Weighted decision:** teknikal 35%, risk mechanics 20%, status pasar 10%, berita 8%, fundamental 10%, OOS 7%, quote 5%, dan breadth 5%.
        7. **Critical blockers:** suspensi/FCA, corporate action yang belum disesuaikan, berita negatif material, edge OOS negatif, fundamental distress, konflik quote, candle belum final, dan portfolio risk penuh.
        8. **Portfolio engine:** setiap posisi dinilai menjadi HOLD, HOLD_NO_AVG, HOLD_TIGHT_STOP, TAKE_PROFIT_PARTIAL, REDUCE, CUT_LOSS, DO_NOT_AVG_DOWN, atau AVG_DOWN_ALLOWED.
        9. **Specialty screens:** core setup ditampilkan terpisah; Sniper memakai strict ICT confluence; BSJP/BPJS wajib intraday dan session window; Multibagger menggabungkan growth-quality-momentum; ARA Hunter tetap speculative-review-only.

        `PENDING_DATA` berarti setup teknikal tetap tersimpan tetapi total confidence belum mencapai threshold order. `EXECUTION_READY` hanya menghasilkan `BUY_LIMIT`; scanner tidak mengirim order otomatis ke Stockbit. `ARA_HUNTER_STRICT` bukan izin direct execution.
        """
    )

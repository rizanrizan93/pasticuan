...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.16.4", page_icon="🛡️", layout="wide")
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.16.4")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.16.4 mengambil IDX/XBRL official-first, lalu Yahoo timeseries direct bounded sebagai fallback/cross-check. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    if not incremental_cache_report.empty:
        ...
        st.caption(
            f"Incremental cache v7.16.4: {cache_hits} stage hit, "
    ...
    result.update({
        ...
        "scanner_version": "7.16.4-database-readback-finite-hotfix",
    })
    ...
    database_bridge = ScannerDatabaseBridge()
    try:
        result["database_sync_report"] = database_bridge.persist_scan_result(result)
    except Exception as exc:
        fail_row = database_bridge.status_row(
            "DATABASE_FAIL_SOFT",
        ...
        result["database_sync_report"] = pd.DataFrame([fail_row])
    scan_profiler.mark(
        "Database sync",
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.16.4 — Currency/Split-Aware Valuation, Complete Universe, Provenance-Gated Evidence, Statistical Selector, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_audit:
        ...
        with db_col1:
            if st.button("Test koneksi database", key="database_health_check_v7164"):
                st.session_state["database_health_report_v7164"] = ScannerDatabaseBridge().health_check()
        with db_col2:
            health_report = st.session_state.get("database_health_report_v7164")
        ...
        with verify_col1:
            if st.button("Verifikasi upload scan terakhir", key="database_readback_verify_v7164"):
                readback_bridge = ScannerDatabaseBridge()
                try:
                    st.session_state["database_readback_report_v7164"] = (
                        readback_bridge.verify_persisted_scan(result)
                    )
                except Exception as exc:
                    fail_row = readback_bridge.status_row(
                        "READBACK_FAIL_SOFT",
                    ...
                    st.session_state["database_readback_report_v7164"] = pd.DataFrame([fail_row])
        with verify_col2:
            readback_report = st.session_state.get("database_readback_report_v7164", pd.DataFrame())
            if isinstance(readback_report, pd.DataFrame) and not readback_report.empty:
                ...
                st.download_button(
                    ...
                    "text/csv",
                    key="download_database_readback_v7164",
                )
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.16.4 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.16.4 — Currency/Split-Aware Valuation, Direct-vs-Proxy Multibagger, IHSG & Core Swing

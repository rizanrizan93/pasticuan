...
def detect_reversal_signals(d: pd.DataFrame) -> pd.DataFrame:
    ...
    x["Swing_Low"] = x["Swing_Low"].fillna(False)
    x["Bullish_FVG"] = x["Bullish_FVG"].fillna(False)
    ...
    for i in range(4, len(x)):
        ...
        if bool(x["Swing_Low"].iloc[i - 1]) and x["Low"].iloc[i - 1] < x["Low"].iloc[i - 3]:
            if x["Close"].iloc[i - 2] > x["Open"].iloc[i - 2]:
                breaker_top = x["High"].iloc[i - 2]
                breaker_bottom = x["Low"].iloc[i - 2]
...
def score_stock_smc(
    ...
    unicorn_present = bool(d["Unicorn_Setup"].tail(8).any())

    smc_points = 0
    smc_points += 4 * int(d["Bullish_FVG"].tail(5).any())
    smc_points += 4 * int(d["Bullish_OB"].tail(5).any())
    smc_points += 4 * int(unicorn_present)
    # removed: volume-based scoring
    smc_points += 2 * int(len(d) >= 5 and float(last["Close"]) > float(d["Close"].iloc[-5])) if len(d) >= 5 else 1
    ...
    smart_money_score += 24.0 * float(d["Bullish_FVG"].tail(8).any())
    smart_money_score += 24.0 * float(d["Bullish_OB"].tail(8).any())
    smart_money_score += 20.0 * float(unicorn_present)
    smart_money_score += 12.0 * float(last["Close"] > last["EMA20"])
    smart_money_score += 10.0 * float(last["EMA20"] > last["EMA50"])
    smart_money_score += 8.0 * float(last["EMA50"] > last["EMA200"])
    smart_money_score += 10.0 * float(last["RSI14"] >= 50)
    smart_money_score += 6.0 * float(last["MACD_HIST"] > 0)
    smart_money_score += 6.0 * float(last["STOCH_K"] > last["STOCH_D"])
    ...
    smc_confirmed = d["Bullish_FVG"].tail(8).any() or d["Bullish_OB"].tail(8).any() or unicorn_present
    ...
    return {
        ...
        "unicorn_present": unicorn_present,
                    ...
                    watch_rows.append(
                        {
                            ...
                            "Unicorn": "🦄 YES" if r.get("unicorn_present", False) else "NO",
    ...
    with sub_detail:
        if analyze_btn:
            ...
            if stock_df.empty or len(stock_df) < min_history_bars:
                ...
                detail_cols[1].write(f"**Unicorn Setup:** `{stock_res.get('unicorn_present', False)}`")

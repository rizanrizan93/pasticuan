...
st.caption(
    "Scanner probabilistik untuk ranking entry, stoploss, takeprofit, dan horizon per emiten."
)
...
if source == "Paste tickers":
    ...
    st.sidebar.caption(
        "CSV dapat berisi daftar ticker saja, atau data OHLCV lengkap dengan kolom symbol/ticker, date, open, high, low, close, volume."
    )
...
# Session state defaults so the dashboard does not reset when widgets rerun
...
def download_one(symbol: str, period: str) -> Tuple[str, pd.DataFrame, str]:
    """Download OHLCV with multiple attempts.

    yfinance can fail on some environments. We try both Ticker.history and download
    with a couple of symbol variants and keep the best non-empty result.
    """
    candidates = [symbol]
...
def build_plan(df: pd.DataFrame, pred: Dict[int, Dict[str, float]], setup: str) -> Dict[str, float | str]:
    ...
    # Entry is defined as a zone. We make the zone earlier for scanability,
    # but keep it structurally tied to the nearest support / reclaim area.
    if setup == "PULLBACK":
        entry_low = np.nanmax([x for x in [support_anchor, close - 0.28 * atr_safe] if np.isfinite(x)])
        entry_high = np.nanmin([x for x in [close + 0.04 * atr_safe, ema20_v * 1.008 if np.isfinite(ema20_v) else np.nan] if np.isfinite(x)])
    elif setup == "UNICORN":
        entry_low = np.nanmax([x for x in [support_anchor, close - 0.22 * atr_safe] if np.isfinite(x)])
        entry_high = np.nanmin([x for x in [close + 0.06 * atr_safe, ema20_v * 1.010 if np.isfinite(ema20_v) else np.nan] if np.isfinite(x)])
    elif setup == "SNIPER":
        entry_low = np.nanmax([x for x in [support_anchor, close - 0.16 * atr_safe] if np.isfinite(x)])
        entry_high = np.nanmin([x for x in [close + 0.05 * atr_safe, resistance_anchor * 1.004 if np.isfinite(resistance_anchor) else np.nan] if np.isfinite(x)])
    else:  # REVERSAL
        entry_low = np.nanmax([x for x in [low_10, close - 0.20 * atr_safe] if np.isfinite(x)])
        entry_high = np.nanmin([x for x in [close + 0.08 * atr_safe, support_anchor * 1.02 if np.isfinite(support_anchor) else np.nan] if np.isfinite(x)])

    if not np.isfinite(entry_low):
        entry_low = close * 0.995
    if not np.isfinite(entry_high) or entry_high <= entry_low:
        entry_high = max(entry_low * 1.008, close)
    ...
    # TP follows structure first, prediction second.
    ...
    # Stop is price-based, not ratio-based.
    stop_price = min(
        entry_low - 0.35 * atr_safe * CFG["stop_pad"],
        structural_stop,
        entry_low * 0.998,
    )
...
def score_plan(df: pd.DataFrame, plan: Dict[str, float | str], setup: str) -> Dict[str, float]:
    ...
    ev = float((prob_tp1 * (plan["tp1"] - plan["entry_mid"])) - ((1 - prob_tp1) * (plan["entry_mid"] - plan["stop_price"])))
...
def run_scan() -> dict | None:
    ...
    if not frames:
        ...
        st.info("Solusi paling stabil: upload CSV OHLCV dari export broker/Stockbit atau gunakan ticker list yang datanya tersedia di source online.")
...
if bundle is None:
    ...
    st.write(
        "Jika source online gagal di deploy, gunakan Upload CSV yang berisi OHLCV lengkap. "
        "Scanner ini akan memproses data lokal tanpa tergantung Yahoo Finance."
    )
    st.stop()
...
show_cols = [
    "symbol", "setup", "score", "close", "entry_low", "entry_high", "stop_price", "stop_pct", "tp1", "tp2",
...
display_df.rename(columns={"stop_price": "stop_price_rp", "stop_pct": "stop_pct_%"}, inplace=True)
st.dataframe(display_df[["symbol", "setup", "score", "close", "entry_low", "entry_high", "stop_price_rp", "stop_pct_%", "tp1", "tp2", "best_horizon", "prob_entry", "prob_tp1", "prob_tp2", "rr1", "rr2", "setup_note"]], use_container_width=True, hide_index=True)
...
st.write(
    ...
    f"Entry: **{row['entry_low']:.2f} - {row['entry_high']:.2f}** | Stop: **{row['stop_price']:.2f}** "
    f"({row['stop_pct']*100:.2f}%) | TP1: **{row['tp1']:.2f}** | TP2: **{row['tp2']:.2f}**"
)

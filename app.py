...
def fetch_ohlcv_for_ticker(ticker: str, period: str, interval: str) -> pd.DataFrame:
    ...
    data = yf.download(yf_ticker, period=period, interval=interval, auto_adjust=False, progress=False, threads=False)
    if data is None or len(data) == 0:
        return pd.DataFrame()
    data = data.reset_index()
    date_col = "Date" if "Date" in data.columns else "Datetime" if "Datetime" in data.columns else data.columns[0]
...
st.dataframe(pd.DataFrame({"ticker": [normalize_display_ticker(t) for t in tickers]}), use_container_width=True, hide_index=True)
...
if st.button("Fetch OHLCV + Scan", type="primary"):
    ...
    for i, t in enumerate(tickers, start=1):
        status.write(f"Memproses {normalize_display_ticker(t)} ({i}/{len(tickers)})")
        try:
            df = fetch_ohlcv_for_ticker(t, period=period, interval=interval)
            if df.empty:
                failed.append(t)
            else:
                ohlcv_chunks.append(df)
        except Exception:
            failed.append(t)
    ...
    if not ohlcv_chunks:
        st.error("Tidak ada data OHLCV yang berhasil diunduh. Cek ticker, period, interval, atau koneksi data.")
        st.stop()
    ...
    with st.expander("Lihat data mentah ticker terpilih"):
        st.dataframe(ohlcv[ohlcv["ticker"] == selected].tail(80), use_container_width=True, hide_index=True)

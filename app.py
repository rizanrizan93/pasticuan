...
st.set_page_config(page_title="IDX Super Scanner Safe Signal Semantics v5.6.1", page_icon="🛡️", layout="wide")
...
st.title("IDX Super Scanner — Safe Signal Semantics v5.6.1")
st.caption("Default SIGNAL_FIRST mempertahankan semua setup valid sebagai radar. Hanya ACCOUNT_GUARDED yang dapat menerbitkan EXECUTION_READY; kandidat manual memakai READY_FOR_STOCKBIT_VERIFY.")
...
with st.sidebar:
    ...
    with st.expander("Compounding Multibagger", expanded=True):
        ...
        st.caption("Jika belum ada ACCUMULATE_NOW, dana tetap dicatat sebagai cash compounding—bukan dipaksa membeli.")
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v5.6.1 mengambil Yahoo dan IDX/XBRL otomatis. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
scan_signature = upload_fingerprint(
    uploaded, portfolio_uploaded, fundamental_history_uploaded,
    broksum_uploaded, orderbook_uploaded,
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "compounding_budget_idr": realized_profit_to_compound * profit_allocation_pct,
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v5.6.1 Safe Signal Semantics</b><br>
...
tab_orders, tab_daily, tab_setups, tab_sniper, tab_fast, tab_multibagger, tab_ara, tab_portfolio, tab_chart, tab_bridge, tab_validation, tab_audit, tab_method = st.tabs(
    [
        "Order Builder", "Daily Radar", "Core Setups", "Sniper Entry", "BSJP / BPJS",
...
with tab_multibagger:
    ...
    accumulate_now = int(multibagger.get("compounding_state", pd.Series(dtype=str)).eq("ACCUMULATE_NOW").sum()) if not multibagger.empty else 0
    c1, c2 = st.columns(2)
    c1.metric("Cash profit untuk compounding", rupiah(budget))
    c2.metric("ACCUMULATE_NOW", accumulate_now)
    st.caption(
        "Profit trading masuk ke bucket Multibagger. Kandidat A sekarang wajib skor fundamental ≥8/10, "
        "data grade A, histori memadai, minimal dua keluarga sumber yang overlap, filing IDX/XBRL otomatis "
        "terverifikasi, consensus ≥75, dan tanpa konflik. "
        "Jika belum ada ACCUMULATE_NOW, dana tetap cash—tidak dipaksa membeli."
    )
...
with tab_method:
    st.markdown(
        """
        ### Hirarki sinyal v5.6.1 Safe Signal Semantics
        ...
        4. **Dua kebijakan eksekusi:** default `SIGNAL_FIRST` menerbitkan `SIGNAL_READY` untuk radar dan `READY_FOR_STOCKBIT_VERIFY` hanya setelah seluruh gate non-akun lolos. `ACCOUNT_GUARDED` menambahkan sizing, cash, slot posisi, dan portfolio heat sebelum `EXECUTION_READY`.
        ...
        6. **ARA Intelligence:** `DAILY_RADAR` selalu meranking kandidat; `SIGNAL_READY` berarti pola terkonfirmasi; `ORDER_READY` merupakan lapisan opsional ACCOUNT_GUARDED. Default SIGNAL_FIRST tidak menyembunyikan setup karena kondisi akun.
        ...
        9. **Sizing:** pada `SIGNAL_FIRST`, lot bersifat informasional dan `SIGNAL_READY` tidak membawa tiket beli. `READY_FOR_STOCKBIT_VERIFY` membawa usulan harga tetapi wajib revalidasi broker. Pada `ACCOUNT_GUARDED`, order yang lolos membawa lot berbasis risiko akun.

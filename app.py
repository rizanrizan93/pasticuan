...
st.set_page_config(page_title="IDX Multibagger & Core Swing Scanner v6.8.0", page_icon="🛡️", layout="wide")
...
REQUIRED_SCANNER_FILES = ("scanner.py", "scanner_focus.py", "ai_engine.py", "time_cycle.py", "eoff_reconstruction.py", "dashboard_v660.py")
...
st.title("IDX Scanner — Multibagger & Core Swing v6.8.0")
...
with st.sidebar:
    ...
    with st.expander("Time-Cycle Intelligence — Swing/Core & Multibagger", expanded=True):
        ...
        st.caption(
            "Implementasi clean-room ini tidak mengklaim formula proprietary asli. Mulai v6.8.0, astro tidak memiliki bobot produksi. EOFF memengaruhi timing hanya setelah cluster waktu, arah harga, dan chronological forward evidence lolos."
        )
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v6.8.0 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v6.8.0 Multibagger & Core Swing Focus</b><br>
...
with main_existing:
    ...
    with tab_multibagger:
        ...
        st.caption(
            ...
            "Execution Readiness dihitung terpisah dari momentum, silent accumulation v2, Core Swing, dan objective EOFF timing; timing tidak dapat mengubah bisnis lemah menjadi kandidat berkualitas. "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v6.8.0 Multibagger & Core Swing

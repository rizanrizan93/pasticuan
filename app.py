...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.6.1", page_icon="🛡️", layout="wide")
...
REQUIRED_SCANNER_FILES = (
    ...
    "ihsg_direction.py", "narrative_engine.py",
)
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.6.1")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.6.1 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "7.6.1-narrative-safety-hardening",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.6.1 — Narrative Intelligence, Coverage-aware Multibagger, Statistical Selector, Execution AI, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_multibagger:
        ...
        with st.expander(
            ...
            st.caption(
                ...
                "Metrik conversion tetap shadow sebelum sampel minimum, tetapi "
                "narrative positif yang bersumber tetap aktif sebagai input produksi "
                "dengan confidence shrinkage. Silent Accumulation menjadi ranking key "
                "terpisah agar tidak dihitung berulang."
            )
            ...
            if not narrative_events_view.empty:
                ...
                st.download_button(
                    ...
                    "narrative_event_ledger_v761.csv",
                    "text/csv",
                    width="stretch",
                )
        ...
        st.caption(
            "Multibagger Quality v7.6.1 memisahkan Growth Compounder dari Turnaround/Cyclical, mengaudit coverage setiap pilar, dan menambahkan Narrative–Flow Convergence point-in-time tanpa mengubah missing evidence menjadi cerita. Growth menilai growth persistence, profitability termasuk gross profitability, cash conversion/accrual quality, balance-sheet safety, serta reinvestment runway yang berbobot langsung 15%. "
    ...
    with tab_audit:
        ...
        st.caption(
            "Bridge v10 membaca cache fundamental, histori laporan, forward quality, coverage-aware lane Growth/Turnaround, kalender IDX, selector outcomes, execution-AI outcomes, arah IHSG, serta narrative safety lineage dari Supabase. "
        ...
        if not database_read_report.empty:
            ...
            misses = int(read_state.isin(["DATABASE_MISS", "DATABASE_EXPIRED", "DATABASE_MODEL_STALE", "DATABASE_EVENT_DUE", "DATABASE_CHECK_DUE", "MIGRATION_REQUIRED_V3", "MIGRATION_REQUIRED_V4", "MIGRATION_REQUIRED_V5", "MIGRATION_REQUIRED_V6", "MIGRATION_REQUIRED_V7", "MIGRATION_REQUIRED_V8", "MIGRATION_REQUIRED_V9", "MIGRATION_REQUIRED_V10"]).sum())
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.6.1 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.6.1 — Narrative Intelligence, Coverage-aware Multibagger, IHSG & Core Swing

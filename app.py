...
def build_macro_liquidity_gate(bench_df: pd.DataFrame, benchmark_symbol: str = "^JKSE") -> dict:
    ...
    required_cols = [
        ...
        "VOL_SMA20", "REL_VOL", "OBV", "OBV_SLOPE10", "CMF20",
        "MFI14", "STOCH_K", "STOCH_D", "CCI20", "ROC12",
    ]
    d = d.dropna(subset=[c for c in required_cols if c in d.columns]).copy()
...
def classify_8_phase(d: pd.DataFrame) -> dict:
    required_cols = [
        ...
        "RSI14", "ADX14", "REL_VOL", "CMF20", "MFI14", "STOCH_K",
        "STOCH_D", "CCI20", "ROC12", "OBV_SLOPE10", "ATR14",
    ]
    x = d.dropna(subset=[c for c in required_cols if c in d.columns]).copy()
...
def score_stock_smc(
    ...
    signal_cols = [
        ...
        "EMA20_Reclaim", "MACD_Bull_Cross", "RSI_Bounce", "Breakout_5D",
        "Volume_Surge", "Bullish_FVG", "Bullish_OB",
    ]
    ...
    required_cols = [
        ...
        "MACD_HIST", "ATR14", "ADX14", "BB_MID", "BB_UPPER", "BB_LOWER",
        "VOL_SMA20", "REL_VOL", "VPT", "OBV", "OBV_SMA10", "OBV_SLOPE10",
        "CMF20", "MFI14", "STOCH_K", "STOCH_D", "CCI20", "ROC12",
    ]
    d = d.replace([np.inf, -np.inf], np.nan)
    d = d.dropna(subset=[c for c in required_cols if c in d.columns]).copy()

    if len(d) < max(50, int(min_history_bars * 0.7)):
        return {"valid": False, "reason": "Kebocoran data setelah dropping NaN", "rows_after_clean": len(d)}
    ...
    try:
        ...
        if not res.get("valid"):
            return {
                "valid": False,
                "symbol": symbol,
                "reason": res.get("reason", "Scoring gagal"),
                "rows_after_clean": res.get("rows_after_clean", np.nan),
            }
...
def run_deep_dive_analysis(
    ...
    if not stock_res.get("valid"):
        return {
            ...
            "ifs_context": None,
            "entry_plan": None,
            "error": stock_res.get("reason", "Scoring gagal"),
        }
    ...
    with sub_detail:
        if analyze_btn:
            ...
            if stock_df.empty or len(stock_df) < min_history_bars:
                ...
                if not stock_res.get("valid"):
                    st.error(f"{deep_ticker}: {stock_res.get('reason', 'Scoring gagal')}")
                    st.stop()

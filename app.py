...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.4.0", page_icon="🛡️", layout="wide")
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.4.0")
st.caption(
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.4.0 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "7.4.0-scipy-statistical-quality-momentum",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.4.0 — Statistical Selector, Execution AI, Multibagger, Core Swing & Arah IHSG</b><br>
...
with main_existing:
        ...
        if not selector_audit_view.empty:
            with st.expander("Walk-forward selector: rule vs independen vs AI vs relative strength", expanded=False):
                ...
                st.caption(
                    ...
                    "tiga horizon, dan maximum drawdown tetap di bawah guard."
                )
        st.divider()
    ...
    with tab_multibagger:
        ...
        st.caption(
            "Multibagger Quality v7.4 menilai lima pilar eksplisit: growth persistence, profitability termasuk gross profitability, cash conversion/accrual quality, balance-sheet safety, dan reinvestment runway. "
    ...
    with tab_audit:
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.4.0 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.4.0 — Statistical Selector, IHSG, Multibagger & Core Swing

...
APP_VERSION = "9.8.2-all-eligible-lite"
APP_ROOT = Path(__file__).resolve().parent
if str(APP_ROOT) not in sys.path:
    sys.path.insert(0, str(APP_ROOT))

st.set_page_config(page_title="IDX Scanner v9.8.2", page_icon="📊", layout="wide")
...
REQUIRED_FILES = (
    ...
    "decision_overlay.py", "real_money_guard.py", "v9_dashboard.py",
    "resumable_app_engine.py", "fast_scan_engine.py",
)
missing = [name for name in REQUIRED_FILES if not (APP_ROOT / name).is_file()]
if missing:
    st.error("Deployment v9.8.2 tidak lengkap.")
...
st.title("IDX Super Scanner v9.8.2 — ALL_ELIGIBLE_LITE")
st.caption(
    f"{APP_VERSION} • fast {FAST_SCAN_VERSION} • macro {MACRO_ENGINE_VERSION} • "
    f"decision {SIMPLE_FOCUS_VERSION} • inventory {DECISION_OVERLAY_VERSION} • dashboard {V9_DASHBOARD_VERSION}"
)
st.markdown(
    """
    <div class="v9-note">
      ...
      Deep fundamental/news/official IDX hanya untuk kandidat prioritas + maintenance rotation.
    </div>
    """, unsafe_allow_html=True,
)
...
if run_scan:
    ...
    try:
        ...
        if db_state == "CIRCUIT_OPEN_FAIL_SOFT":
            st.warning(
                "Supabase tidak responsif pada scan ini. Ranking tetap selesai karena database tidak lagi menjadi dependency. "
                "Data yang tidak sempat dipersist akan dicoba lagi pada scan berikutnya saat Supabase sehat."
            )
        else:

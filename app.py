...
st.set_page_config(page_title="IDX Super Scanner Eye-of-Future Reconstruction AI v6.5.0", page_icon="🛡️", layout="wide")
...
REQUIRED_SCANNER_FILES = ("scanner.py", "scanner_specialty.py", "ai_engine.py", "time_cycle.py", "eoff_reconstruction.py")
...
def cached_twelve_fundamental_history(
    ...
    return fetch_twelve_data_fundamental_history(
        tickers, api_key=_twelve_data_api_key, max_tickers=max_tickers,
    )
...
def enrich_fundamental_shortlist(
    ...
    if fundamentals is None or fundamentals.empty or not tickers:
    ...
    yahoo_history, yahoo_report = cached_yahoo_fundamental_history(
        tickers, max_tickers=int(config.fundamental_history_top_n),
    )
...
st.title("IDX Super Scanner — Eye-of-Future Reconstruction AI v6.5.0")
...
with st.sidebar:
    ...
    st.caption(
        ...
        "Fundamental shortlist: Yahoo + IDX/XBRL otomatis; upload hanya fallback."
    )
    if itick_api_token:
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v6.2.0 mengambil Yahoo dan IDX/XBRL otomatis. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_portfolio and portfolio_uploaded is not None:
    ...
    if use_fundamentals and not fundamentals.empty:
        progress.progress(72, text="Mengambil Yahoo + IDX/XBRL otomatis dan memeriksa kualitas laba…")
...
if run_scan and uploaded is not None:
    ...
    if use_fundamentals and (not signals.empty or portfolio_tickers):
        ...
        if not fundamentals.empty:
            progress.progress(68, text="Mengambil Yahoo + IDX/XBRL otomatis, kualitas laba, dilusi, dan consensus…")
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v6.5.0 Eye-of-Future Reconstruction AI</b><br>
...
tab_multibagger, tab_orders, tab_daily, tab_setups, tab_sniper, tab_fast, tab_ara, tab_portfolio, tab_chart, tab_time_cycle, tab_bridge, tab_validation, tab_audit, tab_method = st.tabs(
    [
        "Multibagger Capital", "Order Builder", "Daily Radar", "Core Setups", "Sniper Entry",
        "BSJP / BPJS", "ARA Hunter", "Portfolio Stockbit", "Chart", "Time-Cycle Intelligence",
    ...
    if not multibagger.empty:
        allocation_columns = [
            column for column in [
                ...
                "multibagger_time_cycle_score", "time_cycle_capital_weight_pct", "eoff_strength_label",
                "eoff_reconstruction_score", "eoff_signal_active", "eoff_direction_bias",
                "eoff_fib_cluster_count", "eoff_reversal_date", "eoff_astro_score",
...
with tab_time_cycle:
    st.subheader("Time-Cycle & Eye-of-Future Reconstruction — Single / Multi Ticker")
    st.caption(
        "Ketik satu ticker untuk deep dive atau beberapa ticker dipisahkan koma. Tidak perlu upload CSV. "
        "Analisis hanya memakai daily OHLCV dan tidak memengaruhi BPJS/BSJP/intraday/ARA."
    )
    with st.form("manual_time_cycle_form"):
        manual_tickers_text = st.text_input("Ticker", value="ANTM", placeholder="ANTM atau ANTM, MDKA, NCKL")
        tc_period = st.selectbox("Lookback", ["5y", "3y", "2y"], index=1, key="time_cycle_period")
        run_time_cycle = st.form_submit_button("Analisis Time Cycle + EOFF", type="primary")
    if run_time_cycle:
        raw_names = [part.strip() for part in manual_tickers_text.replace(";", ",").split(",") if part.strip()]
        manual_tickers = list(dict.fromkeys(normalize_idx_ticker(name) for name in raw_names))[:10]
        if not manual_tickers:
            st.warning("Masukkan minimal satu ticker IDX.")
        else:
            with st.spinner(f"Mengunduh daily OHLCV dan menghitung time cycle {len(manual_tickers)} ticker…"):
                tc_histories, tc_report = download_ohlcv(tuple(manual_tickers), period=tc_period)
                tc_rows = []
                for ticker in manual_tickers:
                    analysis = analyze_time_cycle(
                        tc_histories.get(ticker),
                        TimeCycleConfig(
                            min_bars=int(cfg.time_cycle_min_history_bars),
                            lunar_enabled=bool(cfg.time_cycle_lunar_enabled),
                            eoff_enabled=bool(cfg.eoff_enabled),
                    ...
                    tc_rows.append({"ticker": ticker, **analysis})
                st.session_state["manual_time_cycle_result"] = {
                    "rows": pd.DataFrame(tc_rows), "histories": tc_histories, "report": tc_report,
                }
    manual_tc = st.session_state.get("manual_time_cycle_result", {})
    tc_rows = manual_tc.get("rows", pd.DataFrame()) if isinstance(manual_tc, dict) else pd.DataFrame()
    if not tc_rows.empty:
        display_cols = [column for column in (
            "ticker", "time_cycle_state", "time_cycle_score", "time_cycle_confidence",
            "time_cycle_direction_bias", "time_cycle_phase", "dominant_cycle_bars",
            "cycle_historical_hit_rate", "cycle_validation_samples",
            "price_time_confluence_score", "next_reversal_window_start",
            "next_reversal_window_end", "bars_to_reversal_window", "lunar_phase",
            "lunar_days_to_major_marker", "eoff_state", "eoff_strength_label",
            "eoff_reconstruction_score", "eoff_direction_bias", "eoff_fib_cluster_count",
            "eoff_reversal_date", "eoff_astro_score", "eoff_ephemeris_state",
        ) if column in tc_rows.columns]
        ranking_view = tc_rows[display_cols].sort_values("time_cycle_score", ascending=False, na_position="last")
        st.dataframe(ranking_view, hide_index=True, width="stretch")
        detail_ticker = st.selectbox("Deep dive ticker", tc_rows["ticker"].tolist(), key="time_cycle_detail_ticker")
        detail = tc_rows.loc[tc_rows["ticker"].eq(detail_ticker)].iloc[0].to_dict()
        m1, m2, m3, m4, m5, m6 = st.columns(6)
        m1.metric("Time-cycle", f"{safe_number(detail.get('time_cycle_score'), 0):.1f}")
        m2.metric("Cycle confidence", f"{safe_number(detail.get('time_cycle_confidence'), 0):.1f}%")
        m3.metric("EOFF", f"{safe_number(detail.get('eoff_reconstruction_score'), 0):.1f}")
        m4.metric("EOFF strength", safe_text(detail.get('eoff_strength_label')) or "LOW")
        m5.metric("Fib cluster", int(safe_number(detail.get('eoff_fib_cluster_count'), 0)))
        m6.metric("EOFF active", "YES" if bool(detail.get('eoff_signal_active')) else "SHADOW")
        st.info(safe_text(detail.get("time_cycle_explanation")) or "Tidak ada penjelasan cycle.")
        if safe_text(detail.get("eoff_explanation")):
            st.info(safe_text(detail.get("eoff_explanation")))
        tc_histories = manual_tc.get("histories", {})
        figure = make_time_cycle_chart(tc_histories.get(detail_ticker), detail, detail_ticker)
        if figure is not None:
            st.plotly_chart(figure, width="stretch")
        st.json({
            "direction_bias": detail.get("time_cycle_direction_bias"),
            "phase": detail.get("time_cycle_phase"),
            "next_window": [detail.get("next_reversal_window_start"), detail.get("next_reversal_window_end")],
            "pivot_cycle": detail.get("pivot_cycle_bars"),
            "autocorrelation_cycle": detail.get("autocorr_cycle_bars"),
            "spectral_cycle": detail.get("spectral_cycle_bars"),
            "fibonacci_time_score": detail.get("fibonacci_time_score"),
            "lunar_marker": detail.get("lunar_phase"),
            "lunar_historical_lift": detail.get("lunar_historical_lift"),
            "eoff": {
                "state": detail.get("eoff_state"),
                "strength": detail.get("eoff_strength_label"),
                "signal_active": bool(detail.get("eoff_signal_active")),
                "direction": detail.get("eoff_direction_bias"),
                "time_power": detail.get("eoff_time_power_score"),
                "price_power": detail.get("eoff_price_power_score"),
                "pattern": detail.get("eoff_pattern_score"),
                "momentum": detail.get("eoff_momentum_score"),
                "astro": detail.get("eoff_astro_score"),
                "fib_cluster_count": detail.get("eoff_fib_cluster_count"),
                "fib_unique_anchors": detail.get("eoff_fib_unique_anchor_count"),
                "fib_historical_hit": detail.get("eoff_historical_hit_rate"),
                "fib_historical_baseline": detail.get("eoff_historical_baseline_rate"),
                "fib_historical_lift": detail.get("eoff_historical_lift"),
                "astro_fib_historical_hit": detail.get("eoff_confluence_historical_hit_rate"),
                "astro_fib_historical_events": detail.get("eoff_confluence_historical_events"),
                "astro_fib_historical_lift": detail.get("eoff_confluence_historical_lift"),
                "reversal_date": detail.get("eoff_reversal_date"),
                "ephemeris_date": detail.get("eoff_ephemeris_date"),
                "moon_declination": detail.get("eoff_moon_declination_deg"),
                "moon_phase": detail.get("eoff_moon_phase"),
                "sun_sign": detail.get("eoff_sun_sign"),
                "annual_cycle": detail.get("eoff_sun_annual_cycle_bias"),
                "retrograde": detail.get("eoff_retrograde_planets"),
                "stationary": detail.get("eoff_stationary_planets"),
                "ingress": detail.get("eoff_ingress_events"),
                "aspects": detail.get("eoff_active_aspects"),
                "astro_events": detail.get("eoff_astro_events"),
            },
        })
        try:
            import json as _json
            roadmap_rows = _json.loads(safe_text(detail.get("eoff_roadmap_json")) or "[]")
        except Exception:
            roadmap_rows = []
        if roadmap_rows:
            st.markdown("#### Future Price Road Map — scenario, bukan kepastian")
            st.dataframe(pd.DataFrame(roadmap_rows), hide_index=True, width="stretch")
    else:
        st.info("Masukkan ticker dan tekan Analisis Time Cycle + EOFF.")
...
with tab_method:
    st.markdown(
        """
        ### Hirarki sinyal v6.5.0 Eye-of-Future Reconstruction AI

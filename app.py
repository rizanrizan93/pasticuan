...
st.set_page_config(page_title="IDX Multibagger & Core Swing Scanner v6.8.1", page_icon="🛡️", layout="wide")
...
st.title("IDX Scanner — Multibagger & Core Swing v6.8.1")
...
with st.sidebar:
    ...
    with st.expander("Time-Cycle Intelligence — Swing/Core & Multibagger", expanded=True):
        ...
        st.caption(
            "Implementasi clean-room ini tidak mengklaim formula proprietary asli. Mulai v6.8.1, astro tidak memiliki bobot produksi. EOFF memengaruhi timing hanya setelah cluster waktu, arah harga, dan chronological forward evidence lolos."
        )
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v6.8.1 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "6.8.1-research-maintenance-database-ready",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v6.8.1 Multibagger & Core Swing Focus</b><br>
...
with main_existing:
    ...
    with tab_multibagger:
        ...
        st.caption(
            ...
            "Execution Readiness dihitung terpisah dari momentum, Silent Accumulation v3, Core Swing, dan objective EOFF timing; timing tidak dapat mengubah bisnis lemah menjadi kandidat berkualitas. "
        ...
        if not multibagger.empty:
            allocation_columns = [
                column for column in [
                    ...
                    "multibagger_quality_score", "execution_readiness_score", "research_recommendation_status",
                    "multibagger_candidate_type", "multibagger_status", "compounding_state",
    ...
    with tab_audit:
        ...
        st.subheader("Database readiness")
        st.caption(
            "Database eksternal belum wajib. Mode default DISABLED_NO_DATABASE tidak melakukan network I/O; "
            "setelah Supabase dibuat, sinkronisasi tetap fail-soft dan tidak boleh menghentikan scanner."
        )
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v6.8.1 Multibagger & Core Swing

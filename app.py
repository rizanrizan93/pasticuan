...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.5.1", page_icon="🛡️", layout="wide")
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.5.1")
st.caption(
...
with st.sidebar:
    ...
    multibagger_full_universe = st.checkbox(
        ...
        help=(
            "Mode riset penuh. Default OFF agar scan harian menilai shortlist teknikal lebih dahulu. "
            "Aktifkan saat backfill mingguan/akhir pekan; snapshot fundamental dicache maksimal "
            "21 hari dan histori resmi direfresh bertahap."
        ),
    )
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.5.1 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "7.5.1-multibagger-lanes-research-allocation-split",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.5.1 — Multibagger Lanes, Selection/Execution Split, Statistical Selector, Execution AI, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_multibagger:
        ...
        with growth_tab:
            if isinstance(growth_compounder, pd.DataFrame) and not growth_compounder.empty:
                ...
                st.info("Belum ada Growth Compounder yang lolos gate riset.")
        with turnaround_tab:
        ...
        st.caption(
            "Multibagger Quality v7.5.1 memisahkan Growth Compounder dari Turnaround/Cyclical. Growth menilai growth persistence, profitability termasuk gross profitability, cash conversion/accrual quality, balance-sheet safety, serta reinvestment runway yang kini berbobot langsung 15%. "
    ...
    with tab_audit:
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.5.1 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.5.1 — Multibagger Lanes, Selection/Execution Split, IHSG & Core Swing

...
st.set_page_config(page_title="IDX Wealth & IHSG Direction Scanner v7.5.2", page_icon="🛡️", layout="wide")
...
from dashboard_v660 import (
    ...
    render_time_cycle_main_tab, streamlit_safe_frame,
)
...
def database_first_fundamentals(
    ...
    default_priorities = tuple(dict.fromkeys(priority_tickers or due_names[: min(12, len(due_names))]))
    refresh_names, scheduler_audit = select_round_robin_backfill(
        ...
        max_count=int(getattr(cfg, "fundamental_backfill_max_per_scan", 40)),
    )
...
def enrich_fundamental_shortlist(
    ...
    refresh_names_list, scheduler_audit = select_round_robin_backfill(
        ...
        priority_tickers=due_names[: min(8, len(due_names))],
        event_tickers=event_names,
...
def render_ihsg_direction_panel(
    ...
    if isinstance(horizons, pd.DataFrame) and not horizons.empty:
        ...
        if all(column in horizons for column in probability_columns.values()):
            ...
            horizons["probability_leader_research"] = probability_view.idxmax(
                axis=1,
            ).map(label_by_column).where(probability_view.notna().any(axis=1), "N/A")
            sorted_probability = np.sort(
                probability_view.to_numpy(dtype=float), axis=1,
            )
            horizons["probability_edge_pct"] = np.where(
                probability_view.notna().sum(axis=1).ge(2),
...
st.title("IDX Wealth Scanner — Multibagger, Core Swing & Arah IHSG v7.5.2")
st.caption(
...
with st.sidebar:
    ...
    validation_max_tickers = st.slider(
        ...
        help=(
            "Sampel deterministik berstrata likuiditas—bukan berdasarkan score—agar validasi "
            "tidak mendominasi CPU. Pilih 300 untuk mendekati full-universe; hasil yang belum "
    ...
    multibagger_full_universe = st.checkbox(
        ...
        help=(
            "Mode riset penuh. Database dibaca untuk seluruh universe, tetapi panggilan provider "
            "tetap dibatasi dan dirotasi per cohort agar scan tidak membanjiri layanan gratis. "
            "Snapshot dan histori resmi yang sudah didapat disimpan untuk scan berikutnya."
        ),
    )
    ...
    with st.expander("Time-Cycle Intelligence — Swing/Core & Multibagger", expanded=True):
        ...
        st.caption(
            ...
            "Cycle hanya dipakai untuk timing dan Best Buy Date setelah VALIDATED."
        )
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v7.5.2 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    if validate and result["prepared"]:
        validation_prepared, validation_universe_audit = select_walkforward_universe(
            result["prepared"],
            max_tickers=int(cfg.validation_max_tickers),
        )
        progress.progress(
            45,
            text=(
                "Menjalankan chronological OOS validation dengan live-plan parity "
                f"({len(validation_prepared)}/{len(result['prepared'])} ticker)…"
            ),
        )
        stats, trades = run_walkforward_validation(validation_prepared, cfg)
    scan_profiler.mark(
        "Chronological OOS",
        workload=(
    ...
    if use_fundamentals and (not signals.empty or portfolio_tickers):
        ...
        fundamental_priority_names = tuple(dict.fromkeys(list(portfolio_tickers) + execution_names + ranked_names[:12]))
        ...
        if not fundamentals.empty:
            ...
            if multibagger_full_universe:
                # Read persisted statement history for the whole universe, then
                ...
                history_names = tuple(dict.fromkeys(
                    ...
                    + multibagger_ranked + scan_tickers
                ))
            else:
                history_names = tuple(dict.fromkeys(
                    portfolio_tickers + execution_names + uploaded_names
    ...
    result.update({
        ...
        "scanner_version": "7.5.2-fundamental-provenance-near-miss",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v7.5.2 — Multibagger Lanes, Provenance Repair, Near-Miss Radar, Statistical Selector, Execution AI, Core Swing & Arah IHSG</b><br>
...
with main_existing:
    ...
    with tab_orders:
        ...
        st.caption(
            ...
            "trend, relative strength, dan Silent Accumulation. Setup dicari sesudah seleksi; karena itu "
        ...
        if core_radar.empty:
            ...
            st.dataframe(core_radar[radar_columns], hide_index=True, width="stretch", height=430)
    ...
    with tab_multibagger:
        ...
        if not coverage_view.empty:
            ...
            q5.metric(
                "Menunggu data",
                int(coverage_counts.get("Menunggu snapshot fundamental", 0)),
            )
        ...
        radar_columns = [
            "ticker", "multibagger_lane", "multibagger_candidate_type",
        ...
        with growth_tab:
            if isinstance(growth_compounder, pd.DataFrame) and not growth_compounder.empty:
                ...
                st.info(
                    ...
                    "Kandidat terdekat tetap terlihat pada Near-Miss di bawah."
                )
        with turnaround_tab:
            if isinstance(turnaround, pd.DataFrame) and not turnaround.empty:
                ...
                st.info(
                    ...
                    "recovery dan tanpa risiko kritis. Kandidat recovery yang "
                    "belum lengkap tetap terlihat pada Near-Miss di bawah."
                )
        ...
        diag_coverage, diag_growth, diag_turnaround, diag_pending, diag_gates = st.tabs(
            [
                "Coverage",
                "Growth Near-Miss",
        ...
        st.caption(
            "Multibagger Quality v7.5.2 memisahkan Growth Compounder dari Turnaround/Cyclical. Growth menilai growth persistence, profitability termasuk gross profitability, cash conversion/accrual quality, balance-sheet safety, serta reinvestment runway yang kini berbobot langsung 15%. "
    ...
    with tab_audit:
        ...
        st.caption(
            "Bridge v7 membaca cache fundamental, histori laporan, forward quality, lane Growth/Turnaround, kalender IDX, selector outcomes, execution-AI outcomes, dan snapshot arah IHSG dari Supabase. "
        ...
        with db_col1:
            if st.button("Test koneksi database", key="database_health_check_v710"):
                st.session_state["database_health_report_v710"] = ScannerDatabaseBridge().health_check()
        with db_col2:
            health_report = st.session_state.get("database_health_report_v710")
        ...
        if not database_read_report.empty:
            ...
            misses = int(read_state.isin(["DATABASE_MISS", "DATABASE_EXPIRED", "DATABASE_MODEL_STALE", "DATABASE_EVENT_DUE", "DATABASE_CHECK_DUE", "MIGRATION_REQUIRED_V3", "MIGRATION_REQUIRED_V4", "MIGRATION_REQUIRED_V5", "MIGRATION_REQUIRED_V6", "MIGRATION_REQUIRED_V7"]).sum())
        ...
        if not database_sync_report.empty:
            ...
            if database_states.intersection({"DATABASE_FAIL_SOFT", "PARTIAL_WRITE"}):
                st.warning(
                    "Sinkronisasi database belum penuh. Periksa kolom detail; v7.5.2 menampilkan body error PostgREST "
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v7.5.2 — Multibagger Lanes, Provenance Repair, Near-Miss Radar, IHSG & Core Swing

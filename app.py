...
st.set_page_config(page_title="IDX Super Scanner Resilient v4.2", page_icon="🎯", layout="wide")
...
st.title("IDX Super Scanner — Resilient Evidence v4.2")
...
b1, b2 = st.columns(2)
run_scan = b1.button("Jalankan scanner", type="primary", use_container_width=True, disabled=uploaded is None)
run_portfolio = b2.button(
    "Analisis portfolio saja", type="secondary", use_container_width=True, disabled=portfolio_uploaded is None
)
...
if run_scan and uploaded is not None:
    ...
    context_names = list(dict.fromkeys(potential + portfolio_tickers + specialty_shortlist))
    if context_names:
...
with tab_orders:
    if signals.empty:
        ...
        c1.download_button(
            ...
            "idx_super_scanner_results.csv", "text/csv", use_container_width=True,
        )
        c2.download_button(
            "Download execution-ready CSV",
            execution.drop(columns=["status_rank"], errors="ignore").to_csv(index=False).encode("utf-8"),
            "idx_execution_ready.csv", "text/csv", use_container_width=True,
...
with tab_method:
    st.markdown(
        ...
        7. **Specialty screens:** core setup ditampilkan terpisah; Sniper memakai strict ICT confluence; BSJP/BPJS wajib intraday dan session window; Multibagger menggabungkan growth-quality-momentum; ARA Hunter tetap speculative-review-only.

...
def _flow_score_from_mode(flow_mode: str) -> float:
    mapping = {
        "Big Akumulasi": 95.0,
...
def make_flow_score(flow_mode: str) -> float:
    ...
    return _flow_score_from_mode(flow_mode)
...
def load_fundamental_snapshot(symbol: str) -> dict:
    out = {
        ...
        "growth_data_reliability": np.nan,
...
def detect_reversal_signals(d: pd.DataFrame) -> pd.DataFrame:
    ...
    # Candlestick / price-action reversal primitives
    x["Bullish_Engulfing"] = (
        (x["Close"] > x["Open"])
...
def score_stock_smc(
    ...
    if d.empty or len(d) < min_history_bars:
        return {"valid": False, "reason": "Data historis tidak mencukupi", "df": d.copy(), "last": d.iloc[-1] if len(d) else None}
    ...
    if len(d) < max(50, int(min_history_bars * 0.7)):
        return {"valid": False, "reason": "Kebocoran data setelah dropping NaN", "df": d.copy(), "last": d.iloc[-1] if len(d) else None}
...
flow_val = make_flow_score("Netral")
...
def run_deep_dive_analysis(
    ...
    flow_val_local = make_flow_score(bandarmology_mode)
    ...
    if not stock_res.get("valid", False):
        return {
            ...
            "ifs_context": None,
            "entry_plan": None,
            "error": stock_res.get("reason", "Analisis teknikal tidak valid."),
        }
            ...
            if stock_df.empty or len(stock_df) < min_history_bars:
                ...
                with right:
                    ...
                    if stock_res["decision"] in {"BUY", "STRONG BUY"}:
                        ...
                        tp_price = stock_res["entry_price"] + take_profit_atr_local * float(_safe_float(stock_res.get("last", {}).get("ATR14"), np.nan))

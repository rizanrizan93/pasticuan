...
APP_VERSION = "9.7.1-performance-finalizer-hotfix"
...
def _database_job_error_message(error: object, *, action: str = "membaca") -> str:
    ...
    if any(token in lowered for token in ("transport failure", "readtimeout", "read timed out", "connecttimeout", "connectionerror", "connection aborted", "temporarily unavailable")):
        return (
            ...
            "Scanner v9.7.1 mencoba ulang GET/idempotent write otomatis dengan backoff dan membatch checkpoint ticker. "
            "Ini bukan indikasi permission 403/42501. Coba Segarkan status job; bila berulang, "
...
def _render_resumable_job(job: Mapping[str, object], bridge: ScannerDatabaseBridge) -> None:
    ...
    if status == "FINALIZING":
        finalizing_text = {
            "RANKING_READY": "Ranking provisional sudah siap; menyiapkan validasi kandidat final.",
            "EXECUTION_VERIFY": "Memvalidasi harga/EOD kandidat prioritas (maks. 24 ticker), bukan mengulang scan 400 ticker.",
            "DATABASE_SYNC": "Ranking final sudah dihitung; menyinkronkan snapshot keputusan ke database.",
...
st.title("IDX Super Scanner v9.7 — Macro-First")
st.caption(
...
st.markdown(
    """
    <div class="v9-note">
      <b>Resumable + modular decision architecture</b><br>
      Core data/database/fundamental dipisahkan dari inventory lifecycle dan dashboard. Perubahan UI atau anti-chase tidak mengubah provider/database contract.
      Setiap ticker tetap memiliki checkpoint durable. Output utama: <b>Top 3 Report Card</b>, <b>The Next Leader</b>, dan <b>Swing Ready</b>.
    </div>
    """,
    unsafe_allow_html=True,
)
...
if active_job and str(active_job.get("job_type", "")).upper() == job_type:
    ...
    if status in {"COMPLETE_WITH_FAILURES", "FAILED"}:
        ...
        if not failed_rows.empty:
            st.error(
                "Job lama selesai dengan ticker gagal. v9.7.1 mengulang ticker tersebut di job yang sama, "

...
st.set_page_config(page_title="IDX Super Scanner Resilient v4.3.2", page_icon="🎯", layout="wide")
...
def execution_funnel_summary(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    stages = [
        ("Candle EOD final", "DAILY_BAR_NOT_FINAL"),
...
def build_tradingview_bridge(
    ...
    status_rank = {
        ...
        "MULTIBAGGER_A_CANDIDATE": 1, "ARA_HUNTER_STRICT": 1,
...
st.title("IDX Super Scanner — EOD Integrity v4.3.2")
st.caption("Upload universe ticker; OHLCV, fundamental, berita, status IDX, dan cross-check harga dikumpulkan otomatis. Broker flow aktual tetap dikonfirmasi di Stockbit.")
...
st.success(
    "Tidak perlu upload harga, berita, fundamental, atau market status. Scanner memakai OHLCV flow proxy untuk akumulasi; "
    "broker summary/bandarmology aktual tidak diklaim otomatis dan tetap dikonfirmasi di Stockbit. Portfolio adalah upload opsional."
)

now_jkt_ui = pd.Timestamp.now(tz="Asia/Jakarta")
if now_jkt_ui.weekday() < 5 and not idx_daily_bar_is_final(now_jkt_ui):
    st.warning(
        "Core scanner memakai completed EOD terakhir. Selama sesi berjalan, kandidat teknikal ditampilkan sebagai "
        "PENDING_CLOSE—bukan BLOCKED_CONTEXT. Refresh setelah 16:20 WIB untuk keputusan EXECUTION_READY final."
    )
...
if run_scan and uploaded is not None:
    ...
    # Broker-level data is not fabricated. v4.3.2 keeps the quantitative
    # flow/accumulation proxy automatic from OHLCV, while actual Stockbit
    # broker-summary/bandarmology remains an external manual confirmation.
...
m4.metric("Menunggu close", int(signals["status"].eq("PENDING_CLOSE").sum()) if not signals.empty else 0)
m5.metric("Siap verifikasi", int(signals["status"].eq("READY_FOR_PRICE_VERIFY").sum()) if not signals.empty else 0)
m6.metric("Pending data", int(signals["status"].eq("PENDING_DATA").sum()) if not signals.empty else 0)
m7.metric("Watchlist", int(signals["status"].eq("WATCHLIST_ENTRY").sum()) if not signals.empty else 0)
m8.metric("Breadth > EMA50", f"{context.breadth_ema50:.0f}%" if context.breadth_ema50 is not None else "N/A")
...
with tab_method:
    st.markdown(
        ...
        11. **TradingView bridge:** level scanner diekspor dalam schema tetap dan diverifikasi kembali pada indikator Pine v6. Directional score adalah confluence score, bukan probabilitas.
        12. **Stockbit screener:** preset Stockbit hanya berfungsi sebagai coarse pre-filter. Sweep, BOS/CHOCH, FVG, ORB, dan validitas structural stop tetap dikonfirmasi oleh scanner/TradingView.

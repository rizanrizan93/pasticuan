...
st.set_page_config(page_title="IDX Multibagger & Core Swing Scanner v6.9.0", page_icon="🛡️", layout="wide")
...
st.title("IDX Scanner — Multibagger & Core Swing v6.9.0")
...
with st.sidebar:
    ...
    with st.expander("Time-Cycle Intelligence — Swing/Core & Multibagger", expanded=True):
        ...
        st.caption(
            "Implementasi clean-room ini tidak mengklaim formula proprietary asli. Mulai v6.9.0, astro tidak memiliki bobot produksi. EOFF memengaruhi timing hanya setelah cluster waktu, arah harga, dan chronological forward evidence lolos."
        )
...
with st.expander("Fallback manual laporan fundamental — biasanya tidak diperlukan"):
    st.caption(
        "Default v6.9.0 mengambil IDX/XBRL official-first, lalu Yahoo fallback/cross-check terbatas. Gunakan CSV hanya bila endpoint IDX sedang berubah/terblokir, "
...
if run_scan and uploaded is not None:
    ...
    result.update({
        ...
        "scanner_version": "6.9.0-confidence-database-foundation",
    })
...
if "scan_result" not in st.session_state:
    st.markdown(
        ...
        <div class="scanner-note">
          <b>Alur otomatis v6.9.0 Multibagger & Core Swing Focus</b><br>
...
with main_existing:
    ...
    with tab_audit:
        ...
        st.caption(
            ...
            "Schema target v2 memisahkan fakta laporan, snapshot model, proyek, ownership, EOFF, dan audit scan. "
            "Setelah Supabase dibuat, sinkronisasi tetap fail-soft dan tidak boleh menghentikan scanner."
        )
        db_col1, db_col2 = st.columns([1, 3])
        with db_col1:
            if st.button("Test koneksi database", key="database_health_check_v690"):
                st.session_state["database_health_report_v690"] = ScannerDatabaseBridge().health_check()
        with db_col2:
            health_report = st.session_state.get("database_health_report_v690")
    ...
    with tab_method:
        st.markdown(
            """
            ### Hirarki sinyal v6.9.0 Multibagger & Core Swing

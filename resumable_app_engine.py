...
def process_backfill_chunk(
    ...
    *,
    ...
    fundamentals = normalize_fundamental_classification(_mark_history_eligible(enrich_fundamentals_with_history(baseline, history)))
    ready_snapshot = set()
    ...
    sector_missing = set(fundamentals.loc[fundamentals.get("sector", pd.Series("UNKNOWN", index=fundamentals.index)).astype(str).str.upper().eq("UNKNOWN"), "ticker"].map(_ticker)) if not fundamentals.empty and "ticker" in fundamentals.columns else set()
    fallback = list(dict.fromkeys([ticker for ticker in tickers if ticker in sector_missing] + [ticker for ticker in tickers if ticker not in ready_snapshot]))[:fallback_limit]
    if fallback:
        live = fetch_resilient_fundamentals(fallback, cfg)
        fundamentals = normalize_fundamental_classification(_mark_history_eligible(enrich_fundamentals_with_history(_coalesce_primary_evidence(live, fundamentals), history)))
...
def finalize_daily_scan_job(
    ...
    *,
    ...
    try:
        bridge.persist_scan_job_artifacts_batch(job_id, provisional_artifacts, model_version=ENGINE_VERSION)
    except Exception:
        for artifact_type, payload in provisional_artifacts.items():
            bridge.persist_scan_job_artifact(job_id, artifact_type, payload, model_version=ENGINE_VERSION)
    try:
    ...
    requested_verify_cap = int(config.get("execution_verification_cap", 24) or 24)
    verification_cap = min(
        24,
        max(8, requested_verify_cap),
    ...
    try:
        bridge.persist_scan_job_artifacts_batch(job_id, safe_artifacts, model_version=ENGINE_VERSION)
    except Exception:
        for artifact_type, payload in safe_artifacts.items():
            bridge.persist_scan_job_artifact(job_id, artifact_type, payload, model_version=ENGINE_VERSION)
    return {
        "requested_tickers": len(universe),
        "processed_tickers": len(item_audit.get("processed_tickers", [])),

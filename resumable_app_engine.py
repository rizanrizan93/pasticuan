...
def _job_evidence(
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        event_future = pool.submit(bridge.read_narrative_events, universe, limit=max(1000, len(universe) * 20))
        outcome_future = pool.submit(bridge.read_narrative_event_outcomes, universe, limit=max(1000, len(universe) * 20))
        fundamentals, history, report = fundamental_future.result()
        market, news = aux_future.result()
        events = event_future.result()
        outcomes = outcome_future.result()
...
def finalize_daily_scan_job(
    ...
    *,
    ...
    decision_refresh_cap = min(evidence_cap, max(6, int(config.get("decision_evidence_cap", 12) or 12))) if evidence_cap else 0
    ...
    def _maintenance_priority(ticker: str) -> tuple[int, float, str]:
        ...
        if not row:
            ...
            bucket = 2
        elif counts.get(ticker, 0) < 2:
            bucket = 3
        elif not np.isfinite(age) or age > 210:
            bucket = 4
        else:
            bucket = 9
        return bucket, -(age if np.isfinite(age) else 9999.0), ticker
    ...
    if hasattr(bridge, "persist_scan_result"):
        try:
            ...
            result["database_sync_report"] = bridge.persist_scan_result(result, tables=(

...
def _future_fundamental_impact(pm: Mapping[str, Any], fund: Mapping[str, Any]) -> dict[str, Any]:
    ...
    confidence = 'LOW' if not ownership_known else 'HIGH' if quorum and coverage >= 75 and expected_revenue > 0 and expected_ebitda > 0 else 'MEDIUM' if coverage >= 50 and (expected_revenue > 0 or capex > 0) else 'LOW'
    if not ownership_known:
...
def scan_multibagger_candidates(prepared: Mapping[str, pd.DataFrame], fundamentals: pd.DataFrame | None, core_signals: pd.DataFrame | None=None, project_management: pd.DataFrame | None=None, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for ticker, frame in prepared.items():
        ...
        row = frame.iloc[-1]
        preliminary_accumulation = silent_accumulation_profile(frame)
        if coverage <= 0:
            rows.append({
                'ticker': ticker,
        ...
        accumulation_profile = silent_accumulation_profile(frame)
        ...
        # Research architecture v6.8.0: asset quality and entry timing are separate.
        ...
        elif total >= 82 and fundamental_score_10 >= 8.0 and fundamental_data_gate and grade_a_gate and forward_quality_gate and (adtv >= 1500000000) and (not severe_flags) and (general_solvency_gate or bank_prudential_gate):

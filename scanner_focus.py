...
def collect_automatic_forward_quality(
    ...
    usable_evidence = [frame.dropna(axis=1, how="all") for frame in evidence]
    return (
        pd.concat(usable_evidence, ignore_index=True, sort=False)
        if usable_evidence else pd.DataFrame(),
...
def scan_multibagger_candidates(prepared: Mapping[str, pd.DataFrame], fundamentals: pd.DataFrame | None, core_signals: pd.DataFrame | None=None, project_management: pd.DataFrame | None=None, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    if not result.empty:
        ...
        sort_score = 'confidence_adjusted_multibagger_score' if 'confidence_adjusted_multibagger_score' in result.columns else 'multibagger_score'
        result = result.sort_values(['_rank', sort_score, 'multibagger_score', 'adtv20_idr'], ascending=[True, False, False, False]).drop(columns='_rank').reset_index(drop=True)
...
def build_focus_order_builder(
    ...
    result=result.sort_values(['profit_conviction_score','silent_accumulation_score','target_quality_score','liquidity_score'],ascending=[False,False,False,False],kind='stable')
    ...
    if not result.empty:
        ...
        result = result.sort_values(
            ['final_score','_silent_rank','target_quality_score','liquidity_score'],
...
def build_focus_daily_board(
    ...
    if core_builder is not None and not core_builder.empty:
        score_col='final_score' if 'final_score' in core_builder else 'hybrid_conviction_score' if 'hybrid_conviction_score' in core_builder else 'profit_conviction_score'

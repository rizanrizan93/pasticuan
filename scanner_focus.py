...
def scan_multibagger_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        growth_quality = 100.0 * growth_score / 22.0
        profitability_quality = 100.0 * profitability_score / 18.0
        ...
        profitability_parts = [
            ...
            (gross_profitability_quality, 0.25),
            (gross_margin_quality, 0.10),
        ]
        profitability_parts = [
            (value, weight) for value, weight in profitability_parts if np.isfinite(value)
        ]
        profitability_pillar = (
            sum(value * weight for value, weight in profitability_parts)
            / sum(weight for _, weight in profitability_parts)
            if profitability_parts else 0.0
        )
        gross_growth_quality = _range_quality_score(gross_profit_growth, -0.05, 0.18)
        persistence_parts = [
            (growth_quality, 0.60),
            (100.0 * positive_earnings_ratio, 0.15),
            (100.0 * margin_stability, 0.10),
            (gross_growth_quality, 0.15),
        ]
        persistence_parts = [
            (value, weight) for value, weight in persistence_parts if np.isfinite(value)
        ]
        growth_persistence_pillar = (
            sum(value * weight for value, weight in persistence_parts)
            / sum(weight for _, weight in persistence_parts)
            if persistence_parts else growth_quality
        )
        cashflow_quality = 100.0 * earnings_quality_score / 18.0
        accrual_quality = _range_quality_score(accruals_to_assets, 0.10, -0.02)
        cashflow_pillar = (
            0.80 * cashflow_quality + 0.20 * accrual_quality
            if np.isfinite(accrual_quality) else cashflow_quality
        )
        balance_quality = 100.0 * balance_score / 12.0 if not is_financial else max(0.0, min(100.0, safe_number(fundamental_score_10, 0.0) * 10.0))
        leverage_trend_quality = _range_quality_score(
            leverage_change_yoy, 0.08, -0.04,
        )
        safety_pillar = (
            0.80 * balance_quality + 0.20 * leverage_trend_quality
            if np.isfinite(leverage_trend_quality) and not is_financial else
            balance_quality
        )
        valuation_quality = 100.0 * valuation_score / 8.0
        liquidity_quality = _multibagger_liquidity_quality(adtv)
        base_quality = (
            0.25 * growth_persistence_pillar
            + 0.25 * profitability_pillar
            + 0.20 * cashflow_pillar
            + 0.15 * safety_pillar
            + 0.10 * valuation_quality
        ...
        reinvestment_parts = [
            (roic_quality, 0.30),
            (project_score, 0.25 if project_coverage >= 25.0 else 0.10),
            (future_impact_score, 0.25 if impact_coverage > 0 else 0.10),
            (growth_persistence_pillar, 0.20),
        ]
        reinvestment_parts = [
            (value, weight) for value, weight in reinvestment_parts if np.isfinite(value)
        ]
        reinvestment_runway_pillar = (
            sum(value * weight for value, weight in reinvestment_parts)
            / sum(weight for _, weight in reinvestment_parts)
            if reinvestment_parts else 0.0
        )
        pillar_evidence = (
            any(np.isfinite(value) for value in (revenue_growth, earnings_growth, gross_profit_growth)),
            any(np.isfinite(value) for value in (roe, roa, gross_profitability)),
            any(np.isfinite(value) for value in (cash_conversion, positive_ocf_ratio, fcf, accruals_to_assets)),
            is_financial or any(np.isfinite(value) for value in (debt_equity, current_ratio, cash_to_debt, leverage_change_yoy)),
            np.isfinite(roic_proxy) or project_coverage >= 25.0 or impact_coverage > 0,
        )
        quality_pillar_coverage = 20.0 * sum(pillar_evidence)
        pillar_values = {
        ...
        elif total >= 82 and overall_confidence >= 70.0 and fundamental_score_10 >= 8.0 and fundamental_data_gate and grade_a_gate and quality_pillar_a_gate and forward_quality_gate and (adtv >= 1500000000) and (not severe_flags) and (general_solvency_gate or bank_prudential_gate):
            status = 'MULTIBAGGER_A_CANDIDATE'
        elif total >= 72 and overall_confidence >= 55.0 and fundamental_score_10 >= 7.0 and fundamental_data_grade in {'A', 'B', 'C'} and coverage >= 60 and quality_pillar_gate and (not severe_flags):
    ...
    if not result.empty:
        ...
        if eligible_sector.any():
            result.loc[eligible_sector, 'sector_relative_quality_score'] = (
                quality.loc[eligible_sector]
                .groupby(result.loc[eligible_sector, 'sector'])
        ...
        peer_quality = pd.to_numeric(
            result['sector_relative_quality_score'], errors='coerce',
        ).fillna(50.0)
        quality_for_selection = np.where(
            eligible_sector,
            0.90 * quality.fillna(0.0) + 0.10 * peer_quality,
            quality,
        )
        quality_for_selection = pd.Series(
            quality_for_selection, index=result.index, dtype=float,
        )
        ...
        # Strategic eligibility and Silent Accumulation decide selection.
        # Technical excess-return timing helps discriminate peers; an active
        # setup is intentionally absent from this score.
        result['multibagger_selection_score'] = np.where(
            quality.notna(),
            0.72 * quality_for_selection.fillna(0.0) + 0.20 * silent + 0.08 * timing_selector,
            np.nan,
        )
        result['multibagger_selection_score'] = pd.to_numeric(
            result['multibagger_selection_score'], errors='coerce',
        ).round(1)
        ...
        result['selected_reason'] = [
            ' • '.join(part for part in (fundamental, technical) if safe_text(part))
            or 'Belum ada alasan pemilihan yang cukup kuat'
            for fundamental, technical in zip(fundamental_reason, selector_reason)
        ]
        ...
        result['invalidation_reason'] = [
            ...
            else 'Tesis strategis ditinjau ulang bila kualitas fundamental, flow, atau tren memburuk'
            for value in stop
        ]
        ...
        result['primary_risk'] = [
            (safe_text(fundamental) or safe_text(technical) or 'Risiko utama belum terstruktur').split(' • ')[0]
            for fundamental, technical in zip(fundamental_risks, selector_risks)
        ]
        ...
        result = result.sort_values(
            ['_rank', 'multibagger_selection_score', 'silent_accumulation_score',
...
def _attach_selector_overlay(
    ...
    if 'ticker' not in frame or 'ticker' not in selector_ranking:
        return frame.copy()
    selector_columns = [
        'ticker', 'as_of', 'selection_rank', 'swing_selection_score',
...
def build_focus_order_builder(
    ...
    result=result.sort_values(
        ['core_priority_score','silent_accumulation_score','profit_conviction_score','target_quality_score','liquidity_score'],
    ...
    if not result.empty:
        ...
        result['_silent_rank'] = pd.to_numeric(result.get('silent_accumulation_score'), errors='coerce').fillna(0.0)
        result = result.sort_values(
        ...
        result['profit_rank'] = np.arange(1, len(result)+1)
        result['selected_reason'] = result.get(
            'selector_selected_reason', pd.Series('', index=result.index),
        ).fillna('').astype(str)
        ...
        result['primary_risk'] = [
            (safe_text(warning) or safe_text(risk) or 'Tidak ada risiko utama terstruktur').split(' • ')[0]
            for warning, risk in zip(result['warnings'], selection_risks)
        ]
    else:
...
def build_focus_daily_board(
    ...
    if multibagger is not None and not multibagger.empty:
        ...
        sort_cols=[score_col] + (['silent_accumulation_score'] if 'silent_accumulation_score' in multibagger else [])
...
def build_focus_screens(
    ...
    selector, selector_audit, selector_panel = build_cross_sectional_selector(
        prepared, selector_config, silent_profiles,
    )
    ...
    return {
        ...
        'daily_opportunities':build_focus_daily_board(core_radar,multibagger),

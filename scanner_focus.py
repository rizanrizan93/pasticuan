...
def scan_multibagger_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        persistence_parts = [
            ...
            (
                ...
                0.15 if fundamental_inflection_coverage >= 50.0 else 0.05,
            ),
        ]
        persistence_parts = [
    ...
    if not result.empty:
        ...
        result['growth_compounder_selection_score'] = np.where(
            growth_quality.notna(),
            0.72 * growth_for_selection.fillna(0.0)
            + 0.20 * effective_silent
            + 0.08 * timing_selector,
            np.nan,
        )
        result['turnaround_selection_score'] = np.where(
            turnaround_quality.notna(),
            0.65 * turnaround_for_selection.fillna(0.0)
            + 0.20 * effective_silent
            + 0.15 * timing_selector,
            np.nan,
        )
        lane = result.get(
            'multibagger_lane',
        ...
        result['selected_reason'] = [
            ' • '.join(
                part for part in (lane_summary, fundamental, technical)
                if safe_text(part)
            )
            or 'Belum ada alasan pemilihan yang cukup kuat'
            for lane_summary, fundamental, technical in zip(
                lane_reason, fundamental_reason, selector_reason,
            )
        ]
...
def build_multibagger_diagnostic_views(
    ...
    *,
    ...
    def boolean(name: str) -> pd.Series:
        ...
        return source.fillna(False).map(truthy)
    ...
    data_pending = ~snapshot_available
    scored_blocked = snapshot_available & ~research_eligible
    ...
    coverage_rows = [
        ...
        (
            'Menunggu snapshot fundamental', int(data_pending.sum()),
            'Belum boleh disimpulkan buruk dan tidak boleh diberi skor fundamental nol.',
        ),
    ]
    ...
    common_columns = [
        'ticker', 'near_miss_state', 'fundamental_score',
    ...
    pending_columns = [
        'ticker', 'data_refresh_priority_score',
        ...
        'near_miss_state', 'near_miss_reason', 'capital_state',
    ]
    ...
    def select_columns(local: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
        ...
        return local.loc[:, unique].head(max(1, int(limit))).reset_index(drop=True)

    return {
        'coverage': coverage_view,
        'gate_failures': gate_failures,
        'growth_near_miss': select_columns(growth_near_miss, growth_columns),
        'turnaround_near_miss': select_columns(
            turnaround_near_miss, turnaround_columns,
        ),
        'data_pending': select_columns(pending_view, pending_columns),
    }

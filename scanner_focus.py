...
FOCUS_PIPELINE_CACHE_VERSION = "7.16.0-persistent-core-indicator-signal-cache"
...
def allocate_multibagger_capital(candidates: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        emir_state = safe_text(row.get('emir_method_state')).upper()
        emir_available = 'emir_method_state' in out.columns and emir_state not in {'', 'DISABLED'}
        emir_eligible = (
            truthy(row.get('emir_method_production_eligible'))
            if emir_available else True
        )
        severe = (
            ...
            or emir_state == 'EMIR_FRAMEWORK_REJECT'
        )
        status = safe_text(row.get('multibagger_status')).upper()
        # Legacy callers that predate the evidence-contract column keep their
        # prior behaviour. Every v7.14.0 scanner result includes the column and
        # therefore fails closed when direct evidence is absent.
        evidence_rank_gate = (
        ...
        eligible = bool(
            ...
            and evidence_rank_gate
            and not severe
            and emir_eligible
        )
        cap_pct = _multibagger_tier_cap_pct(tier, cfg) if eligible else 0.0
        emir_cap = safe_number(row.get('emir_position_cap_pct'), np.nan)
        if eligible and emir_available and np.isfinite(emir_cap):
            cap_pct = min(cap_pct, max(0.0, emir_cap))
        ...
        emir_note = (
            f"; Emir layer {emir_state or 'UNAVAILABLE'}"
            f" cap {safe_number(row.get('emir_position_cap_pct'), 0.0):.1f}%"
            if emir_available else ''
        )
        out.at[idx, 'allocation_reason'] = f'{tier}; conviction {conviction:.1f}/100 (rule {rule_conviction:.1f}){ai_note}{cycle_note}{narrative_note}{emir_note}; strongest pillars: {strongest_text}'
        if eligible:
...
def _multibagger_confidence_profile(
    *, coverage: float, data_grade: str, reliability: str, official_verified: bool,
    ...
    eoff_weight = max(0.0, min(0.15, safe_number(eoff_confidence_weight, 0.0)))
...
def scan_multibagger_candidates(
    ...
    if not result.empty:
        ...
        emir_growth_adjustment = pd.to_numeric(
            result.get(
                'emir_growth_rank_adjustment',
                pd.Series(0.0, index=result.index),
            ),
            errors='coerce',
        ).fillna(0.0)
        emir_turnaround_adjustment = pd.to_numeric(
            result.get(
                'emir_turnaround_rank_adjustment',
                pd.Series(0.0, index=result.index),
            ),
            errors='coerce',
        ).fillna(0.0)
        ...
        result['multibagger_final_score_formula'] = (
            'BASE_QUALITY_PLUS_NARRATIVE_MAX_5_7_PLUS_EMIR_MAX_14_16_V7_13_0'
        )
...
def build_focus_order_builder(
    ...
    emir_swing_adjustment = pd.to_numeric(
        result.get(
            'emir_swing_rank_adjustment',
            pd.Series(0.0, index=result.index),
        ),
        errors='coerce',
    ).fillna(0.0)
    ...
    result['core_final_score_formula'] = (
        '70PCT_SELECTION_30PCT_RULE_OR_AI_PLUS_NARRATIVE_MAX5_PLUS_EMIR_MAX18_V7_9'
    )
    narrative_block = result.get(
        'narrative_hard_block',
        pd.Series(False, index=result.index),
    ).map(truthy)
    emir_state = result.get(
        'emir_method_state', pd.Series('', index=result.index),
    ).fillna('').astype(str).str.upper()
    emir_confirmed = result.get(
        'emir_method_production_eligible',
        pd.Series(False, index=result.index),
    ...
    if not result.empty:
        ...
        post_ai_narrative = pd.to_numeric(
            result.get(
                'narrative_swing_rank_adjustment',
                pd.Series(0.0, index=result.index),
            ),
        ...
        result['core_priority_score_pre_overlay'] = post_ai_base.round(1)
        result['core_narrative_contribution_points'] = post_ai_narrative.round(2)
        result['core_emir_contribution_points'] = post_ai_emir.round(2)
        result['core_priority_score'] = (
            post_ai_base + post_ai_narrative + post_ai_emir
        ).clip(0.0, 100.0).round(1)
        result['core_final_score_formula'] = (
            '70PCT_SELECTION_30PCT_HYBRID_PLUS_NARRATIVE_MAX5_PLUS_EMIR_MAX18_V7_9'
        )
...
def _build_focus_screens_uncached(
    ...
    if isinstance(core_radar, pd.DataFrame) and not core_radar.empty:
        ...
        core_radar['swing_selection_score'] = (
            base_swing + narrative_adjustment + emir_adjustment
        ).clip(0.0, 100.0).round(1)
    ...
    return {
        ...
        'multibagger_decision_summary':build_multibagger_decision_summary(multibagger),

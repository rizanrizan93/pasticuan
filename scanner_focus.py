...
def allocate_multibagger_capital(candidates: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        derived_quality = (
            ...
            + 0.22 * normalized['Cash-flow quality'] + 0.14 * solvency
            + 0.10 * normalized['Valuation']
        )
...
def scan_multibagger_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        base_quality = (
            0.28 * growth_quality
            + 0.22 * profitability_quality
            + 0.20 * cashflow_quality
        ...
        elif total >= 82 and overall_confidence >= 70.0 and fundamental_score_10 >= 8.0 and fundamental_data_gate and grade_a_gate and forward_quality_gate and (adtv >= 1500000000) and (not severe_flags) and (general_solvency_gate or bank_prudential_gate):
            status = 'MULTIBAGGER_A_CANDIDATE'
        elif total >= 72 and overall_confidence >= 55.0 and fundamental_score_10 >= 7.0 and fundamental_data_grade in {'A', 'B', 'C'} and coverage >= 60 and (not severe_flags):
    ...
    if not result.empty:
        ...
        result['multibagger_selection_score'] = np.where(
            quality.notna(),
            0.68 * quality.fillna(0.0) + 0.20 * silent + 0.12 * timing_selector,
            np.nan,
        )

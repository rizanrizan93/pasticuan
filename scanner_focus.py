...
from scanner import Any, BinaryIO, Iterable, Mapping, ScanConfig, ThreadPoolExecutor, read_csv_input, safe_number, safe_text, silent_accumulation_metrics, silent_accumulation_profile, truthy, as_completed, math, normalize_idx_ticker, np, pd, enrich_fundamentals_with_valuation
...
class LocalAIConfig:
    """Compatibility shell: AI is removed from the v8 production path."""

    def __init__(self, **kwargs):
        self.__dict__.update(kwargs)


def enrich_profit_ranking_with_ai(frame, **kwargs):
    out = frame.copy() if isinstance(frame, pd.DataFrame) else pd.DataFrame()
    if not out.empty:
        base = pd.to_numeric(
            out.get('profit_conviction_score', pd.Series(0.0, index=out.index)),
            errors='coerce',
        ).fillna(0.0)
        out['hybrid_conviction_score'] = base.round(1)
        out['ai_production_weight_pct'] = 0.0
        out['ai_state'] = 'REMOVED_FROM_V8_PRODUCTION'
    audit = pd.DataFrame([{
        'metric': 'AI production state',
        'value': 'REMOVED_FROM_V8_PRODUCTION',
        'production_weight_pct': 0.0,
    }])
    return out, audit


def enrich_multibagger_with_peer_ai(frame, **kwargs):
    out = frame.copy() if isinstance(frame, pd.DataFrame) else pd.DataFrame()
    if not out.empty:
        out['ai_production_weight_pct'] = 0.0
        out['ai_state'] = 'REMOVED_FROM_V8_PRODUCTION'
...
class TimeCycleConfig:
    """Compatibility shell: EOFF/time-cycle is removed from production."""

    def __init__(self, **kwargs):
        self.__dict__.update(kwargs)


def analyze_time_cycle(*args, **kwargs):
    return {
        'time_cycle_state': 'REMOVED_FROM_V8_PRODUCTION',
        'time_cycle_confidence': 0.0,
        'bullish_timing_score': 50.0,
        'continuation_timing_score': 50.0,
        'time_cycle_score': 50.0,
        'time_cycle_explanation': 'EOFF/time-cycle has zero production weight in v8.',
        'quick_buy_state': 'DISABLED',
        'quick_buy_action': 'WAIT_FOR_PRICE_STRUCTURE',
        'best_buy_date': '',
        'best_buy_confidence': 0.0,
        'eoff_state': 'REMOVED_FROM_V8_PRODUCTION',
        'eoff_public_validation_state': 'REMOVED',
        'eoff_public_directional_events': 0.0,
        'eoff_public_lift': 0.0,
    }


def setup_time_alignment(*args, **kwargs):
    return 50.0
...
FOCUS_PIPELINE_CACHE_VERSION = "8.0.2-slim-pure-evidence-entry-integrity"
...
def allocate_multibagger_capital(
    candidates: pd.DataFrame,
    config: ScanConfig | None = None,
) -> pd.DataFrame:
    """Allocate the Multibagger sleeve from the single v8 strategic score.

    V8 deliberately does not create a second capital-conviction model.  The
    strategic score already contains the four disjoint production pillars, so
    capital sizing may apply only hard-risk gates, concentration caps and lot
    rounding.  Re-scoring projects, management, flow, AI, Emir or time-cycle
    ...
    out = candidates.copy()
    defaults = {
        'rule_capital_conviction_score': np.nan,
        'capital_conviction_score': np.nan,
        'capital_tier': 'WATCH_ONLY',
        'capital_priority_score': np.nan,
        ...
        'allocation_reason': '',
        'multibagger_budget_idr': max(
            0.0, cfg.multibagger_capital_budget_idr,
        ),
        'multibagger_cash_reserve_idr': max(
            0.0, cfg.multibagger_capital_budget_idr,
        ),
        'production_ai_weight_pct': 0.0,
        'production_eoff_weight_pct': 0.0,
        'production_legacy_overlay_weight_pct': 0.0,
    }
    ...
    minimum = max(
        0.0,
        min(100.0, cfg.multibagger_min_capital_conviction),
    )
    ...
    for idx, row in out.iterrows():
        conviction = safe_number(score_series.loc[idx], np.nan)
        if not np.isfinite(conviction):
            out.at[idx, 'allocation_action'] = 'WAIT_FOR_V8_SCORE'
            out.at[idx, 'allocation_reason'] = (
                'V8 strategic score unavailable; capital is not allocated.'
            )
            continue
        tier, tier_multiplier = _multibagger_capital_tier(row, conviction)
        status = safe_text(row.get('multibagger_status')).upper()
        rank_gate = (
            truthy(row.get('multibagger_rank_eligible'))
            if 'multibagger_rank_eligible' in out.columns else True
        )
        red_text = safe_text(row.get('red_flags')).upper()
        severe_tokens = (
            'OCF NEGATIF', 'MARGIN BERSIH NEGATIF', 'DER TINGGI',
            'DILUSI TINGGI',
        )
        severe = bool(
            ...
            or bool(safe_text(row.get('management_governance_flags')).strip())
            or safe_text(row.get('management_related_party_risk')).upper()
            == 'CRITICAL'
            or 'CRITICAL' in safe_text(
                row.get('project_execution_flags'),
            ).upper()
        ...
        cap_pct = (
            _multibagger_tier_cap_pct(tier, cfg) if eligible else 0.0
        )
        out.at[idx, 'rule_capital_conviction_score'] = round(conviction, 1)
        out.at[idx, 'capital_conviction_score'] = round(conviction, 1)
        out.at[idx, 'capital_tier'] = tier
        out.at[idx, 'capital_priority_score'] = round(conviction, 1)
        out.at[idx, 'allocation_eligible'] = eligible
        out.at[idx, 'allocation_cap_pct'] = round(cap_pct, 1)
        out.at[idx, 'production_ai_weight_pct'] = 0.0
        out.at[idx, 'production_eoff_weight_pct'] = 0.0
        out.at[idx, 'production_legacy_overlay_weight_pct'] = 0.0
        out.at[idx, 'allocation_reason'] = (
            f"{tier}; v8 strategic score {conviction:.1f}/100; "
            f"fundamental {safe_number(row.get('v8_fundamental_future_score'), 50.0):.1f}; "
            f"narrative/alignment {safe_number(row.get('v8_narrative_alignment_score'), 50.0):.1f}; "
            f"market/sector {safe_number(row.get('v8_market_sector_score'), 50.0):.1f}; "
            f"flow {safe_number(row.get('v8_flow_score'), 50.0):.1f}."
        )
        if eligible:
            eligible_rows.append((conviction, idx))
            raw_weights[idx] = (
                max(1.0, conviction - 55.0) ** 2 * tier_multiplier
            )
            caps[idx] = cap_pct
    ...
    raw_weights = {
        idx: value for idx, value in raw_weights.items() if idx in selected
    }
    caps = {idx: value for idx, value in caps.items() if idx in selected}
    ...
    ranked_indices = sorted(
        selected,
        key=lambda idx: safe_number(
            out.at[idx, 'capital_priority_score'], 0.0,
        ),
        reverse=True,
    )
    for rank, idx in enumerate(ranked_indices, start=1):
        ...
        deploy = (
            target_weight
            if state in {'ACCUMULATE_NOW', 'STARTER_NOW'} else 0.0
        )
        action = (
            'ALLOCATE_LARGEST'
            if rank == 1 and deploy > 0
            else 'ALLOCATE_NOW'
            if deploy > 0
            else 'WAIT_ENTRY_ZONE'
        )
    ...
    budget = max(
        0.0, safe_number(cfg.multibagger_capital_budget_idr, 0.0),
    )
    deployed_value = 0.0
    for idx, row in out.iterrows():
        ...
        target_amount = (
            budget
            * safe_number(row.get('strategic_target_weight_pct'), 0.0)
            / 100.0
        )
        ...
        lots = (
            int(math.floor(proposed / (reference * 100.0)))
            if proposed > 0 and np.isfinite(reference) and reference > 0
            else 0
        )
        ...
        out.at[idx, 'strategic_target_amount_idr'] = round(
            target_amount, 0,
        )
    ...
    out['multibagger_cash_reserve_idr'] = round(
        max(0.0, budget - deployed_value), 0,
    )
...
def _multibagger_confidence_profile(
    *, coverage: float, data_grade: str, reliability: str, official_verified: bool,
    ...
    eoff_confidence_weight: float=0.0,
) -> dict[str, Any]:
    ...
    # EOFF/time-cycle is a shadow diagnostic when its configured production
    # weight is zero. It must not silently penalise or promote asset-quality
    # ranking through the confidence channel.
    eoff_weight = 0.0
    total_weight = 0.40 + 0.20 + 0.25 + eoff_weight
    overall = max(0.0, min(100.0, (
        0.40 * fundamental_confidence
        + 0.20 * future_confidence
        + 0.25 * technical_confidence
        + eoff_weight * eoff_confidence
    ...
    return {
        ...
        'eoff_confidence_effective_weight_pct': round(100.0 * eoff_weight, 1),
...
def _sort_multibagger_ranking_contract(frame: pd.DataFrame) -> pd.DataFrame:
    """Apply the hard-gate → score → silent-flow → execution rank contract."""
    if frame is None or frame.empty:
        return frame.copy() if isinstance(frame, pd.DataFrame) else pd.DataFrame()
    out = frame.copy()
    qualified = out.get(
        'multibagger_status', pd.Series('', index=out.index),
    ).fillna('').astype(str).str.upper().isin({
        'MULTIBAGGER_A_CANDIDATE', 'MULTIBAGGER_B_CANDIDATE',
        'MULTIBAGGER_WATCHLIST',
    })
    research = out.get(
        'research_eligible', pd.Series(False, index=out.index),
    ).map(truthy)
    direct_eligible = out.get(
        'multibagger_rank_eligible', pd.Series(False, index=out.index),
    ).map(truthy)
    proxy_eligible = out.get(
        'multibagger_proxy_rank_eligible', pd.Series(False, index=out.index),
    ).map(truthy)
    hard_block = out.get(
        'narrative_hard_block', pd.Series(False, index=out.index),
    ).map(truthy)
    # A NOT_QUALIFIED row can never jump ahead merely because its evidence
    # coverage is high. Evidence completeness and investment qualification are
    # separate gates; score orders rows only after those gates.
    out['_hard_eligibility_rank'] = np.select(
        [
            hard_block,
            direct_eligible & qualified,
            proxy_eligible & qualified,
            research & qualified,
            qualified,
            research,
        ],
    ...
    sort_columns = [
        '_hard_eligibility_rank', 'multibagger_selection_score',
        'effective_silent_accumulation_score', 'execution_readiness_score',
    ...
    if 'adtv20_idr' in out:
        sort_columns.append('adtv20_idr')
    ...
    ).drop(columns=['_hard_eligibility_rank']).reset_index(drop=True)
    # Rows retained only for universe completeness must stay visibly unscored.
    # Giving them a numeric tail rank would imply a comparison the model never
    # made and could leak into downstream exports or user-defined filters.
    out['multibagger_selection_rank'] = np.nan
    scored_indices = out.index[
        pd.to_numeric(
            out.get(
                'multibagger_selection_score',
                pd.Series(np.nan, index=out.index),
    ...
    out.loc[scored_indices, 'multibagger_selection_rank'] = np.arange(
        1, len(scored_indices) + 1,
    )
    out['multibagger_production_rank'] = np.nan
    post_qualified = out.get(
        'multibagger_status', pd.Series('', index=out.index),
    ).fillna('').astype(str).str.upper().isin({
        'MULTIBAGGER_A_CANDIDATE', 'MULTIBAGGER_B_CANDIDATE',
        'MULTIBAGGER_WATCHLIST',
    })
    post_research = out.get(
        'research_eligible', pd.Series(False, index=out.index),
    ).map(truthy)
    post_block = out.get(
        'narrative_hard_block', pd.Series(False, index=out.index),
    ...
    production_indices = out.index[
        post_direct & post_qualified & post_research & ~post_block
    ].tolist()
    out.loc[production_indices, 'multibagger_production_rank'] = np.arange(
        1, len(production_indices) + 1,
    )
    out['multibagger_proxy_research_rank'] = np.nan
    post_proxy = out.get(
        'multibagger_proxy_rank_eligible', pd.Series(False, index=out.index),
    ).map(truthy)
    proxy_indices = out.index[
        post_proxy & post_qualified & post_research & ~post_block
    ].tolist()
    out.loc[proxy_indices, 'multibagger_proxy_research_rank'] = np.arange(
        1, len(proxy_indices) + 1,
    )
    out['growth_compounder_rank'] = np.nan
    out['turnaround_rank'] = np.nan
    lane = out.get(
        'multibagger_lane', pd.Series('', index=out.index),
    ).fillna('').astype(str)
    research = out.get(
        'research_eligible', pd.Series(False, index=out.index),
    ).map(truthy)
    growth_indices = out.index[research & lane.eq('GROWTH_COMPOUNDER')].tolist()
    turnaround_indices = out.index[
        research & lane.eq('TURNAROUND_CYCLICAL')
    ].tolist()
    out.loc[growth_indices, 'growth_compounder_rank'] = np.arange(
        1, len(growth_indices) + 1,
    )
    out.loc[turnaround_indices, 'turnaround_rank'] = np.arange(
        1, len(turnaround_indices) + 1,
    )
    return out


def _qualified_multibagger_lane(
    current_lane: Any,
    *,
    growth_research_eligible: bool,
    turnaround_research_eligible: bool,
) -> str:
    if turnaround_research_eligible and not growth_research_eligible:
        return 'TURNAROUND_CYCLICAL'
    if growth_research_eligible and not turnaround_research_eligible:
        return 'GROWTH_COMPOUNDER'
    return safe_text(current_lane).upper() or 'GROWTH_COMPOUNDER'


def _direct_broker_flow_mask(frame: pd.DataFrame) -> pd.Series:
    """Return direct-flow eligibility only for provenance-verified evidence."""
    coverage = pd.to_numeric(
        frame.get(
            'broker_summary_coverage_pct',
    ...
    direct = frame.get(
        'broker_summary_direct_verified',
        pd.Series(False, index=frame.index),
    ).map(truthy)
    return (
        coverage.ge(55.0)
        & direct
        & mode.eq('DIRECT_BROKER_SUMMARY_VERIFIED')
    )


def _insufficient_history_multibagger_row(
    ticker: str,
    frame: pd.DataFrame | None,
    fundamental: Mapping[str, Any] | None,
    *,
    required_bars: int=220,
) -> dict[str, Any]:
    """Keep an undersized listing visible without manufacturing a score."""
    fund = dict(fundamental or {})
    bars = len(frame) if isinstance(frame, pd.DataFrame) else 0
    latest = (
        frame.iloc[-1]
    ...
    reason = f'INSUFFICIENT_TECHNICAL_HISTORY_{bars}_OF_{required_bars}_BARS'
    return {
        'ticker': ticker,
        'multibagger_status': 'DATA_NOT_SCORED',
        'multibagger_scoring_state': 'DATA_NOT_SCORED_INSUFFICIENT_TECHNICAL_HISTORY',
        'multibagger_lane': 'UNCLASSIFIED',
        'research_eligible': False,
        'growth_research_eligible': False,
        'turnaround_research_eligible': False,
        'turnaround_research_state': 'DATA_PENDING_TECHNICAL_HISTORY',
        'research_eligibility_reason': reason,
        'multibagger_score': np.nan,
        'multibagger_quality_score': np.nan,
        'confidence_adjusted_multibagger_score': np.nan,
        'turnaround_recovery_score': np.nan,
        'multibagger_score_reason': (
            f'Histori harga hanya {bars} bar; minimum {required_bars}. '
            'Ticker dipertahankan di audit, tetapi tidak diberi skor.'
        ),
        ...
        'quality_pillar_coverage_pct': 0.0,
        'quality_pillars_strong': 0,
        'quality_pillars_critical': 0,
        'quality_pillar_gate': False,
        'quality_pillar_a_gate': False,
        'fundamental_score': fund.get('fundamental_score', np.nan),
        'fundamental_score_10': fund.get('fundamental_score_10', np.nan),
        ...
        'fundamental_complete_for_multibagger': truthy(
            fund.get('fundamental_complete_for_multibagger', False),
        ),
        'fundamental_acquisition_state': safe_text(
            fund.get('fundamental_acquisition_state'),
        ) or 'UNKNOWN',
        'fundamental_missing_core_fields': safe_text(
            fund.get('fundamental_missing_core_fields'),
        ),
        'technical_history_bars': bars,
        'technical_history_required_bars': required_bars,
        'technical_history_coverage_pct': round(
            100.0 * min(1.0, bars / max(required_bars, 1)), 1,
        ),
        'technical_history_complete': False,
        'technical_history_state': 'INSUFFICIENT_HISTORY',
        'last_price': safe_number(latest.get('Close'), np.nan),
        'adtv20_idr': safe_number(latest.get('ADTV20'), 0.0),
        'silent_accumulation_score': np.nan,
        'silent_accumulation_confidence': 0.0,
        'silent_accumulation_state': 'NOT_SCORED_INSUFFICIENT_HISTORY',
        'overall_research_confidence': 0.0,
        'overall_research_confidence_grade': 'VERY_LOW',
        'technical_entry_state': 'INSUFFICIENT_HISTORY',
        'top_positive_drivers': '',
        'top_negative_drivers': f'Histori teknikal {bars}/{required_bars} bar',
        'scoring_reason_codes': reason,
        'selected_reason': 'Data teknikal belum cukup untuk ranking yang sebanding',
        'primary_risk': 'Listing/history terlalu pendek untuk validasi teknikal',
        'capital_tier': 'WATCH_ONLY',
        'allocation_action': 'WAIT_FOR_TECHNICAL_HISTORY',
        'allocation_reason': 'Tidak dialokasikan sampai minimum histori terpenuhi.',
        'compounding_state': 'WAIT_FOR_EVIDENCE',
        'review_action': 'REFRESH_AFTER_MORE_COMPLETED_SESSIONS',
        'note': 'N/A means not scored; it does not mean issuer quality is zero.',
    }
...
def scan_multibagger_candidates(
    ...
    for ticker, frame in prepared.items():
        ...
        if frame is None or frame.empty or len(frame) < 220:
            rows.append(_insufficient_history_multibagger_row(
                ticker, frame, fund, required_bars=220,
            ))
            continue
        ...
        if coverage <= 0:
            rows.append({
                ...
                'fundamental_acquisition_state': safe_text(fund.get('fundamental_acquisition_state')) or 'UNAVAILABLE_NOT_SCORED',
                'fundamental_missing_core_fields': safe_text(fund.get('fundamental_missing_core_fields')) or 'ALL_CORE_FIELDS_PENDING',
                'fundamental_statement_family_coverage_pct': safe_number(fund.get('fundamental_statement_family_coverage_pct'), 0.0),
                'fundamental_history_period_coverage_pct': safe_number(fund.get('fundamental_history_period_coverage_pct'), 0.0),
                'fundamental_income_statement_coverage_pct': safe_number(fund.get('fundamental_income_statement_coverage_pct'), 0.0),
                'fundamental_balance_sheet_coverage_pct': safe_number(fund.get('fundamental_balance_sheet_coverage_pct'), 0.0),
                'fundamental_cashflow_statement_coverage_pct': safe_number(fund.get('fundamental_cashflow_statement_coverage_pct'), 0.0),
        ...
        trailing_pe = safe_number(fund.get('trailing_pe'), np.nan)
        earnings_yield = safe_number(fund.get('earnings_yield'), np.nan)
        price_to_book = safe_number(fund.get('price_to_book'), np.nan)
        ev_ebitda = safe_number(fund.get('ev_ebitda'), np.nan)
        valuation_data_coverage = safe_number(
            fund.get('valuation_data_coverage_pct'), 0.0,
        )
        valuation_score_eligible = truthy(
            fund.get('valuation_score_eligible', False)
        )
        valuation_production_eligible = truthy(
            fund.get('valuation_production_eligible', False)
        )
        valuation_state = safe_text(fund.get('valuation_state')) or 'VALUATION_UNAVAILABLE'
        valuation_confidence = safe_text(fund.get('valuation_confidence')) or 'VERY_LOW'
        fundamental_reliability = safe_text(fund.get('fundamental_reliability')).upper() or 'UNKNOWN'
        fundamental_data_grade = safe_text(fund.get('fundamental_data_grade')).upper() or 'D'
        ...
        statement_family_coverage = safe_number(fund.get('fundamental_statement_family_coverage_pct'), 0.0)
        period_coverage = safe_number(fund.get('fundamental_history_period_coverage_pct'), 0.0)
        fundamental_complete_for_multibagger = truthy(fund.get('fundamental_complete_for_multibagger', False))
        fundamental_acquisition_state = safe_text(fund.get('fundamental_acquisition_state')) or (
            'COMPLETE_FOR_MULTIBAGGER' if fundamental_complete_for_multibagger else 'PARTIAL_NOT_PRODUCTION_ELIGIBLE'
        )
        income_statement_coverage = safe_number(fund.get('fundamental_income_statement_coverage_pct'), 0.0)
        balance_sheet_coverage = safe_number(fund.get('fundamental_balance_sheet_coverage_pct'), 0.0)
        cashflow_statement_coverage = safe_number(fund.get('fundamental_cashflow_statement_coverage_pct'), 0.0)
        missing_core_fields = safe_text(fund.get('fundamental_missing_core_fields'))
        ...
        peg_quality = (
            ...
            else 65.0 if np.isfinite(peg) and 1.5 < peg <= 2.5
            else 25.0 if np.isfinite(peg) and 2.5 < peg <= 3.5
            else 0.0 if np.isfinite(peg) else np.nan
        )
        fcf_yield_quality = (
            100.0 if np.isfinite(fcf_yield) and fcf_yield >= 0.08
            else 70.0 if np.isfinite(fcf_yield) and fcf_yield >= 0.04
            else 30.0 if np.isfinite(fcf_yield) and fcf_yield > 0.0
            else 0.0 if np.isfinite(fcf_yield) else np.nan
        )
        earnings_yield_quality = (
            100.0 if np.isfinite(earnings_yield) and earnings_yield >= 0.08
            else 70.0 if np.isfinite(earnings_yield) and earnings_yield >= 0.05
            else 30.0 if np.isfinite(earnings_yield) and earnings_yield > 0.0
            else 0.0 if np.isfinite(earnings_yield) else np.nan
        )
        ev_ebitda_quality = (
            100.0 if np.isfinite(ev_ebitda) and 0.0 < ev_ebitda <= 6.0
            else 70.0 if np.isfinite(ev_ebitda) and ev_ebitda <= 10.0
            else 30.0 if np.isfinite(ev_ebitda) and ev_ebitda <= 15.0
            else 10.0 if np.isfinite(ev_ebitda) and ev_ebitda > 0.0
            else np.nan
        )
        if is_financial:
            price_to_book_quality = (
                100.0 if np.isfinite(price_to_book) and 0.0 < price_to_book <= 1.2
                else 70.0 if np.isfinite(price_to_book) and price_to_book <= 2.0
                else 30.0 if np.isfinite(price_to_book) and price_to_book <= 3.0
                else 10.0 if np.isfinite(price_to_book) and price_to_book > 0.0
                else np.nan
            )
            valuation_components = (
                (earnings_yield_quality, 3.5, float(valuation_score_eligible and np.isfinite(earnings_yield))),
                (price_to_book_quality, 3.5, float(valuation_score_eligible and np.isfinite(price_to_book))),
                (
                    peg_quality, 1.5,
                    float(valuation_score_eligible and np.isfinite(peg) and peg > 0.0),
                ),
            )
        else:
            price_to_book_quality = (
                100.0 if np.isfinite(price_to_book) and 0.0 < price_to_book <= 1.5
                else 60.0 if np.isfinite(price_to_book) and price_to_book <= 3.0
                else 25.0 if np.isfinite(price_to_book) and price_to_book <= 5.0
                else 10.0 if np.isfinite(price_to_book) and price_to_book > 0.0
                else np.nan
            )
            valuation_components = (
                (earnings_yield_quality, 2.0, float(valuation_score_eligible and np.isfinite(earnings_yield))),
                (fcf_yield_quality, 2.5, float(valuation_score_eligible and np.isfinite(fcf_yield))),
                (ev_ebitda_quality, 2.0, float(valuation_score_eligible and np.isfinite(ev_ebitda))),
                (
                    peg_quality, 1.5,
                    float(valuation_score_eligible and np.isfinite(peg) and peg > 0.0),
                ),
                (price_to_book_quality, 1.0, float(valuation_score_eligible and np.isfinite(price_to_book))),
            )
        (
            valuation_quality,
            valuation_metric_coverage,
            valuation_quality_raw,
        ) = _coverage_adjusted_component(valuation_components)
        valuation_score = (
        ...
        timing_globally_enabled = bool(
            getattr(cfg, 'time_cycle_enabled', True)
        )
        timing_production_weight = max(
            0.0,
            safe_number(
                getattr(cfg, 'time_cycle_multibagger_max_weight', 0.0),
                0.0,
            ),
        )
        full_timing_requested = bool(
            timing_globally_enabled
            and (
                bool(getattr(
                    cfg, 'multibagger_time_cycle_full_refresh_enabled', False,
                ))
                or timing_production_weight > 0.0
            )
        )
        run_full_timing = bool(
            full_timing_requested
            and (
                total >= 55.0
                or (
        ...
        time_cycle = analyze_time_cycle(
            ...
            'time_cycle_state': (
                'DISABLED'
                if not timing_globally_enabled
                else 'SKIPPED_ZERO_MULTIBAGGER_PRODUCTION_WEIGHT'
                if not full_timing_requested
                else 'SKIPPED_LOW_MULTIBAGGER_QUALITY'
            ),
            'time_cycle_confidence': 0.0,
            'bullish_timing_score': 50.0,
            'continuation_timing_score': 50.0,
            'time_cycle_explanation': (
                'Full time-cycle/EOFF disabled for Multibagger daily scans '
                'because its production weight is zero; enable explicit full '
                'refresh only for shadow research.'
                if timing_globally_enabled and not full_timing_requested
                else 'Full time-cycle/EOFF deferred because strategic quality '
                'is below the research threshold.'
                if timing_globally_enabled
                else 'Time-cycle/EOFF disabled by configuration.'
            ),
        }
        ...
        fundamental_data_gate = bool(
            ...
            and history_coverage >= 55.0
            and fundamental_complete_for_multibagger
            and statement_family_coverage >= 65.0
            and period_coverage >= 75.0
        )
        grade_a_gate = bool(
        ...
        confidence_profile = _multibagger_confidence_profile(
            ...
            eoff_confidence_weight=min(0.15, timing_production_weight),
        )
        ...
        elif total >= 72 and overall_confidence >= 55.0 and fundamental_score_10 >= 7.0 and fundamental_data_grade in {'A', 'B', 'C'} and coverage >= 60 and metric_data_gate and fundamental_complete_for_multibagger and quality_pillar_gate and (not severe_flags):
        ...
        # A descriptive archetype can explain the issuer, but only a passed
        # lane gate decides which score enters ranking.
        archetype_profile['multibagger_lane'] = _qualified_multibagger_lane(
            archetype_profile.get('multibagger_lane'),
            growth_research_eligible=growth_research_eligible,
            turnaround_research_eligible=turnaround_research_eligible,
        )
        ...
        rows[-1].update({
            ...
            **archetype_profile,
            ...
            'time_cycle_evaluation_mode': (
                'FULL_CANDIDATE' if run_full_timing
                else 'DISABLED' if not timing_globally_enabled
                else 'DEFERRED_ZERO_PRODUCTION_WEIGHT'
                if not full_timing_requested
                else 'DEFERRED_LOW_QUALITY'
            ),
            'technical_history_bars': len(frame),
            'technical_history_required_bars': 220,
            'technical_history_coverage_pct': 100.0,
            'technical_history_complete': True,
            'technical_history_state': 'COMPLETE',
    ...
    if not result.empty:
        ...
        result['sector_relative_quality_score'] = np.nan
        result['turnaround_sector_relative_score'] = np.nan
        growth_sector = (
        ...
        # V8 never adds legacy narrative or composite Emir adjustments.
        # Their evidence is scored once inside the disjoint production pillars.
        growth_narrative_adjustment = pd.Series(0.0, index=result.index)
        turnaround_narrative_adjustment = pd.Series(0.0, index=result.index)
        emir_growth_adjustment = pd.Series(0.0, index=result.index)
        emir_turnaround_adjustment = pd.Series(0.0, index=result.index)
        ...
        result['growth_narrative_contribution_points'] = growth_narrative_adjustment.round(2)
        result['growth_emir_contribution_points'] = emir_growth_adjustment.round(2)
        result['turnaround_narrative_contribution_points'] = turnaround_narrative_adjustment.round(2)
        result['turnaround_emir_contribution_points'] = emir_turnaround_adjustment.round(2)
        result['growth_compounder_selection_score'] = (
            pd.to_numeric(
                result['growth_compounder_selection_score'],
                errors='coerce',
        ...
        result['turnaround_selection_score'] = (
            ...
            + emir_turnaround_adjustment
        ).clip(0.0, 100.0)
        result['multibagger_final_score_formula'] = (
            'LEGACY_BASE_ONLY_V8_OVERLAYS_DISABLED'
        )
        fundamental_complete = result.get(
            'fundamental_complete_for_multibagger',
            pd.Series(False, index=result.index),
        ).map(truthy)
        technical_history_complete = result.get(
            'technical_history_complete',
            pd.Series(False, index=result.index),
        ).map(truthy)
        technical_history_coverage = pd.to_numeric(
            result.get(
                'technical_history_coverage_pct',
                pd.Series(0.0, index=result.index),
            ),
            errors='coerce',
        ).fillna(0.0)
        narrative_coverage = pd.to_numeric(
            result.get('narrative_evidence_coverage_pct', pd.Series(0.0, index=result.index)),
            errors='coerce',
        ).fillna(0.0)
        alignment_coverage = pd.to_numeric(
            result.get('issuer_alignment_coverage_pct', pd.Series(0.0, index=result.index)),
            errors='coerce',
        ).fillna(0.0)
        emir_coverage = pd.to_numeric(
            result.get('emir_method_coverage_pct', pd.Series(0.0, index=result.index)),
            errors='coerce',
        ).fillna(0.0)
        narrative_state = result.get(
            'narrative_score_state', pd.Series('', index=result.index),
        ).fillna('').astype(str).str.upper()
        full_narrative = (
            narrative_coverage.ge(35.0)
            & ~narrative_state.str.contains('PARTIAL|NOT_SCORED', regex=True)
        )
        proxy_complete = (
            fundamental_complete
            & technical_history_complete
            & full_narrative
            & alignment_coverage.ge(30.0)
            & emir_coverage.ge(55.0)
        )
        narrative_mode = result.get(
            'narrative_evidence_mode', pd.Series('', index=result.index),
        ).fillna('').astype(str).str.upper()
        active_event_count = pd.to_numeric(
            result.get('narrative_active_event_count', pd.Series(0.0, index=result.index)),
            errors='coerce',
        ).fillna(0.0)
        official_event_count = pd.to_numeric(
            result.get('narrative_official_event_count', pd.Series(0.0, index=result.index)),
            errors='coerce',
        ).fillna(0.0)
        items_reviewed = pd.to_numeric(
            result.get('narrative_items_reviewed', pd.Series(0.0, index=result.index)),
            errors='coerce',
        ).fillna(0.0)
        news_complete = result.get(
            'narrative_news_collection_state', pd.Series('', index=result.index),
        ).fillna('').astype(str).str.upper().eq('COMPLETE')
        source_event_mode = narrative_mode.isin({
            'SOURCE_EVENT', 'EVENT_PLUS_STRUCTURED_FINANCIAL',
            'EVENT_PLUS_PARTIAL_STRUCTURED_FINANCIAL',
        })
        direct_event_verified = (
            source_event_mode
            & active_event_count.ge(1.0)
            & (official_event_count.ge(1.0) | (news_complete & items_reviewed.ge(1.0)))
        )
        structured_production = result.get(
            'structured_financial_production_eligible',
            pd.Series(False, index=result.index),
        ).map(truthy)
        direct_alignment = result.get(
            'issuer_alignment_score_state', pd.Series('', index=result.index),
        ).fillna('').astype(str).str.upper().eq('SCORED_DIRECT_AND_PROXY')
        broker_coverage = pd.to_numeric(
            result.get('broker_summary_coverage_pct', pd.Series(0.0, index=result.index)),
            errors='coerce',
        ).fillna(0.0)
        direct_flow_verified = _direct_broker_flow_mask(result)
        emir_production = result.get(
        ...
        direct_evidence_complete = (
            ...
            & direct_alignment
            & direct_flow_verified
            & emir_production
        )
        any_research_evidence = (
            fundamental_complete
            | narrative_coverage.gt(0.0)
            | alignment_coverage.gt(0.0)
            | emir_coverage.gt(0.0)
        )
        partial_evidence = any_research_evidence & ~proxy_complete
        result['multibagger_evidence_class'] = np.select(
            [direct_evidence_complete, proxy_complete, partial_evidence],
            [
                'DIRECT_EVIDENCE_RANKING', 'FULL_PROXY_COVERAGE',
                'PARTIAL_EVIDENCE_RESEARCH',
            ],
            default='DATA_PENDING',
        )
        qualified_status = result.get(
            'multibagger_status', pd.Series('', index=result.index),
        ).fillna('').astype(str).str.upper().isin({
            'MULTIBAGGER_A_CANDIDATE', 'MULTIBAGGER_B_CANDIDATE',
            'MULTIBAGGER_WATCHLIST',
        })
        research_eligible = result.get(
            'research_eligible', pd.Series(False, index=result.index),
        ).map(truthy)
        narrative_block = result.get(
            'narrative_hard_block', pd.Series(False, index=result.index),
        ).map(truthy)
        result['multibagger_direct_event_verified'] = direct_event_verified
        result['multibagger_direct_flow_verified'] = direct_flow_verified
        result['multibagger_direct_alignment_verified'] = direct_alignment
        result['multibagger_direct_evidence_complete'] = direct_evidence_complete
        result['multibagger_proxy_evidence_complete'] = proxy_complete
        result['multibagger_rank_eligible'] = (
            direct_evidence_complete & research_eligible
            & qualified_status & ~narrative_block
        )
        result['multibagger_proxy_rank_eligible'] = (
            proxy_complete & research_eligible
            & qualified_status & ~narrative_block
        )
        result['multibagger_proxy_comparability_pct'] = np.minimum.reduce([
            np.where(fundamental_complete, 100.0, 0.0),
            technical_history_coverage.to_numpy(dtype=float),
            narrative_coverage.to_numpy(dtype=float),
            alignment_coverage.to_numpy(dtype=float),
            emir_coverage.to_numpy(dtype=float),
        ]).round(1)
        result['multibagger_score_comparability_pct'] = np.minimum.reduce([
            np.where(fundamental_complete, 100.0, 0.0),
            technical_history_coverage.to_numpy(dtype=float),
            np.where(direct_event_verified, 100.0, 0.0),
            np.where(direct_alignment, alignment_coverage, 0.0),
            broker_coverage.to_numpy(dtype=float),
            np.where(emir_production, emir_coverage, 0.0),
        ]).round(1)
        lane = result.get(
            'multibagger_lane',
        ...
        # v8 owns the only production score. Legacy narrative/Emir additive
        # overlays are retained as diagnostics above but are overwritten here.
        result = apply_multibagger_production_scoring(result)
        # V8 eligibility is based on one score-coverage contract. Direct
        # narrative, broker, Emir and time-cycle gates no longer duplicate or
        # veto evidence that has already been represented in the score.
        v8_coverage = pd.to_numeric(
            result.get(
                'v8_production_score_coverage_pct',
                pd.Series(0.0, index=result.index),
            ),
            errors='coerce',
        ).fillna(0.0)
        v8_min_coverage = max(
            ...
            min(
                90.0,
                safe_number(
                    getattr(cfg, 'multibagger_min_data_coverage_pct', 65.0),
                    65.0,
                ),
            ),
        )
        result['multibagger_score_comparability_pct'] = v8_coverage.round(1)
        result['multibagger_proxy_comparability_pct'] = v8_coverage.round(1)
        result['multibagger_rank_eligible'] = (
            result.get(
                'research_eligible', pd.Series(False, index=result.index),
            ).map(truthy)
            ...
            & v8_coverage.ge(v8_min_coverage)
            & ~result.get(
        ...
        result['multibagger_proxy_rank_eligible'] = (
            ...
            & v8_coverage.ge(max(50.0, v8_min_coverage - 15.0))
            & ~result.get(
                'narrative_hard_block',
                pd.Series(False, index=result.index),
            ).map(truthy)
        )
        result['multibagger_direct_evidence_complete'] = (
            result['multibagger_rank_eligible']
        )
        result['multibagger_proxy_evidence_complete'] = (
            result['multibagger_proxy_rank_eligible']
        )
        result['multibagger_evidence_class'] = np.select(
            [
            ...
            ['V8_PRODUCTION_READY', 'V8_RESEARCH_ONLY'],
            default='V8_DATA_PENDING',
        )
        result['selection_before_setup'] = True
        result = _sort_multibagger_ranking_contract(result)
...
def build_multibagger_diagnostic_views(
    ...
    *,
    ...
    common_columns = [
        ...
        'narrative_effective_score', 'narrative_score_state',
        ...
        'issuer_alignment_score_state', 'issuer_alignment_evidence_basis',
        'retail_adoption_stage', 'narrative_flow_effective_score',
        'narrative_flow_research_score', 'narrative_flow_score_state',
        ...
        'smart_money_flow_evidence_mode',
        'narrative_lifecycle_score', 'narrative_lifecycle_state',
        'flow_preceded_narrative', 'emir_method_score',
        'emir_method_state', 'emir_method_production_eligible',
        'emir_method_reliability_pct', 'emir_selection_reason',
        'emir_risk_flags', 'emir_position_cap_pct',
    ...
    score_columns = [
        ...
        'effective_silent_accumulation_score',
        'narrative_effective_score', 'operating_narrative_proxy_score',
        'narrative_flow_effective_score', 'narrative_flow_research_score',
        'issuer_alignment_effective_score',
        'stock_universe_familiarity_score', 'smart_money_behavior_score',
        'narrative_lifecycle_score', 'emir_method_score',
        'narrative_conversion_rate_20d_pct',
        'multibagger_metric_coverage_pct', 'growth_pillar_coverage_pct',
        'profitability_pillar_coverage_pct', 'cashflow_pillar_coverage_pct',
        'safety_pillar_coverage_pct', 'runway_pillar_coverage_pct',
        'valuation_pillar_coverage_pct', 'overall_research_confidence',
        'selector_trend_score', 'selector_relative_strength_score',
    ]
    dispersion_rows: list[dict[str, Any]] = []
    narrative_coverage_series = numeric(
        'narrative_evidence_coverage_pct', 0.0,
    ).fillna(0.0)
    alignment_coverage_series = numeric(
        'issuer_alignment_coverage_pct', 0.0,
    ).fillna(0.0)
    conversion_resolved_series = numeric(
        'narrative_conversion_resolved_20d', 0.0,
    ).fillna(0.0)
    for column in score_columns:
        ...
        unique_count = int(observed.nunique()) if observed_count else 0
        if column in {
            'narrative_effective_score', 'narrative_flow_effective_score',
        } and narrative_coverage_series.le(0.0).all():
            audit_state = 'NOT_SCORED_SOURCE_EVIDENCE_PENDING'
            explanation = 'Event aktif bersumber belum tersedia; neutral 50 tidak boleh ditampilkan.'
        elif column == 'issuer_alignment_effective_score' and alignment_coverage_series.le(0.0).all():
            audit_state = 'NOT_SCORED_ALIGNMENT_EVIDENCE_PENDING'
            explanation = 'Ownership/governance/capital-allocation evidence belum tersedia.'
        elif column == 'narrative_conversion_rate_20d_pct' and conversion_resolved_series.le(0.0).all():
            audit_state = 'NOT_SCORED_OUTCOME_SAMPLE_PENDING'
            explanation = 'Belum ada outcome 20D resolved; rate 50 bukan hasil statistik.'
        elif observed_count == 0:
            audit_state = 'NOT_SCORED_NO_OBSERVATION'
            explanation = 'Tidak ada nilai valid; dashboard harus menampilkan kosong/—.'
        elif unique_count > 1:
            audit_state = 'VARIES_CROSS_SECTIONALLY'
            explanation = 'Nilai membedakan emiten pada cohort scan ini.'
        elif column.endswith('_coverage_pct'):
            audit_state = 'CONSTANT_COVERAGE_CURRENT_COHORT'
            explanation = 'Coverage mengukur ketersediaan field, bukan kualitas emiten; dapat sama bila field yang tersedia sama.'
        else:
            audit_state = 'CONSTANT_CURRENT_COHORT_REVIEW'
            explanation = 'Nilai sama pada cohort saat ini; periksa input/state, tetapi jangan membuat variasi buatan.'
        dispersion_rows.append({
            'metric': column,
            'observed_tickers': observed_count,
            'missing_tickers': int(values.isna().sum()),
            'unique_values': unique_count,
            'minimum': round(float(observed.min()), 3) if observed_count else np.nan,
            'maximum': round(float(observed.max()), 3) if observed_count else np.nan,
            'mean': round(float(observed.mean()), 3) if observed_count else np.nan,
            'std_dev': round(float(observed.std(ddof=0)), 3) if observed_count else np.nan,
            'audit_state': audit_state,
            'explanation': explanation,
        })
    score_dispersion = pd.DataFrame(dispersion_rows)
...
def build_multibagger_decision_summary(
    multibagger: pd.DataFrame | None,
) -> pd.DataFrame:
    """Curated decision surface; the full frame remains available for audit."""
    if multibagger is None or multibagger.empty:
        return pd.DataFrame()
    columns = [
        'multibagger_selection_rank', 'multibagger_production_rank',
        'multibagger_proxy_research_rank', 'ticker',
        'multibagger_evidence_class', 'multibagger_rank_eligible',
        'multibagger_proxy_rank_eligible', 'multibagger_status',
        'multibagger_lane', 'multibagger_selection_score',
        'effective_silent_accumulation_score', 'execution_readiness_score',
        ...
        'fundamental_data_grade', 'fundamental_history_quarters',
        'valuation_score', 'valuation_pillar_coverage_pct',
        'valuation_data_coverage_pct', 'valuation_state',
        'valuation_confidence', 'valuation_source_tier',
        'valuation_statement_official_verified',
        'valuation_score_eligible',
        'valuation_production_eligible', 'market_cap', 'trailing_pe',
        'earnings_yield', 'price_to_book', 'peg_ratio', 'fcf_yield',
        'ev_ebitda', 'valuation_peg_growth_pct_used',
        'valuation_sector_model_source',
        'valuation_fx_source', 'valuation_split_state',
        'valuation_flags',
        'technical_history_bars', 'technical_history_required_bars',
        'technical_history_state',
        'narrative_effective_score', 'narrative_evidence_mode',
        'issuer_alignment_effective_score', 'broker_summary_score',
        'broker_summary_direct_verified',
        'broker_summary_provenance_state',
        'emir_method_score', 'emir_method_production_eligible',
        'selector_relative_strength_score', 'selector_trend_score',
        'technical_entry_state', 'entry_low', 'entry_high', 'stop_loss',
        'tp1', 'tp2', 'selected_reason', 'primary_risk',
        'trigger_waiting', 'invalidation_reason',
    ]
    return multibagger.loc[:, [
        column for column in columns if column in multibagger.columns
    ]].copy()
...
def build_focus_order_builder(
    ...
    for setup_name, spec in specs.items():
        ...
        for _, row in local.iterrows():
            ...
            # Production structure is deliberately flow-free.  The legacy
            # structure score above remains diagnostic because it includes
            # supply/demand, which is already represented by the v8 flow
            # pillar.
            v8_structure_pure = max(
                0.0,
                min(
                    100.0,
                    0.65 * structural
            ...
            cycle_weight = max(0.0, min(0.04, safe_number(row.get('time_cycle_effective_weight_pct'), 0.0)/100.0)) if cycle_state == 'VALIDATED' else 0.0
            ...
            rows.append({
                ...
                'v8_structure_pure_score': round(v8_structure_pure, 1),
                'v8_structure_pure_coverage_pct': round(component_coverages[0], 1),
                'timing_score': round(timing, 1),
                'timing_coverage_pct': round(component_coverages[1], 1),
                'flow_score': round(flow, 1),
                'liquidity_score': round(liquidity, 1),
                'target_quality_score': round(target, 1),
                'target_quality_coverage_pct': round(component_coverages[4], 1),
    ...
    result = attach_narrative_profiles(result, narrative_profiles)
    # Compatibility columns remain zero; production narrative and issuer
    # actions enter exactly once through apply_core_production_scoring().
    narrative_adjustment = pd.Series(0.0, index=result.index)
    emir_swing_adjustment = pd.Series(0.0, index=result.index)
    result['core_priority_score_pre_overlay'] = result['core_priority_score']
    result['core_priority_score_pre_narrative'] = result['core_priority_score']
    result['core_narrative_contribution_points'] = narrative_adjustment.round(2)
    result['core_emir_contribution_points'] = emir_swing_adjustment.round(2)
    result['core_priority_score'] = (
        pd.to_numeric(result['core_priority_score'], errors='coerce')
        + narrative_adjustment
        + emir_swing_adjustment
    ).clip(0.0, 100.0).round(1)
    result['core_final_score_formula'] = (
        'LEGACY_CORE_BASE_ONLY_V8_OVERLAYS_DISABLED'
    )
    narrative_block = result.get(
        'narrative_hard_block',
        pd.Series(False, index=result.index),
    ...
    # The single disjoint v8 score is applied before any threshold or ranking.
    # Legacy selection, narrative/Emir overlays and AI cannot pre-filter the
    # production candidate set.
    result = apply_core_production_scoring(result)
    result['profit_conviction_score_legacy'] = pd.to_numeric(
        result.get('profit_conviction_score'), errors='coerce',
    )
    result['profit_conviction_score'] = pd.to_numeric(
        result.get('core_priority_score'), errors='coerce',
    ).round(1)
    result['selection_before_setup'] = True
    raw=result.copy()
    coverage_columns = [
    ...
    result=result[
        pd.to_numeric(result['core_priority_score'], errors='coerce').ge(minimum)
        & result['order_builder_eligible']
    ].copy()
    ...
    for strategy in specs:
        ...
        local_above=local_raw[
            pd.to_numeric(
                local_raw['core_priority_score'], errors='coerce',
            ).ge(minimum)
    ...
    if not result.empty:
        result, ai_audit=enrich_profit_ranking_with_ai(
            ...
            memory_events=ai_memory,
            config=ai_cfg,
        )
        # Compatibility audit only: the production score was already frozen
        # before filtering. AI and legacy overlays have exactly zero weight.
        result['final_score'] = pd.to_numeric(
            result.get('core_priority_score'), errors='coerce',
        ).round(1)
        result['profit_conviction_score'] = result['final_score']
        result['core_narrative_contribution_points'] = 0.0
        result['core_emir_contribution_points'] = 0.0
...
def _complete_core_radar_universe(
    core_radar: pd.DataFrame | None,
    prepared: Mapping[str, pd.DataFrame],
    silent_profiles: Mapping[str, Mapping[str, Any]],
    *,
    required_bars: int=220,
) -> pd.DataFrame:
    """Append explicit DATA_PENDING rows for selector-excluded tickers."""
    out = (
        core_radar.copy()
        if isinstance(core_radar, pd.DataFrame)
        else pd.DataFrame()
    )
    existing = set(
    ...
    if not out.empty and 'daily_session_current' not in out.columns:
        # Existing selector rows were built from the current prepared session.
        # Adding pending rows must not turn the implicit-true legacy default
        # into NaN/false for every already-scored ticker.
        out['daily_session_current'] = True
    rows: list[dict[str, Any]] = []
    for raw_ticker, frame in prepared.items():
        ticker = safe_text(raw_ticker).upper()
        ...
        profile = dict(silent_profiles.get(ticker, {}) or {})
        rows.append({
            'ticker': ticker,
            'as_of': (
            ...
            'selector_rank_eligible': False,
            'selector_data_state': 'DATA_NOT_SCORED_INSUFFICIENT_TECHNICAL_HISTORY',
            'technical_feature_coverage_pct': round(
                100.0 * min(1.0, bars / max(required_bars, 1)), 1,
            ),
            'selector_missing_feature_count': np.nan,
            'selector_missing_features': 'MINIMUM_HISTORY',
            'swing_selection_score': np.nan,
            'technical_selection_score': np.nan,
            'selected_reason': (
                f'Histori {bars}/{required_bars} bar; ticker tetap terlihat '
                'tetapi tidak diberi ranking Core Swing.'
            ),
            'selection_risks': 'INSUFFICIENT_TECHNICAL_HISTORY',
            'selection_reason_codes': (
                f'INSUFFICIENT_TECHNICAL_HISTORY_{bars}_OF_{required_bars}_BARS'
            ),
            'daily_session_current': bool(
                isinstance(frame, pd.DataFrame) and not frame.empty
            ),
            'last_price': safe_number(latest.get('Close'), np.nan),
            'adtv20_idr': safe_number(latest.get('ADTV20'), 0.0),
            'effective_silent_accumulation_score': profile.get(
                'effective_silent_accumulation_score', np.nan,
            ),
            'silent_accumulation_confidence': profile.get(
                'silent_accumulation_confidence', 0.0,
            ),
        })
    if not rows:
    ...
    return pd.concat([out, pending], ignore_index=True, sort=False)


def _incremental_cache_ttl_seconds(config: ScanConfig) -> float:
    hours = max(0.25, safe_number(
        getattr(config, 'incremental_cache_max_age_hours', 8.0), 8.0,
    ))
    return 3600.0 * hours


def _append_cache_report(
...
def _load_or_build_incremental_stage(
    *,
    ...
    if store is None or not bool(getattr(config, 'incremental_cache_stage_enabled', True)):
        started = time.perf_counter()
        payload = builder()
        _append_cache_report(
    ...
    stored = store.put(
        ...
        ttl_seconds=_incremental_cache_ttl_seconds(config),
        metadata={'pipeline_version': FOCUS_PIPELINE_CACHE_VERSION},
    )
    _append_cache_report(
        report, stage=stage,
        state=f'{lookup.state}_REBUILT', cache_key=cache_key,
        load_ms=lookup.elapsed_ms, build_ms=build_ms,
        store_ms=stored.elapsed_ms, payload_bytes=stored.payload_bytes,
        detail='; '.join(
            part for part in (lookup.detail, stored.detail) if safe_text(part)
        ),
    )
    return payload


def _focus_input_fingerprints(
    *,
    prepared: Mapping[str, pd.DataFrame],
    fundamentals: pd.DataFrame | None,
    core_signals: pd.DataFrame | None,
    ...
    narrative_events: pd.DataFrame | None,
    narrative_outcomes: pd.DataFrame | None,
    benchmark: pd.DataFrame | None,
    validation_events: pd.DataFrame | None,
    ai_memory: pd.DataFrame | None,
    ...
    for frame in prepared.values():
        if isinstance(frame, pd.DataFrame) and not frame.empty:
            try:
                latest_dates.append(pd.Timestamp(frame.index[-1]))
            except Exception:
                continue
    data_as_of = max(latest_dates).isoformat() if latest_dates else 'NO_DATA_AS_OF'
    run_partition = pd.Timestamp.now(tz='Asia/Jakarta').strftime('%Y-%m-%d')
    return {
        'prepared': fingerprint_prepared(prepared),
        'fundamentals': fingerprint_dataframe(fundamentals),
        'core_signals': fingerprint_dataframe(core_signals),
        'project_management': fingerprint_dataframe(project_management),
        'news_review': fingerprint_dataframe(news_review),
        'market_status': fingerprint_dataframe(market_status),
        'narrative_events': fingerprint_dataframe(narrative_events),
        'narrative_outcomes': fingerprint_dataframe(narrative_outcomes),
        'benchmark': fingerprint_dataframe(benchmark),
        'validation_events': fingerprint_dataframe(validation_events),
        'ai_memory': fingerprint_dataframe(ai_memory),
        'reference_fx': fingerprint_dataframe(reference_fx),
        'config': fingerprint_value(config_payload),
        'data_as_of': fingerprint_value(data_as_of),
        'run_partition': fingerprint_value(run_partition),
    }


def _build_focus_screens_uncached(
    prepared: Mapping[str, pd.DataFrame],
    fundamentals: pd.DataFrame | None=None,
    core_signals: pd.DataFrame | None=None,
    project_management: pd.DataFrame | None=None,
    ...
    _cache_store: IncrementalEvidenceStore | None=None,
    _cache_report: list[dict[str, Any]] | None=None,
    _input_fingerprints: Mapping[str, str] | None=None,
) -> dict[str, pd.DataFrame]:
    """Build the two production focuses: Multibagger and Core Swing."""
    cfg=config or ScanConfig()
    fingerprints = dict(_input_fingerprints or {})
    if not fingerprints:
        fingerprints = _focus_input_fingerprints(
            prepared=prepared, fundamentals=fundamentals,
            core_signals=core_signals, project_management=project_management,
            news_review=news_review, market_status=market_status,
            narrative_events=narrative_events,
            narrative_outcomes=narrative_outcomes, benchmark=benchmark,
            validation_events=validation_events, ai_memory=ai_memory,
            reference_fx=reference_fx, config=cfg,
        )

    valuation_key = combine_fingerprints(
        FOCUS_PIPELINE_CACHE_VERSION, 'VALUATION',
        fingerprints['prepared'], fingerprints['fundamentals'],
        fingerprints['reference_fx'], fingerprints['config'],
    )
    fundamentals = _load_or_build_incremental_stage(
        store=_cache_store, report=_cache_report,
        stage='fundamentals_enriched', cache_key=valuation_key,
        config=cfg,
        builder=lambda: enrich_fundamentals_with_valuation(
            fundamentals if isinstance(fundamentals, pd.DataFrame) else pd.DataFrame(),
            prepared,
            reference_fx=reference_fx,
        ),
    )
    enriched_fundamentals_fp = fingerprint_dataframe(fundamentals)

    def build_silent_profiles() -> dict[str, dict[str, Any]]:
        profiles = current_silent_profiles(prepared)
        # Broker-summary is explicit user/provider evidence, not an OHLCV inference.
        if isinstance(core_signals, pd.DataFrame) and not core_signals.empty and 'ticker' in core_signals:
            broker_columns = [
                column for column in (
                    'broksum_asof', 'broksum_days', 'broksum_net',
                    'broksum_net_ratio', 'broksum_signal',
                    'top_net_buy_brokers', 'top_net_sell_brokers',
                ...
                ) if column in core_signals.columns
            ]
            if broker_columns:
                broker_rows = core_signals.drop_duplicates('ticker', keep='last')
                for _, broker_row in broker_rows.iterrows():
                    ticker = safe_text(broker_row.get('ticker')).upper()
                    if not ticker or ticker not in profiles:
                        continue
                    profiles[ticker] = {
                        **dict(profiles.get(ticker, {})),
                        **{column: broker_row.get(column) for column in broker_columns},
                    }
        return profiles

    silent_key = combine_fingerprints(
        FOCUS_PIPELINE_CACHE_VERSION, 'SILENT_PROFILES',
        fingerprints['prepared'], fingerprints['core_signals'],
    )
    silent_profiles = _load_or_build_incremental_stage(
        store=_cache_store, report=_cache_report,
        stage='silent_profiles', cache_key=silent_key,
        config=cfg, builder=build_silent_profiles,
    )
    silent_profiles_fp = fingerprint_value(silent_profiles)
    ...
    selector_key = combine_fingerprints(
        FOCUS_PIPELINE_CACHE_VERSION, 'SELECTOR',
        fingerprints['prepared'], fingerprint_value(selector_config),
        fingerprint_value(sector_map), silent_profiles_fp,
    )
    def build_selector_payload() -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
        selector_frame, audit_frame, panel_frame = build_cross_sectional_selector(
            prepared, selector_config, silent_profiles, sector_map,
        )
        training_meta = {
            'training_panel_rows': len(panel_frame),
            'training_panel_tickers': int(panel_frame['ticker'].nunique())
            if not panel_frame.empty and 'ticker' in panel_frame else 0,
            'training_panel_dates': int(panel_frame['as_of'].nunique())
            if not panel_frame.empty and 'as_of' in panel_frame else 0,
            'selector_version': selector_frame.get(
                'selector_version', pd.Series(dtype=str),
            ).iloc[0] if not selector_frame.empty else '',
        }
        return selector_frame, audit_frame, training_meta

    selector, selector_audit, selector_training_meta = _load_or_build_incremental_stage(
        store=_cache_store, report=_cache_report,
        stage='selector_bundle_compact', cache_key=selector_key, config=cfg,
        builder=build_selector_payload,
    )
    selector_fp = combine_fingerprints(
        selector_key, fingerprint_dataframe(selector),
        fingerprint_dataframe(selector_audit),
        fingerprint_value(selector_training_meta),
    )

    narrative_key = combine_fingerprints(
        FOCUS_PIPELINE_CACHE_VERSION, 'NARRATIVE',
        fingerprints['prepared'], enriched_fundamentals_fp,
        fingerprints['news_review'], fingerprints['project_management'],
        fingerprints['market_status'], fingerprints['narrative_events'],
        fingerprints['narrative_outcomes'], fingerprints['benchmark'],
        silent_profiles_fp, fingerprints['config'],
        fingerprints['data_as_of'], fingerprints['run_partition'],
    )
    narrative_intelligence = _load_or_build_incremental_stage(
        store=_cache_store, report=_cache_report,
        stage='narrative_bundle', cache_key=narrative_key, config=cfg,
        builder=lambda: build_narrative_intelligence(
            prepared=prepared,
    ...
    core_radar = _complete_core_radar_universe(
        core_radar, prepared, silent_profiles,
    )
    ...
    if isinstance(core_radar, pd.DataFrame) and not core_radar.empty:
        base_swing = pd.to_numeric(
            core_radar.get(
                ...
                pd.Series(np.nan, index=core_radar.index),
            ),
            errors='coerce',
        )
        missing_base = base_swing.isna()
        core_radar['core_score_state'] = np.where(
            missing_base, 'CORE_BASE_NOT_SCORED', 'CORE_BASE_SCORED',
        )
        if 'selector_rank_eligible' in core_radar:
            core_radar['selector_rank_eligible'] = (
                core_radar['selector_rank_eligible'].map(truthy) & ~missing_base
            )
        narrative_adjustment = pd.Series(0.0, index=core_radar.index)
        emir_adjustment = pd.Series(0.0, index=core_radar.index)
        core_radar['swing_selection_score_pre_narrative'] = base_swing
        core_radar['swing_selection_score'] = base_swing.clip(0.0, 100.0).round(1)
        core_radar['narrative_swing_rank_adjustment'] = 0.0
        core_radar['emir_swing_rank_adjustment'] = 0.0
        core_radar['production_legacy_overlay_weight_pct'] = 0.0
        core_radar['production_scoring_version'] = PRODUCTION_SCORING_VERSION
        narrative_cov = pd.to_numeric(
            core_radar.get('narrative_evidence_coverage_pct', pd.Series(0.0, index=core_radar.index)),
            errors='coerce',
        ).fillna(0.0)
        alignment_cov = pd.to_numeric(
            core_radar.get('issuer_alignment_coverage_pct', pd.Series(0.0, index=core_radar.index)),
            errors='coerce',
        ).fillna(0.0)
        emir_cov = pd.to_numeric(
            core_radar.get('emir_method_coverage_pct', pd.Series(0.0, index=core_radar.index)),
            errors='coerce',
        ).fillna(0.0)
        selector_ok = core_radar.get(
            'selector_rank_eligible', pd.Series(False, index=core_radar.index),
        ).map(truthy)
        current_ok = core_radar.get(
            'daily_session_current', pd.Series(True, index=core_radar.index),
        ).map(truthy)
        proxy_swing_evidence = (
            selector_ok & current_ok & narrative_cov.ge(35.0)
            & alignment_cov.ge(25.0) & emir_cov.ge(55.0)
        )
        swing_mode = core_radar.get(
            'narrative_evidence_mode', pd.Series('', index=core_radar.index),
        ).fillna('').astype(str).str.upper()
        swing_active_events = pd.to_numeric(
            core_radar.get('narrative_active_event_count', pd.Series(0.0, index=core_radar.index)),
            errors='coerce',
        ).fillna(0.0)
        swing_official_events = pd.to_numeric(
            core_radar.get('narrative_official_event_count', pd.Series(0.0, index=core_radar.index)),
            errors='coerce',
        ).fillna(0.0)
        swing_items_reviewed = pd.to_numeric(
            core_radar.get('narrative_items_reviewed', pd.Series(0.0, index=core_radar.index)),
            errors='coerce',
        ).fillna(0.0)
        swing_news_complete = core_radar.get(
            'narrative_news_collection_state', pd.Series('', index=core_radar.index),
        ).fillna('').astype(str).str.upper().eq('COMPLETE')
        swing_direct_event = (
            swing_mode.isin({
                'SOURCE_EVENT', 'EVENT_PLUS_STRUCTURED_FINANCIAL',
            ...
            & swing_active_events.ge(1.0)
            & (
                swing_official_events.ge(1.0)
                | (swing_news_complete & swing_items_reviewed.ge(1.0))
            )
        )
        swing_direct_alignment = core_radar.get(
            'issuer_alignment_score_state', pd.Series('', index=core_radar.index),
        ).fillna('').astype(str).str.upper().eq('SCORED_DIRECT_AND_PROXY')
        swing_broker_coverage = pd.to_numeric(
            core_radar.get('broker_summary_coverage_pct', pd.Series(0.0, index=core_radar.index)),
            errors='coerce',
        ).fillna(0.0)
        swing_direct_flow = _direct_broker_flow_mask(core_radar)
        swing_emir_production = core_radar.get(
            'emir_method_production_eligible', pd.Series(False, index=core_radar.index),
        ).map(truthy)
        direct_swing_evidence = (
            proxy_swing_evidence & swing_direct_event & swing_direct_alignment
            & swing_direct_flow & swing_emir_production
        )
        partial_swing_evidence = selector_ok & current_ok & ~proxy_swing_evidence
        core_radar['swing_evidence_class'] = np.select(
            [direct_swing_evidence, proxy_swing_evidence, partial_swing_evidence],
            [
                'DIRECT_EVIDENCE_RANKING', 'FULL_PROXY_COVERAGE',
                'PARTIAL_EVIDENCE_RESEARCH',
            ],
        ...
        core_radar['swing_rank_eligible'] = direct_swing_evidence
        core_radar['swing_proxy_rank_eligible'] = proxy_swing_evidence
        core_radar['swing_direct_event_verified'] = swing_direct_event
        core_radar['swing_direct_flow_verified'] = swing_direct_flow
        core_radar['swing_score_comparability_pct'] = np.minimum.reduce([
            pd.to_numeric(core_radar.get('technical_feature_coverage_pct', pd.Series(0.0, index=core_radar.index)), errors='coerce').fillna(0.0).to_numpy(dtype=float),
            np.where(swing_direct_event, 100.0, 0.0),
            np.where(swing_direct_alignment, alignment_cov, 0.0),
            swing_broker_coverage.to_numpy(dtype=float),
            np.where(swing_emir_production, emir_cov, 0.0),
        ]).round(1)
        core_radar['swing_proxy_comparability_pct'] = np.minimum.reduce([
            pd.to_numeric(core_radar.get('technical_feature_coverage_pct', pd.Series(0.0, index=core_radar.index)), errors='coerce').fillna(0.0).to_numpy(dtype=float),
            narrative_cov.to_numpy(dtype=float), alignment_cov.to_numpy(dtype=float), emir_cov.to_numpy(dtype=float),
        ]).round(1)
        ...
        emir_reason = core_radar.get(
            'emir_selection_reason',
            pd.Series('', index=core_radar.index),
        ).fillna('').astype(str)
        core_radar['selected_reason'] = [
            ' • '.join(
                part for part in (technical, narrative, emir)
                if safe_text(part)
            )
            for technical, narrative, emir in zip(
                technical_reason, narrative_reason, emir_reason,
            )
        ]
        ...
        core_radar['_selector_eligible_sort'] = (
            ...
            ).map(truthy).astype(int)
        )
        emir_state = core_radar.get(
            'emir_method_state', pd.Series('', index=core_radar.index),
        ).fillna('').astype(str).str.upper()
        emir_ready = core_radar.get(
            'emir_method_production_eligible',
            pd.Series(False, index=core_radar.index),
        ).map(truthy) | emir_state.isin({'', 'DISABLED'})
        core_radar['_emir_ready_sort'] = emir_ready.astype(int)
        core_radar['_evidence_rank_sort'] = core_radar.get(
            'swing_evidence_class', pd.Series('DATA_PENDING', index=core_radar.index),
        ).map({
            'DIRECT_EVIDENCE_RANKING': 0, 'FULL_PROXY_COVERAGE': 1,
            'PARTIAL_EVIDENCE_RESEARCH': 2, 'DATA_PENDING': 3,
        }).fillna(3)
        core_radar = core_radar.sort_values(
            [
                '_evidence_rank_sort', '_selector_eligible_sort', '_emir_ready_sort',
                '_narrative_block_sort', 'swing_selection_score',
            ...
            columns=[
                '_evidence_rank_sort', '_selector_eligible_sort', '_emir_ready_sort',
                '_narrative_block_sort',
            ],
        ).reset_index(drop=True)
        core_radar['swing_selection_rank'] = np.arange(
            1, len(core_radar) + 1,
        )
        core_radar['swing_production_rank'] = np.nan
        swing_prod_idx = core_radar.index[core_radar.get('swing_rank_eligible', pd.Series(False, index=core_radar.index)).map(truthy)].tolist()
        core_radar.loc[swing_prod_idx, 'swing_production_rank'] = np.arange(1, len(swing_prod_idx) + 1)
    narrative_profiles_fp = fingerprint_dataframe(narrative_profiles)
    multibagger_key = combine_fingerprints(
        FOCUS_PIPELINE_CACHE_VERSION, 'MULTIBAGGER',
        fingerprints['prepared'], enriched_fundamentals_fp,
        fingerprints['core_signals'], fingerprints['project_management'],
        fingerprints['config'], selector_fp, silent_profiles_fp,
        narrative_profiles_fp,
    )
    multibagger = _load_or_build_incremental_stage(
        store=_cache_store, report=_cache_report,
        stage='multibagger_frame', cache_key=multibagger_key, config=cfg,
        builder=lambda: scan_multibagger_candidates(
    ...
    order_builder_key = combine_fingerprints(
        FOCUS_PIPELINE_CACHE_VERSION, 'ORDER_BUILDER',
        fingerprints['core_signals'], fingerprints['validation_events'],
        fingerprints['ai_memory'], fingerprints['config'], selector_fp,
        narrative_profiles_fp,
    )
    core_builder = _load_or_build_incremental_stage(
        store=_cache_store, report=_cache_report,
        stage='order_builder', cache_key=order_builder_key, config=cfg,
        builder=lambda: build_focus_order_builder(
    ...
    # Build downstream summaries before enforcing the bounded production
    # schemas. The full internal evidence frame never reaches Streamlit/export.
    multibagger_decision_summary = build_multibagger_decision_summary(
        multibagger,
    )
    daily_opportunities = build_focus_daily_board(
        core_builder, research_multibagger,
    )
    production_evidence_detail = build_production_evidence_detail(
        multibagger, core_builder,
    )
    multibagger = slim_production_frame(
        multibagger, model='multibagger',
    )
    research_multibagger = slim_production_frame(
        research_multibagger, model='multibagger',
    )
    growth_compounder = slim_production_frame(
        growth_compounder, model='multibagger',
    )
    turnaround = slim_production_frame(
        turnaround, model='multibagger',
    )
    core_builder = slim_production_frame(core_builder, model='core')
    narrative_profile_columns = [
        'ticker', 'narrative_engine_version', 'narrative_as_of',
        'narrative_state', 'narrative_score_state',
        'narrative_event_score', 'narrative_event_effective_score',
        'narrative_event_coverage_pct', 'narrative_event_count',
        'narrative_active_event_count', 'narrative_official_event_count',
        'latest_narrative_event', 'latest_narrative_event_type',
        'latest_narrative_event_date', 'narrative_source_quality_score',
        'narrative_novelty_score', 'issuer_action_alignment_score',
        'issuer_action_alignment_effective_score',
        'issuer_action_alignment_coverage_pct',
        'issuer_action_alignment_basis', 'narrative_hard_block',
        'narrative_primary_reason', 'narrative_primary_risk',
        'evidence_acquisition_status', 'evidence_acquisition_missing',
    ]
    narrative_profiles_production = (
        narrative_profiles.loc[:, [
            column for column in narrative_profile_columns
            if column in narrative_profiles.columns
        ]].copy()
        if isinstance(narrative_profiles, pd.DataFrame)
        else pd.DataFrame()
    )
    narrative_profiles_production.attrs = {}
    return {
        'fundamentals_enriched':fundamentals,
        'multibagger':multibagger,
        'multibagger_decision_summary':multibagger_decision_summary,
        ...
        'multibagger_score_dispersion':multibagger_diagnostics['score_dispersion'],
        ...
        'narrative_profiles':narrative_profiles_production,
        'narrative_engine_audit':narrative_intelligence.get(
            'audit', pd.DataFrame(),
        ),
        'selector_training_summary':pd.DataFrame([selector_training_meta]),
        'profit_order_builder':core_builder,
        'daily_opportunities':daily_opportunities,
        'production_evidence_detail':production_evidence_detail,
        'production_scoring_audit':production_scoring_audit_frame(),
...
def build_focus_screens(
    ...
    """Build Focus screens with deterministic SQLite incremental reuse.

    Cache identity includes all computational inputs, scanner configuration,
    data as-of partition, and pipeline version. A miss executes the original
    decision path. Cache errors never create synthetic scores or eligibility.
    """
    cfg = config or ScanConfig()
    if not incremental_cache_enabled(cfg):
        return _build_focus_screens_uncached(
            prepared, fundamentals=fundamentals, core_signals=core_signals,
            project_management=project_management, news_review=news_review,
            market_status=market_status, narrative_events=narrative_events,
            narrative_outcomes=narrative_outcomes, benchmark=benchmark,
            config=cfg, validation_events=validation_events,
            ai_memory=ai_memory, reference_fx=reference_fx,
        )
    ...
    try:
        ...
        _append_cache_report(
            report, stage='input_fingerprint', state='READY',
            cache_key=combine_fingerprints(*fingerprints.values()),
            build_ms=1000.0 * (time.perf_counter() - fingerprint_started),
        )
        store = IncrementalEvidenceStore(
            max_entries=max(4, int(safe_number(
                getattr(cfg, 'incremental_cache_max_entries', 48), 48,
            ))),
            max_bytes=max(16, int(safe_number(
                getattr(cfg, 'incremental_cache_max_bytes_mb', 1024), 1024,
            ))) * 1024 * 1024,
            read_only=bool(getattr(cfg, 'incremental_cache_read_only', False)),
        )
    except Exception as exc:
        result = _build_focus_screens_uncached(
            prepared, fundamentals=fundamentals, core_signals=core_signals,
            project_management=project_management, news_review=news_review,
            market_status=market_status, narrative_events=narrative_events,
            narrative_outcomes=narrative_outcomes, benchmark=benchmark,
        ...
        result['incremental_cache_report'] = pd.DataFrame([{
            'stage': 'cache_initialisation', 'cache_state': 'ERROR_FAIL_SOFT',
            'cache_key_prefix': '', 'load_ms': 0.0,
            'build_ms': round(1000.0 * (time.perf_counter() - fingerprint_started), 2),
            'store_ms': 0.0, 'payload_bytes': 0,
            'detail': f'{type(exc).__name__}: {str(exc)[:180]}',
        }])
        return result

    full_key = combine_fingerprints(
        FOCUS_PIPELINE_CACHE_VERSION, 'FULL_FOCUS_RESULT',
        *[fingerprints[key] for key in sorted(fingerprints)],
    )
    full_cache_enabled = bool(
        getattr(cfg, 'incremental_cache_full_result_enabled', True)
    )
    if full_cache_enabled:
        lookup = store.get('focus_full_result', full_key)
        if lookup.state == 'HIT' and isinstance(lookup.payload, dict):
            _append_cache_report(
                report, stage='focus_full_result', state='HIT',
                cache_key=full_key, load_ms=lookup.elapsed_ms,
                payload_bytes=lookup.payload_bytes,
            )
            result = lookup.payload
            result['incremental_cache_report'] = pd.DataFrame(report)
            return result
        _append_cache_report(
            report, stage='focus_full_result',
            state=f'{lookup.state}_REBUILD_REQUIRED', cache_key=full_key,
            load_ms=lookup.elapsed_ms, payload_bytes=lookup.payload_bytes,
    ...
    result = _build_focus_screens_uncached(
        ...
        config=cfg, validation_events=validation_events,
        ai_memory=ai_memory, reference_fx=reference_fx,
        _cache_store=store, _cache_report=report,
        _input_fingerprints=fingerprints,
    )
    if full_cache_enabled:
        payload = {
            key: value for key, value in result.items()
            if key != 'incremental_cache_report'
        }
        stored = store.put(
            'focus_full_result', full_key, payload,
            ttl_seconds=_incremental_cache_ttl_seconds(cfg),
            metadata={
                'pipeline_version': FOCUS_PIPELINE_CACHE_VERSION,
                'ticker_count': len(prepared),
            },
        )
        _append_cache_report(
            report, stage='focus_full_result_write', state=stored.state,
            cache_key=full_key, store_ms=stored.elapsed_ms,

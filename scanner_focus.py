...
def allocate_multibagger_capital(candidates: pd.DataFrame, config: ScanConfig | None=None) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        # prior behaviour. Every v7.13.0 scanner result includes the column and
...
def build_focus_screens(
    ...
    # Carry the existing normalized CSV fields into the public-framework layer
    # while retaining the disclaimer that broker code != beneficial owner.
    if isinstance(core_signals, pd.DataFrame) and not core_signals.empty and 'ticker' in core_signals:
        ...
        if broker_columns:
            ...
            for _, broker_row in broker_rows.iterrows():
                ...
                if not ticker or ticker not in silent_profiles:
                    continue
                silent_profiles[ticker] = {
                    **dict(silent_profiles.get(ticker, {})),
